...
@Schema(description = "管理后台 - 事件工单分页 Request VO")
@Data
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
public class EvtWoPageReqVO extends PageParam {

    @Schema(description = "工单ID，唯一编码，UUID生成", example = "19126")
    private String woId;

    @Schema(description = "工单编号，系统自动生成，格式“WO+年份(4位)+月份(2位)+8位流水号”")
    private String woNo;

    @Schema(description = "事件编码，关联事件分级分拨表")
    private String evtCode;

    @Schema(description = "事件类型名称，从分拨表同步", example = "王五")
    private String evtTypeName;

    @Schema(description = "分拨部门代码，从分拨表同步")
    private String allocateDeptCode;

    @Schema(description = "分拨部门名称，从分拨表同步", example = "张三")
