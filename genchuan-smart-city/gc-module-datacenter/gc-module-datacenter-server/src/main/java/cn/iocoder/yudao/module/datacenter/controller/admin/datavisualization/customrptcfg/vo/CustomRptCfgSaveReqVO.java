...
@Schema(description = "管理后台 - 自定义报表配置新增/修改 Request VO")
@Data
public class CustomRptCfgSaveReqVO {
    ...
    @NotEmpty(message = "报表模板名称，如“XX街道月度监测事件报表”不能为空")
    ...
    @NotEmpty(message = "报表类型：分域分析/决策效果/资源调度，标识报表数据来源类型，数据字典表（sys_dict）不能为空")
    private String rptType;

    @Schema(description = "数据来源表，关联的数据库表名（如“stat_analysis_mon_evt”）", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotEmpty(message = "数据来源表，关联的数据库表名（如“stat_analysis_mon_evt”）不能为空")
    private String dataSourceTable;

    @Schema(description = "统计维度配置，JSON格式存储维度配置（如[{\"dimension\":\"stat_cycle\",\"name\":\"统计周期\",\"type\":\"date\",\"default\":\"month\"}]）", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotEmpty(message = "统计维度配置不能为空")
