...
@Schema(description = "管理后台 - 处置结果评估新增/修改 Request VO")
@Data
public class DisposalResultEvalSaveReqVO {
    ...
    @NotNull(message = "资源利用得分，1-10分，评估资源调配合理性、使用率，如“9分(冲锋舟全部投入使用,无浪费)”不能为空")
    ...
    @NotNull(message = "效果达成得分，1-10分，评估事件处置目标达成度，如“10分(被困20人全部转移,无伤亡)”不能为空")
    ...
    @NotNull(message = "综合得分，(效率 + 资源 + 效果) / 3，保留1位小数不能为空")
    ...
    @NotEmpty(message = "评估人ID，评估管理员账号，用户信息表（sys_user）不能为空")
    ...
    @NotEmpty(message = "评估人姓名，与评估人ID同步，用户信息表（sys_user）不能为空")
    ...
    @NotNull(message = "评估时间，格式 yyyy-MM-dd HH:mm:ss，系统自动生成不能为空")
