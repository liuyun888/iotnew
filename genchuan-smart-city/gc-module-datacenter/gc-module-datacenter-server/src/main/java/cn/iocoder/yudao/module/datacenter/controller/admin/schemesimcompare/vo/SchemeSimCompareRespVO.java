...
@Schema(description = "管理后台 - 方案模拟对比统计 Response VO")
@Data
@ExcelIgnoreUnannotated
public class SchemeSimCompareRespVO {
    ...
    @ExcelProperty("对比ID，唯一编码，UUID生成")
    ...
    @ExcelProperty("对比名称，如“XX区域燃气方案A vs 方案B”")
    ...
    @ExcelProperty("参与方案IDs，决策方案ID，英文逗号分隔（如“id1,id2”），决策方案表（biz_decision_scheme）")
    ...
    @ExcelProperty("参与方案名称，与方案IDs同步，逗号分隔，决策方案表（biz_decision_scheme）")
    ...
    @ExcelProperty("资源投入权重，%，10-50，资源投入维度权重，默认30%，权重配置表（sys_scheme_weight_cfg）")
    ...
    @ExcelProperty("预期效果权重，%，30-70，预期效果维度权重，默认50%，权重配置表（sys_scheme_weight_cfg）")
    ...
    @ExcelProperty("实施周期权重，%，10-30，实施周期维度权重，默认20%，权重配置表（sys_scheme_weight_cfg）")
    ...
    @ExcelProperty("方案1_ID，第一个参与方案ID，决策方案表（biz_decision_scheme）")
    ...
    @ExcelProperty("方案1_资源投入得分，分，0-100，方案1资源投入得分（预算越低、资源利用率越高得分越高）")
    ...
    @ExcelProperty("方案1_预期效果得分，分，0-100，方案1预期效果得分（事件下降率越高、响应时长越短得分越高）")
    ...
    @ExcelProperty("方案1_周期得分，分，0-100，方案1周期得分（周期越短得分越高）")
    ...
    @ExcelProperty("方案1_综合得分，分，0-100，（资源得分×权重+效果得分×权重+周期得分×权重）/100")
