...
@Data
public class LocalRegulationSaveReqVO {
    ...
    @NotEmpty(message = "行政区划代码，符合GB/T 2260，如330100=杭州市，行政区划表（sys_area）不能为空")
    ...
    @NotEmpty(message = "法规文件路径，法规文件存储路径，文件存储表（sys_file_storage）不能为空")
    ...
    @NotEmpty(message = "法规状态：有效/失效，标识法规当前状态不能为空")
