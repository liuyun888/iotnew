...
@Schema(description = "管理后台 - 救援车辆分类配置新增/修改 Request VO")
@Data
public class EmerVehicleCatSaveReqVO {

    @Schema(description = "自增主键ID", requiredMode = Schema.RequiredMode.REQUIRED, example = "28537")
    private Long id;

    @Schema(description = "分类ID，唯一编码，UUID", requiredMode = Schema.RequiredMode.REQUIRED, example = "15536")
    @NotEmpty(message = "分类ID，唯一编码，UUID不能为空")
    private String vehicleCatId;

    @Schema(description = "父分类ID，关联本表“分类ID”", requiredMode = Schema.RequiredMode.REQUIRED, example = "17936")
    ...
    @Schema(description = "分类名称，标准名称", requiredMode = Schema.RequiredMode.REQUIRED, example = "赵六")
