...
@Schema(description = "管理后台 - 行业典型经验新增/修改 Request VO")
@Data
public class IndustryTypicalExpSaveReqVO {
    ...
    @NotEmpty(message = "经验标题，如“智慧城管事件处置效率提升经验”不能为空")
    ...
    @NotEmpty(message = "行业领域代码，取值如01=城管，02=环保，03=交通，行业领域字典表（sys_dict_industry_field）不能为空")
    ...
    @NotEmpty(message = "行业领域名称，与行业领域代码同步，行业领域字典表（sys_dict_industry_field）不能为空")
    ...
    @NotEmpty(message = "案例单位，经验来源案例单位，如“XX市城管执法局”，部门信息表（sys_org）不能为空")
    ...
    @NotEmpty(message = "经验核心亮点，支持富文本，如“1.引入AI分派：自动匹配处置部门；2.建立台账：跟踪处置进度”不能为空")
    ...
    @NotEmpty(message = "经验实施效果，如“事件处置时长从24小时缩短至8小时，效率提升66%”不能为空")
    ...
    @NotNull(message = "点赞次数，用户点赞次数，默认0不能为空")
