...
@Schema(description = "管理后台 - 监测事件分类配置分页 Request VO")
@Data
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
public class MonEvtCatPageReqVO extends PageParam {

    @Schema(description = "分类ID，唯一编码，UUID生成", example = "21747")
    private String monEvtCatId;

    @Schema(description = "父类ID，关联本表格“分类ID”，大类父类ID为“0”；监测事件分类配置表（sys_mon_evt_cat）", example = "13260")
    private String parentEvtId;

    @Schema(description = "分类层级，标识分类所处层级，1 (大类)/2 (中类)/3 (小类)")
    private String evtCatLevel;

    @Schema(description = "分类代码，大类/中类从01顺序编排，小类从001顺序编排，扩展类小类从080倒排")
    private String evtCatCode;

    @Schema(description = "分类名称，参照GB/T XXXXX.6标准名称，扩展类标注 “自定义”", example = "张三")
    private String evtCatName;

    @Schema(description = "分类说明，描述事件范围，如“燃气泄漏：燃气从管道或设备泄漏的事件”")
    private String evtCatDesc;

    @Schema(description = "启用状态，标识分类是否可用，默认1", example = "1")
