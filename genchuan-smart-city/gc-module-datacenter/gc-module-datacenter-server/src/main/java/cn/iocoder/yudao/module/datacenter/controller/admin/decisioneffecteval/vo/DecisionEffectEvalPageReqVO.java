...
@Schema(description = "管理后台 - 决策效果评估分页 Request VO")
@Data
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
public class DecisionEffectEvalPageReqVO extends PageParam {

    @Schema(description = "评估ID，唯一编码，UUID生成", example = "1710")
    private String effectEvalId;

    @Schema(description = "关联方案ID，关联决策方案表的方案ID，决策方案表（biz_decision_scheme）", example = "27806")
    ...
    @Schema(description = "实施周期，实际实施周期（如“2025-10-01至2025-12-20”），决策执行跟踪表（biz_decision_exec_track）")
    private String implementationCycle;

    @Schema(description = "预期效果，方案预期效果（如“事件下降50%”），决策方案表（biz_decision_scheme）")
    private String expectedEffect;

    @Schema(description = "实际效果，最终实际效果（如“事件下降55%”），分域分析统计表（如stat_analysis_mon_evt）")
    private String actualEffect;

    @Schema(description = "效果达成率，实际效果/预期效果×100，取值范围：0.00-100.00，单位：%，无")
    private BigDecimal effectAchievementRate;

    @Schema(description = "资源总投入，实施过程中资源投入总额（设备+人员+其他），单位：万元，资源投入表（biz_res_input）")
    ...
    @Schema(description = "评估等级：优秀/良好/合格/不合格，判定规则：效果达成率≥120%=优秀，100%-119%=良好，80%-99%=合格，<80%=不合格，无")
    ...
    @Schema(description = "评估人，负责评估的用户ID，用户信息表（sys_user）")
    private String evalUser;

    @Schema(description = "评估时间，格式：yyyy-MM-dd HH:mm:ss")
