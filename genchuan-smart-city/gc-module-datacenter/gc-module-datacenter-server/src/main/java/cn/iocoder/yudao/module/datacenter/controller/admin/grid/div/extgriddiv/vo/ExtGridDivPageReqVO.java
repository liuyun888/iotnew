package cn.iocoder.yudao.module.datacenter.controller.admin.grid.div.extgriddiv.vo;

import lombok.*;
import java.util.*;
import io.swagger.v3.oas.annotations.media.Schema;
import cn.iocoder.yudao.framework.common.pojo.PageParam;
import org.springframework.format.annotation.DateTimeFormat;
import java.time.LocalDateTime;

import static cn.iocoder.yudao.framework.common.util.date.DateUtils.FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND;

@Schema(description = "管理后台 - 扩展网格划分分页 Request VO")
@Data
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
public class ExtGridDivPageReqVO extends PageParam {

    @Schema(description = "扩展网格ID", example = "395")
    private String extGridId;

    @Schema(description = "扩展网格名称", example = "张三")
    private String extGridName;

    @Schema(description = "扩展网格类型", example = "1")
    private String extType;

    @Schema(description = "所含基础网格IDs")
    private String includedBasicIds;

    @Schema(description = "基础网格类型", example = "2")
    private String basicGridType;

    @Schema(description = "面积(m²)")
    private Integer area;

    @Schema(description = "边界坐标")
    private String boundaryCoords;

    @Schema(description = "申请原因", example = "不香")
    private String applyReason;

    @Schema(description = "审核状态", example = "1")
    private String auditStatus;

    @Schema(description = "申请人ID", example = "20045")
    private String applyUserId;

    @Schema(description = "申请时间")
    @DateTimeFormat(pattern = FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND)
    private LocalDateTime[] applyTime;

    @Schema(description = "审核人ID", example = "21987")
    private String auditUserId;

    @Schema(description = "审核时间")
    @DateTimeFormat(pattern = FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND)
    private LocalDateTime[] auditTime;

    @Schema(description = "审核意见")
    private String auditOpinion;

    @Schema(description = "分类扩展字段1")
    private String extCat1;

    @Schema(description = "分类扩展字段2")
    private String extCat2;

    @Schema(description = "创建时间")
    @DateTimeFormat(pattern = FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND)
    private LocalDateTime[] createTime;

}