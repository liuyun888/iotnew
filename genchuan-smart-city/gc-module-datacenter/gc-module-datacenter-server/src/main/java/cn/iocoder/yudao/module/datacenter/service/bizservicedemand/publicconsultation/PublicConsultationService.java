package cn.iocoder.yudao.module.datacenter.service.bizservicedemand.publicconsultation;

import cn.iocoder.yudao.module.datacenter.controller.admin.bizservicedemand.publicconsultation.vo.PublicConsultationPageReqVO;
import cn.iocoder.yudao.module.datacenter.controller.admin.bizservicedemand.publicconsultation.vo.PublicConsultationSaveReqVO;
import jakarta.validation.*;
import cn.iocoder.yudao.module.datacenter.dal.dataobject.bizservicedemand.publicconsultation.PublicConsultationDO;
import cn.iocoder.yudao.framework.common.pojo.PageResult;

/**
 * 咨询建议 Service 接口
 *
 * @author 亘川智城
 */
public interface PublicConsultationService {

    /**
     * 创建咨询建议
     *
     * @param createReqVO 创建信息
     * @return 编号
     */
    Long createPublicConsultation(@Valid PublicConsultationSaveReqVO createReqVO);

    /**
     * 更新咨询建议
     *
     * @param updateReqVO 更新信息
     */
    void updatePublicConsultation(@Valid PublicConsultationSaveReqVO updateReqVO);

    /**
     * 删除咨询建议
     *
     * @param id 编号
     */
    void deletePublicConsultation(Long id);

    /**
     * 获得咨询建议
     *
     * @param id 编号
     * @return 咨询建议
     */
    PublicConsultationDO getPublicConsultation(Long id);

    /**
     * 获得咨询建议分页
     *
     * @param pageReqVO 分页查询
     * @return 咨询建议分页
     */
    PageResult<PublicConsultationDO> getPublicConsultationPage(PublicConsultationPageReqVO pageReqVO);

}