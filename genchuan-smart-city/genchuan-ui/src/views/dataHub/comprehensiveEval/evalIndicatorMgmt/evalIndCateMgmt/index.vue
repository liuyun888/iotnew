<template>
  ...
  <ContentWrap class="mt-4">
    <el-card shadow="hover" class="rounded-lg">
      ...
      >
        ...
        <el-table-column label="指标分类名称" align="center" prop="idxCatName" width="180" />
        <el-table-column label="分类编码" align="center" prop="catCode" width="150" />
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EvalIdxCatForm ref="formRef" @success="getList" />
  <!-- 详情抽屉 -->
  <EvalIdxCatDetailDrawer ref="detailRef" />
</template>
...
import EvalIdxCatForm from './EvalIdxCatForm.vue'
import EvalIdxCatDetailDrawer from './EvalIdxCatDetailDrawer.vue'

/** 指标分类管理 列表 */
defineOptions({ name: 'EvalIdxCat' })
const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化
const loading = ref(true) // 列表的加载中
const list = ref<EvalIdxCatVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EvalIdxCatApi.getEvalIdxCatPage(queryParams)
...
/** 详情操作 */
const openDetail = (row: EvalIdxCatVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EvalIdxCatApi.deleteEvalIdxCat(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EvalIdxCatApi.exportEvalIdxCat(queryParams)
    download.excel(data, '指标分类管理.xls')
  } catch {
  } finally {
