<template>
  ...
  <ContentWrap class="mt-4">
    <el-card shadow="hover" class="rounded-lg">
      ...
      >
        ...
        <el-table-column label="指标体系名称" align="center" prop="idxSystemName" width="180" />
        <el-table-column label="体系编码" align="center" prop="systemCode" width="150" />
        <el-table-column label="适用对象类型" align="center" prop="applyObjectType" width="150" />
        <el-table-column label="体系版本" align="center" prop="systemVersion" width="120" />
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EvalIdxSystemForm ref="formRef" @success="getList" />
  <!-- 详情抽屉 -->
  <EvalIdxSystemDetailDrawer ref="detailRef" />
</template>
...
import EvalIdxSystemForm from './EvalIdxSystemForm.vue'
import EvalIdxSystemDetailDrawer from './EvalIdxSystemDetailDrawer.vue'

/** 指标体系管理 列表 */
defineOptions({ name: 'EvalIdxSystem' })
const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化
const loading = ref(true) // 列表的加载中
const list = ref<EvalIdxSystemVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EvalIdxSystemApi.getEvalIdxSystemPage(queryParams)
...
/** 详情操作 */
const openDetail = (row: EvalIdxSystemVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EvalIdxSystemApi.deleteEvalIdxSystem(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EvalIdxSystemApi.exportEvalIdxSystem(queryParams)
    download.excel(data, '指标体系管理.xls')
  } catch {
  } finally {
