  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">结果报表详情</span>
    ...
    <div class="p-4 bg-gray-50 min-h-[calc(100vh-120px)]">
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-blue-600">{{ formData.resultRptId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">报表名称：</span>
              <span class="value font-bold text-blue-600">{{ formData.resultRptName || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">报表编码：</span>
              <span class="value font-bold text-blue-600">{{ formData.resultRptCode || '-' }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-pink-600">{{ formData.evalTaskName || '-' }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-green-50 text-green-700">报表配置信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">报表类型：</span>
              <span class="value font-bold text-green-600">{{ formData.rptType || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">统计维度：</span>
              <span class="value font-bold text-green-600">{{
                formData.statDimension || '-'
              }}</span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-green-600">{{ formData.regionName || '-' }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-yellow-50 text-yellow-700">报表状态信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
            ...
            <div class="info-item">
              <span class="label">报表状态：</span>
              <span class="value">
                <el-tag :type="['GENERATED'].includes(formData.rptStatus) ? 'success' : 'danger'">
                  {{ formData.rptStatus === 'GENERATED' ? '已生成' : '-' }}
                </el-tag>
              </span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <span class="value">{{ formData.filePath || '-' }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow" span="2">
          <template #header>
            <div class="card-header bg-purple-50 text-purple-700">报表字段信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">报表字段：</span>
              <span class="value whitespace-pre-line">{{ formData.rptFields || '-' }}</span>
            </div>
          </div>
        </el-card>
...
import { EvalResultRptVO } from '@/api/dataHub/comprehensiveEval/evalResultApply/evalResultReportMgmt'

/** 结果报表详情抽屉 */
defineOptions({ name: 'EvalResultRptDetailDrawer' })
...
const formData = ref<EvalResultRptVO>({
  id: 0,
  resultRptId: '',
  resultRptName: '',
  resultRptCode: '',
...
/** 打开抽屉 */
const open = (row: EvalResultRptVO) => {
  formData.value = { ...row }
