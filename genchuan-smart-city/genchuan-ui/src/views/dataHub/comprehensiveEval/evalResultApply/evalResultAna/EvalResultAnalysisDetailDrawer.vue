  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">评价结果分析详情</span>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              <span class="label">分析ID：</span>
              <span class="value font-bold text-blue-600">{{
                formData.resultAnalysisId || '-'
              }}</span>
            </div>
            <div class="info-item">
              <span class="label">分析名称：</span>
              <span class="value font-bold text-blue-600">{{
                formData.resultAnalysisName || '-'
              }}</span>
            </div>
            <div class="info-item">
              <span class="label">分析编码：</span>
              <span class="value font-bold text-blue-600">{{
                formData.resultAnalysisCode || '-'
              }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-green-50 text-green-700">分析配置信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">分析维度：</span>
              <span class="value font-bold text-green-600">{{
                formData.analysisDimension || '-'
              }}</span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-green-600">{{ formData.idxItemName || '-' }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-yellow-50 text-yellow-700">分析参数信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">时间范围：</span>
              <span class="value font-bold text-yellow-600">{{ formData.timeRange || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">图表类型：</span>
              <span class="value font-bold text-yellow-600">{{ formData.chartType || '-' }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow" span="2">
          <template #header>
            <div class="card-header bg-purple-50 text-purple-700">分析结果详情</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">分析结果：</span>
              <span class="value whitespace-pre-line">{{ formData.analysisResult || '-' }}</span>
            </div>
          </div>
        </el-card>
...
import { EvalResultAnalysisVO } from '@/api/dataHub/comprehensiveEval/evalResultApply/evalResultAna'

/** 评价结果分析详情抽屉 */
defineOptions({ name: 'EvalResultAnalysisDetailDrawer' })
...
const formData = ref<EvalResultAnalysisVO>({
  id: 0,
  resultAnalysisId: '',
  resultAnalysisName: '',
  resultAnalysisCode: '',
...
/** 打开抽屉 */
const open = (row: EvalResultAnalysisVO) => {
  formData.value = { ...row }
