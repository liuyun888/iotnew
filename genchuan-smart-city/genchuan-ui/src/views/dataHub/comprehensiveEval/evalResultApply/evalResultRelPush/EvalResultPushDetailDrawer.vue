  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">结果关联推送详情</span>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              <span class="label">推送ID：</span>
              <span class="value font-bold text-blue-600">{{ formData.resultPushId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">推送编号：</span>
              <span class="value font-bold text-blue-600">{{
                formData.resultPushCode || '-'
              }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-green-50 text-green-700">推送配置信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">推送目标：</span>
              <span class="value font-bold text-green-600">{{ formData.pushTarget || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">推送方式：</span>
              <span class="value font-bold text-green-600">{{ formData.pushMethod || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">推送地址：</span>
              <span class="value">{{ formData.pushAddress || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">推送时间：</span>
              <span class="value font-bold text-green-600">{{
                formatDate(formData.pushTime)
              }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-yellow-50 text-yellow-700">推送状态信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">推送状态：</span>
              <span class="value">
                <el-tag :type="['SUCCESS'].includes(formData.pushStatus) ? 'success' : 'danger'">
                  {{ formData.pushStatus === 'SUCCESS' ? '成功' : '失败' }}
                </el-tag>
              </span>
            </div>
            <div class="info-item">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow" span="2">
          <template #header>
            <div class="card-header bg-purple-50 text-purple-700">推送内容详情</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">推送内容：</span>
              <span class="value whitespace-pre-line">{{ formData.pushContent || '-' }}</span>
            </div>
          </div>
        </el-card>
...
import { EvalResultPushVO } from '@/api/dataHub/comprehensiveEval/evalResultApply/evalResultRelPush'

/** 结果关联推送详情抽屉 */
defineOptions({ name: 'EvalResultPushDetailDrawer' })
...
const formData = ref<EvalResultPushVO>({
  id: 0,
  resultPushId: '',
  resultPushCode: '',
  resultArchId: '',
...
/** 打开抽屉 */
const open = (row: EvalResultPushVO) => {
  formData.value = { ...row }
