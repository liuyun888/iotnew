  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">报告归档详情</span>
    ...
    <div class="p-4 bg-gray-50 min-h-[calc(100vh-120px)]">
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-blue-600">{{ formData.rptArchId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">归档编号：</span>
              <span class="value font-bold text-blue-600">{{ formData.rptArchCode || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">报告版本：</span>
              <span class="value font-bold text-blue-600">{{ formData.rptVersion || '-' }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-pink-50 text-pink-700">关联报告信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">关联报告ID：</span>
              <span class="value">{{ formData.evalRptId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">报告名称：</span>
              <span class="value font-bold text-pink-600">{{ formData.evalRptName || '-' }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-yellow-50 text-yellow-700">归档配置信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">归档位置：</span>
              <span class="value">{{ formData.archLocation || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">归档方式：</span>
              <span class="value font-bold text-yellow-600">{{ formData.archMethod || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">归档时间：</span>
              <span class="value font-bold text-yellow-600">{{
                formatDate(formData.archTime)
              }}</span>
            </div>
            <div class="info-item">
              <span class="label">归档状态：</span>
              <span class="value">
                <el-tag :type="['SUCCESS'].includes(formData.archStatus) ? 'success' : 'danger'">
                  {{ formData.archStatus === 'SUCCESS' ? '成功' : '失败' }}
                </el-tag>
              </span>
            </div>
          </div>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow" span="2">
          <template #header>
            <div class="card-header bg-red-50 text-red-700">异常信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">失败原因：</span>
              <span class="value">{{ formData.failReason || '无' }}</span>
            </div>
          </div>
        </el-card>
...
import { EvalRptArchVO } from '@/api/dataHub/comprehensiveEval/evalReportMgmt/evalReportFileExport'

/** 报告归档详情抽屉 */
defineOptions({ name: 'EvalRptArchDetailDrawer' })
...
const formData = ref<EvalRptArchVO>({
  id: 0,
  rptArchId: '',
  rptArchCode: '',
  evalRptId: '',
