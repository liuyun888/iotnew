<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="报告编码" prop="evalRptCode">
        <el-input
          v-model="queryParams.evalRptCode"
          placeholder="请输入报告编码"
          clearable
          @keyup.enter="handleQuery"
          class="!w-200px rounded-md"
  ...
  <ContentWrap class="mt-4">
    <el-card shadow="hover" class="rounded-lg">
      ...
      >
        ...
        <el-table-column label="报告编码" align="center" prop="evalRptCode" width="150" />
        <el-table-column label="关联模板名称" align="center" prop="rptTemplateName" width="180" />
        <el-table-column label="评价对象名称" align="center" prop="evalObjectName" width="180" />
        <el-table-column label="报告状态" align="center" prop="rptStatus" width="120">
          <template #default="scope">
            <el-tag :type="['GENERATED'].includes(scope.row.rptStatus) ? 'success' : 'warning'">
              {{ scope.row.rptStatus === 'GENERATED' ? '已生成' : '处理中' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EvalRptForm ref="formRef" @success="getList" />
  <!-- 详情抽屉 -->
  <EvalRptDetailDrawer ref="detailRef" />
</template>
...
import EvalRptForm from './EvalRptForm.vue'
import EvalRptDetailDrawer from './EvalRptDetailDrawer.vue'

/** 评价报告 列表 */
defineOptions({ name: 'EvalRpt' })
const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化
const loading = ref(true) // 列表的加载中
const list = ref<EvalRptVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EvalRptApi.getEvalRptPage(queryParams)
...
/** 详情操作 */
const openDetail = (row: EvalRptVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EvalRptApi.deleteEvalRpt(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EvalRptApi.exportEvalRpt(queryParams)
    download.excel(data, '评价报告.xls')
  } catch {
  } finally {
