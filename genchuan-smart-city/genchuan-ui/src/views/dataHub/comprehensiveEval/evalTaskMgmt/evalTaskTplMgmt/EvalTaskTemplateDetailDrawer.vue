  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">任务模板详情</span>
    ...
    <div class="p-4 bg-gray-50 min-h-[calc(100vh-120px)]">
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            <div class="info-item">
              <span class="label">关联指标体系ID：</span>
              <span class="value">{{ formData.idxSystemId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">关联指标体系名称：</span>
              <span class="value font-bold text-pink-600">{{ formData.idxSystemName || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">关联评价主体ID：</span>
              <span class="value">{{ formData.evalSubjectId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">关联评价主体名称：</span>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-green-50 text-green-700">周期配置信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">任务周期：</span>
              <span class="value font-bold text-green-600">{{ formData.taskCycle || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">自定义周期：</span>
              <span class="value font-bold text-green-600">{{ formData.customCycle || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">模板描述：</span>
              <span class="value">{{ formData.templateDesc || '-' }}</span>
            </div>
            <div class="info-item">
...
import { EvalTaskTemplateVO } from '@/api/dataHub/comprehensiveEval/evalTaskMgmt/evalTaskTplMgmt'

/** 任务模板详情抽屉 */
defineOptions({ name: 'EvalTaskTemplateDetailDrawer' })
...
const formData = ref<EvalTaskTemplateVO>({
  id: 0,
  taskTemplateId: '',
  taskTemplateName: '',
  taskTemplateCode: '',
...
/** 打开抽屉 */
const open = (row: EvalTaskTemplateVO) => {
  formData.value = { ...row }
