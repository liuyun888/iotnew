<template>
  <ContentWrap>
    ...
    >
      <el-form-item label="模板名称" prop="taskTemplateName">
        <el-input
          v-model="queryParams.taskTemplateName"
          placeholder="请输入模板名称"
          clearable
      ...
      <el-form-item label="模板编码" prop="taskTemplateCode">
        <el-input
          v-model="queryParams.taskTemplateCode"
          placeholder="请输入模板编码"
          clearable
          @keyup.enter="handleQuery"
          class="!w-200px rounded-md"
      ...
      <el-form-item label="关联指标体系" prop="idxSystemName">
        <el-input
  ...
  <ContentWrap class="mt-4">
    <el-card shadow="hover" class="rounded-lg">
      ...
      >
        ...
        <el-table-column label="模板名称" align="center" prop="taskTemplateName" width="180" />
        <el-table-column label="模板编码" align="center" prop="taskTemplateCode" width="150" />
        <el-table-column label="适用对象类型" align="center" prop="applyObjectType" width="150" />
        <el-table-column label="关联指标体系" align="center" prop="idxSystemName" width="180" />
        <el-table-column label="关联评价主体" align="center" prop="evalSubjectName" width="180" />
        <el-table-column label="任务周期" align="center" prop="taskCycle" width="120" />
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EvalTaskTemplateForm ref="formRef" @success="getList" />
  <!-- 详情抽屉 -->
  <EvalTaskTemplateDetailDrawer ref="detailRef" />
</template>
...
import EvalTaskTemplateForm from './EvalTaskTemplateForm.vue'
import EvalTaskTemplateDetailDrawer from './EvalTaskTemplateDetailDrawer.vue'

/** 任务模板管理 列表 */
defineOptions({ name: 'EvalTaskTemplate' })
const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化
const loading = ref(true) // 列表的加载中
const list = ref<EvalTaskTemplateVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EvalTaskTemplateApi.getEvalTaskTemplatePage(queryParams)
...
/** 详情操作 */
const openDetail = (row: EvalTaskTemplateVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EvalTaskTemplateApi.deleteEvalTaskTemplate(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EvalTaskTemplateApi.exportEvalTaskTemplate(queryParams)
    download.excel(data, '任务模板管理.xls')
  } catch {
  } finally {
