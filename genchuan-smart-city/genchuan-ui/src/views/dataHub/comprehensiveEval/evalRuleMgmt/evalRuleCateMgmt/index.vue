<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="分类编码" prop="ruleCatCode">
        <el-input
          v-model="queryParams.ruleCatCode"
          placeholder="请输入分类编码"
          clearable
          @keyup.enter="handleQuery"
          class="!w-200px rounded-md"
  ...
  <ContentWrap class="mt-4">
    <el-card shadow="hover" class="rounded-lg">
      ...
      >
        ...
        <el-table-column label="规则分类名称" align="center" prop="ruleCatName" width="180" />
        <el-table-column label="分类编码" align="center" prop="ruleCatCode" width="150" />
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EvalRuleCatForm ref="formRef" @success="getList" />
  <!-- 详情抽屉 -->
  <EvalRuleCatDetailDrawer ref="detailRef" />
</template>
...
import EvalRuleCatForm from './EvalRuleCatForm.vue'
import EvalRuleCatDetailDrawer from './EvalRuleCatDetailDrawer.vue'

/** 规则分类管理 列表 */
defineOptions({ name: 'EvalRuleCat' })
const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化
const loading = ref(true) // 列表的加载中
const list = ref<EvalRuleCatVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EvalRuleCatApi.getEvalRuleCatPage(queryParams)
...
/** 详情操作 */
const openDetail = (row: EvalRuleCatVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EvalRuleCatApi.deleteEvalRuleCat(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EvalRuleCatApi.exportEvalRuleCat(queryParams)
    download.excel(data, '规则分类管理.xls')
  } catch {
  } finally {
