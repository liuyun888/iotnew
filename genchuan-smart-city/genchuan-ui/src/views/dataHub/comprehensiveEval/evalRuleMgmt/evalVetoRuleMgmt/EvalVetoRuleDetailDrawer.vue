  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">否决项规则详情</span>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              <span class="label">否决项ID：</span>
              <span class="value font-bold text-blue-600">{{ formData.vetoRuleId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">否决项名称：</span>
              <span class="value font-bold text-blue-600">{{ formData.vetoRuleName || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">否决项编码：</span>
              <span class="value font-bold text-blue-600">{{ formData.vetoRuleCode || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">适用对象类型：</span>
              <span class="value font-bold text-blue-600">{{
                formData.applyObjectType || '-'
              }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-pink-50 text-pink-700">关联指标信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
            ...
            <div class="info-item">
              <span class="label">指标阈值：</span>
              <span class="value font-bold text-pink-600">{{ formData.idxThreshold || '-' }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-green-50 text-green-700">否决规则配置</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">否决条件：</span>
              <span class="value">{{ formData.vetoCondition || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">生效周期：</span>
              <span class="value font-bold text-green-600">{{ formData.validCycle || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">否决结果：</span>
              <span class="value font-bold text-green-600">{{ formData.vetoResult || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">否决描述：</span>
              <span class="value">{{ formData.vetoDesc || '-' }}</span>
            </div>
            <div class="info-item">
...
import { EvalVetoRuleVO } from '@/api/dataHub/comprehensiveEval/evalRuleMgmt/evalVetoRuleMgmt'

/** 否决项规则详情抽屉 */
defineOptions({ name: 'EvalVetoRuleDetailDrawer' })
...
const formData = ref<EvalVetoRuleVO>({
  id: 0,
  vetoRuleId: '',
  vetoRuleName: '',
  vetoRuleCode: '',
