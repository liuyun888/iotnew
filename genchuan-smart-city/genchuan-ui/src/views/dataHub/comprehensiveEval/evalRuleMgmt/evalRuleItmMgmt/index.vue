<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="规则项编码" prop="ruleItemCode">
        <el-input
          v-model="queryParams.ruleItemCode"
      ...
      <el-form-item label="所属规则分类" prop="ruleCatName">
        <el-input
          v-model="queryParams.ruleCatName"
  ...
  <ContentWrap class="mt-4">
    <el-card shadow="hover" class="rounded-lg">
      ...
      >
        ...
        <el-table-column label="规则项名称" align="center" prop="ruleItemName" width="180" />
        <el-table-column label="规则项编码" align="center" prop="ruleItemCode" width="150" />
        <el-table-column label="所属规则分类" align="center" prop="ruleCatName" width="180" />
        <el-table-column label="关联指标项" align="center" prop="idxItemName" width="180" />
        <el-table-column label="满分值" align="center" prop="fullScore" width="100">
          <template #default="scope">
            <span class="font-bold text-green-600">{{ scope.row.fullScore || 0 }}</span>
          </template>
        </el-table-column>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EvalRuleItemForm ref="formRef" @success="getList" />
  <!-- 详情抽屉 -->
  <EvalRuleItemDetailDrawer ref="detailRef" />
</template>
...
import EvalRuleItemForm from './EvalRuleItemForm.vue'
import EvalRuleItemDetailDrawer from './EvalRuleItemDetailDrawer.vue'

/** 规则项管理 列表 */
defineOptions({ name: 'EvalRuleItem' })
const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化
const loading = ref(true) // 列表的加载中
const list = ref<EvalRuleItemVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EvalRuleItemApi.getEvalRuleItemPage(queryParams)
...
/** 详情操作 */
const openDetail = (row: EvalRuleItemVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EvalRuleItemApi.deleteEvalRuleItem(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EvalRuleItemApi.exportEvalRuleItem(queryParams)
    download.excel(data, '规则项管理.xls')
  } catch {
  } finally {
