  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">结果存档详情</span>
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-blue-600">{{ formData.resultArchId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">存档编号：</span>
              <span class="value font-bold text-blue-600">{{ formData.archCode || '-' }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              <span class="label">最终评价等级：</span>
              <el-tag
                :type="
                  formData.finalEvalLevel === 'A'
                    ? 'success'
                    : formData.finalEvalLevel === 'C'
                      ? 'warning'
                      : 'primary'
                "
              >
                {{ formData.finalEvalLevel || '-' }}
              </el-tag>
            </div>
            <div class="info-item">
              <span class="label">最终评价得分：</span>
              <span
                class="font-bold"
                :class="{
                  'text-green-600': formData.finalEvalLevel === 'A',
                  'text-blue-600': formData.finalEvalLevel === 'B',
                  'text-orange-600': formData.finalEvalLevel === 'C'
                }"
                >{{ formData.finalEvalScore || 0 }}</span
              >
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-green-50 text-green-700">存档配置信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">存档原因：</span>
              <span class="value">{{ formData.archReason || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">存档时间：</span>
              <span class="value font-bold text-green-600">{{
                formatDate(formData.archTime)
              }}</span>
            </div>
            <div class="info-item">
              <span class="label">存档附件：</span>
              <span class="value">{{ formData.archAttachs || '-' }}</span>
            </div>
          </div>
        </el-card>
...
import { EvalResultArchVO } from '@/api/dataHub/comprehensiveEval/evalResultMgmt/evalResultFilingMgmt'

/** 结果存档详情抽屉 */
defineOptions({ name: 'EvalResultArchDetailDrawer' })
...
const formData = ref<EvalResultArchVO>({
  id: 0,
  resultArchId: '',
  archCode: '',
  evalTaskId: '',
...
/** 打开抽屉 */
const open = (row: EvalResultArchVO) => {
  formData.value = { ...row }
