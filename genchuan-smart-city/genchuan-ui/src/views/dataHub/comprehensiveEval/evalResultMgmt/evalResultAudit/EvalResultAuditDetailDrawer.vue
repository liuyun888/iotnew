  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">结果审核详情</span>
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-blue-600">{{ formData.resultAuditId || '-' }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              <span class="label">自动计算结果：</span>
              <span class="value font-bold text-pink-600">{{
                formData.autoCalcResult || '-'
              }}</span>
            </div>
            <div class="info-item">
              <span class="label">自动计算得分：</span>
              <span
                class="font-bold"
                :class="{
                  'text-green-600': formData.auditStatus === 'APPROVED',
                  'text-red-600': formData.auditStatus === 'REJECTED',
                  'text-orange-600': formData.auditStatus === 'PENDING'
                }"
                >{{ formData.autoCalcScore || 0 }}</span
              >
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-green-50 text-green-700">审核信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-green-600">{{ formData.auditorName || '-' }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-yellow-50 text-yellow-700">审核意见</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">审核意见：</span>
              <span class="value whitespace-pre-line">{{ formData.auditOpinion || '-' }}</span>
            </div>
          </div>
        </el-card>
...
import { EvalResultAuditVO } from '@/api/dataHub/comprehensiveEval/evalResultMgmt/evalResultAudit'

/** 结果审核详情抽屉 */
defineOptions({ name: 'EvalResultAuditDetailDrawer' })
...
const formData = ref<EvalResultAuditVO>({
  id: 0,
  resultAuditId: '',
  evalTaskId: '',
  evalTaskName: '',
...
/** 打开抽屉 */
const open = (row: EvalResultAuditVO) => {
  formData.value = { ...row }
