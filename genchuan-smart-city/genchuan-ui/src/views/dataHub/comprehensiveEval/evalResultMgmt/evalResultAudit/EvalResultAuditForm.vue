<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="800px">
    ...
    >
      ...
      <el-form-item label="自动计算结果" prop="autoCalcResult">
        <el-input
          v-model="formData.autoCalcResult"
          placeholder="请输入自动计算结果"
          class="w-full rounded-md"
        />
      </el-form-item>
      <el-form-item label="自动计算得分" prop="autoCalcScore">
        <el-input
          v-model="formData.autoCalcScore"
          placeholder="请输入自动计算得分"
          type="number"
          class="w-full rounded-md"
        />
      </el-form-item>
...
/** 结果审核 表单 */
defineOptions({ name: 'EvalResultAuditForm' })
const { t } = useI18n() // 国际化
...
const formData = ref({
  ...
  autoCalcResult: undefined,
  autoCalcScore: undefined,
...
const formRules = reactive({
  ...
  autoCalcResult: [{ required: true, message: '自动计算结果不能为空', trigger: 'blur' }],
  autoCalcScore: [
    { required: true, message: '自动计算得分不能为空', trigger: 'blur' },
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const res = await EvalResultAuditApi.getEvalResultAudit(id)
      formData.value = {
        ...res,
        // 只保留需要展示的字段
        auditOpinion: res.auditOpinion || '',
        autoCalcScore: res.autoCalcScore || 0
      }
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as EvalResultAuditVO
    if (formType.value === 'create') {
      await EvalResultAuditApi.createEvalResultAudit(data)
      message.success(t('common.createSuccess'))
    } else {
      await EvalResultAuditApi.updateEvalResultAudit(data)
