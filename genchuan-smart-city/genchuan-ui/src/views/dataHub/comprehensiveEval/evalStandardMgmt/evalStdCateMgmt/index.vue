<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="分类编码" prop="stdCatCode">
        <el-input
          v-model="queryParams.stdCatCode"
          placeholder="请输入分类编码"
          clearable
          @keyup.enter="handleQuery"
          class="!w-200px rounded-md"
      ...
      <el-form-item label="适用指标体系" prop="idxSystemName">
        <el-input
  ...
  <ContentWrap class="mt-4">
    <el-card shadow="hover" class="rounded-lg">
      ...
      >
        ...
        <el-table-column label="标准分类名称" align="center" prop="stdCatName" width="180" />
        <el-table-column label="分类编码" align="center" prop="stdCatCode" width="150" />
        <el-table-column label="适用指标体系" align="center" prop="idxSystemName" width="180" />
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EvalStdCatForm ref="formRef" @success="getList" />
  <!-- 详情抽屉 -->
  <EvalStdCatDetailDrawer ref="detailRef" />
</template>
...
import EvalStdCatForm from './EvalStdCatForm.vue'
import EvalStdCatDetailDrawer from './EvalStdCatDetailDrawer.vue'

/** 标准分类管理 列表 */
defineOptions({ name: 'EvalStdCat' })
const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化
const loading = ref(true) // 列表的加载中
const list = ref<EvalStdCatVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EvalStdCatApi.getEvalStdCatPage(queryParams)
...
/** 详情操作 */
const openDetail = (row: EvalStdCatVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EvalStdCatApi.deleteEvalStdCat(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EvalStdCatApi.exportEvalStdCat(queryParams)
    download.excel(data, '标准分类管理.xls')
  } catch {
  } finally {
