  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">申诉详情</span>
    ...
    <div class="p-4 bg-gray-50 min-h-[calc(100vh-120px)]">
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          ...
          <div class="card-content">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-blue-600">{{ formData.appealId || '-' }}</span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              <span class="label">关联公示ID：</span>
              <span class="value">{{ formData.resultPublicId || '-' }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-green-50 text-green-700">申诉人信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">申诉人ID：</span>
              <span class="value">{{ formData.appealUserId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">申诉人姓名：</span>
              <span class="value font-bold text-green-600">{{
                formData.appealUserName || '-'
              }}</span>
            </div>
            <div class="info-item">
              <span class="label">提交时间：</span>
              <span class="value font-bold text-green-600">{{
                formatDate(formData.submitTime)
              }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-yellow-50 text-yellow-700">申诉状态信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">申诉状态：</span>
              <span class="value">
                <el-tag :type="formData.appealStatus === 'SUBMITTED' ? 'success' : 'warning'">
                  {{ formData.appealStatus === 'SUBMITTED' ? '已提交' : '处理中' }}
                </el-tag>
              </span>
            </div>
            <div class="info-item">
              <span class="label">证明材料：</span>
              <span class="value">{{ formData.proofMats || '-' }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow" span="2">
          <template #header>
            <div class="card-header bg-purple-50 text-purple-700">申诉理由</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">申诉理由：</span>
              <span class="value whitespace-pre-line">{{ formData.appealReason || '-' }}</span>
            </div>
          </div>
        </el-card>
...
import { EvalAppealVO } from '@/api/dataHub/comprehensiveEval/evalAppealReview/evalAppealMgmt'

/** 申诉详情抽屉 */
defineOptions({ name: 'EvalAppealDetailDrawer' })
...
const formData = ref<EvalAppealVO>({
  id: 0,
  appealId: '',
  appealCode: '',
  resultPublicId: '',
