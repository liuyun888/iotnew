  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">复核结果反馈详情</span>
    ...
    <div class="p-4 bg-gray-50 min-h-[calc(100vh-120px)]">
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-blue-50 text-blue-700">基础关联信息</div>
          </template>
          <div class="card-content">
            ...
            <div class="info-item">
              ...
              <span class="value font-bold text-blue-600">{{
                formData.reviewFeedbackId || '-'
              }}</span>
            </div>
            <div class="info-item">
              <span class="label">关联复核ID：</span>
              <span class="value">{{ formData.reviewId || '-' }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-green-50 text-green-700">评价得分对比</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">原评价等级：</span>
              <el-tag
                :type="
                  formData.originalEvalLevel === 'A'
                    ? 'success'
                    : formData.originalEvalLevel === 'C'
                      ? 'warning'
                      : 'primary'
                "
              >
                {{ formData.originalEvalLevel || '-' }}
              </el-tag>
            </div>
            <div class="info-item">
              <span class="label">原评价得分：</span>
              <span
                class="font-bold"
                :class="{
                  'text-green-600': formData.originalEvalLevel === 'A',
                  'text-blue-600': formData.originalEvalLevel === 'B',
                  'text-orange-600': formData.originalEvalLevel === 'C'
                }"
                >{{ formData.originalEvalScore || 0 }}</span
              >
            </div>
            <div class="info-item">
              <span class="label">修正后等级：</span>
              <el-tag
                :type="
                  formData.revisedLevel === 'A'
                    ? 'success'
                    : formData.revisedLevel === 'C'
                      ? 'warning'
                      : 'primary'
                "
              >
                {{ formData.revisedLevel || '-' }}
              </el-tag>
            </div>
            <div class="info-item">
              <span class="label">修正后得分：</span>
              <span
                class="font-bold"
                :class="{
                  'text-green-600': formData.revisedLevel === 'A',
                  'text-blue-600': formData.revisedLevel === 'B',
                  'text-orange-600': formData.revisedLevel === 'C'
                }"
                >{{ formData.revisedScore || 0 }}</span
              >
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow">
          <template #header>
            <div class="card-header bg-yellow-50 text-yellow-700">反馈基本信息</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">最终复核结果：</span>
              <span class="value font-bold text-yellow-600">{{
                formData.finalReviewResult || '-'
              }}</span>
            </div>
            <div class="info-item">
              <span class="label">反馈方式：</span>
              <span class="value font-bold text-yellow-600">{{
                formData.feedbackMethod || '-'
              }}</span>
            </div>
            <div class="info-item">
              <span class="label">反馈时间：</span>
              <span class="value font-bold text-yellow-600">{{
                formatDate(formData.feedbackTime)
              }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-beautify" :shadow="hoverShadow" span="2">
          <template #header>
            <div class="card-header bg-purple-50 text-purple-700">反馈意见详情</div>
          </template>
          <div class="card-content">
            <div class="info-item">
              <span class="label">反馈意见：</span>
              <span class="value whitespace-pre-line">{{ formData.feedbackOpinion || '-' }}</span>
            </div>
          </div>
        </el-card>
...
import { EvalReviewFeedbackVO } from '@/api/dataHub/comprehensiveEval/evalAppealReview/evalReviewResultFeed'

/** 复核结果反馈详情抽屉 */
defineOptions({ name: 'EvalReviewFeedbackDetailDrawer' })
...
const formData = ref<EvalReviewFeedbackVO>({
  id: 0,
  reviewFeedbackId: '',
  reviewId: '',
  appealId: '',
...
/** 打开抽屉 */
const open = (row: EvalReviewFeedbackVO) => {
  formData.value = { ...row }
