<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column label="资产总数" align="center" prop="totalAssetCount" />
      <el-table-column label="在用资产数" align="center" prop="inUseAssetCount" />
      <el-table-column label="资产在用率" align="center" prop="assetInUseRate" />
      <el-table-column label="维护总次数" align="center" prop="totalMntCount" />
      <el-table-column
        label="报表生成时间"
        align="center"
        prop="rptCreateTime"
  ...
  <!-- 表单弹窗：添加/修改 -->
  <DomAssetMonRptForm ref="formRef" @success="getList" />

  <!-- 详情抽屉 -->
  <DomAssetMonRptDetail ref="detailRef" />
</template>
...
import DomAssetMonRptForm from './DomAssetMonRptForm.vue'
import DomAssetMonRptDetail from './DomAssetMonRptDetail.vue'

/** 资产分域监测报表 列表 */
defineOptions({ name: 'DomAssetMonRpt' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<DomAssetMonRptVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await DomAssetMonRptApi.getDomAssetMonRptPage(queryParams)
...
/** 详情操作 */
const detailRef = ref()
const openDetail = (row: DomAssetMonRptVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await DomAssetMonRptApi.deleteDomAssetMonRpt(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await DomAssetMonRptApi.exportDomAssetMonRpt(queryParams)
    download.excel(data, '资产分域监测报表.xls')
  } catch {
  } finally {
