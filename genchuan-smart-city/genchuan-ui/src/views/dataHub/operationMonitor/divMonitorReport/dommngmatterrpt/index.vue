<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="管理事项" prop="mngMatterMajorName">
        <el-input
          v-model="queryParams.mngMatterMajorName"
      ...
      <el-form-item label="处置部门" prop="deptName">
        <el-input
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column label="管理事项大类" align="center" prop="mngMatterMajorName" width="160" />
      <el-table-column label="管理事项小类" align="center" prop="mngMatterMinorName" width="160" />
      <el-table-column label="分域名称" align="center" prop="domName" width="120" />
      <el-table-column label="处置部门" align="center" prop="deptName" width="140" />
      <el-table-column label="事件总数" align="center" prop="totalEvtCount" width="100" />
      <el-table-column label="已办结事件数" align="center" prop="completedEvtCount" width="120" />
  ...
  <!-- 表单弹窗：添加/修改 -->
  <DomMngMatterRptForm ref="formRef" @success="getList" />

  <!-- 详情抽屉 -->
  <DomMngMatterRptDetail ref="detailRef" :visible="detailVisible" @close="detailVisible = false" />
</template>
...
import {
  DomMngMatterRptApi,
  DomMngMatterRptVO
} from '@/api/dataHub/operationMonitor/divMonitorReport/dommngmatterrpt'
import DomMngMatterRptForm from './DomMngMatterRptForm.vue'
import DomMngMatterRptDetail from './DomMngMatterRptDetail.vue'

/** 管理事项分域监测报表 列表 */
defineOptions({ name: 'DomMngMatterRpt' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<DomMngMatterRptVO[]>([]) // 列表的数据
...
const queryParams = reactive({
  ...
  mngMatterMajorName: undefined,
  mngMatterMinorId: undefined,
  mngMatterMinorName: undefined,
  domId: undefined,
  domName: undefined,
  deptCode: undefined,
  deptName: undefined,
...
const detailRef = ref()
const openDetail = (row: DomMngMatterRptVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await DomMngMatterRptApi.getDomMngMatterRptPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await DomMngMatterRptApi.deleteDomMngMatterRpt(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await DomMngMatterRptApi.exportDomMngMatterRpt(queryParams)
    download.excel(data, '管理事项分域监测报表.xls')
  } catch {
  } finally {
