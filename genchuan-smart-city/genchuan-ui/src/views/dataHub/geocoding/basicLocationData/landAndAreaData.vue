    ...
    <div class="cc-main-content">
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="标识码" prop="zoneCode">
            <el-input
          ...
          <el-form-item>
            ...
            <el-button
              ...
              v-hasPermi="['basicLocationData:landAndAreaData:create']"
            >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('空间表示类型')"
          >
            <template #default="scope">
              {{ handleTypeOptions(scope.row.spaceType, OptionsAll.spaceTypeOptions) }}
            </template>
          </el-table-column>
          <el-table-column
            label="数据来源"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['basicLocationData:landAndAreaData:query']"
                >详细
              </el-button>
              <el-button
                type="primary"
                ...
                v-hasPermi="['basicLocationData:landAndAreaData:update']"
                >编辑
              </el-button>
              <el-button
                type="danger"
                ...
                v-hasPermi="['basicLocationData:landAndAreaData:delete']"
                >删除
              </el-button>
            </template>
          </el-table-column>
        ...
        >
          ...
          <div class="drawer-content">
            ...
            <template v-if="drawerConfig.active == 1">
              ...
              >
                <ul>
                  <li class="width50"><span>名称：</span>{{ queryData.zoneName }}</li>
                  <li class="width50"><span>标识码：</span>{{ queryData.zoneCode }}</li>
                  <li class="width50"><span>别名：</span>{{ queryData.alias }}</li>
                  <li class="width50"><span>曾用名：</span>{{ queryData.oldName }}</li>
                  <li class="width50"><span>四至范围：</span>{{ queryData.boundary }}</li>
                  <li class="width50"><span>街道名称：</span>{{ queryData.streetName }}</li>
                  ...
                  <li class="width50">
                    <span>空间表示类型：</span
                    >{{ handleTypeOptions(queryData.spaceType, OptionsAll.spaceTypeOptions) }}
                  </li>
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                ...
                >
                  <el-form-item
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入名称" v-model="formData.zoneName" clearable />
                  </el-form-item>
                  <el-form-item
                    label="标识码："
                    prop="zoneCode"
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入标识码" v-model="formData.zoneCode" clearable />
                  </el-form-item>
                  <el-form-item
                    label="别名："
                    prop="alias"
                  ...
                  <el-form-item
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入四至范围" v-model="formData.boundary" clearable />
                  </el-form-item>
                  <el-form-item
                    label="街道名称："
                    prop="streetName"
                  ...
                  >
                    <el-select
                      placeholder="请选择空间表示类型"
                      v-model="formData.spaceType"
                      filterable
                      clearable
                    >
                      <el-option
                        v-for="item in OptionsAll.spaceTypeOptions"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                      />
...
import * as landAndAreaDataApi from '@/api/dataHub/geocoding/basicLocationData/landAndAreaData'
...
const tableInitData = async () => {
  ...
  try {
    ...
    const res = await landAndAreaDataApi.listPage(merged)
  ...
  if (valid) {
    try {
      ...
      if (drawerConfig.value.type === 'add' && !formData.value.id) {
        await landAndAreaDataApi.createData(data)
        ElMessage.success('保存成功')
      } else {
        await landAndAreaDataApi.updateData(data)
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '地片与区片数据-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  drawerConfig.value.title = '编辑地片与区片数据'
}
// 点击删除按钮
const delClick = (row) => {
  ElMessageBox.confirm('请注意：确认后，所有相关数据将不可恢复。', `确定要删除吗？`, {
    ...
    .then(async () => {
      try {
        await landAndAreaDataApi.delData(row.id)
        await tableInitData()
        ElMessage({
          type: 'success',
          message: '删除成功'
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增地片与区片数据'
  drawerConfig.value.active = '1'
}
//drawer 配置
