    ...
    <div class="cc-main-content">
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="标识码" prop="houseCode">
            <el-input
          ...
          <el-form-item label="地片/区片" prop="zoneName">
            <el-input
              v-model="searchForm.zoneName"
              placeholder="请输入地片/区片"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
          ...
          <el-form-item label="门(楼)牌号" prop="houseNum">
            <el-input
              ...
              placeholder="请输入门(楼)牌号"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
          </el-form-item>
          <el-form-item label="街道名称" prop="parentStreetName">
            <el-input
              v-model="searchForm.parentStreetName"
              placeholder="请输入街道名称"
              clearable
              class="!w-240px"
              @change="searchClick"
          ...
          <el-form-item>
            ...
            <el-button
              ...
              v-hasPermi="['basicLocationData:gateBuildingNumber:create']"
            >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            prop="houseNum"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('门(楼)牌号')"
          />
          <el-table-column
            label="街道名称"
            min-width="150"
            align="center"
            prop="parentStreetName"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('街道名称')"
          />
          <el-table-column
            label="街道代码"
            min-width="150"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['basicLocationData:gateBuildingNumber:query']"
                >详细
              </el-button>
              <el-button
                type="primary"
                ...
                v-hasPermi="['basicLocationData:gateBuildingNumber:update']"
                >编辑
              </el-button>
              <el-button
                type="danger"
                ...
                v-hasPermi="['basicLocationData:gateBuildingNumber:delete']"
                >删除
              </el-button>
            </template>
          </el-table-column>
        ...
        >
          ...
          <div class="drawer-content">
            ...
            <template v-if="drawerConfig.active == 1">
              ...
              >
                <ul>
                  <li class="width50"><span>名称：</span>{{ queryData.houseName }}</li>
                  <li class="width50"><span>标识码：</span>{{ queryData.houseCode }}</li>
                  <li class="width50"><span>北斗网格位置码：</span>{{ queryData.bdGridCode }}</li>
                  <li class="width50"><span>街巷：</span>{{ queryData.streetName }}</li>
                  <li class="width50"><span>地片/区片：</span>{{ queryData.zoneName }}</li>
                  <li class="width50"><span>门(楼)牌号：</span>{{ queryData.houseNum }}</li>
                  <li class="width50"><span>街道名称：</span>{{ queryData.parentStreetName }}</li>
                  <li class="width50"><span>街道代码：</span>{{ queryData.streetCode }}</li>
                  <li class="width50"><span>社区名称：</span>{{ queryData.communityName }}</li>
                  <li class="width50"><span>社区代码：</span>{{ queryData.communityCode }}</li>
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                ...
                >
                  <el-form-item
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入名称" v-model="formData.houseName" clearable />
                  </el-form-item>
                  <el-form-item
                    label="标识码："
                    prop="houseCode"
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入标识码" v-model="formData.houseCode" clearable />
                  </el-form-item>
                  <el-form-item
                    label="北斗网格位置码："
                    prop="bdGridCode"
                  ...
                  <el-form-item
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入街巷" v-model="formData.streetName" clearable />
                  </el-form-item>
                  <el-form-item
                    label="地片/区片："
                    prop="zoneName"
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入地片/区片" v-model="formData.zoneName" clearable />
                  </el-form-item>
                  <el-form-item
                    label="门(楼)牌号："
                    prop="houseNum"
                  ...
                  >
                    <el-input
                      placeholder="请输入街道名称"
                      v-model="formData.parentStreetName"
                      clearable
                    />
                  </el-form-item>
                  <el-form-item
...
import * as gateBuildingNumberApi from '@/api/dataHub/geocoding/basicLocationData/gateBuildingNumber'
...
const tableInitData = async () => {
  ...
  try {
    ...
    const res = await gateBuildingNumberApi.listPage(merged)
  ...
  if (valid) {
    try {
      ...
      if (drawerConfig.value.type === 'add' && !formData.value.id) {
        await gateBuildingNumberApi.createData(data)
        ElMessage.success('保存成功')
      } else {
        await gateBuildingNumberApi.updateData(data)
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '门（楼）牌数据-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  drawerConfig.value.title = '编辑门（楼）牌数据'
}
// 点击删除按钮
const delClick = (row) => {
  ElMessageBox.confirm('请注意：确认后，所有相关数据将不可恢复。', `确定要删除吗？`, {
    ...
    .then(async () => {
      try {
        await gateBuildingNumberApi.delData(row.id)
        await tableInitData()
        ElMessage({
          type: 'success',
          message: '删除成功'
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增门（楼）牌数据'
  drawerConfig.value.active = '1'
}
//drawer 配置
...
const tableColumns = ref([
  ...
  { label: '街巷', visible: true },
  { label: '地片/区片', visible: true },
  { label: '门(楼)牌号', visible: true },
  { label: '街道名称', visible: true },
  { label: '街道代码', visible: true },
  { label: '社区名称', visible: true },
  { label: '社区代码', visible: true },
