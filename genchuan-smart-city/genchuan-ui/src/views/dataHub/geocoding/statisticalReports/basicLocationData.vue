<template>
  <div class="cc-main">
    <div class="cc-main-content">
      ...
      <!--右边的按钮-->
      <!--        <div class="right">
                <el-button type="primary" plain :icon="Plus" @click="addClick(titleConfig.addButName)" v-hasPermi="['statisticalReports:basicLocationData:create']">
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="区域数据数量" prop="regionCount">
            <el-input
              v-model="searchForm.regionCount"
              placeholder="请输入区域数据数量"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
          </el-form-item>
          <el-form-item label="地片与区片数据数量" prop="zoneCount">
            <el-input
              v-model="searchForm.zoneCount"
              placeholder="请输入地片与区片数据数量"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
          </el-form-item>
          <el-form-item label="街巷数据数量" prop="streetCount">
            <el-input
              v-model="searchForm.streetCount"
              placeholder="请输入街巷数据数量"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            prop="zoneCount"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('地片与区片数据数量')"
          />
          <el-table-column
            label="街巷数据数量"
            min-width="150"
          ...
          <el-table-column width="100" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['statisticalReports:basicLocationData:query']"
                >详细
              </el-button>
              <!--              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['statisticalReports:basicLocationData:update']">编辑</el-button>
                            <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['statisticalReports:basicLocationData:delete']">删除</el-button>-->
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
            ...
            <template v-if="drawerConfig.active == 1">
              ...
              >
                <ul>
                  ...
                  <li class="width50"><span>区域数据数量：</span>{{ queryData.regionCount }}</li>
                  <li class="width50"
                    ><span>地片与区片数据数量：</span>{{ queryData.zoneCount }}</li
                  >
                  <li class="width50"><span>街巷数据数量：</span>{{ queryData.streetCount }}</li>
                  <li class="width50"><span>门牌楼牌数据数量：</span>{{ queryData.houseCount }}</li>
                  <li class="width50"><span>兴趣点数据数量：</span>{{ queryData.poiCount }}</li>
                  <li class="width50"><span>新增数据数量：</span>{{ queryData.newDataCount }}</li>
                  <li class="width50"
                    ><span>更新数据数量：</span>{{ queryData.updateDataCount }}</li
                  >
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                ...
                >
                  ...
                  >
                    <el-input
                      placeholder="请输入兴趣点数据数量"
                      v-model="formData.poiCount"
                      clearable
                    />
                  </el-form-item>
                  <el-form-item
                  ...
                  >
                    <el-input
                      placeholder="请输入新增数据数量"
                      v-model="formData.newDataCount"
                      clearable
                    />
                  </el-form-item>
                  <el-form-item
                  ...
                  >
                    <el-input
                      placeholder="请输入更新数据数量"
                      v-model="formData.updateDataCount"
                      clearable
                    />
                  </el-form-item>
                  <el-form-item
...
import * as basicLocationDataApi from '@/api/dataHub/geocoding/statisticalReports/basicLocationData'
...
const titleConfig = ref({
  name: '基本地点数据统计报表', //标题名称
  addButName: '新增基本地点数据统计报表' //新增按钮名称
})
// 搜索 字段
...
const tableInitData = async () => {
  ...
  try {
    ...
    const res = await basicLocationDataApi.listPage(merged)
  ...
  if (valid) {
    try {
      ...
      if (drawerConfig.value.type === 'add' && !formData.value.id) {
        await basicLocationDataApi.createData(data)
        ElMessage.success('保存成功')
      } else {
        await basicLocationDataApi.updateData(data)
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '基本地点数据统计报表-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  drawerConfig.value.title = '编辑基本地点数据统计报表'
}
// 点击删除按钮
const delClick = (row) => {
  ElMessageBox.confirm('请注意：确认后，所有相关数据将不可恢复。', `确定要删除吗？`, {
    ...
    .then(async () => {
      try {
        await basicLocationDataApi.delData(row.id)
        await tableInitData()
        ElMessage({
          type: 'success',
          message: '删除成功'
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增基本地点数据统计报表'
  drawerConfig.value.active = '1'
}
//drawer 配置
...
const tableColumns = ref([
  ...
  { label: '区域数据数量', visible: true },
  { label: '地片与区片数据数量', visible: true },
  { label: '街巷数据数量', visible: true },
  { label: '门牌楼牌数据数量', visible: true },
  { label: '兴趣点数据数量', visible: true },
  { label: '新增数据数量', visible: true },
  { label: '更新数据数量', visible: true },
