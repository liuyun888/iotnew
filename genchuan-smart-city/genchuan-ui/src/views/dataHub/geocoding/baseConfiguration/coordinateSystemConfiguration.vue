  ...
  if (valid) {
    try {
      ...
      if (drawerConfig.value.type === 'add' && !formData.value.id) {
        await coordinateSystemConfigurationApi.createData(data)
        ElMessage.success('保存成功')
      } else {
        await coordinateSystemConfigurationApi.updateData(data)
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '坐标系-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  drawerConfig.value.title = '编辑坐标系'
}
// 点击删除按钮
const delClick = (row) => {
