    ...
    <div class="cc-main-content">
      ...
      <div class="cc-search-css" ref="searchRef">
        <el-form
          ...
          label-width="68px"
        >
          <el-form-item label="图层代码" prop="layerCode">
            <el-input
          ...
          <el-form-item>
            ...
            <el-button
              ...
              v-hasPermi="['baseConfiguration:layerCodeConfiguration:create']"
            >
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['baseConfiguration:layerCodeConfiguration:query']"
                >详细
              </el-button>
              <el-button
                type="primary"
                ...
                v-hasPermi="['baseConfiguration:layerCodeConfiguration:update']"
                >编辑
              </el-button>
              <el-button
                type="danger"
                ...
                v-hasPermi="['baseConfiguration:layerCodeConfiguration:delete']"
                >删除
              </el-button>
            </template>
          </el-table-column>
        ...
        >
          ...
          <div class="drawer-content">
            ...
            <template v-if="drawerConfig.active == 1">
              ...
              >
                <ul>
                  <li class="width50"><span>图层代码：</span>{{ queryData.layerCode }}</li>
                  <li class="width50"><span>图层名称：</span>{{ queryData.layerName }}</li>
                  <li class="width50"
                    ><span>启用状态：</span
                  ...
                  >
                  <li class="width50"><span>图层说明：</span>{{ queryData.layerDesc }}</li>
                </ul>
              </div>
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                ...
                >
                  <el-form-item
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入图层代码" v-model="formData.layerCode" clearable />
                  </el-form-item>
                  <el-form-item
                    label="图层名称："
                    prop="layerName"
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入图层名称" v-model="formData.layerName" clearable />
                  </el-form-item>
                  <el-form-item
                    label="启用状态："
                    prop="enableStatus"
...
import * as layerCodeConfigurationApi from '@/api/dataHub/geocoding/baseConfiguration/layerCodeConfiguration'
...
const tableInitData = async () => {
  ...
  try {
    ...
    const res = await layerCodeConfigurationApi.listPage(merged)
  ...
  if (valid) {
    try {
      ...
      if (drawerConfig.value.type === 'add' && !formData.value.id) {
        await layerCodeConfigurationApi.createData(data)
        ElMessage.success('保存成功')
      } else {
        await layerCodeConfigurationApi.updateData(data)
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '图层代码-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  drawerConfig.value.title = '编辑图层代码'
}
// 点击删除按钮
const delClick = (row) => {
  ElMessageBox.confirm('请注意：确认后，所有相关数据将不可恢复。', `确定要删除吗？`, {
    ...
    .then(async () => {
      try {
        await layerCodeConfigurationApi.delData(row.id)
        await tableInitData()
        ElMessage({
          type: 'success',
          message: '删除成功'
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增图层代码'
  drawerConfig.value.active = '1'
}
//drawer 配置
