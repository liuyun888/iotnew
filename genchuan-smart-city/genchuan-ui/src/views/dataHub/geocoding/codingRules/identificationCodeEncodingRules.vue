<template>
  <div class="cc-main">
    <div class="cc-main-content">
      ...
      <!--        &lt;!&ndash;右边的按钮&ndash;&gt;-->
      <!--        <div class="right">-->
      <!--          <el-button type="primary" plain :icon="Plus" @click="addClick(titleConfig.addButName)" v-hasPermi="['codingRules:identificationCodeEncodingRules:create']">-->
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="县级行政代码位数" prop="adminCodeLength">
            <el-input
              v-model="searchForm.adminCodeLength"
              placeholder="请输入县级行政代码位数"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
          </el-form-item>
          <el-form-item label="街道代码位数" prop="streetCodeLength">
            <el-input
              v-model="searchForm.streetCodeLength"
              placeholder="请输入街道代码位数"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
          </el-form-item>
          <el-form-item label="图层代码位数" prop="layerCodeLength">
            <el-input
              v-model="searchForm.layerCodeLength"
              placeholder="请输入图层代码位数"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
          </el-form-item>
          <el-form-item label="顺序码位数" prop="seqCodeLength">
            <el-input
              v-model="searchForm.seqCodeLength"
              placeholder="请输入顺序码位数"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
          ...
          <el-form-item>
            ...
            <el-button
              ...
              v-hasPermi="['codingRules:identificationCodeEncodingRules:create']"
            >
              {{ titleConfig.addButName }}
            </el-button>
            <!--表格字段是否显示-->
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            min-width="150"
            align="center"
            prop="adminCodeLength"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('县级行政代码位数')"
          />
          <el-table-column
            label="街道代码位数"
            min-width="150"
            align="center"
            prop="streetCodeLength"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('街道代码位数')"
          />
          <el-table-column
            label="图层代码位数"
            min-width="150"
            align="center"
            prop="layerCodeLength"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('图层代码位数')"
          />
          <el-table-column
            label="顺序码位数"
            min-width="150"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['codingRules:identificationCodeEncodingRules:query']"
                >详细
              </el-button>
              <el-button
                type="primary"
                ...
                v-hasPermi="['codingRules:identificationCodeEncodingRules:update']"
                >编辑
              </el-button>
              <el-button
                type="danger"
                ...
                v-hasPermi="['codingRules:identificationCodeEncodingRules:delete']"
                >删除
              </el-button>
            </template>
          </el-table-column>
        ...
        >
          ...
          <div class="drawer-content">
            ...
            <template v-if="drawerConfig.active == 1">
              ...
              >
                <ul>
                  <li class="width50"><span>规则名称：</span>{{ queryData.ruleName }}</li>
                  <li class="width50"
                    ><span>县级行政代码位数：</span>{{ queryData.adminCodeLength }}</li
                  >
                  <li class="width50"
                    ><span>街道代码位数：</span>{{ queryData.streetCodeLength }}</li
                  >
                  <li class="width50"
                    ><span>图层代码位数：</span>{{ queryData.layerCodeLength }}</li
                  >
                  <li class="width50"><span>顺序码位数：</span>{{ queryData.seqCodeLength }}</li>
                  <li class="width50"><span>顺序码生成规则：</span>{{ queryData.seqGenRule }}</li>
                  <li class="width50"
                    ><span>启用状态：</span
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                ...
                >
                  <el-form-item
                    ...
                    :rules="[]"
                  >
                    <el-input placeholder="请输入规则名称" v-model="formData.ruleName" clearable />
                  </el-form-item>
                  <el-form-item
                    label="县级行政代码位数："
                    prop="adminCodeLength"
                  ...
                  >
                    <el-input
                      placeholder="请输入县级行政代码位数"
                      v-model="formData.adminCodeLength"
                      clearable
                    />
                  </el-form-item>
                  <el-form-item
                  ...
                  >
                    <el-input
                      placeholder="请输入街道代码位数"
                      v-model="formData.streetCodeLength"
                      clearable
                    />
                  </el-form-item>
                  <el-form-item
                  ...
                  >
                    <el-input
                      placeholder="请输入图层代码位数"
                      v-model="formData.layerCodeLength"
                      clearable
                    />
                  </el-form-item>
                  <el-form-item
                  ...
                  >
                    <el-input
                      placeholder="请输入顺序码位数"
                      v-model="formData.seqCodeLength"
                      clearable
                    />
                  </el-form-item>
                  <el-form-item
...
import * as identificationCodeEncodingRulesApi from '@/api/dataHub/geocoding/codingRules/identificationCodeEncodingRules'
...
const titleConfig = ref({
  name: '标识码编码规则配置', //标题名称
  addButName: '新增标识码编码规则' //新增按钮名称
})
// 搜索 字段
const initSearchForm = {
  ruleName: '', // 规则名称
  adminCodeLength: '', // 县级行政代码位数
  streetCodeLength: '', // 街道代码位数
  layerCodeLength: '', // 图层代码位数
  seqCodeLength: '', // 顺序码位数
  seqGenRule: '', // 顺序码生成规则
  enableStatus: '', // 启用状态
  configUser: '' // 配置人
...
const tableInitData = async () => {
  ...
  try {
    ...
    const res = await identificationCodeEncodingRulesApi.listPage(merged)
  ...
  if (valid) {
    try {
      ...
      if (drawerConfig.value.type === 'add' && !formData.value.id) {
        await identificationCodeEncodingRulesApi.createData(data)
        ElMessage.success('保存成功')
      } else {
        await identificationCodeEncodingRulesApi.updateData(data)
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '标识码编码规则-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  drawerConfig.value.title = '编辑标识码编码规则'
}
// 点击删除按钮
const delClick = (row) => {
  ElMessageBox.confirm('请注意：确认后，所有相关数据将不可恢复。', `确定要删除吗？`, {
    ...
    .then(async () => {
      try {
        await identificationCodeEncodingRulesApi.delData(row.id)
        await tableInitData()
        ElMessage({
          type: 'success',
          message: '删除成功'
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增标识码编码规则'
  drawerConfig.value.active = '1'
}
//drawer 配置
...
const tableColumns = ref([
  { label: '规则名称', visible: true },
  { label: '县级行政代码位数', visible: true },
  { label: '街道代码位数', visible: true },
  { label: '图层代码位数', visible: true },
  { label: '顺序码位数', visible: true },
  { label: '顺序码生成规则', visible: true },
