<template>
  <div class="cc-main">
    <div class="cc-main-content">
      ...
      <!--        &lt;!&ndash;右边的按钮&ndash;&gt;-->
      <!--        <div class="right">-->
      <!--          <el-button type="primary" plain :icon="Plus" @click="addClick(titleConfig.addButName)" v-hasPermi="['dataQualityConfiguration:positionAccuracyStandard:create']">-->
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          <el-form-item label="地形类型" prop="terrainType">
            <el-select
              v-model="searchForm.terrainType"
              placeholder="请选择地形类型"
              class="!w-240px"
              clearable
              @change="searchClick"
            >
              <el-option
                v-for="(item, key) in OptionsAll.terrainTypeOptions"
                :key="key"
                :label="item.label"
                :value="item.value"
              />
          ...
          <el-form-item label="适用数据类型" prop="applicableDataType">
            <el-select
              v-model="searchForm.applicableDataType"
            ...
            >
              <el-option
                v-for="(item, key) in OptionsAll.applicableDataTypeOptions"
                :key="key"
                :label="item.label"
                :value="item.value"
              />
          ...
          <el-form-item>
            ...
            <el-button
              ...
              v-hasPermi="['dataQualityConfiguration:positionAccuracyStandard:create']"
            >
              {{ titleConfig.addButName }}
            </el-button>
            <!--表格字段是否显示-->
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            v-if="tableColumnShow('地形类型')"
          >
            <template #default="scope">
              <div @click="tableOneClick(scope.row)" class="table-active">
                {{ handleTypeOptions(scope.row.terrainType, OptionsAll.terrainTypeOptions) }}
              </div>
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('适用数据类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  scope.row.applicableDataType,
                  OptionsAll.applicableDataTypeOptions
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['dataQualityConfiguration:positionAccuracyStandard:query']"
                >详细
              </el-button>
              <el-button
                type="primary"
                ...
                v-hasPermi="['dataQualityConfiguration:positionAccuracyStandard:update']"
                >编辑
              </el-button>
              <el-button
                type="danger"
                ...
                v-hasPermi="['dataQualityConfiguration:positionAccuracyStandard:delete']"
                >删除
              </el-button>
            </template>
          </el-table-column>
            ...
            <template v-if="drawerConfig.active == 1">
              ...
              >
                <ul>
                  <li class="width50">
                    <span>地形类型：</span
                    >{{ handleTypeOptions(queryData.terrainType, OptionsAll.terrainTypeOptions) }}
                  </li>
                  <li class="width50"
                    ><span>适用数据类型：</span
                    >{{
                      handleTypeOptions(
                        queryData.applicableDataType,
                        OptionsAll.applicableDataTypeOptions
                      )
                    }}
                  </li>
                  <li class="width100"><span>比例尺说明：</span>{{ queryData.scaleDesc }}</li>
                  <li class="width50"
                    ><span>启用状态：</span
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                ...
                >
                  ...
                  >
                    <el-select
                      placeholder="请选择地形类型"
                      v-model="formData.terrainType"
                      filterable
                      clearable
                    >
                      <el-option
                        v-for="item in OptionsAll.terrainTypeOptions"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                      />
                  ...
                  >
                    <el-select
                      placeholder="请选择适用数据类型"
                      v-model="formData.applicableDataType"
                      filterable
                      clearable
                    >
                      <el-option
                        v-for="item in OptionsAll.applicableDataTypeOptions"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                      />
                  ...
                  >
                    <el-input
                      placeholder="请输入比例尺说明"
                      v-model="formData.scaleDesc"
                      type="textarea"
...
import * as positionAccuracyStandardApi from '@/api/dataHub/geocoding/dataQualityConfiguration/positionAccuracyStandard'
...
const titleConfig = ref({
  name: '位置精度标准配置', //标题名称
  addButName: '新增位置精度标准' //新增按钮名称
})
// 搜索 字段
const initSearchForm = {
  terrainType: '', // 地形类型
  applicableDataType: '', // 适用数据类型
  enableStatus: '', // 启用状态
  configUser: '' // 配置人
}
// 搜索初始化数据
...
const tableInitData = async () => {
  ...
  try {
    ...
    const res = await positionAccuracyStandardApi.listPage(merged)
  ...
  if (valid) {
    try {
      ...
      if (drawerConfig.value.type === 'add' && !formData.value.id) {
        await positionAccuracyStandardApi.createData(data)
        ElMessage.success('保存成功')
      } else {
        await positionAccuracyStandardApi.updateData(data)
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '位置精度标准-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  drawerConfig.value.title = '编辑位置精度标准'
}
// 点击删除按钮
const delClick = (row) => {
  ElMessageBox.confirm('请注意：确认后，所有相关数据将不可恢复。', `确定要删除吗？`, {
    ...
    .then(async () => {
      try {
        await positionAccuracyStandardApi.delData(row.id)
        await tableInitData()
        ElMessage({
          type: 'success',
          message: '删除成功'
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增位置精度标准'
  drawerConfig.value.active = '1'
}
//drawer 配置
...
const tableColumns = ref([
  { label: '地形类型', visible: true },
  { label: '适用数据类型', visible: true },
  { label: '比例尺说明', visible: true },
