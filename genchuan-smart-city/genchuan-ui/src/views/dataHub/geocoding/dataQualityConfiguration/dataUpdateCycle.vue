<template>
  <div class="cc-main">
    <div class="cc-main-content">
      ...
      <!--        &lt;!&ndash;右边的按钮&ndash;&gt;-->
      <!--        <div class="right">-->
      <!--          <el-button type="primary" plain :icon="Plus" @click="addClick(titleConfig.addButName)" v-hasPermi="['dataQualityConfiguration:dataUpdateCycle:create']">-->
      ...
      <div class="cc-search-css" ref="searchRef">
        <el-form
          ...
          label-width="110px"
        >
          <el-form-item label="更新对象类型" prop="updateObjectType">
            <el-select
              v-model="searchForm.updateObjectType"
              placeholder="请选择更新对象类型"
              class="!w-240px"
              clearable
              @change="searchClick"
            >
              <el-option
                v-for="(item, key) in OptionsAll.updateObjectTypeOptions"
                :key="key"
                :label="item.label"
                :value="item.value"
              />
          ...
          <el-form-item label="常规更新周期" prop="regularCycle">
            <el-select
              v-model="searchForm.regularCycle"
              placeholder="请选择常规更新周期"
              class="!w-240px"
              clearable
              @change="searchClick"
            >
              <el-option
                v-for="(item, key) in OptionsAll.regularCycleOptions"
                :key="key"
                :label="item.label"
                :value="item.value"
              />
          ...
          <el-form-item label="特殊更新触发条件" prop="specialTriggerCondition">
            <el-input
              v-model="searchForm.specialTriggerCondition"
              placeholder="请输入特殊更新触发条件"
              clearable
              class="!w-240px"
              @change="searchClick"
            />
          </el-form-item>
          <el-form-item label="更新责任部门" prop="updateDept">
            <el-tree-select
              v-model="searchForm.updateDept"
              class="!w-240px"
              :data="deptList"
              :props="defaultProps"
              check-strictly
          ...
          <el-form-item>
            ...
            <el-button
              ...
              v-hasPermi="['dataQualityConfiguration:dataUpdateCycle:create']"
            >
              {{ titleConfig.addButName }}
            </el-button>
            <!--表格字段是否显示-->
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            v-if="tableColumnShow('更新对象类型')"
          >
            <template #default="scope">
              <div @click="tableOneClick(scope.row)" class="table-active">
                {{
                  handleTypeOptions(scope.row.updateObjectType, OptionsAll.updateObjectTypeOptions)
                }}
              </div>
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('常规更新周期')"
          >
            <template #default="scope">
              {{ handleTypeOptions(scope.row.regularCycle, OptionsAll.regularCycleOptions) }}
            </template>
          </el-table-column>
          <el-table-column
            label="特殊更新触发条件"
            min-width="150"
            align="center"
            prop="specialTriggerCondition"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('特殊更新触发条件')"
          />
          <el-table-column
            label="更新责任部门"
            min-width="150"
            ...
            prop="updateDept"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('更新责任部门')"
          />
          <el-table-column
            label="更新流程说明"
            min-width="150"
            align="center"
            prop="updateProcessDesc"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('更新流程说明')"
          />
          <el-table-column
            label="启用状态"
            min-width="150"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['dataQualityConfiguration:dataUpdateCycle:query']"
                >详细
              </el-button>
              <el-button
                type="primary"
                ...
                v-hasPermi="['dataQualityConfiguration:dataUpdateCycle:update']"
                >编辑
              </el-button>
              <el-button
                type="danger"
                ...
                v-hasPermi="['dataQualityConfiguration:dataUpdateCycle:delete']"
                >删除
              </el-button>
            </template>
          </el-table-column>
            ...
            <template v-if="drawerConfig.active == 1">
              ...
              >
                <ul>
                  <li class="width50"
                    ...
                    >{{
                      handleTypeOptions(
                        queryData.updateObjectType,
                        OptionsAll.updateObjectTypeOptions
                      )
                    }}
                  </li>
                  <li class="width50"
                    ><span>常规更新周期：</span
                    >{{ handleTypeOptions(queryData.regularCycle, OptionsAll.regularCycleOptions) }}
                  </li>
                  <li class="width50"
                    ><span>特殊更新触发条件：</span>{{ queryData.specialTriggerCondition }}</li
                  >
                  <li class="width50"><span>更新责任部门：</span>{{ queryData.updateDept }}</li>
                  <li class="width100"
                    ><span>更新流程说明：</span>{{ queryData.updateProcessDesc }}</li
                  >
                  <li class="width50"
                    ><span>启用状态：</span
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                ...
                >
                  <el-form-item
                    label="更新对象类型："
                    prop="updateObjectType"
                    class="edit-content-form-li width50"
                    :rules="[]"
                  >
                    <el-select
                      placeholder="请选择更新对象类型"
                      v-model="formData.updateObjectType"
                      filterable
                      clearable
                    >
                      <el-option
                        v-for="item in OptionsAll.updateObjectTypeOptions"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                      />
                  ...
                  >
                    <el-select
                      placeholder="请选择常规更新周期"
                      v-model="formData.regularCycle"
                      filterable
                      clearable
                    >
                      <el-option
                        v-for="item in OptionsAll.regularCycleOptions"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                      />
                  ...
                  >
                    <el-input
                      placeholder="请输入特殊更新触发条件"
                      v-model="formData.specialTriggerCondition"
                      clearable
                    />
                  </el-form-item>
                  <el-form-item
                  ...
                  >
                    <el-tree-select
                      v-model="formData.updateDept"
                      :data="deptList"
                      :props="defaultProps"
                      check-strictly
                      node-key="id"
                      placeholder="请选择更新责任部门"
                    />
                  </el-form-item>
                  <el-form-item
                    label="更新流程说明："
                  ...
                  >
                    <el-input
                      placeholder="请输入更新流程说明"
                      v-model="formData.updateProcessDesc"
                      type="textarea"
...
import * as dataUpdateCycleApi from '@/api/dataHub/geocoding/dataQualityConfiguration/dataUpdateCycle'
...
const titleConfig = ref({
  name: '数据更新周期配置', //标题名称
  addButName: '新增数据更新周期' //新增按钮名称
})
// 搜索 字段
const initSearchForm = {
  updateObjectType: '', // 更新对象类型
  regularCycle: '', // 常规更新周期
  specialTriggerCondition: '', // 特殊更新触发条件
  updateDept: '', // 更新责任部门
  enableStatus: '', // 启用状态
  configUser: '' // 配置人
}
...
const tableInitData = async () => {
  ...
  try {
    ...
    const res = await dataUpdateCycleApi.listPage(merged)
...
const initFormData = {
  id: '',
  updateObjectType: '1', // 更新对象类型
  regularCycle: '1', // 常规更新周期
  specialTriggerCondition: '', // 特殊更新触发条件
  updateDept: '', // 更新责任部门
  updateProcessDesc: '', // 更新流程说明
  enableStatus: 1, // 启用状态
  configUser: '', // 配置人
  configTime: '', // 配置时间
  remark: '' // 备注
  ...
  if (valid) {
    try {
      ...
      if (drawerConfig.value.type === 'add' && !formData.value.id) {
        await dataUpdateCycleApi.createData(data)
        ElMessage.success('保存成功')
      } else {
        await dataUpdateCycleApi.updateData(data)
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '数据更新周期-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  drawerConfig.value.title = '编辑数据更新周期'
}
// 点击删除按钮
const delClick = (row) => {
  ElMessageBox.confirm('请注意：确认后，所有相关数据将不可恢复。', `确定要删除吗？`, {
    ...
    .then(async () => {
      try {
        await dataUpdateCycleApi.delData(row.id)
        await tableInitData()
        ElMessage({
          type: 'success',
          message: '删除成功'
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增数据更新周期'
  drawerConfig.value.active = '1'
}
//drawer 配置
...
const tableColumns = ref([
  { label: '更新对象类型', visible: true },
  { label: '常规更新周期', visible: true },
  { label: '特殊更新触发条件', visible: true },
  { label: '更新责任部门', visible: true },
  { label: '更新流程说明', visible: true },
