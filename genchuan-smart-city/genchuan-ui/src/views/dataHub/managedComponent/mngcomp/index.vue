  ...
  <div class="mng-comp-page">
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="统计ID" align="center" prop="statMngCompId" min-width="140" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:mng-comp:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
    ...
    >
      <div class="detail-header flex justify-between items-center p-4 border-b">
        <h3 class="text-lg font-semibold">管理部件统计详情</h3>
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="统计ID">{{ selectedRow?.statMngCompId || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="部件总数">{{ selectedRow?.totalCompCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="完好部件数">{{ selectedRow?.normalCompCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="破损部件数">{{ selectedRow?.damagedCompCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="丢失部件数">{{ selectedRow?.lostCompCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="废弃部件数">{{ selectedRow?.discardedCompCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="新增部件数">{{ selectedRow?.newCompCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="更新部件数">{{ selectedRow?.updateCompCount || '-' }}</el-descriptions-item>
...
const list = ref<MngCompVO[]>([]) // 列表数据
const total = ref(0) // 总记录数
const exportLoading = ref(false) // 导出加载状态

// 详情抽屉相关状态
const isDetailShow = ref(false)
const selectedRow = ref<MngCompVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: MngCompVO) => {
...
const getList = async () => {
  ...
  try {
    ...
    console.error('查询管理部件统计列表失败:', error)
...
const handleDelete = async (id: number) => {
  try {
    ...
    console.error('删除管理部件统计失败:', error)
...
const handleExport = async () => {
  try {
    ...
    console.error('导出管理部件统计失败:', error)
