<template>
  <div class="biz-mng-comp-ext-page">
    <!-- 搜索区域 -->
        ...
        >
          <div class="form-row">
            <el-form-item label="扩展ID" prop="mngCompExtId">
              <el-input
                v-model="queryParams.mngCompExtId"
                placeholder="请输入扩展ID"
                clearable
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp-ext:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp-ext:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="扩展ID" align="center" prop="mngCompExtId" min-width="140" />
        <el-table-column label="归属大类ID" align="center" prop="majorId" min-width="140" />
        <el-table-column label="归属大类名称" align="center" prop="majorName" min-width="160" />
        <el-table-column label="扩展小类代码" align="center" prop="extMinorCode" min-width="140" />
        <el-table-column label="扩展小类名称" align="center" prop="extMinorName" min-width="160" />
        <el-table-column
          label="系统创建时间"
          align="center"
          prop="createTimeSys"
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp-ext:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp-ext:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp-ext:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="扩展ID">{{ selectedRow?.mngCompExtId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="归属大类ID">{{ selectedRow?.majorId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="归属大类名称">{{ selectedRow?.majorName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="扩展小类代码">{{ selectedRow?.extMinorCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="扩展小类名称">{{ selectedRow?.extMinorName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="扩展小类说明">{{ selectedRow?.extMinorDesc || '-' }}</el-descriptions-item>
          <el-descriptions-item label="建议主管部门代码">{{ selectedRow?.suggestDeptCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="建议主管部门名称">{{ selectedRow?.suggestDeptName || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <BizMngCompExtForm ref="formRef" @success="getList" />
  </div>
</template>
...
import BizMngCompExtForm from './BizMngCompExtForm.vue'

/** 管理部件扩展管理部件配置 列表 */
defineOptions({ name: 'BizMngCompExt' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<BizMngCompExtVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<BizMngCompExtVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: BizMngCompExtVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await BizMngCompExtApi.getBizMngCompExtPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await BizMngCompExtApi.deleteBizMngCompExt(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await BizMngCompExtApi.exportBizMngCompExt(queryParams)
    download.excel(data, '管理部件扩展管理部件配置.xls')
  } catch {
  } finally {
