<template>
  <div class="managed-component-major-page">
    <!-- 搜索区域 -->
          ...
          <div class="form-row">
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:managed-component-major-config:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:managed-component-major-config:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="大类ID" align="center" prop="majorId" min-width="140" />
        <el-table-column label="大类代码" align="center" prop="majorCode" min-width="140" />
        <el-table-column label="大类名称" align="center" prop="majorName" min-width="160" />
        <el-table-column label="排序序号" align="center" prop="sortNum" width="120" />
        <el-table-column
          label="创建时间"
          align="center"
          prop="createTime"
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:managed-component-major-config:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:managed-component-major-config:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:managed-component-major-config:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="大类ID">{{ selectedRow?.majorId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="大类代码">{{ selectedRow?.majorCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="大类名称">{{ selectedRow?.majorName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="大类说明">{{ selectedRow?.majorDesc || '-' }}</el-descriptions-item>
...
/** 管理部件大类配置表 列表 */
defineOptions({ name: 'ManagedComponentMajorConfig' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<ManagedComponentMajorConfigVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<ManagedComponentMajorConfigVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: ManagedComponentMajorConfigVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await ManagedComponentMajorConfigApi.getManagedComponentMajorConfigPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await ManagedComponentMajorConfigApi.deleteManagedComponentMajorConfig(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await ManagedComponentMajorConfigApi.exportManagedComponentMajorConfig(queryParams)
    download.excel(data, '管理部件大类配置表.xls')
  } catch {
  } finally {
