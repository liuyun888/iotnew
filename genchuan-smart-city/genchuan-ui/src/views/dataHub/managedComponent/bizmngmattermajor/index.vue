<template>
  <div class="biz-mng-matter-major-page">
    <!-- 搜索区域 -->
        ...
        >
          <div class="form-row">
            <el-form-item label="管理事项大类ID" prop="mngMatterMajorId">
              <el-input
                v-model="queryParams.mngMatterMajorId"
                placeholder="请输入管理事项大类ID"
                clearable
            ...
            <el-form-item label="大类代码" prop="matterMajorCode">
              <el-input
                v-model="queryParams.matterMajorCode"
                placeholder="请输入大类代码"
                clearable
                @keyup.enter="handleQuery"
                class="!w-160px"
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-matter-major:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-matter-major:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="管理事项大类ID" align="center" prop="mngMatterMajorId" min-width="160" />
        <el-table-column label="大类代码" align="center" prop="matterMajorCode" min-width="140" />
        <el-table-column label="大类名称" align="center" prop="matterMajorName" min-width="180" />
        <el-table-column label="大类说明" align="center" prop="matterMajorDesc" min-width="200" />
        <el-table-column label="排序序号" align="center" prop="sortNum" min-width="120" />
        <el-table-column label="启用状态" align="center" prop="enableStatus" min-width="120" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-matter-major:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-matter-major:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-matter-major:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="管理事项大类ID">{{ selectedRow?.mngMatterMajorId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="大类代码">{{ selectedRow?.matterMajorCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="大类名称">{{ selectedRow?.matterMajorName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="大类说明">{{ selectedRow?.matterMajorDesc || '-' }}</el-descriptions-item>
          <el-descriptions-item label="排序序号">{{ selectedRow?.sortNum || '-' }}</el-descriptions-item>
          <el-descriptions-item label="启用状态">{{ selectedRow?.enableStatus || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <BizMngMatterMajorForm ref="formRef" @success="getList" />
  </div>
</template>
...
import BizMngMatterMajorForm from './BizMngMatterMajorForm.vue'

/** 管理事项大类 列表 */
defineOptions({ name: 'BizMngMatterMajor' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<BizMngMatterMajorVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<BizMngMatterMajorVO | null>(null)
...
const queryParams = reactive({
  ...
  createTimeSys: undefined, // 修正为undefined，而非数组
})
const queryFormRef = ref() // 搜索的表单
const formRef = ref() // 表单弹窗

/** 打开详情伪抽屉 */
const openDetail = (row: BizMngMatterMajorVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await BizMngMatterMajorApi.getBizMngMatterMajorPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await BizMngMatterMajorApi.deleteBizMngMatterMajor(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await BizMngMatterMajorApi.exportBizMngMatterMajor(queryParams)
    download.excel(data, '管理事项大类.xls')
  } catch {
  } finally {
