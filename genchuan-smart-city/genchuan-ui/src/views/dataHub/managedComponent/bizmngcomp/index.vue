  ...
  <div class="biz-mng-comp-page">
        ...
        >
          <div class="form-row">
            ...
            <el-form-item label="部件状态" prop="compStatus">
              <el-select
                v-model="queryParams.compStatus"
                placeholder="请选择部件状态"
                clearable
                class="!w-160px"
              >
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="部件ID" align="center" prop="mngCompId" min-width="140" />
        <el-table-column label="部件标识码" align="center" prop="compCode" min-width="140" />
        <el-table-column label="部件名称" align="center" prop="compName" min-width="160" />
        <el-table-column label="部件状态" align="center" prop="compStatus" min-width="120" />
        <el-table-column label="主管部门名称" align="center" prop="deptName" min-width="160" />
        <el-table-column label="关联网格名称" align="center" prop="gridName" min-width="160" />
        <el-table-column
          label="系统创建时间"
          align="center"
          prop="createTimeSys"
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:biz-mng-comp:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      <div class="detail-header flex justify-between items-center p-4 border-b">
        <h3 class="text-lg font-semibold">管理部件信息管理详情</h3>
      ...
      <div class="detail-content p-6">
        <el-descriptions
          ...
          :column="1"
          border
          :label-style="{ 'width': '180px', 'font-weight': '500', 'text-align': 'left' }"
          :content-style="{ 'flex': '1', 'text-align': 'left' }"
        >
          <el-descriptions-item label="主键ID">{{ selectedRow?.id || '-' }}</el-descriptions-item>
          <el-descriptions-item label="部件ID">{{ selectedRow?.mngCompId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="部件标识码">{{ selectedRow?.compCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="部件名称">{{ selectedRow?.compName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="关联管理部件小类ID">{{ selectedRow?.minorId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="关联管理部件小类名称">{{ selectedRow?.minorName || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="部件状态">{{ selectedRow?.compStatus || '-' }}</el-descriptions-item>
          <el-descriptions-item label="部件普查日期">{{ dateFormatter(selectedRow?.initDate) || '-' }}</el-descriptions-item>
          <el-descriptions-item label="状态/权属变更更新日期">{{ dateFormatter(selectedRow?.changeDate) || '-' }}</el-descriptions-item>
          <el-descriptions-item label="数据来源">{{ selectedRow?.dataSource || '-' }}</el-descriptions-item>
          <el-descriptions-item label="录入人账号">{{ selectedRow?.createUser || '-' }}</el-descriptions-item>
          <el-descriptions-item label="系统生成时间">{{ dateFormatter(selectedRow?.createTime) || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <BizMngCompForm ref="formRef" @success="getList" />
  </div>
</template>
...
import BizMngCompForm from './BizMngCompForm.vue'

/** 管理部件信息管理 列表 */
defineOptions({ name: 'BizMngComp' })
...
const list = ref<BizMngCompVO[]>([]) // 列表数据
const total = ref(0) // 总记录数
const exportLoading = ref(false) // 导出加载状态

// 详情抽屉相关状态
const isDetailShow = ref(false)
const selectedRow = ref<BizMngCompVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: BizMngCompVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await BizMngCompApi.getBizMngCompPage(queryParams)
    list.value = data.list
    total.value = data.total
  } catch (error) {
    console.error('查询管理部件信息管理列表失败:', error)
...
const handleDelete = async (id: number) => {
  try {
    ...
    await BizMngCompApi.deleteBizMngComp(id)
    message.success(t('common.delSuccess'))
    await getList()
  } catch (error) {
    console.error('删除管理部件信息管理失败:', error)
...
const handleExport = async () => {
  try {
    ...
    const data = await BizMngCompApi.exportBizMngComp(queryParams)
    download.excel(data, '管理部件信息管理.xls')
    message.success(t('common.exportSuccess'))
  } catch (error) {
    console.error('导出管理部件信息管理失败:', error)
