  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="扩展ID" prop="mngMatterExtId">
        <el-input v-model="formData.mngMatterExtId" placeholder="请输入扩展ID" />
      </el-form-item>
      <el-form-item label="归属大类ID" prop="matterMajorId">
        <el-input v-model="formData.matterMajorId" placeholder="请输入归属大类ID" />
      </el-form-item>
      <el-form-item label="归属大类名称" prop="matterMajorName">
        <el-input v-model="formData.matterMajorName" placeholder="请输入归属大类名称" />
      </el-form-item>
...
/** 管理事项扩展管理事项配置 表单 */
defineOptions({ name: 'BizMngMatterExtForm' })
...
const formRules = reactive({
  mngMatterExtId: [{ required: true, message: '扩展ID不能为空', trigger: 'blur' }],
  matterMajorId: [{ required: true, message: '归属大类ID不能为空', trigger: 'blur' }],
  matterMajorName: [{ required: true, message: '归属大类名称不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await BizMngMatterExtApi.getBizMngMatterExt(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as BizMngMatterExtVO
    if (formType.value === 'create') {
      await BizMngMatterExtApi.createBizMngMatterExt(data)
      message.success(t('common.createSuccess'))
    } else {
      await BizMngMatterExtApi.updateBizMngMatterExt(data)
