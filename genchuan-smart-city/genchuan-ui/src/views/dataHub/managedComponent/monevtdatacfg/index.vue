<template>
  <div class="mon-evt-data-cfg-page">
    <!-- 搜索区域 -->
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="配置ID" align="center" prop="monEvtDataId" min-width="140" />
        <el-table-column label="事件分类ID" align="center" prop="evtCatId" min-width="140" />
        <el-table-column label="事件分类名称" align="center" prop="evtCatName" min-width="160" />
        <el-table-column label="字段名称" align="center" prop="fieldName" min-width="140" />
        <el-table-column label="字段代码" align="center" prop="fieldCode" min-width="140" />
        <el-table-column label="字段类型" align="center" prop="fieldType" min-width="120" />
        <el-table-column label="字段长度" align="center" prop="fieldLength" min-width="100" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:mon-evt-data-cfg:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="配置ID">{{ selectedRow?.monEvtDataId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="事件分类ID">{{ selectedRow?.evtCatId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="事件分类名称">{{ selectedRow?.evtCatName || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="字段类型">{{ selectedRow?.fieldType || '-' }}</el-descriptions-item>
          <el-descriptions-item label="字段长度">{{ selectedRow?.fieldLength || '-' }}</el-descriptions-item>
          <el-descriptions-item label="约束条件">{{ selectedRow?.constraintType || '-' }}</el-descriptions-item>
          <el-descriptions-item label="值域范围">{{ selectedRow?.valueRange || '-' }}</el-descriptions-item>
          <el-descriptions-item label="字段说明">{{ selectedRow?.fieldDesc || '-' }}</el-descriptions-item>
...
const selectedRow = ref<MonEvtDataCfgVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: MonEvtDataCfgVO) => {
