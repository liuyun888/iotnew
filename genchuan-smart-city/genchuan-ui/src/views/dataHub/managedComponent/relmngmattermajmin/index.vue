<template>
  <div class="rel-mng-matter-maj-min-page">
    <!-- 搜索区域 -->
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="关联ID" align="center" prop="mngMatterMajorMinorId" min-width="140" />
        ...
        <el-table-column label="关联状态" align="center" prop="relStatus" width="120">
          <template #default="scope">
            <el-tag :type="getRelStatusType(scope.row.relStatus)">
              {{ getRelStatusText(scope.row.relStatus) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:rel-mng-matter-maj-min:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="关联ID">{{ selectedRow?.mngMatterMajorMinorId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="关联管理事项大类ID">{{ selectedRow?.majorId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="关联管理事项大类名称">{{ selectedRow?.majorName || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="关联状态">
            <el-tag :type="getRelStatusType(selectedRow?.relStatus)">
              {{ getRelStatusText(selectedRow?.relStatus) }}
            </el-tag>
...
const list = ref<RelMngMatterMajMinVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<RelMngMatterMajMinVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: RelMngMatterMajMinVO) => {
...
/** 获取关联状态显示文本 */
const getRelStatusText = (status: any) => {
  if (status === null || status === undefined) return '未知';

  const statusStr = String(status).toLowerCase();
  return (statusStr === '1' || statusStr === 'true') ? '有效' : '无效';
}

/** 获取关联状态标签类型 */
const getRelStatusType = (status: any) => {
