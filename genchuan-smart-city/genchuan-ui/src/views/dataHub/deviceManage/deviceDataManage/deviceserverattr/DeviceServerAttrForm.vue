...
/** 设备服务器属性配置 表单 */
defineOptions({ name: 'DeviceServerAttrForm' })
...
const formRules = reactive({
  ...
  isRequired: [{ required: true, message: '是否必选不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await DeviceServerAttrApi.getDeviceServerAttr(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as DeviceServerAttrVO
    if (formType.value === 'create') {
      await DeviceServerAttrApi.createDeviceServerAttr(data)
      message.success(t('common.createSuccess'))
    } else {
      await DeviceServerAttrApi.updateDeviceServerAttr(data)
