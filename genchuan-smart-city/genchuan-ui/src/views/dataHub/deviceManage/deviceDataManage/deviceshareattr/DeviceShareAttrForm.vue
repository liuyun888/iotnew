  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="600px">
    ...
    >
      <el-form-item label="属性配置ID" prop="shareAttrCfgId">
        <el-input v-model="formData.shareAttrCfgId" placeholder="请输入属性配置ID" />
      </el-form-item>
      ...
      <el-form-item label="维护单位名称" prop="mntDeptName">
        <el-input v-model="formData.mntDeptName" placeholder="请输入维护单位名称" />
      </el-form-item>
...
/** 设备共享属性配置 表单 */
defineOptions({ name: 'DeviceShareAttrForm' })
...
const formData = ref({
  ...
  attrDesc: undefined,
  mntDeptCode: undefined,
  mntDeptName: undefined,
  cfgTime: undefined,
  cfgUser: undefined,
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await DeviceShareAttrApi.getDeviceShareAttr(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as DeviceShareAttrVO
    if (formType.value === 'create') {
      await DeviceShareAttrApi.createDeviceShareAttr(data)
      message.success(t('common.createSuccess'))
    } else {
      await DeviceShareAttrApi.updateDeviceShareAttr(data)
