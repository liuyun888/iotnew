<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column label="资产编码" align="center" prop="assetCode" min-width="120" />
      <el-table-column
        label="关联时间"
        align="center"
        prop="relTime"
      ...
      <el-table-column label="关联状态" align="center" prop="relStatus" min-width="100">
  ...
  <!-- 表单弹窗：添加/修改 -->
  <DeviceAssetForm ref="formRef" @success="getList" />

  <!-- 详情抽屉 -->
  <DeviceAssetDetail ref="detailRef" :visible="detailVisible" @close="detailVisible = false" />
</template>
...
import DeviceAssetForm from './DeviceAssetForm.vue'
import DeviceAssetDetail from './DeviceAssetDetail.vue'
...
const list = ref<DeviceAssetVO[]>([]) // 列表的数据
...
const openDetail = (row: DeviceAssetVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await DeviceAssetApi.getDeviceAssetPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await DeviceAssetApi.deleteDeviceAsset(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await DeviceAssetApi.exportDeviceAsset(queryParams)
    download.excel(data, '设备关联资产.xls')
  } catch {
  } finally {
