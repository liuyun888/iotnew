  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">巡查巡检记录档案详情</span>
      ...
      <div class="grid grid-cols-2 gap-6">
        ...
        <el-card class="card-hover">
          ...
          <div class="card-content space-y-3 text-gray-700">
            <div class="flex items-center">
              ...
              <span class="font-bold text-blue-600">{{ data?.archId || '-' }}</span>
            </div>
            <div class="flex items-center">
              <span class="w-36 text-gray-500">档案编码：</span>
              <span class="font-bold text-blue-600">{{ data?.archCode || '-' }}</span>
            </div>
            <div class="flex items-center">
              <span class="w-36 text-gray-500">档案名称：</span>
              <span class="font-medium">{{ data?.archName || '-' }}</span>
            </div>
            <div class="flex items-center">
              <span class="w-36 text-gray-500">档案类型：</span>
              <span>{{ data?.archType || '-' }}</span>
            </div>
            <div class="flex items-center">
        ...
        <el-card class="card-hover">
          ...
          <div class="card-content space-y-3 text-gray-700">
            ...
            <div class="flex items-center">
              ...
              <span>{{ data?.archCycle || '-' }}</span>
            </div>
            <div class="flex items-center">
              <span class="w-36 text-gray-500">包含记录数：</span>
              <span class="font-bold text-blue-600">{{ data?.includeRecCount || 0 }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="card-hover">
          <div class="card-header bg-green-50 px-4 py-3 rounded-t-lg mb-4">
            <span class="text-green-700 font-semibold">归档信息</span>
          </div>
          <div class="card-content space-y-3 text-gray-700">
            <div class="flex items-center">
              <span class="w-36 text-gray-500">归档人：</span>
              <span>{{ data?.archUser || '-' }}</span>
            </div>
            <div class="flex items-center">
              <span class="w-36 text-gray-500">归档时间：</span>
              <span>{{ formatDate(data?.archTime) }}</span>
            </div>
            <div class="flex items-center">
              <span class="w-36 text-gray-500">销毁时间：</span>
              <span>{{ formatDate(data?.destroyTime) }}</span>
            </div>
            <div class="flex items-center">
              <span class="w-36 text-gray-500">档案状态：</span>
              <el-tag :type="data?.archStatus === '已归档' ? 'success' : 'warning'">
                {{ data?.archStatus || '-' }}
              </el-tag>
            </div>
          </div>
        ...
        <el-card class="card-hover">
          <div class="card-header bg-yellow-50 px-4 py-3 rounded-t-lg mb-4">
            <span class="text-yellow-700 font-semibold">附件信息</span>
          </div>
          <div class="card-content space-y-3 text-gray-700">
        ...
        <el-card class="card-hover grid-cols-2">
          <div class="card-header bg-purple-50 px-4 py-3 rounded-t-lg mb-4">
            <span class="text-purple-700 font-semibold">扩展信息</span>
          </div>
...
import { InspectRecArchVO } from '@/api/dataHub/supervisionInspection/patrolArchiveManagement/recordArchive'

const props = defineProps<{
  modelValue: boolean
  data?: InspectRecArchVO | null
}>()
