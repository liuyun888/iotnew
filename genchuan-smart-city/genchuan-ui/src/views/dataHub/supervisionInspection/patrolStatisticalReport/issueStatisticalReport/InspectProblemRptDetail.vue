  ...
  >
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">问题统计详情</span>
    ...
    <div class="detail-container">
      <div class="grid-layout">
        ...
        <!-- 区域与问题类型卡片 -->
        <el-card class="info-card area-type-card">
          <div class="card-header">区域与问题类型</div>
          <div class="card-body">
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <span class="value">{{ detailData.problemTypeId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">问题类型名称：</span>
              <span class="value">{{ detailData.problemTypeName || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">问题集中区域：</span>
              <span class="value">{{ detailData.concentratedArea || '-' }}</span>
            </div>
          </div>
        </el-card>

        <!-- 问题统计卡片 -->
        <el-card class="info-card problem-count-card">
          <div class="card-header">问题统计</div>
          <div class="card-body">
            <div class="info-item">
              <span class="label">上报问题数：</span>
              <span class="value">{{ detailData.rptProblemCount || 0 }}</span>
            </div>
            <div class="info-item">
              <span class="label">一级问题数：</span>
              <span class="value">{{ detailData.level1ProblemCount || 0 }}</span>
            </div>
            <div class="info-item">
              <span class="label">一级问题占比：</span>
              <span class="value">
                {{
                  detailData.rptProblemCount
                    ? Math.round(
                        (detailData.level1ProblemCount / detailData.rptProblemCount) * 100
                      ) + '%'
                    : '0%'
                }}
              </span>
        ...
        <!-- 整改情况卡片 -->
        <el-card class="info-card rectify-card">
          <div class="card-header">整改情况</div>
          <div class="card-body">
            <div class="info-item">
            ...
            <div class="info-item">
              <span class="label">未整改原因：</span>
              <span class="value">{{ detailData.unrectifiedReason || '-' }}</span>
            </div>
          </div>
        </el-card>
...
import { InspectProblemRptVO } from '@/api/dataHub/supervisionInspection/patrolStatisticalReport/issueStatisticalReport'
...
const props = defineProps({
  ...
  data: {
    type: Object as () => InspectProblemRptVO,
    default: () => ({})
  }
})
...
const detailData = ref<InspectProblemRptVO>({} as InspectProblemRptVO)
...
.area-type-card .card-header {
  background-color: #fff7e6;
}

.problem-count-card .card-header {
  background-color: #f6ffed;
}

.rectify-card .card-header {
