<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="整改截止时间" prop="rectifyDeadlineTime">
        <el-date-picker
          v-model="queryParams.rectifyDeadlineTime"
  ...
  <!-- 表单弹窗：添加/修改 -->
  <InspectRectifyTaskForm ref="formRef" @success="getList" />

  <!-- 详情抽屉 -->
  <InspectRectifyTaskDetail
    ref="detailRef"
...
import InspectRectifyTaskForm from './InspectRectifyTaskForm.vue'
import InspectRectifyTaskDetail from './InspectRectifyTaskDetail.vue'

/** 巡查巡检整改任务派发 列表 */
defineOptions({ name: 'InspectRectifyTask' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<InspectRectifyTaskVO[]>([]) // 列表的数据
...
const detailData = ref<InspectRectifyTaskVO | null>(null)
const detailRef = ref()
...
const getList = async () => {
  loading.value = true
  try {
    const data = await InspectRectifyTaskApi.getInspectRectifyTaskPage(queryParams)
...
/** 详情操作 */
const openDetail = (row: InspectRectifyTaskVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await InspectRectifyTaskApi.deleteInspectRectifyTask(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await InspectRectifyTaskApi.exportInspectRectifyTask(queryParams)
    download.excel(data, '巡查巡检整改任务派发.xls')
  } catch {
  } finally {
