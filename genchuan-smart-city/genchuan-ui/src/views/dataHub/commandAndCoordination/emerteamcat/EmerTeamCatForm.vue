  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="分类ID" prop="teamCatId">
        <el-input v-model="formData.teamCatId" placeholder="请输入分类ID" />
      </el-form-item>
...
/** 救援队伍分类配置 表单 */
defineOptions({ name: 'EmerTeamCatForm' })
...
const formRules = reactive({
  teamCatId: [{ required: true, message: '分类ID不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await EmerTeamCatApi.getEmerTeamCat(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as EmerTeamCatVO
    if (formType.value === 'create') {
      await EmerTeamCatApi.createEmerTeamCat(data)
      message.success(t('common.createSuccess'))
    } else {
      await EmerTeamCatApi.updateEmerTeamCat(data)
