  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="分类ID" prop="deviceCatId">
        <el-input v-model="formData.deviceCatId" placeholder="请输入分类ID" />
      </el-form-item>
...
/** 救援设备分类配置 表单 */
defineOptions({ name: 'EmerDeviceCatForm' })
...
const formRules = reactive({
  deviceCatId: [{ required: true, message: '分类ID不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await EmerDeviceCatApi.getEmerDeviceCat(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as EmerDeviceCatVO
    if (formType.value === 'create') {
      await EmerDeviceCatApi.createEmerDeviceCat(data)
      message.success(t('common.createSuccess'))
    } else {
      await EmerDeviceCatApi.updateEmerDeviceCat(data)
