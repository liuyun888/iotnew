<template>
  <div class="analysis-evt-classify-allocate-page">
    <!-- 搜索区域 -->
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="分拨ID" align="center" prop="allocateId" min-width="140" />
        <el-table-column label="事件编码" align="center" prop="evtCode" min-width="160" />
        <el-table-column label="事件类型名称" align="center" prop="evtTypeName" min-width="180" />
        <el-table-column label="紧急程度" align="center" prop="urgencyLevel" min-width="120" />
        <el-table-column label="分拨部门名称" align="center" prop="allocateDeptName" min-width="180" />
        <el-table-column label="分拨人姓名" align="center" prop="allocatorName" min-width="120" />
        <el-table-column
          label="分拨时间"
          align="center"
          prop="allocateTime"
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:evt-classify-allocate:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
    ...
    >
      <div class="detail-header flex justify-between items-center p-4 border-b">
        <h3 class="text-lg font-semibold">事件分级分拨详情</h3>
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="分拨ID">{{ selectedRow?.allocateId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="事件编码">{{ selectedRow?.evtCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="事件类型ID">{{ selectedRow?.evtTypeId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="事件类型名称">{{ selectedRow?.evtTypeName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="紧急程度">{{ selectedRow?.urgencyLevel || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="分拨人ID">{{ selectedRow?.allocatorId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="分拨人姓名">{{ selectedRow?.allocatorName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="分拨时间">{{ dateFormatter(selectedRow?.allocateTime) || '-' }}</el-descriptions-item>
          <el-descriptions-item label="分拨说明">{{ selectedRow?.allocateDesc || '-' }}</el-descriptions-item>
        </el-descriptions>
      </div>
    </div>
...
const selectedRow = ref<EvtClassifyAllocateVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: EvtClassifyAllocateVO) => {
...
.analysis-evt-classify-allocate-page {
  padding: 8px;
  height: 100vh;
