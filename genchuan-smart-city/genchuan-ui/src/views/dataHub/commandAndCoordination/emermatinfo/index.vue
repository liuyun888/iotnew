<template>
      ...
      >
        ...
        <div class="form-actions">
          ...
          <el-button
            ...
            v-hasPermi="['datacenter:emer-mat-info:create']"
          >
          ...
          <el-button
            ...
            v-hasPermi="['datacenter:emer-mat-info:export']"
          >
  ...
  <ContentWrap>
    <el-card class="table-card" shadow="never">
      <template #header>
        <div class="card-header">
          <span class="card-title">应急物资信息列表</span>
      ...
      >
        <el-table-column label="物资信息" align="center" min-width="240">
          <template #default="scope">
            <div class="material-info">
              <div class="mat-code">{{ scope.row.matCode }}</div>
              <div class="mat-name">{{ scope.row.matName }}</div>
              <div class="mat-spec-unit">
                <span v-if="scope.row.matSpec" class="mat-spec">{{ scope.row.matSpec }}</span>
                <span v-if="scope.row.matUnit" class="mat-unit">{{ scope.row.matUnit }}</span>
              </div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="库存信息" align="center" width="120">
          <template #default="scope">
            <div class="inventory-info">
              <div class="total-quantity">
                总量: <span class="quantity-value">{{ scope.row.totalQuantity || 0 }}</span>
              </div>
              <div class="available-quantity">
                可用: <span class="quantity-value">{{ scope.row.availableQuantity || 0 }}</span>
              </div>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="状态" align="center" width="80">
          <template #default="scope">
            <el-tag
              :type="scope.row.matStatus === '1' ? 'success' : 'danger'"
              size="small"
            >
              {{ getStatusText(scope.row.matStatus) }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="120" fixed="right">
          <template #default="scope">
            <el-button
              ...
              v-hasPermi="['datacenter:emer-mat-info:update']"
            >
              编辑
            </el-button>
            <el-button
              ...
              v-hasPermi="['datacenter:emer-mat-info:delete']"
            >
              删除
            </el-button>
          </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EmerMatInfoForm ref="formRef" @success="getList" />
</template>
...
import EmerMatInfoForm from './EmerMatInfoForm.vue'

/** 应急物资信息 列表 */
defineOptions({ name: 'EmerMatInfo' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<EmerMatInfoVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EmerMatInfoApi.getEmerMatInfoPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EmerMatInfoApi.deleteEmerMatInfo(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EmerMatInfoApi.exportEmerMatInfo(queryParams)
    download.excel(data, '应急物资信息.xls')
  } catch {
  } finally {
