<template>
      ...
      >
        ...
        <div class="form-section" v-show="showFullSearch">
          ...
          <div class="form-row">
            ...
            <el-form-item label="可用数量" prop="availableDeviceCount">
              <el-input
                v-model="queryParams.availableDeviceCount"
                placeholder="请输入可用数量"
                clearable
                @keyup.enter="handleQuery"
                class="!w-200px"
        ...
        <div class="form-actions">
          ...
          <el-button
            ...
            v-hasPermi="['datacenter:emer-device-info:create']"
          >
          ...
          <el-button
            ...
            v-hasPermi="['datacenter:emer-device-info:export']"
          >
  ...
  <ContentWrap>
    <el-card class="table-card" shadow="never">
      <template #header>
        <div class="card-header">
          <span class="card-title">救援设备信息列表</span>
      ...
      >
        <el-table-column label="设备信息" align="center" min-width="240">
          <template #default="scope">
            <div class="device-info">
              <div class="device-code">{{ scope.row.deviceCode }}</div>
              <div class="device-name">{{ scope.row.deviceName }}</div>
              <div v-if="scope.row.deviceModel" class="device-model">
                型号: {{ scope.row.deviceModel }}
              </div>
              <div class="device-id">ID: {{ scope.row.deviceId }}</div>
            </div>
          </template>
        </el-table-column>
          ...
          <template #default="scope">
            <div class="quantity-info">
              <div class="total-devices">
                总数: <span class="quantity-value">{{ scope.row.deviceCount || 0 }}</span>
              </div>
              <div class="available-devices">
                可用: <span class="quantity-value">{{ scope.row.availableDeviceCount || 0 }}</span>
              </div>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="状态" align="center" width="80">
          <template #default="scope">
            <el-tag
              :type="scope.row.deviceStatus === '1' ? 'success' : 'danger'"
              size="small"
            >
        ...
        <el-table-column label="操作" align="center" width="120" fixed="right">
          <template #default="scope">
            <el-button
              ...
              v-hasPermi="['datacenter:emer-device-info:update']"
            >
              编辑
            </el-button>
            <el-button
              ...
              v-hasPermi="['datacenter:emer-device-info:delete']"
            >
              删除
            </el-button>
          </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EmerDeviceInfoForm ref="formRef" @success="getList" />
</template>
...
import EmerDeviceInfoForm from './EmerDeviceInfoForm.vue'

/** 救援设备信息 列表 */
defineOptions({ name: 'EmerDeviceInfo' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<EmerDeviceInfoVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EmerDeviceInfoApi.getEmerDeviceInfoPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EmerDeviceInfoApi.deleteEmerDeviceInfo(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EmerDeviceInfoApi.exportEmerDeviceInfo(queryParams)
    download.excel(data, '救援设备信息.xls')
  } catch {
  } finally {
