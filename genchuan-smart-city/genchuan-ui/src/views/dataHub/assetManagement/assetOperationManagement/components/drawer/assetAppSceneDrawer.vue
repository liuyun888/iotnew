  ...
  >
    <template #header>
      <div class="drawer-header">
        <span class="text-lg font-bold">资产关联应用场景详情</span>
    ...
    <el-scrollbar height="calc(100vh - 150px)">
      ...
      >
        ...
        <el-descriptions-item label="应用场景ID">
          {{ detail?.appSceneId ?? '-' }}
        </el-descriptions-item>

        <el-descriptions-item label="应用场景编码">
          {{ detail?.appSceneCode ?? '-' }}
        </el-descriptions-item>

        <el-descriptions-item label="应用场景名称">
          {{ detail?.appSceneName ?? '-' }}
        </el-descriptions-item>
...
const detail = ref<AssetAppSceneVO | null>(props.assetAppScene ?? null)
...
const handleRefresh = async () => {
  ...
  try {
    loading.value = true
    const res = await AssetAppSceneApi.getAssetAppScene(props.id)
    detail.value = res
...
watch(
  () => visible.value,
  (val) => {
    if (val && props.id && !props.assetAppScene) {
      handleRefresh()
    }
  },
  { immediate: true }
