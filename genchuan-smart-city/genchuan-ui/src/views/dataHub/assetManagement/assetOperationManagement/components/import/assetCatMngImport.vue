...
/** 资产分类管理导入组件 */
defineOptions({ name: 'AssetCatMngImport' })
...
const downloadTemplate = async () => {
  try {
    const data = await AssetCatMngApi.importAssetCatMngTemplate()
    download.excel(data, '资产分类管理模板.xls')
...
const submitFileForm = () => {
  ...
  // 使用 upload 方法进行导入
  upload.isUploading = true
  AssetCatMngApi.importAssetCatMng(formData)
    .then(response => {
