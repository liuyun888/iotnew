<template>
  <ContentWrap>
    ...
    <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" label-width="68px">
      ...
      <el-form-item label="关联资产名称" prop="relAssetName">
        <el-select v-model="queryParams.relAssetName" placeholder="请选择关联资产名称" filterable clearable class="!w-240px">
          <el-option v-for="op in dataMaps.options" :key="op.value" :label="op.label" :value="op.label" />
        </el-select>
      </el-form-item>
      <el-form-item label="网格编码" prop="gridCode">
        <el-input v-model="queryParams.gridCode" placeholder="请输入网格编码" clearable @keyup.enter="handleQuery"
          class="!w-240px" />
      </el-form-item>
      <el-form-item label="网格名称" prop="gridName">
        <el-select v-model="queryParams.gridName" placeholder="请选择网格名称" filterable clearable class="!w-240px">
          <el-option v-for="item in OptionsAll.gridNameOptions" :key="item.value" :label="item.label"
            :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="网格级别" prop="gridLevel">
        <el-select v-model="queryParams.gridLevel" placeholder="请选择网格级别" clearable class="!w-240px">
          <el-option v-for="item in OptionsAll.gridLevelOptions" :key="item.value" :label="item.label"
            :value="item.value" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-button type="primary" plain @click="openForm('create')"
          v-hasPermi="['assetOperationManagement:assetGrid:create']">
          <Icon icon="ep:plus" class="mr-5px" /> 新增
        </el-button>
        <el-button type="success" plain @click="handleExport" :loading="exportLoading"
          v-hasPermi="['assetOperationManagement:assetGrid:export']">
      ...
      <el-table-column label="操作" align="center" min-width="220px">
        <template #default="scope">
          ...
          <el-button link type="primary" @click="openForm('update', scope.row.id)"
            v-hasPermi="['assetOperationManagement:assetGrid:update']">
            编辑
          </el-button>
          <el-button link type="danger" @click="handleDelete(scope.row.id)"
            v-hasPermi="['assetOperationManagement:assetGrid:delete']">
            删除
          </el-button>
        </template>
      </el-table-column>
  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" v-loading="formLoading">
      ...
      <el-form-item label="网格名称" prop="gridName">
        <el-select v-model="formData.gridName" placeholder="请选择网格名称">
          <el-option v-for="item in OptionsAll.gridNameOptions" :key="item.value" :label="item.label"
            :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="网格级别" prop="gridLevel">
        <el-select v-model="formData.gridLevel" placeholder="请选择网格级别">
          <el-option v-for="item in OptionsAll.gridLevelOptions" :key="item.value" :label="item.label"
            :value="item.value" />
        </el-select>
      </el-form-item>
  ...
  <!-- 抽屉组件 -->
  <assetGridDrawer v-model="detailDrawerVisible" :id="selectedDetailId" />
</template>
...
import assetGridDrawer from './components/drawer/assetGridDrawer.vue'

/** 资产关联网格 列表 */
defineOptions({ name: 'AssetGrid' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<AssetGridVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const queryParams = reactive({
  pageNo: 1,
  pageSize: 15,
  assetRelGridId: undefined,
...
const getList = async () => {
  loading.value = true
  try {
    const data = await AssetGridApi.getAssetGridPage(queryParams)
...
const formRules = reactive({
  assetRelGridId: [{ required: true, message: '关联ID不能为空', trigger: 'blur' }],
  ...
  gridName: [{ required: true, message: '网格名称不能为空', trigger: 'change' }],
  gridLevel: [{ required: true, message: '网格级别不能为空', trigger: 'change' }],
...
const openForm = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await AssetGridApi.getAssetGrid(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as AssetGridVO
    if (formType.value === 'create') {
      await AssetGridApi.createAssetGrid(data)
      message.success(t('common.createSuccess'))
    } else {
      await AssetGridApi.updateAssetGrid(data)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await AssetGridApi.deleteAssetGrid(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await AssetGridApi.exportAssetGrid(queryParams)
    download.excel(data, '资产关联网格.xls')
  } catch {
  } finally {
