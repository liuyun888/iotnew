      ...
      <div class="drawer-content">
        ...
        <template v-if="localDrawerConfig.active==1">
          ...
          <div class="see-content" v-loading="localDrawerConfig.loading" v-if="localDrawerConfig.type === 'query'">
            <ul>
              <li class="width50"><span>服务端属性ID：</span>{{ localQueryData.assetServerAttrId }}</li>
              ...
              <li class="width50"><span>采集频率：</span>{{ localQueryData.collectFreq }}</li>
              <li class="width50"><span>最后采集时间：</span>{{ formatDate(localQueryData.lastCollectTime, 'YYYY-MM-DD') }}</li>
          ...
          <div class="edit-content" v-loading="localDrawerConfig.loading" v-else>
            <el-form :model="localFormData" ref="drawerFormRef" class="edit-content-form" label-width="140px">
              <!-- 服务端属性ID -->
              <el-form-item label="服务端属性ID：" prop="assetServerAttrId" class="edit-content-form-li width50" 
                :rules="[{ required: true, message: '服务端属性ID不能为空', trigger: 'blur' }]">
                <el-input placeholder="请输入服务端属性ID" v-model="localFormData.assetServerAttrId" clearable/>
              </el-form-item>
              ...
              <!-- 采集频率 -->
              <el-form-item label="采集频率：" prop="collectFreq" class="edit-content-form-li width50" 
                :rules="[{ required: true, message: '采集频率不能为空', trigger: 'change' }]">
                <el-select placeholder="请选择采集频率" v-model="localFormData.collectFreq" filterable clearable>
                  <el-option v-for="item in collectFreqOptions" :key="item.value" :label="item.label" :value="item.value"/>
                </el-select>
              </el-form-item>
              
              <!-- 最后采集时间 -->
              <el-form-item label="最后采集时间：" prop="lastCollectTime" class="edit-content-form-li width50">
                <el-date-picker class="width100" v-model="localFormData.lastCollectTime" type="date" placeholder="请选择最后采集时间" format="YYYY-MM-DD" value-format="x"/>
              </el-form-item>
