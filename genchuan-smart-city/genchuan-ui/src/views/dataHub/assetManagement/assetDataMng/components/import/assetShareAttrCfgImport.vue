...
import { AssetShareAttrCfgApi } from '@/api/dataHub/assetManagement/assetDataMng/assetShareAttrCfg'
import download from '@/utils/download'

/** 资产共享属性配置导入组件 */
defineOptions({ name: 'AssetShareAttrCfgImport' })
...
const downloadTemplate = async () => {
  try {
    const data = await AssetShareAttrCfgApi.importAssetShareAttrCfgTemplate()
    download.excel(data, '资产共享属性配置管理模板.xls')
...
const submitFileForm = () => {
  ...
  // 使用 upload 方法进行导入
  upload.isUploading = true
  AssetShareAttrCfgApi.importAssetShareAttrCfg(formData)
    .then(response => {
