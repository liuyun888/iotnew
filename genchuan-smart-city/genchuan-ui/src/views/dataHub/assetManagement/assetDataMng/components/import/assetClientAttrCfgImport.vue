...
import { AssetClientAttrCfgApi } from '@/api/dataHub/assetManagement/assetDataMng/assetClientAttrCfg'
import download from '@/utils/download'

/** 资产客户端属性配置导入组件 */
defineOptions({ name: 'AssetClientAttrCfgImport' })
...
const downloadTemplate = async () => {
  try {
    const data = await AssetClientAttrCfgApi.importAssetClientAttrCfgTemplate()
    download.excel(data, '资产客户端属性配置管理模板.xls')
...
const submitFileForm = () => {
  ...
  // 使用 upload 方法进行导入
  upload.isUploading = true
  AssetClientAttrCfgApi.importAssetClientAttrCfg(formData)
    .then(response => {
