...
import { AssetSpatialDataApi } from '@/api/dataHub/assetManagement/assetDataMng/assetSpatialData'
import download from '@/utils/download'

/** 资产空间数据导入组件 */
defineOptions({ name: 'AssetSpatialDataImport' })
...
const downloadTemplate = async () => {
  try {
    const data = await AssetSpatialDataApi.importAssetSpatialDataTemplate()
    download.excel(data, '资产空间数据管理模板.xls')
...
const submitFileForm = () => {
  ...
  // 使用 upload 方法进行导入
  upload.isUploading = true
  AssetSpatialDataApi.importAssetSpatialData(formData)
    .then(response => {
