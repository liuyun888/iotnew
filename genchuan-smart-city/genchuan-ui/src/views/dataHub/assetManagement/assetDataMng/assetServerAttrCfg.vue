<template>
  <div class="page-container">
      ...
      <div class="right-panel">
        <ContentWrap>
          ...
          <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" label-width="68px">
            ...
            <el-form-item label="采集频率" prop="collectFreq">
              <el-select v-model="queryParams.collectFreq" placeholder="请选择采集频率" clearable class="!w-240px">
                <el-option v-for="item in OptionsAll.collectFreqOptions" :key="item.value" :label="item.label"
                  :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item>
              ...
              <el-button type="primary" plain @click="addClick"
                v-hasPermi="['assetDataMng:assetServerAttrCfg:create']">
                <Icon icon="ep:plus" class="mr-5px" /> 新增
              </el-button>
              <el-button type="success" plain @click="handleExport" :loading="exportLoading"
                v-hasPermi="['assetDataMng:assetServerAttrCfg:export']">
                <Icon icon="ep:download" class="mr-5px" /> 导出
              </el-button>

              <!-- 导入 -->
              <el-button type="warning" plain @click="handleImport" v-hasPermi="['assetDataMng:assetServerAttrCfg:import']">
        ...
        <ContentWrap>
          ...
          <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true"
            ...
            <el-table-column label="服务端属性ID" align="center" prop="assetServerAttrId" />
            ...
            <el-table-column label="采集频率" align="center" prop="collectFreq" />
            <el-table-column label="最后采集时间" align="center" prop="lastCollectTime" :formatter="dateFormatter" width="180px"
            ...
            <el-table-column label="操作" align="center" min-width="200px">
              <template #default="scope">
                <el-button link type="primary" @click="queryClick(scope.row)"
                  v-hasPermi="['assetDataMng:assetServerAttrCfg:query']">
                  详细
                </el-button>
                <el-button link type="primary" @click="editClick(scope.row)"
                  v-hasPermi="['assetDataMng:assetServerAttrCfg:update']">
                  编辑
                </el-button>
                <el-button link type="danger" @click="handleDelete(scope.row.id)"
                  v-hasPermi="['assetDataMng:assetServerAttrCfg:delete']">
                  删除
                </el-button>
              </template>
            </el-table-column>
    ...
    <!-- 导入组件 -->
    <AssetServerAttrCfgImport ref="importRef" @success="handleImportSuccess" />

    <!-- 抽屉组件 -->
    <AssetServerAttrCfgDrawer
...
import { AssetServerAttrCfgApi, AssetServerAttrCfgVO } from '@/api/dataHub/assetManagement/assetDataMng/assetServerAttrCfg'
import AssetServerAttrCfgImport from './components/import/assetServerAttrCfgImport.vue'
import AssetServerAttrCfgDrawer from './components/drawer/assetServerAttrCfgDrawer.vue'
import AssetServerAttrCfgTree from './components/tree/assetServerAttrCfgTree.vue'

/** 资产服务端属性配置 列表 */
defineOptions({ name: 'AssetServerAttrCfg' })
...
const list = ref<AssetServerAttrCfgVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const queryParams = reactive({
  pageNo: 1,
  pageSize: 15,
  assetServerAttrId: undefined,
...
const getList = async () => {
  loading.value = true
  try {
    const data = await AssetServerAttrCfgApi.getAssetServerAttrCfgPage(queryParams)
...
const handleDrawerSubmit = async (formEl: any) => {
  try {
    const data = formData.value as unknown as AssetServerAttrCfgVO
    if (drawerConfig.value.type === 'add') {
      await AssetServerAttrCfgApi.createAssetServerAttrCfg(data)
      message.success(t('common.createSuccess'))
    } else {
      await AssetServerAttrCfgApi.updateAssetServerAttrCfg(data)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await AssetServerAttrCfgApi.deleteAssetServerAttrCfg(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await AssetServerAttrCfgApi.exportAssetServerAttrCfg(queryParams)
    download.excel(data, '资产服务端属性配置.xls')
  } catch {
  } finally {
...
const handleSortChange = (sort: { prop: string; order: string }) => {
  if (sort.prop === 'lastCollectTime') {
    queryParams.orderByColumn = 'lastCollectTime'
