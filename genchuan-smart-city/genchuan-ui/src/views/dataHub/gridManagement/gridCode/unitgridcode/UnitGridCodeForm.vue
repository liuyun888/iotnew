  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="编码ID" prop="unitCodeId">
        <el-input v-model="formData.unitCodeId" placeholder="请输入编码ID" />
      </el-form-item>
      <el-form-item label="标识码" prop="unitGridCode">
        <el-input v-model="formData.unitGridCode" placeholder="请输入单元网格标识码" />
      </el-form-item>
      <el-form-item label="所属网格ID" prop="unitGridId">
        <el-input v-model="formData.unitGridId" placeholder="请输入所属单元网格ID" />
      </el-form-item>
...
/** 单元网格编码 表单 */
defineOptions({ name: 'UnitGridCodeForm' })
...
const formRules = reactive({
  unitGridCode: [{ required: true, message: '单元网格标识码不能为空', trigger: 'blur' }],
  unitGridId: [{ required: true, message: '所属单元网格ID不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await UnitGridCodeApi.getUnitGridCode(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as UnitGridCodeVO
    if (formType.value === 'create') {
      await UnitGridCodeApi.createUnitGridCode(data)
      message.success(t('common.createSuccess'))
    } else {
      await UnitGridCodeApi.updateUnitGridCode(data)
