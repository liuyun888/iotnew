  ...
  <div class="analysis-asset-page">
          ...
          <div class="form-row">
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-asset:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-asset:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="资产大类名称" align="center" prop="assetMajorName" min-width="160" />
        <el-table-column label="资产小类名称" align="center" prop="assetMinorName" min-width="160" />
        <el-table-column label="资产总存量" align="center" prop="totalAssetCount" min-width="120" />
        <el-table-column label="资产完好率" align="center" prop="assetIntactRate" min-width="120" />
        <el-table-column
          label="系统创建时间"
          align="center"
          prop="createTimeSys"
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-asset:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-asset:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-asset:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="资产大类ID">{{ selectedRow?.assetMajorId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资产大类名称">{{ selectedRow?.assetMajorName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资产小类ID">{{ selectedRow?.assetMinorId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资产小类名称">{{ selectedRow?.assetMinorName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资产总存量">{{ selectedRow?.totalAssetCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资产完好率">{{ selectedRow?.assetIntactRate || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <AnalysisAssetForm ref="formRef" @success="getList" />
  </div>
</template>
...
import { AnalysisAssetApi, AnalysisAssetVO } from '@/api/dataHub/analysisDecision/analysisasset'
import AnalysisAssetForm from './AnalysisAssetForm.vue'

/** 按资产分域分析研判统计 列表 */
defineOptions({ name: 'AnalysisAsset' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<AnalysisAssetVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<AnalysisAssetVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: AnalysisAssetVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await AnalysisAssetApi.getAnalysisAssetPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await AnalysisAssetApi.deleteAnalysisAsset(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await AnalysisAssetApi.exportAnalysisAsset(queryParams)
    download.excel(data, '按资产分域分析研判统计.xls')
  } catch {
  } finally {
