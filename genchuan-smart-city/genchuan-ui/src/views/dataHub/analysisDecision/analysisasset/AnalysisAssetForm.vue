<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="资产总存量" prop="totalAssetCount">
        <el-input v-model="formData.totalAssetCount" placeholder="请输入资产总存量" />
      </el-form-item>
      <el-form-item label="资产完好率" prop="assetIntactRate">
        <el-input v-model="formData.assetIntactRate" placeholder="请输入资产完好率" />
      </el-form-item>
...
/** 按资产分域分析研判统计 表单 */
defineOptions({ name: 'AnalysisAssetForm' })
...
const formRules = reactive({
  ...
  totalAssetCount: [{ required: true, message: '资产总存量不能为空', trigger: 'blur' }],
  assetIntactRate: [{ required: true, message: '资产完好率不能为空', trigger: 'blur' }],
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await AnalysisAssetApi.getAnalysisAsset(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as AnalysisAssetVO
    if (formType.value === 'create') {
      await AnalysisAssetApi.createAnalysisAsset(data)
      message.success(t('common.createSuccess'))
    } else {
      await AnalysisAssetApi.updateAnalysisAsset(data)
