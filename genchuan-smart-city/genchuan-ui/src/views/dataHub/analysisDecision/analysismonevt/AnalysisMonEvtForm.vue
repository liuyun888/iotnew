...
/** 按监测事件分析研判统计 表单 */
defineOptions({ name: 'AnalysisMonEvtForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await AnalysisMonEvtApi.getAnalysisMonEvt(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as AnalysisMonEvtVO
    if (formType.value === 'create') {
      await AnalysisMonEvtApi.createAnalysisMonEvt(data)
      message.success(t('common.createSuccess'))
    } else {
      await AnalysisMonEvtApi.updateAnalysisMonEvt(data)
