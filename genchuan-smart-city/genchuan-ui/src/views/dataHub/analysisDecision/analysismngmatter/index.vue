<template>
  <div class="analysis-mng-matter-page">
    <!-- 搜索区域 -->
          ...
          <div class="form-row">
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mng-matter:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mng-matter:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="事项小类名称" align="center" prop="matterMinorName" min-width="160" />
        <el-table-column label="处置部门名称" align="center" prop="deptName" min-width="160" />
        <el-table-column label="事项上报总量" align="center" prop="totalRptCount" min-width="120" />
        <el-table-column label="事项办结率" align="center" prop="matterCompleteRate" min-width="120" />
        <el-table-column
          label="创建时间"
          align="center"
          prop="createTimeSys"
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mng-matter:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mng-matter:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mng-matter:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="事项上报总量">{{ selectedRow?.totalRptCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="事项办结率">{{ selectedRow?.matterCompleteRate || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <AnalysisMngMatterForm ref="formRef" @success="getList" />
  </div>
</template>
...
import { AnalysisMngMatterApi, AnalysisMngMatterVO } from '@/api/dataHub/analysisDecision/analysismngmatter'
import AnalysisMngMatterForm from './AnalysisMngMatterForm.vue'

/** 按管理事项分析研判统计 列表 */
defineOptions({ name: 'AnalysisMngMatter' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<AnalysisMngMatterVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<AnalysisMngMatterVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: AnalysisMngMatterVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await AnalysisMngMatterApi.getAnalysisMngMatterPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await AnalysisMngMatterApi.deleteAnalysisMngMatter(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await AnalysisMngMatterApi.exportAnalysisMngMatter(queryParams)
    download.excel(data, '按管理事项分析研判统计.xls')
  } catch {
  } finally {
