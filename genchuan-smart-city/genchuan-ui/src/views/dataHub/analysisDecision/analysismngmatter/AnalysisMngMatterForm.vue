<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="事项上报总量" prop="totalRptCount">
        <el-input v-model="formData.totalRptCount" placeholder="请输入事项上报总量" />
      </el-form-item>
      <el-form-item label="事项办结率" prop="matterCompleteRate">
        <el-input v-model="formData.matterCompleteRate" placeholder="请输入事项办结率" />
      </el-form-item>
...
/** 按管理事项分析研判统计 表单 */
defineOptions({ name: 'AnalysisMngMatterForm' })
...
const formRules = reactive({
  ...
  totalRptCount: [{ required: true, message: '事项上报总量不能为空', trigger: 'blur' }],
  matterCompleteRate: [{ required: true, message: '事项办结率不能为空', trigger: 'blur' }],
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await AnalysisMngMatterApi.getAnalysisMngMatter(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as AnalysisMngMatterVO
    if (formType.value === 'create') {
      await AnalysisMngMatterApi.createAnalysisMngMatter(data)
      message.success(t('common.createSuccess'))
    } else {
      await AnalysisMngMatterApi.updateAnalysisMngMatter(data)
