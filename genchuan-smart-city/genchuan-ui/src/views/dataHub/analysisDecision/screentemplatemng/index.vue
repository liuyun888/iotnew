<template>
  <div class="analysis-screen-template-mng-page">
    <!-- 搜索区域 -->
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="模板ID" align="center" prop="screenTemplateId" min-width="140" />
        <el-table-column label="模板名称" align="center" prop="templateName" min-width="180" />
        <el-table-column label="大屏布局" align="center" prop="screenLayout" min-width="160" />
        <el-table-column label="刷新频率" align="center" prop="refreshFreq" min-width="120" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:screen-template-mng:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
    ...
    >
      <div class="detail-header flex justify-between items-center p-4 border-b">
        <h3 class="text-lg font-semibold">大屏模板管理详情</h3>
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="模板ID">{{ selectedRow?.screenTemplateId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="模板名称">{{ selectedRow?.templateName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="大屏布局">{{ selectedRow?.screenLayout || '-' }}</el-descriptions-item>
          <el-descriptions-item label="数据来源配置">{{ selectedRow?.dataSourceCfg || '-' }}</el-descriptions-item>
          <el-descriptions-item label="组件配置">{{ selectedRow?.componentCfg || '-' }}</el-descriptions-item>
          <el-descriptions-item label="刷新频率">{{ selectedRow?.refreshFreq || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="模板预览图">{{ selectedRow?.templatePreview || '-' }}</el-descriptions-item>
...
const selectedRow = ref<ScreenTemplateMngVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: ScreenTemplateMngVO) => {
...
.analysis-screen-template-mng-page {
  padding: 8px;
  height: 100vh;
