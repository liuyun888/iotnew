<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="部件总数量" prop="totalCompCount">
        <el-input v-model="formData.totalCompCount" placeholder="请输入部件总数量" />
      </el-form-item>
      <el-form-item label="在线部件数" prop="onlineCompCount">
        <el-input v-model="formData.onlineCompCount" placeholder="请输入在线部件数" />
      </el-form-item>
...
/** 按监测部件分析研判统计 表单 */
defineOptions({ name: 'AnalysisMonCompForm' })
...
const formRules = reactive({
  ...
  totalCompCount: [{ required: true, message: '部件总数量不能为空', trigger: 'blur' }],
  onlineCompCount: [{ required: true, message: '在线部件数不能为空', trigger: 'blur' }],
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await AnalysisMonCompApi.getAnalysisMonComp(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as AnalysisMonCompVO
    if (formType.value === 'create') {
      await AnalysisMonCompApi.createAnalysisMonComp(data)
      message.success(t('common.createSuccess'))
    } else {
      await AnalysisMonCompApi.updateAnalysisMonComp(data)
