<template>
  <div class="analysis-mon-comp-page">
    <!-- 搜索区域 -->
          ...
          <div class="form-row">
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mon-comp:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mon-comp:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="部件总数量" align="center" prop="totalCompCount" min-width="120" />
        <el-table-column label="在线部件数" align="center" prop="onlineCompCount" min-width="120" />
        <el-table-column
          label="创建时间"
          align="center"
          prop="createTimeSys"
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mon-comp:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mon-comp:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-mon-comp:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="部件总数量">{{ selectedRow?.totalCompCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="在线部件数">{{ selectedRow?.onlineCompCount || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <AnalysisMonCompForm ref="formRef" @success="getList" />
  </div>
</template>
...
import { AnalysisMonCompApi, AnalysisMonCompVO } from '@/api/dataHub/analysisDecision/analysismoncomp'
import AnalysisMonCompForm from './AnalysisMonCompForm.vue'

/** 按监测部件分析研判统计 列表 */
defineOptions({ name: 'AnalysisMonComp' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<AnalysisMonCompVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<AnalysisMonCompVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: AnalysisMonCompVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await AnalysisMonCompApi.getAnalysisMonCompPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await AnalysisMonCompApi.deleteAnalysisMonComp(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await AnalysisMonCompApi.exportAnalysisMonComp(queryParams)
    download.excel(data, '按监测部件分析研判统计.xls')
  } catch {
  } finally {
