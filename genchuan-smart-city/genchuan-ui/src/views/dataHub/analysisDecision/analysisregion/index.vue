<template>
  <div class="analysis-region-page">
    <!-- 搜索区域 -->
          ...
          <div class="form-row">
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-region:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-region:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="正常监测部件数" align="center" prop="normalMonCompCount" min-width="140" />
        <el-table-column label="异常监测部件数" align="center" prop="abnMonCompCount" min-width="140" />
        <el-table-column label="监测事件办结率" align="center" prop="monEvtCompleteRate" min-width="140" />
        <el-table-column
          label="统计时间"
          align="center"
          prop="statTime"
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-region:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-region:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-region:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="行政区划级别">{{ selectedRow?.regionLevel || '-' }}</el-descriptions-item>
          <el-descriptions-item label="正常监测部件数">{{ selectedRow?.normalMonCompCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="异常监测部件数">{{ selectedRow?.abnMonCompCount || '-' }}</el-descriptions-item>
          <el-descriptions-item label="监测事件办结率">{{ selectedRow?.monEvtCompleteRate || '-' }}</el-descriptions-item>
          <el-descriptions-item label="统计人">{{ selectedRow?.statUser || '-' }}</el-descriptions-item>
          <el-descriptions-item label="统计时间">{{ dateFormatter(selectedRow?.statTime) || '-' }}</el-descriptions-item>
          <el-descriptions-item label="报表备注">{{ selectedRow?.rptRemark || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <AnalysisRegionForm ref="formRef" @success="getList" />
  </div>
</template>
...
import AnalysisRegionForm from './AnalysisRegionForm.vue'

/** 按行政区划分析研判统计 列表 */
defineOptions({ name: 'AnalysisRegion' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<AnalysisRegionVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<AnalysisRegionVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: AnalysisRegionVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await AnalysisRegionApi.getAnalysisRegionPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await AnalysisRegionApi.deleteAnalysisRegion(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await AnalysisRegionApi.exportAnalysisRegion(queryParams)
    download.excel(data, '按行政区划分析研判统计.xls')
  } catch {
  } finally {
