<template>
  <div class="analysis-app-scene-page">
    <!-- 搜索区域 -->
          ...
          <div class="form-row">
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-app-scene:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-app-scene:export']"
              >
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-app-scene:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-app-scene:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:analysis-app-scene:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <AnalysisAppSceneForm ref="formRef" @success="getList" />
  </div>
</template>
...
import { AnalysisAppSceneApi, AnalysisAppSceneVO } from '@/api/dataHub/analysisDecision/analysisappscene'
import AnalysisAppSceneForm from './AnalysisAppSceneForm.vue'

/** 按应用场景分析研判统计 列表 */
defineOptions({ name: 'AnalysisAppScene' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<AnalysisAppSceneVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<AnalysisAppSceneVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: AnalysisAppSceneVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await AnalysisAppSceneApi.getAnalysisAppScenePage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await AnalysisAppSceneApi.deleteAnalysisAppScene(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await AnalysisAppSceneApi.exportAnalysisAppScene(queryParams)
    download.excel(data, '按应用场景分析研判统计.xls')
  } catch {
  } finally {
