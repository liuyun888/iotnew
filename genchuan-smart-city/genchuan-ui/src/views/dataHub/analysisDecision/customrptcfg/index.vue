<template>
  <div class="analysis-custom-rpt-cfg-page">
    <!-- 搜索区域 -->
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="配置ID" align="center" prop="customRptCfgId" min-width="140" />
        <el-table-column label="报表模板名称" align="center" prop="rptTemplateName" min-width="180" />
        <el-table-column label="报表类型" align="center" prop="rptType" min-width="120" />
        <el-table-column label="数据来源表" align="center" prop="dataSourceTable" min-width="160" />
        <el-table-column label="模板状态" align="center" prop="templateStatus" min-width="120" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:custom-rpt-cfg:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
    ...
    >
      <div class="detail-header flex justify-between items-center p-4 border-b">
        <h3 class="text-lg font-semibold">自定义报表配置详情</h3>
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="配置ID">{{ selectedRow?.customRptCfgId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="报表模板名称">{{ selectedRow?.rptTemplateName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="报表类型">{{ selectedRow?.rptType || '-' }}</el-descriptions-item>
          <el-descriptions-item label="数据来源表">{{ selectedRow?.dataSourceTable || '-' }}</el-descriptions-item>
          <el-descriptions-item label="统计维度配置">{{ selectedRow?.statDimensionCfg || '-' }}</el-descriptions-item>
          <el-descriptions-item label="统计指标配置">{{ selectedRow?.statIdxCfg || '-' }}</el-descriptions-item>
          <el-descriptions-item label="展示样式配置">{{ selectedRow?.displayStyleCfg || '-' }}</el-descriptions-item>
          <el-descriptions-item label="筛选条件配置">{{ selectedRow?.filterCfg || '-' }}</el-descriptions-item>
          <el-descriptions-item label="模板状态">{{ selectedRow?.templateStatus || '-' }}</el-descriptions-item>
...
const selectedRow = ref<CustomRptCfgVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: CustomRptCfgVO) => {
