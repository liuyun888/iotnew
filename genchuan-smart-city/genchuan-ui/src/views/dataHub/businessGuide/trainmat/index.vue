  ...
  <div class="train-mat-page">
        ...
        >
          <div class="form-row">
            <el-form-item label="资料ID" prop="trainMatId">
              <el-input
                v-model="queryParams.trainMatId"
                placeholder="请输入资料ID"
                clearable
            ...
            <el-form-item label="培训类型名称" prop="trainTypeName">
              <el-input
                v-model="queryParams.trainTypeName"
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:train-mat:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:train-mat:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="资料ID" align="center" prop="trainMatId" min-width="140" />
        <el-table-column label="资料名称" align="center" prop="matName" min-width="160" />
        <el-table-column label="培训类型名称" align="center" prop="trainTypeName" min-width="140" />
        <el-table-column label="培训对象" align="center" prop="trainObject" min-width="140" />
        <el-table-column label="资料状态" align="center" prop="matStatus" width="120" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:train-mat:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:train-mat:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:train-mat:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="资料ID">{{ selectedRow?.trainMatId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资料名称">{{ selectedRow?.matName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="培训类型代码">{{ selectedRow?.trainTypeCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="培训类型名称">{{ selectedRow?.trainTypeName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="培训对象">{{ selectedRow?.trainObject || '-' }}</el-descriptions-item>
          <el-descriptions-item label="培训时长">{{ selectedRow?.trainEndure || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资料格式">{{ selectedRow?.matFormat || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资料大小">{{ selectedRow?.matSize || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资料摘要">{{ selectedRow?.matAbstract || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资料文件ID">{{ selectedRow?.matFileId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资料文件路径">{{ selectedRow?.matFilePath || '-' }}</el-descriptions-item>
          <el-descriptions-item label="资料状态">{{ selectedRow?.matStatus || '-' }}</el-descriptions-item>
          <el-descriptions-item label="下载次数">{{ selectedRow?.downloadCount || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <TrainMatForm ref="formRef" @success="getList" />
  </div>
</template>
...
import TrainMatForm from './TrainMatForm.vue'

/** 培训资料 列表 */
defineOptions({ name: 'TrainMat' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<TrainMatVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<TrainMatVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: TrainMatVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await TrainMatApi.getTrainMatPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await TrainMatApi.deleteTrainMat(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await TrainMatApi.exportTrainMat(queryParams)
    download.excel(data, '培训资料.xls')
  } catch {
  } finally {
