  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="动态ID" prop="localDynamicId">
        <el-input v-model="formData.localDynamicId" placeholder="请输入动态ID" />
      </el-form-item>
...
/** 本地行业动态 表单 */
defineOptions({ name: 'LocalDynamicForm' })
...
const formRules = reactive({
  localDynamicId: [{ required: true, message: '动态ID不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await LocalDynamicApi.getLocalDynamic(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as LocalDynamicVO
    if (formType.value === 'create') {
      await LocalDynamicApi.createLocalDynamic(data)
      message.success(t('common.createSuccess'))
    } else {
      await LocalDynamicApi.updateLocalDynamic(data)
