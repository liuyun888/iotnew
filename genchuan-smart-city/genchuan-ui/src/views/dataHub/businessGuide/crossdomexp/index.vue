  ...
  <div class="cross-dom-exp-page">
        ...
        >
          <div class="form-row">
            <el-form-item label="经验ID" prop="crossDomExpId">
              <el-input
                v-model="queryParams.crossDomExpId"
                placeholder="请输入经验ID"
                clearable
            ...
            <el-form-item label="经验领域名称" prop="expFieldName">
              <el-input
                v-model="queryParams.expFieldName"
                placeholder="请输入经验领域名称"
                clearable
                @keyup.enter="handleQuery"
                class="!w-160px"
              />
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:cross-dom-exp:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:cross-dom-exp:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="经验ID" align="center" prop="crossDomExpId" min-width="140" />
        <el-table-column label="经验标题" align="center" prop="expTitle" min-width="160" />
        <el-table-column label="经验领域名称" align="center" prop="expFieldName" min-width="140" />
        <el-table-column label="来源城市" align="center" prop="sourceCity" min-width="120" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:cross-dom-exp:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:cross-dom-exp:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:cross-dom-exp:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="经验ID">{{ selectedRow?.crossDomExpId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="经验标题">{{ selectedRow?.expTitle || '-' }}</el-descriptions-item>
          <el-descriptions-item label="经验领域代码">{{ selectedRow?.expFieldCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="经验领域名称">{{ selectedRow?.expFieldName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="来源城市">{{ selectedRow?.sourceCity || '-' }}</el-descriptions-item>
          <el-descriptions-item label="经验详细内容">{{ selectedRow?.expDetail || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="浏览次数">{{ selectedRow?.viewCount || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <CrossDomExpForm ref="formRef" @success="getList" />
  </div>
</template>
...
import CrossDomExpForm from './CrossDomExpForm.vue'

/** 跨域经验交流 列表 */
defineOptions({ name: 'CrossDomExp' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<CrossDomExpVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<CrossDomExpVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: CrossDomExpVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await CrossDomExpApi.getCrossDomExpPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await CrossDomExpApi.deleteCrossDomExp(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await CrossDomExpApi.exportCrossDomExp(queryParams)
    download.excel(data, '跨域经验交流.xls')
  } catch {
  } finally {
