  ...
  <div class="faq-page">
        ...
        >
          <div class="form-row">
            <el-form-item label="FAQ ID" prop="faqId">
              <el-input
                v-model="queryParams.faqId"
                placeholder="请输入FAQ ID"
                clearable
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:faq:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:faq:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="FAQ ID" align="center" prop="faqId" min-width="140" />
        <el-table-column label="问题类型名称" align="center" prop="questionTypeName" min-width="140" />
        <el-table-column label="问题描述" align="center" prop="questionDesc" min-width="200" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:faq:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:faq:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:faq:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="FAQ ID">{{ selectedRow?.faqId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="问题类型代码">{{ selectedRow?.questionTypeCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="问题类型名称">{{ selectedRow?.questionTypeName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="问题描述">{{ selectedRow?.questionDesc || '-' }}</el-descriptions-item>
          <el-descriptions-item label="解答方案">{{ selectedRow?.answerSolution || '-' }}</el-descriptions-item>
          <el-descriptions-item label="关联指南ID">{{ selectedRow?.relatedGuideId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="关联指南名称">{{ selectedRow?.relatedGuideName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="排序序号">{{ selectedRow?.sortNum || '-' }}</el-descriptions-item>
          <el-descriptions-item label="是否置顶">
            <el-tag :type="selectedRow?.isTop === '1' ? 'success' : 'info'">
              {{ selectedRow?.isTop === '1' ? '是' : '否' }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="查看次数">{{ selectedRow?.viewCount || '-' }}</el-descriptions-item>
...
const list = ref<FaqVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<FaqVO | null>(null)
...
const getList = async () => {
  loading.value = true
  try {
    const data = await FaqApi.getFaqPage(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await FaqApi.exportFaq(queryParams)
    download.excel(data, '常见问题解答.xls')
  } catch {
  } finally {
