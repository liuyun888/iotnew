<template>
  <div class="local-level-exp-page">
    <!-- 搜索区域 -->
        ...
        >
          <div class="form-row">
            <el-form-item label="经验ID" prop="localLevelExpId">
              <el-input
                v-model="queryParams.localLevelExpId"
                placeholder="请输入经验ID"
                clearable
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:local-level-exp:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:local-level-exp:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="经验ID" align="center" prop="localLevelExpId" min-width="140" />
        <el-table-column label="经验标题" align="center" prop="expTitle" min-width="200" />
        <el-table-column label="行政区划名称" align="center" prop="regionName" min-width="160" />
        <el-table-column label="关联部门名称" align="center" prop="deptName" min-width="160" />
        <el-table-column label="发布状态" align="center" prop="publishStatus" width="120" />
        <el-table-column label="分享次数" align="center" prop="shareCount" width="100" />
        <el-table-column
          label="创建时间"
          align="center"
          prop="createTime"
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:local-level-exp:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:local-level-exp:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:local-level-exp:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="经验ID">{{ selectedRow?.localLevelExpId || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="关联部门代码">{{ selectedRow?.deptCode || '-' }}</el-descriptions-item>
          <el-descriptions-item label="关联部门名称">{{ selectedRow?.deptName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="经验详细内容">{{ selectedRow?.expContent || '-' }}</el-descriptions-item>
          <el-descriptions-item label="经验实施步骤">{{ selectedRow?.implementationSteps || '-' }}</el-descriptions-item>
          <el-descriptions-item label="实施成果">{{ selectedRow?.achievementData || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="分享次数">{{ selectedRow?.shareCount || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <LocalLevelExpForm ref="formRef" @success="getList" />
  </div>
</template>
...
import LocalLevelExpForm from './LocalLevelExpForm.vue'

/** 本级经验分享 列表 */
defineOptions({ name: 'LocalLevelExp' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<LocalLevelExpVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<LocalLevelExpVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: LocalLevelExpVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await LocalLevelExpApi.getLocalLevelExpPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await LocalLevelExpApi.deleteLocalLevelExp(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await LocalLevelExpApi.exportLocalLevelExp(queryParams)
    download.excel(data, '本级经验分享.xls')
  } catch {
  } finally {
