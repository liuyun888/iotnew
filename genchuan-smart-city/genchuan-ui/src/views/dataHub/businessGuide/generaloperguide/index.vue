<template>
  <div class="general-oper-guide-page">
    <!-- 搜索区域 -->
        ...
        >
          <div class="form-row">
            <el-form-item label="指南ID" prop="generalOperGuideId">
              <el-input
                v-model="queryParams.generalOperGuideId"
                placeholder="请输入指南ID"
                clearable
            ...
            <el-form-item label="操作场景" prop="operScenario">
              <el-input
                v-model="queryParams.operScenario"
                placeholder="请输入操作场景"
                clearable
                @keyup.enter="handleQuery"
                class="!w-160px"
            ...
            <div class="search-buttons-group">
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:general-oper-guide:create']"
              >
              ...
              <el-button
                ...
                v-hasPermi="['datacenter:general-oper-guide:export']"
              >
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="指南ID" align="center" prop="generalOperGuideId" min-width="140" />
        <el-table-column label="指南标题" align="center" prop="guideTitle" min-width="160" />
        <el-table-column label="操作场景" align="center" prop="operScenario" min-width="160" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:general-oper-guide:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:general-oper-guide:update']"
                class="operation-btn"
              >
                编辑
              </el-button>
              <el-button
                ...
                v-hasPermi="['datacenter:general-oper-guide:delete']"
                class="operation-btn"
              >
                删除
              </el-button>
    ...
    >
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="指南ID">{{ selectedRow?.generalOperGuideId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="指南标题">{{ selectedRow?.guideTitle || '-' }}</el-descriptions-item>
          <el-descriptions-item label="操作场景">{{ selectedRow?.operScenario || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="版本号">{{ selectedRow?.version || '-' }}</el-descriptions-item>
    ...
    <!-- 表单弹窗：添加/修改 -->
    <GeneralOperGuideForm ref="formRef" @success="getList" />
  </div>
</template>
...
import GeneralOperGuideForm from './GeneralOperGuideForm.vue'

/** 通用操作指南 列表 */
defineOptions({ name: 'GeneralOperGuide' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<GeneralOperGuideVO[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const exportLoading = ref(false) // 导出的加载中

// 详情伪抽屉相关变量
const isDetailShow = ref(false)
const selectedRow = ref<GeneralOperGuideVO | null>(null)
...
/** 打开详情伪抽屉 */
const openDetail = (row: GeneralOperGuideVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await GeneralOperGuideApi.getGeneralOperGuidePage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await GeneralOperGuideApi.deleteGeneralOperGuide(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await GeneralOperGuideApi.exportGeneralOperGuide(queryParams)
    download.excel(data, '通用操作指南.xls')
  } catch {
  } finally {
