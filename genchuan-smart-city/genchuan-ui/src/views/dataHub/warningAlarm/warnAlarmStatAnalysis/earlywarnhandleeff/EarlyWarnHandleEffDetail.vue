  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">预警告警处置效率统计详情</span>
    ...
    <div class="detail-content">
      <div class="grid-layout">
        ...
        <el-card class="info-card basic-card">
          ...
          <div class="card-body">
            ...
            <div class="info-item">
              ...
              <span class="value key-field">{{ detailData?.efficacyStatId || '-' }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="info-card dept-card">
          <template #header>
            <div class="card-header dept-header">
              <span class="header-title">处置部门信息</span>
            </div>
          </template>
          <div class="card-body">
            <div class="info-item">
              <span class="label">处置部门ID：</span>
              <span class="value key-field">{{ detailData?.handleDeptId || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">处置部门名称：</span>
              <span class="value key-field">{{ detailData?.handleDeptName || '-' }}</span>
            </div>
          </div>
        </el-card>

        <!-- 处置统计卡片：马克龙浅橙 -->
        <el-card class="info-card stat-card">
          <template #header>
            <div class="card-header stat-header">
              <span class="header-title">处置统计信息</span>
            </div>
          </template>
          <div class="card-body">
            <div class="info-item">
              <span class="label">处置预警总数：</span>
              <span class="value count-total">{{ detailData?.totalHandleCount || 0 }}</span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <span
                class="value"
                :class="detailData?.handleRate > 80 ? 'status-success' : 'status-warning'"
              >
                {{ detailData?.handleRate ? `${detailData.handleRate.toFixed(2)}%` : '0%' }}
              </span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="info-card time-card">
          <template #header>
            <div class="card-header time-header">
              <span class="header-title">处置时效信息</span>
            </div>
          </template>
          <div class="card-body">
            <div class="info-item">
              <span class="label">平均处置时长：</span>
              <span class="value count-total">{{ detailData?.avgHandleEndure || 0 }}</span>
            </div>
            <div class="info-item">
              <span class="label">严重等级平均时长：</span>
              <span class="value status-warning">{{ detailData?.criticalAvgEndure || 0 }}</span>
            </div>
          </div>
        </el-card>

        <!-- 处置类型卡片：马克龙浅粉 -->
        <el-card class="info-card type-card">
          <template #header>
            <div class="card-header type-header">
              <span class="header-title">处置类型信息</span>
            </div>
          </template>
          <div class="card-body">
            <div class="info-item">
              <span class="label">主要处置类型：</span>
              <span class="value">{{ detailData?.mainHandleType || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">主要处置类型占比：</span>
              <span class="value rate-field">{{
                detailData?.mainTypeProportion
                  ? `${detailData.mainTypeProportion.toFixed(2)}%`
                  : '0%'
              }}</span>
            </div>
          </div>
...
import { EarlyWarnHandleEffVO } from '@/api/dataHub/warningAlarm/warnAlarmStatAnalysis/earlywarnhandleeff'

defineOptions({ name: 'EarlyWarnHandleEffDetail' })
...
const props = defineProps({
  ...
  detailData: {
    type: Object as () => EarlyWarnHandleEffVO | null,
    default: null
  }
})
