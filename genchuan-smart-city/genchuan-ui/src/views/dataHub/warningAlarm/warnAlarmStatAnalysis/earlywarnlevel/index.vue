<template>
  <ContentWrap>
    ...
    >
      <el-form-item label="统计ID" prop="levelStatId">
        <el-input
          v-model="queryParams.levelStatId"
          placeholder="请输入统计ID"
          clearable
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column label="统计ID" align="center" prop="levelStatId" />
      <el-table-column label="统计周期" align="center" prop="statCycle" />
      <el-table-column label="预警等级" align="center" prop="warnLevel">
        <template #default="scope">
          <span :class="getWarnLevelClass(scope.row.warnLevelCode)">{{
            scope.row.warnLevel || '-'
          }}</span>
        </template>
      </el-table-column>
      <el-table-column label="预警等级编码" align="center" prop="warnLevelCode" />
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EarlyWarnLevelForm ref="formRef" @success="getList" />
  <!-- 详情抽屉 -->
  <EarlyWarnLevelDetail
...
import EarlyWarnLevelForm from './EarlyWarnLevelForm.vue'
import EarlyWarnLevelDetail from './EarlyWarnLevelDetail.vue'

/** 预警告警等级维度统计 列表 */
defineOptions({ name: 'EarlyWarnLevel' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<EarlyWarnLevelVO[]>([]) // 列表的数据
...
const currentDetail = ref<EarlyWarnLevelVO | null>(null)

/** 打开详情抽屉 */
const openDetail = (row: EarlyWarnLevelVO) => {
  currentDetail.value = { ...row }
  detailVisible.value = true
}

/** 预警等级颜色标识 */
const getWarnLevelClass = (code?: string) => {
  switch (code) {
    case 'critical':
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EarlyWarnLevelApi.getEarlyWarnLevelPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EarlyWarnLevelApi.deleteEarlyWarnLevel(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EarlyWarnLevelApi.exportEarlyWarnLevel(queryParams)
    download.excel(data, '预警告警等级维度统计.xls')
  } catch {
  } finally {
