<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column label="主要分布区域" align="center" prop="mainRegionName" />
      <el-table-column
        label="统计时间"
        align="center"
        prop="statTime"
  ...
  <!-- 表单弹窗：添加/修改 -->
  <EarlyWarnTypeForm ref="formRef" @success="getList" />
  <!-- 详情抽屉 -->
  <EarlyWarnTypeDetail
...
import {
  EarlyWarnTypeApi,
  EarlyWarnTypeVO
} from '@/api/dataHub/warningAlarm/warnAlarmStatAnalysis/earlywarntype'
import EarlyWarnTypeForm from './EarlyWarnTypeForm.vue'
import EarlyWarnTypeDetail from './EarlyWarnTypeDetail.vue'

/** 预警告警类型维度统计 列表 */
defineOptions({ name: 'EarlyWarnType' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<EarlyWarnTypeVO[]>([]) // 列表的数据
...
const currentDetail = ref<EarlyWarnTypeVO | null>(null)

/** 打开详情抽屉 */
const openDetail = (row: EarlyWarnTypeVO) => {
...
const getList = async () => {
  loading.value = true
  try {
    const data = await EarlyWarnTypeApi.getEarlyWarnTypePage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await EarlyWarnTypeApi.deleteEarlyWarnType(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await EarlyWarnTypeApi.exportEarlyWarnType(queryParams)
    download.excel(data, '预警告警类型维度统计.xls')
  } catch {
  } finally {
