  ...
  >
    ...
    <template #header>
      <div class="flex justify-between items-center w-full px-2">
        <span class="text-lg font-bold text-gray-800">预警告警类型维度统计详情</span>
        ...
        <el-card class="info-card basic-card">
          ...
          <div class="card-body">
            ...
            <div class="info-item">
              ...
              <span class="value key-field">{{ detailData?.typeStatId || '-' }}</span>
            </div>
            <div class="info-item">
        ...
        <el-card class="info-card type-card">
          <template #header>
            <div class="card-header type-header">
              <span class="header-title">预警类型信息</span>
            </div>
          </template>
          <div class="card-body">
            <div class="info-item">
              <span class="label">预警类型：</span>
              <span class="value">{{ detailData?.warnType || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">预警类型编码：</span>
              <span class="value key-field">{{ detailData?.warnTypeCode || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">占比：</span>
              <span class="value rate-field">{{
                detailData?.proportion ? `${detailData.proportion.toFixed(2)}%` : '0%'
              }}</span>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="info-card region-card">
          <template #header>
            <div class="card-header region-header">
              <span class="header-title">分布区域信息</span>
            </div>
          </template>
          <div class="card-body">
            <div class="info-item">
              <span class="label">主要分布区域名称：</span>
              <span class="value">{{ detailData?.mainRegionName || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="label">主要分布区域数量：</span>
              <span class="value count-total">{{ detailData?.mainRegionCount || 0 }}</span>
            </div>
          </div>
        </el-card>
...
import { EarlyWarnTypeVO } from '@/api/dataHub/warningAlarm/warnAlarmStatAnalysis/earlywarntype'

defineOptions({ name: 'EarlyWarnTypeDetail' })
...
const props = defineProps({
  ...
  detailData: {
    type: Object as () => EarlyWarnTypeVO | null,
    default: null
  }
})
