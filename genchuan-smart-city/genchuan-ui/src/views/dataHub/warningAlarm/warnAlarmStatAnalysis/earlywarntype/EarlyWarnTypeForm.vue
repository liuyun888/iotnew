  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="700px">
    ...
    >
      ...
      <el-row :gutter="24">
        ...
        <el-col :span="12">
          <el-form-item label="预警类型编码" prop="warnTypeCode">
            <el-input v-model="formData.warnTypeCode" placeholder="请输入预警类型编码" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="主要分布区域数量" prop="mainRegionCount">
            <el-input
              v-model="formData.mainRegionCount"
              placeholder="请输入主要分布区域数量"
              type="number"
            />
          </el-form-item>
        </el-col>
...
/** 预警告警类型维度统计 表单 */
defineOptions({ name: 'EarlyWarnTypeForm' })
...
const formData = ref<Partial<EarlyWarnTypeVO>>({
  id: undefined,
...
const formRules = reactive({
  ...
  warnType: [{ required: true, message: '预警类型不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const res = await EarlyWarnTypeApi.getEarlyWarnType(id)
      formData.value = { ...res }
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const submitData = formData.value as EarlyWarnTypeVO
    if (formType.value === 'create') {
      await EarlyWarnTypeApi.createEarlyWarnType(submitData)
      message.success(t('common.createSuccess'))
    } else {
      await EarlyWarnTypeApi.updateEarlyWarnType(submitData)
