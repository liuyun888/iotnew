  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="650px">
    ...
    >
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="统计ID" prop="warnMonEvtStatId">
            <el-input v-model="formData.warnMonEvtStatId" placeholder="请输入统计ID" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="事件大类ID" prop="evtMajorId">
            <el-input v-model="formData.evtMajorId" placeholder="请输入事件大类ID" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="事件大类名称" prop="evtMajorName">
            <el-input v-model="formData.evtMajorName" placeholder="请输入事件大类名称" />
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="事件小类ID" prop="evtMinorId">
            <el-input v-model="formData.evtMinorId" placeholder="请输入事件小类ID" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="事件小类名称" prop="evtMinorName">
            <el-input v-model="formData.evtMinorName" placeholder="请输入事件小类名称" />
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <el-form-item label="平均响应时间" prop="avgRespTime">
            <el-date-picker
              v-model="formData.avgRespTime"
              type="date"
              value-format="x"
              placeholder="选择平均响应时间"
            />
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="涉及区域数" prop="regionCount">
            <el-input v-model.number="formData.regionCount" placeholder="请输入涉及区域数" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
/** 按监测事件预警告警统计 表单 */
defineOptions({ name: 'EarlyWarnMonEvtForm' })
...
const formData = ref<Partial<EarlyWarnMonEvtVO>>({
  id: undefined,
...
const formRules = reactive({
  warnMonEvtStatId: [{ required: true, message: '统计ID不能为空', trigger: 'blur' }],
  statCycle: [{ required: true, message: '统计周期不能为空', trigger: 'blur' }],
  evtMajorId: [{ required: true, message: '事件大类ID不能为空', trigger: 'blur' }],
  evtMajorName: [{ required: true, message: '事件大类名称不能为空', trigger: 'blur' }],
  evtMinorId: [{ required: true, message: '事件小类ID不能为空', trigger: 'blur' }],
  evtMinorName: [{ required: true, message: '事件小类名称不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await EarlyWarnMonEvtApi.getEarlyWarnMonEvt(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as EarlyWarnMonEvtVO
    if (formType.value === 'create') {
      await EarlyWarnMonEvtApi.createEarlyWarnMonEvt(data)
      message.success(t('common.createSuccess'))
    } else {
      await EarlyWarnMonEvtApi.updateEarlyWarnMonEvt(data)
