<template>
  <Dialog v-model="dialogVisible" title="告警预警导入" width="400">
...
const uploadRef = ref()
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/datacenter/warning-alert-list-table/import'
const uploadHeaders = ref() // 上传 Header 头
const fileList = ref([]) // 文件列表
const updateSupport = ref(0) // 是否更新已经存在的告警预警信息数据
...
const submitFormSuccess = (response: any) => {
  ...
  // for (let sampleName of data.createSampleNames) {
  //   text += '< ' + sampleName + ' >'
  // }
...
const importTemplate = async () => {
  const res = await WarningAlertListTableApi.importAlarmWarningTemplate()
  download.excel(res, '告警预警信息导入模版.xls')
}
</script>
