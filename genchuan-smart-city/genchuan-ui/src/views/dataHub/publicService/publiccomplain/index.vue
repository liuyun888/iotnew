<template>
  <div class="public-complain-page">
    <!-- 搜索区域 -->
    ...
    <ContentWrap class="list-container">
      ...
      >
        ...
        <el-table-column label="投诉ID" align="center" prop="complainId" min-width="120" />
        <el-table-column label="投诉单号" align="center" prop="complainNo" min-width="140" />
          ...
          <template #default="scope">
            <div class="operation-btn-group">
              <el-button
                ...
                v-hasPermi="['datacenter:public-complain:detail']"
                class="operation-btn"
              >
                详情
              </el-button>
    ...
    >
      <div class="detail-header flex justify-between items-center p-4 border-b">
        <h3 class="text-lg font-semibold">投诉举报详情</h3>
      ...
      <div class="detail-content p-6">
        ...
        >
          ...
          <el-descriptions-item label="投诉ID">{{ selectedRow?.complainId || '-' }}</el-descriptions-item>
          <el-descriptions-item label="投诉单号">{{ selectedRow?.complainNo || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="事发地址">{{ selectedRow?.incidentAddr || '-' }}</el-descriptions-item>
          <el-descriptions-item label="投诉内容">{{ selectedRow?.complainContent || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="受理时间">{{ dateFormatter(selectedRow?.acceptTime) || '-' }}</el-descriptions-item>
...
const selectedRow = ref<PublicComplainVO | null>(null)
...
/** 打开详情抽屉 */
const openDetail = (row: PublicComplainVO) => {
...
const handleDelete = async (id: number) => {
  try {
    ...
    await PublicComplainApi.deletePublicComplain(id) // 发起删除请求
...
const handleExport = async () => {
  try {
    ...
    download.excel(data, '投诉举报.xls') // 下载Excel
  } catch {
  } finally {
