        ...
        >
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              ...
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['profiles:deviceProfile:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['profiles:deviceProfile:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
...
import {deviceProfileQueryList, deviceProfileAddEdit, deviceProfileDel} from "@/api/genchuan/thingsBoard/profiles";
...
const tableInitData = async () => {
  ...
  try {
    ...
    let res = await deviceProfileQueryList(params);
...
const drawerSubmitForm = async (formEl: FormInstance | undefined) => {
  ...
  if (valid) {
    try {
      await deviceProfileAddEdit(formData.value);
...
const delClick = (row) => {
  ElMessageBox.confirm(
      ...
      .then(async () => {
        try {
          await deviceProfileDel(row.id.id)
          await tableInitData();
