    ...
    <div class="cc-main-content">
      ...
      <div class="cc-search-css" ref="searchRef">
        <el-form class="-mb-15px" :model="searchForm" ref="searchFormRef" :inline="true" label-width="68px">
          ...
          <el-form-item label="配置名称" prop="name">
            <el-input v-model="searchForm.name" placeholder="请输入配置名称" clearable class="!w-240px"/>
          </el-form-item>
          <el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column label="配置名称" min-width="150" align="center" prop="name" :show-overflow-tooltip="true" v-if="tableColumnShow('配置名称')">
            <template #default="scope">
              <div @click="idCopy(scope.row.name)" class="copy-css">{{scope.row.name}}</div>
            </template>
          </el-table-column>
          <el-table-column label="说明" min-width="150" align="center" prop="description" :show-overflow-tooltip="true" v-if="tableColumnShow('说明')"/>
          <el-table-column label="创建时间" align="center" prop="createdTime" :formatter="dateFormatter" width="200px" v-if="tableColumnShow('创建时间')"/>
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button type="success" :icon="View" :plain="true" @click="queryClick(scope.row)" v-hasPermi="['profiles:assetProfile:query']">详细</el-button>
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['profiles:assetProfile:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['profiles:assetProfile:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
      ...
      >
        ...
        <div class="drawer-content">
          ...
          <div class="see-content" v-loading="drawerConfig.loading" v-if="drawerConfig.type==='query'">
            <ul>
              <li class="width100"><span>配置名称：</span> {{ formData.name }}</li>
              <li class="width100"><span>说明：</span>{{ formData.description }}</li>
              <li class="width100"><span>创建时间：</span> {{ formatDate(formData.createdTime) }}</li>
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            <el-form :model="formData" ref="drawerFromRef" class="edit-content-form" label-width="140px">
              <el-form-item label="名称：" prop="name" class="edit-content-form-li width100" :rules="[{ required: true, message: '请输入名称', trigger: 'blur' }]">
                <el-input placeholder="请输入名称" v-model="formData.name" clearable/>
              </el-form-item>
              <el-form-item label="说明：" prop="description" class="edit-content-form-li width100" :rules="[]">
                <el-input placeholder="请输入说明" v-model="formData.description" :autosize="{ minRows: 4, maxRows: 10 }" type="textarea" clearable/>
              </el-form-item>
            </el-form>
          </div>
        </div>
...
import {assetProfilesQueryList, assetProfileAddEdit, assetProfilesDel} from "@/api/genchuan/thingsBoard/profiles";


// 点击复制id
const idCopy = async (copyText) => {
  try {
    // 复制内容到剪贴板
  ...
  ElMessageBox.confirm(
      ...
      .then(async () => {
        try {
          ...
          // 处理错误
          ElMessage.error('删除失败，资产中有使用这资产配置');
          console.log(error)
        }
      })
      .catch(() => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增资产配置';
}
