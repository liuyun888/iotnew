  ...
  <div class="cc-main">
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <!--右边的按钮-->
        <div class="right">
          <el-button type="primary" plain :icon="Plus" @click="addClick(titleConfig.addButName)" v-hasPermi="['profiles:test2:create']"> {{ titleConfig.addButName }}</el-button>
        </div>
      </div>
      <!--搜索内容字段-->
      <div class="cc-search-css" ref="searchRef">
        <el-form class="-mb-15px" :model="searchForm" ref="searchFormRef" :inline="true" label-width="68px">
          ...
          <el-form-item label="道路名称" prop="name">
            <el-input v-model="searchForm.name" placeholder="请输入道路名称" clearable class="!w-240px"/>
          </el-form-item>
          <el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column label="道路名称" min-width="150" align="center" prop="name" :show-overflow-tooltip="true" v-if="tableColumnShow('道路名称')"/>
          <el-table-column label="道路编号" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('道路编号')">
            <template #default="scope">
              {{ tableFieldShow('道路编号', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column label="我是日期" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('我是日期')">
            <template #default="scope">
              {{ tableFieldShow('我是日期', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column label="道路分类代码" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('道路分类代码')">
            <template #default="scope">
              {{ handleTypeOptions(tableFieldShow('道路分类代码', scope.row.field), optionsLisi.道路分类代码Options) }}
            </template>
          </el-table-column>
          <el-table-column label="我是文本框" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('我是文本框')">
            <template #default="scope">
              {{ tableFieldShow('我是文本框', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column label="我是单选框" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('我是单选框')">
            <template #default="scope">
              {{ handleTypeOptions(tableFieldShow('我是单选框', scope.row.field), optionsLisi.我是单选框Options) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              ...
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['profiles:test2:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['profiles:test2:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
        ...
        <div class="drawer-content">
          ...
          <div class="see-content" v-loading="drawerConfig.loading" v-if="drawerConfig.type==='query'">
            <ul>
              ...
              <li class="width50"><span>道路编号：</span>{{ tableFieldShow('道路编号', queryData.field) }}</li>
              <li class="width50"><span>我是日期：</span>{{ tableFieldShow('我是日期', queryData.field) }}</li>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            <el-form :model="formData" ref="drawerFromRef" class="edit-content-form" label-width="140px">
              <el-form-item label="道路名称：" prop="name" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入道路名称', trigger: 'blur' }]">
                <el-input placeholder="请输入道路名称" v-model="formData.name" clearable/>
              </el-form-item>
              <el-form-item label="道路编号：" prop="field.道路编号" class="edit-content-form-li width50" :rules="[]">
                <el-input placeholder="请输入道路编号" v-model="formData.field.道路编号" clearable/>
              </el-form-item>
              <el-form-item label="我是日期：" prop="field.我是日期" class="edit-content-form-li width50" :rules="[{ required: true, message: '请选择我是日期', trigger: 'blur' }]">
                <el-date-picker class="width100" v-model="formData.field.我是日期" type="date" placeholder="请选择我是日期" format="YYYY-MM-DD" value-format="YYYY-MM-DD"/>
              </el-form-item>
              <el-form-item label="道路分类代码：" prop="field.道路分类代码" class="edit-content-form-li width50" :rules="[]">
                <el-select placeholder="请选择道路分类代码" v-model="formData.field.道路分类代码" filterable clearable>
                  <el-option v-for="item in optionsLisi.道路分类代码Options" :key="item.value" :label="item.label" :value="item.value"/>
                </el-select>
              </el-form-item>
              <el-form-item label="我是文本框：" prop="field.我是文本框" class="edit-content-form-li width50" :rules="[]">
                <el-input placeholder="请输入我是文本框" v-model="formData.field.我是文本框" clearable/>
              </el-form-item>
              <el-form-item label="我是单选框：" prop="field.我是单选框" class="edit-content-form-li width50" :rules="[{ required: true, message: '请选择我是单选框', trigger: 'blur' }]">
                <el-radio-group v-model="formData.field.我是单选框">
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'e5571380-73fb-11f0-bd39-2dc5ccdb39ef',//资产配置id
    }
...
const optionsLisi = ref({
  //道路分类代码 选项
  道路分类代码Options: [
    {label: '快速路', value: '1'}, {label: '主干路', value: '2'}, {label: '次干路', value: '3'}, {label: '支路及街坊路', value: '4'},
  ],
  //我是单选框 选项
  我是单选框Options: [
    {label: '单选1', value: '1'},
...
const queryClick = async (row) => {
 ...
 let attachmentUrlres= tableFieldShow('attachmentUrl', queryData.value.field)
  ...
  drawerConfig.value.title = '测试标题名称-详细信息';
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.我是日期 = tableFieldShow('我是日期', row.field);//我是日期
  ...
  drawerConfig.value.title = '编辑测试标题名称';
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增测试标题名称';
}
