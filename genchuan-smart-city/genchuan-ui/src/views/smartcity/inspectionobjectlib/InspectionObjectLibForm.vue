...
/** 双随机行政检查 表单 */
defineOptions({ name: 'InspectionObjectLibForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await InspectionObjectLibApi.getInspectionObjectLib(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as InspectionObjectLibVO
    if (formType.value === 'create') {
      await InspectionObjectLibApi.createInspectionObjectLib(data)
      message.success(t('common.createSuccess'))
    } else {
      await InspectionObjectLibApi.updateInspectionObjectLib(data)
