<template>
    ...
    >
      ...
      <el-form-item style="margin-left: 60px">
        ...
        <el-button
          ...
          v-hasPermi="['smartcity:inspection-object-lib:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['smartcity:inspection-object-lib:export']"
        >
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="企业名称" align="center" prop="entName" width="200px" />
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['smartcity:inspection-object-lib:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['smartcity:inspection-object-lib:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <InspectionObjectLibForm ref="formRef" @success="getList" />
</template>

<script setup lang="ts">
import { dateFormatter } from '@/utils/formatTime'
import download from '@/utils/download'
import { InspectionObjectLibApi, InspectionObjectLibVO } from '@/api/smartcity/inspectionobjectlib'
import InspectionObjectLibForm from './InspectionObjectLibForm.vue'

/** 检查对象库管理 列表 */
defineOptions({ name: 'InspectionObjectLib' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<InspectionObjectLibVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await InspectionObjectLibApi.getInspectionObjectLibPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await InspectionObjectLibApi.deleteInspectionObjectLib(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await InspectionObjectLibApi.exportInspectionObjectLib(queryParams)
    download.excel(data, '检查对象库管理.xls')
  } catch {
  } finally {
