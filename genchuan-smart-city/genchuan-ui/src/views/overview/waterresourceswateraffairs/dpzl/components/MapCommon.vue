  ...
  props.geometriesArray.forEach((item, index) => {
    if (item && typeof item.coord_x === 'number' && typeof item.coord_y === 'number') {
      geometriesData.push({
        ...
        properties: {
          water_fac_id: item.water_fac_id,
          water_fac_name: item.water_fac_name,
...
const getTooltipContent = (properties) => {
  ...
  return `
    <div style="padding: 10px; font-size: 14px; color: #333; background: white; border: 1px solid #ccc; min-width: 260px; border-radius: 4px;">
      <div style="margin-bottom: 8px; font-weight: bold; color: #1E90FF; border-bottom: 1px solid #eee; padding-bottom: 4px; text-align: center;">水务设施信息</div>
      <div style="${rowStyle}">
        <span style="${labelStyle}">设施ID：</span>
        <span style="${valueStyle}">${properties.water_fac_id || '未知'}</span>
      </div>
      <div style="${rowStyle}">
        <span style="${labelStyle}">名称：</span>
        <span style="${valueStyle}">${properties.water_fac_name || '未知'}</span>
      </div>
      <div style="${rowStyle}">
        <span style="${labelStyle}">类型：</span>
        <span style="${valueStyle}">${properties.fac_type || '未知'}</span>
      </div>
      <div style="${rowStyle}">
