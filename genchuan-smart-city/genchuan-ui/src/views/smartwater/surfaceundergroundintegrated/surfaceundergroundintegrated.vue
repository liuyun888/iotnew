    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:surfaceundergroundintegrated:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="地下管线ID" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入地下管线ID"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            v-if="tableColumnShow('地表类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('地表类型', scope.row.field),
                  optionsLisi.地表类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('地表设施名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('地表设施名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="地表高程"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('地表高程')"
          >
            <template #default="scope">
              {{ tableFieldShow('地表高程', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="地下管线ID"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('管线埋深')"
          >
            <template #default="scope">
              {{ tableFieldShow('管线埋深', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="空间距离"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('空间距离')"
          >
            <template #default="scope">
              {{ tableFieldShow('空间距离', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:surfaceundergroundintegrated:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:surfaceundergroundintegrated:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:surfaceundergroundintegrated:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
          ...
          >
            <ul>
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('地表类型', queryData.field),
                    optionsLisi.地表类型Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>地表设施名称：</span
                >{{ tableFieldShow('地表设施名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>地表高程：</span>{{ tableFieldShow('地表高程', queryData.field) }}</li
              >
              <li class="width50"><span>地下管线ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>管线埋深：</span>{{ tableFieldShow('管线埋深', queryData.field) }}</li
              >
              <li class="width50"
                ><span>空间距离：</span>{{ tableFieldShow('空间距离', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请选择地表类型', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择地表类型"
                  v-model="formData.field.地表类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.地表类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.地表设施名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入地表设施名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入地表设施名称"
                  v-model="formData.field.地表设施名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.地表高程"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入地表高程', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入地表高程" v-model="formData.field.地表高程" clearable>
              ...
              <el-form-item
                ...
                prop="name"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入地下管线ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入地下管线ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="管线埋深："
                prop="field.管线埋深"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入管线埋深', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入管线埋深" v-model="formData.field.管线埋深" clearable>
                  <template #append>米(m)</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.空间距离"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入空间距离', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入空间距离"
                  v-model="formData.field.空间距离"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '3fb712f0-7981-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '地表与地下一体化展示', //资产配置，都是写死固定的资产配置
  field: {
    地表类型: '', //地表类型
    地表设施名称: '', //地表设施名称
    地表高程: '', //地表高程
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '地表与地下一体化展示-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.地表类型 = tableFieldShow('地表类型', row.field) //地表类型
  formData.value.field.地表设施名称 = tableFieldShow('地表设施名称', row.field) //地表设施名称
  formData.value.field.地表高程 = tableFieldShow('地表高程', row.field) //地表高程
  formData.value.field.管线埋深 = tableFieldShow('管线埋深', row.field) //管线埋深
  formData.value.field.空间距离 = tableFieldShow('空间距离', row.field) //空间距离

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑地表与地下一体化展示'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增地表与地下一体化展示'
}
...
const titleConfig = ref({
  name: '地表与地下一体化展示', //标题名称
  addButName: '新增地表与地下一体化展示' //新增按钮名称
})
// 搜索 字段
...
const tableColumns = ref([
  { label: '地表类型', visible: true },
  { label: '地表设施名称', visible: true },
  { label: '地表高程', visible: true },
  { label: '地下管线ID', visible: true },
  { label: '管线埋深', visible: true },
  { label: '空间距离', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
