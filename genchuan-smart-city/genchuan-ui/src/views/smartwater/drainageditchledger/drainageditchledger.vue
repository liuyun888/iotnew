    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:drainageditchledger:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="排水渠标识码" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入排水渠标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('渠道类别')"
          >
            <template #default="scope">
              {{ tableFieldShow('渠道类别', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="断面形式"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('断面形式')"
          >
            <template #default="scope">
              {{ tableFieldShow('断面形式', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="渠道材质"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('渠道材质')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('渠道材质', scope.row.field),
                  optionsLisi.渠道材质Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('渠道长度')"
          >
            <template #default="scope">
              {{ tableFieldShow('渠道长度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="渠道坡度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('渠道坡度')"
          >
            <template #default="scope">
              {{ tableFieldShow('渠道坡度', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="渠道宽度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('渠道宽度')"
          >
            <template #default="scope">
              {{ tableFieldShow('渠道宽度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="渠道深度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('渠道深度')"
          >
            <template #default="scope">
              {{ tableFieldShow('渠道深度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="坐标串"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('坐标串')"
          >
            <template #default="scope">
              {{ tableFieldShow('坐标串', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:drainageditchledger:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:drainageditchledger:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:drainageditchledger:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>排水渠标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>渠道类别：</span>{{ tableFieldShow('渠道类别', queryData.field) }}</li
              >
              <li class="width50"
                ><span>断面形式：</span>{{ tableFieldShow('断面形式', queryData.field) }}</li
              >
              <li class="width50"
                ><span>渠道材质：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('渠道材质', queryData.field),
                    optionsLisi.渠道材质Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>渠道长度：</span>{{ tableFieldShow('渠道长度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>渠道坡度：</span>{{ tableFieldShow('渠道坡度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>渠道宽度：</span>{{ tableFieldShow('渠道宽度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>渠道深度：</span>{{ tableFieldShow('渠道深度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>坐标串：</span>{{ tableFieldShow('坐标串', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入排水渠标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入排水渠标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="渠道类别："
                prop="field.渠道类别"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入渠道类别', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入渠道类别"
                  v-model="formData.field.渠道类别"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.断面形式"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入断面形式', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入断面形式"
                  v-model="formData.field.断面形式"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.渠道材质"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择渠道材质', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择渠道材质"
                  v-model="formData.field.渠道材质"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.渠道材质Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.渠道长度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入渠道长度', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入渠道长度" v-model="formData.field.渠道长度" clearable>
              ...
              <el-form-item
                ...
                prop="field.渠道坡度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入渠道坡度', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入渠道坡度"
                  v-model="formData.field.渠道坡度"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.渠道宽度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入渠道宽度', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入渠道宽度" v-model="formData.field.渠道宽度" clearable>
                  <template #append>米(m)</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.渠道深度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入渠道深度', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入渠道深度" v-model="formData.field.渠道深度" clearable>
                  <template #append>米(m)</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.坐标串"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入坐标串', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入坐标串" v-model="formData.field.坐标串" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '6c882870-77f9-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '排水渠台账管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.渠道类别 = tableFieldShow('渠道类别', row.field) //渠道类别
  formData.value.field.断面形式 = tableFieldShow('断面形式', row.field) //断面形式
  formData.value.field.渠道材质 = tableFieldShow('渠道材质', row.field) //渠道材质
  formData.value.field.渠道长度 = tableFieldShow('渠道长度', row.field) //渠道长度
  formData.value.field.渠道坡度 = tableFieldShow('渠道坡度', row.field) //渠道坡度
  formData.value.field.渠道宽度 = tableFieldShow('渠道宽度', row.field) //渠道宽度
  formData.value.field.渠道深度 = tableFieldShow('渠道深度', row.field) //渠道深度
  formData.value.field.坐标串 = tableFieldShow('坐标串', row.field) //坐标串

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑排水渠台账管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增排水渠台账管理'
}
...
const tableColumns = ref([
  { label: '排水渠标识码', visible: true },
  { label: '渠道类别', visible: true },
  { label: '断面形式', visible: true },
  { label: '渠道材质', visible: true },
  { label: '渠道长度', visible: true },
  { label: '渠道坡度', visible: true },
  { label: '渠道宽度', visible: true },
  { label: '渠道深度', visible: true },
  { label: '坐标串', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
