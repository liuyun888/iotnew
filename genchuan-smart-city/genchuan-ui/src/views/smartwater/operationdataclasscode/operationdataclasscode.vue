    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:operationdataclasscode:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="运行数据类型编码" prop="name" label-width="140px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入运行数据类型编码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('数据名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('数据名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="关联设施编码"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('关联设施编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('关联设施编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="采集频率"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('采集频率')"
          >
            <template #default="scope">
              {{ tableFieldShow('采集频率', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:operationdataclasscode:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:operationdataclasscode:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:operationdataclasscode:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>运行数据类型编码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>数据名称：</span>{{ tableFieldShow('数据名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>关联设施编码：</span
                >{{ tableFieldShow('关联设施编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>采集频率：</span>{{ tableFieldShow('采集频率', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入运行数据类型编码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入运行数据类型编码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="数据名称："
                prop="field.数据名称"
              ...
              <el-form-item
                ...
                prop="field.关联设施编码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入关联设施编码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入关联设施编码"
                  v-model="formData.field.关联设施编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.采集频率"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入采集频率', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入采集频率"
                  v-model="formData.field.采集频率"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '66e32120-78aa-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '运行数据分类编码', //资产配置，都是写死固定的资产配置
  field: {
    数据名称: '', //数据名称
    关联设施编码: '', //关联设施编码
    采集频率: '' //采集频率
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '运行数据分类编码-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.数据名称 = tableFieldShow('数据名称', row.field) //数据名称
  formData.value.field.关联设施编码 = tableFieldShow('关联设施编码', row.field) //关联设施编码
  formData.value.field.采集频率 = tableFieldShow('采集频率', row.field) //采集频率

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑运行数据分类编码'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增运行数据分类编码'
}
...
const tableColumns = ref([
  { label: '运行数据类型编码', visible: true },
  { label: '数据名称', visible: true },
  { label: '关联设施编码', visible: true },
  { label: '采集频率', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
