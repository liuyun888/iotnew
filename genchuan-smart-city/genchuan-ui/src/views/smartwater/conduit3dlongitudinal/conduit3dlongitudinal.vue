    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:conduit3dlongitudinal:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="管段标识码" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入管段标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('管径')"
          >
            <template #default="scope">
              {{ tableFieldShow('管径', scope.row.field) }} 毫米(mm)
            </template>
          </el-table-column>
          <el-table-column
            label="起点管底标高"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('起点管底标高')"
          >
            <template #default="scope">
              {{ tableFieldShow('起点管底标高', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="终点管底标高"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('终点管底标高')"
          >
            <template #default="scope">
              {{ tableFieldShow('终点管底标高', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="坡度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('坡度')"
          >
            <template #default="scope">
              {{ tableFieldShow('坡度', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="埋深"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('埋深')"
          >
            <template #default="scope">
              {{ tableFieldShow('埋深', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:conduit3dlongitudinal:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:conduit3dlongitudinal:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:conduit3dlongitudinal:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>管段标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>管径：</span>{{ tableFieldShow('管径', queryData.field) }}</li
              >
              <li class="width50"
                ><span>起点管底标高：</span
                >{{ tableFieldShow('起点管底标高', queryData.field) }}</li
              >
              <li class="width50"
                ><span>终点管底标高：</span
                >{{ tableFieldShow('终点管底标高', queryData.field) }}</li
              >
              <li class="width50"
                ><span>坡度：</span>{{ tableFieldShow('坡度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>埋深：</span>{{ tableFieldShow('埋深', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入管段标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入管段标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="管径："
                prop="field.管径"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入管径', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入管径" v-model="formData.field.管径" clearable>
              ...
              <el-form-item
                ...
                prop="field.起点管底标高"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入起点管底标高', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入起点管底标高"
                  v-model="formData.field.起点管底标高"
                  clearable
                >
              ...
              <el-form-item
                ...
                prop="field.终点管底标高"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入终点管底标高', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入终点管底标高"
                  v-model="formData.field.终点管底标高"
                  clearable
                >
                  <template #append>米(m)</template>
                </el-input>
              ...
              <el-form-item
                ...
                prop="field.坡度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入坡度', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入坡度" v-model="formData.field.坡度" clearable />
              </el-form-item>
              <el-form-item
                label="埋深："
                prop="field.埋深"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入埋深', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入埋深" v-model="formData.field.埋深" clearable>
                  <template #append>米(m)</template>
                </el-input>
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'a4426420-797e-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '管渠三维与纵断面展示', //资产配置，都是写死固定的资产配置
  field: {
    管径: '', //管径
    起点管底标高: '', //起点管底标高
    终点管底标高: '', //终点管底标高
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '管渠三维与纵断面展示-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.管径 = tableFieldShow('管径', row.field) //管径
  formData.value.field.起点管底标高 = tableFieldShow('起点管底标高', row.field) //起点管底标高
  formData.value.field.终点管底标高 = tableFieldShow('终点管底标高', row.field) //终点管底标高
  formData.value.field.坡度 = tableFieldShow('坡度', row.field) //坡度
  formData.value.field.埋深 = tableFieldShow('埋深', row.field) //埋深

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑管渠三维与纵断面展示'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增管渠三维与纵断面展示'
}
...
const titleConfig = ref({
  name: '管渠三维与纵断面展示', //标题名称
  addButName: '新增管渠三维与纵断面展示' //新增按钮名称
})
// 搜索 字段
...
const tableColumns = ref([
  { label: '管段标识码', visible: true },
  { label: '管径', visible: true },
  { label: '起点管底标高', visible: true },
  { label: '终点管底标高', visible: true },
  { label: '坡度', visible: true },
  { label: '埋深', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
