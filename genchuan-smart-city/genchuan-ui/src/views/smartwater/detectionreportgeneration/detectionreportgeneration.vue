    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:detectionreportgeneration:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('区域编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('区域编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="检测时段"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检测时段')"
          >
            <template #default="scope">
              {{ tableFieldShow('检测时段', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="缺陷统计图表"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('缺陷统计图表')"
          >
            <template #default="scope">
              {{ tableFieldShow('缺陷统计图表', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="缺陷类型统计"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('缺陷类型统计')"
          >
            <template #default="scope">
              {{ tableFieldShow('缺陷类型统计', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="缺陷等级统计"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('缺陷等级统计')"
          >
            <template #default="scope">
              {{ tableFieldShow('缺陷等级统计', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="养护建议措施"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('养护建议措施')"
          >
            <template #default="scope">
              {{ tableFieldShow('养护建议措施', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="生成时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('生成时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('生成时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:detectionreportgeneration:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:detectionreportgeneration:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:detectionreportgeneration:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>报告编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>区域编码：</span>{{ tableFieldShow('区域编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>检测时段：</span>{{ tableFieldShow('检测时段', queryData.field) }}</li
              >
              <li class="width50"
                ><span>缺陷统计图表：</span
                >{{ tableFieldShow('缺陷统计图表', queryData.field) }}</li
              >
              <li class="width50"
                ><span>缺陷类型统计：</span
                >{{ tableFieldShow('缺陷类型统计', queryData.field) }}</li
              >
              <li class="width50"
                ><span>缺陷等级统计：</span
                >{{ tableFieldShow('缺陷等级统计', queryData.field) }}</li
              >
              <li class="width50"
                ><span>养护建议措施：</span
                >{{ tableFieldShow('养护建议措施', queryData.field) }}</li
              >
              <li class="width50"
                ><span>生成时间：</span>{{ dateFormatter(null, null, tableFieldShow('生成时间', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入报告编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入报告编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="区域编码："
                prop="field.区域编码"
              ...
              <el-form-item
                ...
                prop="field.检测时段"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检测时段', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入检测时段"
                  v-model="formData.field.检测时段"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.缺陷统计图表"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入缺陷统计图表', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入缺陷统计图表"
                  v-model="formData.field.缺陷统计图表"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.缺陷类型统计"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入缺陷类型统计', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入缺陷类型统计"
                  v-model="formData.field.缺陷类型统计"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.缺陷等级统计"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入缺陷等级统计', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入缺陷等级统计"
                  v-model="formData.field.缺陷等级统计"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.养护建议措施"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入养护建议措施', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入养护建议措施"
                  v-model="formData.field.养护建议措施"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.生成时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入生成时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.生成时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择生成时间"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '76345460-799e-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '检测报告生成-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.区域编码 = tableFieldShow('区域编码', row.field) //区域编码
  formData.value.field.检测时段 = tableFieldShow('检测时段', row.field) //检测时段
  formData.value.field.缺陷统计图表 = tableFieldShow('缺陷统计图表', row.field) //缺陷统计图表
  formData.value.field.缺陷类型统计 = tableFieldShow('缺陷类型统计', row.field) //缺陷类型统计
  formData.value.field.缺陷等级统计 = tableFieldShow('缺陷等级统计', row.field) //缺陷等级统计
  formData.value.field.养护建议措施 = tableFieldShow('养护建议措施', row.field) //养护建议措施
  formData.value.field.生成时间 = tableFieldShow('生成时间', row.field) //生成时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑检测报告生成'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增检测报告生成'
}
...
const tableColumns = ref([
  { label: '报告编号', visible: true },
  { label: '区域编码', visible: true },
  { label: '检测时段', visible: true },
  { label: '缺陷统计图表', visible: true },
  { label: '缺陷类型统计', visible: true },
  { label: '缺陷等级统计', visible: true },
  { label: '养护建议措施', visible: true },
  { label: '生成时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
