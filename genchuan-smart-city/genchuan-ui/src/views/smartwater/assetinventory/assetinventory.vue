    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:assetinventory:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="清查单号" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('账面数量')"
          >
            <template #default="scope">
              {{ tableFieldShow('账面数量', scope.row.field) }} 个/米
            </template>
          </el-table-column>
          <el-table-column
            label="实盘数量"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('实盘数量')"
          >
            <template #default="scope">
              {{ tableFieldShow('实盘数量', scope.row.field) }} 个/米
            </template>
          </el-table-column>
          <el-table-column
            label="差异值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('差异值')"
          >
            <template #default="scope">
              {{ tableFieldShow('差异值', scope.row.field) }} 个/米
            </template>
          </el-table-column>
          <el-table-column
            label="差异原因"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('差异原因')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('差异原因', scope.row.field),
                  optionsLisi.差异原因Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('差异说明')"
          >
            <template #default="scope">
              {{ tableFieldShow('差异说明', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="清查日期"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('清查日期')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('清查日期', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="清查人"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('清查人')"
          >
            <template #default="scope">
              {{ tableFieldShow('清查人', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:assetinventory:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:assetinventory:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:assetinventory:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>清查单号：</span>{{ queryData.name }}</li>
              <li class="width50"
              ...
              >
              <li class="width50"
                ><span>账面数量：</span>{{ tableFieldShow('账面数量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>实盘数量：</span>{{ tableFieldShow('实盘数量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>差异值：</span>{{ tableFieldShow('差异值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>差异原因：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('差异原因', queryData.field),
                    optionsLisi.差异原因Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>差异说明：</span>{{ tableFieldShow('差异说明', queryData.field) }}</li
              >
              <li class="width50"
                ><span>清查日期：</span
                >{{ dateFormatter2(null, null, tableFieldShow('清查日期', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>清查人：</span>{{ tableFieldShow('清查人', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入清查单号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入清查单号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施标识码："
                prop="field.设施标识码"
              ...
              <el-form-item
                ...
                prop="field.账面数量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入账面数量', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入账面数量" v-model="formData.field.账面数量" clearable>
              ...
              <el-form-item
                ...
                prop="field.实盘数量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入实盘数量', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入实盘数量" v-model="formData.field.实盘数量" clearable>
                  <template #append>个/米</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.差异值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入差异值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入差异值" v-model="formData.field.差异值" clearable>
                  <template #append>个/米</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.差异原因"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择差异原因', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择差异原因"
                  v-model="formData.field.差异原因"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.差异原因Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.差异说明"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入差异说明', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入差异说明"
                  v-model="formData.field.差异说明"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.清查日期"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入清查日期', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.清查日期"
                  type="date"
                  value-format="x"
                  placeholder="选择清查日期"
                />
              ...
              <el-form-item
                ...
                prop="field.清查人"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入清查人', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入清查人" v-model="formData.field.清查人" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'abb34db0-7810-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '资产清查管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.账面数量 = tableFieldShow('账面数量', row.field) //账面数量
  formData.value.field.实盘数量 = tableFieldShow('实盘数量', row.field) //实盘数量
  formData.value.field.差异值 = tableFieldShow('差异值', row.field) //差异值
  formData.value.field.差异原因 = tableFieldShow('差异原因', row.field) //差异原因
  formData.value.field.差异说明 = tableFieldShow('差异说明', row.field) //差异说明
  formData.value.field.清查日期 = tableFieldShow('清查日期', row.field) //清查日期
  formData.value.field.清查人 = tableFieldShow('清查人', row.field) //清查人

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑资产清查管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增资产清查管理'
}
...
const tableColumns = ref([
  { label: '清查单号', visible: true },
  { label: '设施标识码', visible: true },
  { label: '设施类型', visible: true },
  { label: '账面数量', visible: true },
  { label: '实盘数量', visible: true },
  { label: '差异值', visible: true },
  { label: '差异原因', visible: true },
  { label: '差异说明', visible: true },
  { label: '清查日期', visible: true },
  { label: '清查人', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
