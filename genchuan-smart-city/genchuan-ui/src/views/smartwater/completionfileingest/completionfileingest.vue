      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:completionfileingest:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('档案名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('档案名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="档案类型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('档案类型')"
          >
            <template #default="scope">
              {{ tableFieldShow('档案类型', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="存储路径"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('存储路径')"
          >
            <template #default="scope">
              {{ tableFieldShow('存储路径', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="上传时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('上传时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('上传时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:completionfileingest:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:completionfileingest:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:completionfileingest:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              >
              <li class="width50"
                ><span>档案名称：</span>{{ tableFieldShow('档案名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>档案类型：</span>{{ tableFieldShow('档案类型', queryData.field) }}</li
              >
              <li class="width50"
                ><span>存储路径：</span>{{ tableFieldShow('存储路径', queryData.field) }}</li
              >
              <li class="width50"
                ><span>上传时间：</span>{{ dateFormatter(null, null, tableFieldShow('上传时间', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.档案名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入档案名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入档案名称"
                  v-model="formData.field.档案名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.档案类型"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入档案类型', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入档案类型"
                  v-model="formData.field.档案类型"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.上传时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入上传时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.上传时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择上传时间"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '79e404e0-78ac-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '竣工档案接入-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.档案名称 = tableFieldShow('档案名称', row.field) //档案名称
  formData.value.field.档案类型 = tableFieldShow('档案类型', row.field) //档案类型
  formData.value.field.存储路径 = tableFieldShow('存储路径', row.field) //存储路径
  formData.value.field.上传时间 = tableFieldShow('上传时间', row.field) //上传时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑竣工档案接入'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增竣工档案接入'
}
...
const tableColumns = ref([
  ...
  { label: '档案名称', visible: true },
  { label: '档案类型', visible: true },
  { label: '存储路径', visible: true },
  { label: '上传时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
