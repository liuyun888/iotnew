    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:topologyrelationchecktool:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="拓扑校验编号" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入拓扑校验编号"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('涉及设施编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('涉及设施编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="问题描述"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('问题描述')"
          >
            <template #default="scope">
              {{ tableFieldShow('问题描述', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="校验人员"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('校验人员')"
          >
            <template #default="scope">
              {{ tableFieldShow('校验人员', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="处理结果"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('处理结果')"
          >
            <template #default="scope">
              {{ tableFieldShow('处理结果', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="处理人"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('处理人')"
          >
            <template #default="scope">
              {{ tableFieldShow('处理人', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:topologyrelationchecktool:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:topologyrelationchecktool:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:topologyrelationchecktool:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>拓扑校验编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>问题类型：</span
                >{{
                  handleTypeOptions(
              ...
              <li class="width50"
                ><span>涉及设施编码：</span
                >{{ tableFieldShow('涉及设施编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>问题描述：</span>{{ tableFieldShow('问题描述', queryData.field) }}</li
              >
              <li class="width50"
                ><span>校验人员：</span>{{ tableFieldShow('校验人员', queryData.field) }}</li
              >
              <li class="width50"
                ><span>处理结果：</span>{{ tableFieldShow('处理结果', queryData.field) }}</li
              >
              <li class="width50"
                ><span>处理人：</span>{{ tableFieldShow('处理人', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入拓扑校验编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入拓扑校验编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="问题类型："
                prop="field.问题类型"
              ...
              <el-form-item
                ...
                prop="field.涉及设施编码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入涉及设施编码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入涉及设施编码"
                  v-model="formData.field.涉及设施编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.校验人员"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入校验人员', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入校验人员"
                  v-model="formData.field.校验人员"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input placeholder="请输入处理人" v-model="formData.field.处理人" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '84fd93b0-78b5-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '拓扑关系校验工具', //资产配置，都是写死固定的资产配置
  field: {
    问题类型: '', //问题类型
    涉及设施编码: '', //涉及设施编码
    问题描述: '', //问题描述
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '拓扑关系校验工具-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.涉及设施编码 = tableFieldShow('涉及设施编码', row.field) //涉及设施编码
  formData.value.field.问题描述 = tableFieldShow('问题描述', row.field) //问题描述
  formData.value.field.校验人员 = tableFieldShow('校验人员', row.field) //校验人员
  formData.value.field.处理结果 = tableFieldShow('处理结果', row.field) //处理结果
  formData.value.field.处理人 = tableFieldShow('处理人', row.field) //处理人

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑拓扑关系校验工具'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增拓扑关系校验工具'
}
...
const tableColumns = ref([
  { label: '拓扑校验编号', visible: true },
  { label: '问题类型', visible: true },
  { label: '涉及设施编码', visible: true },
  { label: '问题描述', visible: true },
  { label: '校验人员', visible: true },
  { label: '处理结果', visible: true },
  { label: '处理人', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
