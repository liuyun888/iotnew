      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:infrastructurelayer:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('材质')"
          >
            <template #default="scope">
              {{ tableFieldShow('材质', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="状态"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:infrastructurelayer:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:infrastructurelayer:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:infrastructurelayer:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              >
              <li class="width50"
                ><span>材质：</span>{{ tableFieldShow('材质', queryData.field) }}</li
              >
              <li class="width50"
                ><span>状态：</span
                >{{
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              >
                <el-input placeholder="请输入材质" v-model="formData.field.材质" clearable />
              </el-form-item>
              <el-form-item
                label="状态："
                prop="field.状态"
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '5f9ec2e0-7975-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '基础设施图层管理', //资产配置，都是写死固定的资产配置
  field: {
    坐标X: '', //坐标X
    坐标Y: '', //坐标Y
    材质: '', //材质
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '基础设施图层管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.材质 = tableFieldShow('材质', row.field) //材质
  formData.value.field.状态 = tableFieldShow('状态', row.field) //状态

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑基础设施图层管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增基础设施图层管理'
}
