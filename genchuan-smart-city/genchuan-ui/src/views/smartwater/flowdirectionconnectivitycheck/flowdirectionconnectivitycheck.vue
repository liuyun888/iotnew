    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:flowdirectionconnectivitycheck:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="节点标识码" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入节点标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('链接管线数量')"
          >
            <template #default="scope">
              {{ tableFieldShow('链接管线数量', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="是否连通"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('是否连通')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('是否连通', scope.row.field),
                  optionsLisi.是否连通Options
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:flowdirectionconnectivitycheck:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:flowdirectionconnectivitycheck:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:flowdirectionconnectivitycheck:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>节点标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>链接管线数量：</span
                >{{ tableFieldShow('链接管线数量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>是否连通：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('是否连通', queryData.field),
                    optionsLisi.是否连通Options
                  )
                }}</li
              >
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入节点标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入节点标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="链接管线数量："
                prop="field.链接管线数量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入链接管线数量', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入链接管线数量"
                  v-model="formData.field.链接管线数量"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.是否连通"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择是否连通', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择是否连通"
                  v-model="formData.field.是否连通"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.是否连通Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '1da20840-7978-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '流向与连通性检查', //资产配置，都是写死固定的资产配置
  field: {
    链接管线数量: '', //链接管线数量
    是否连通: '', //是否连通
    问题类型: '' //问题类型
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '流向与连通性检查-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.链接管线数量 = tableFieldShow('链接管线数量', row.field) //链接管线数量
  formData.value.field.是否连通 = tableFieldShow('是否连通', row.field) //是否连通
  formData.value.field.问题类型 = tableFieldShow('问题类型', row.field) //问题类型

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑流向与连通性检查'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增流向与连通性检查'
}
...
const tableColumns = ref([
  { label: '节点标识码', visible: true },
  { label: '链接管线数量', visible: true },
  { label: '是否连通', visible: true },
