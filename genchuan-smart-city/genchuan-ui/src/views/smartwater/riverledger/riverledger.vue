    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:riverledger:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="河道标识码" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入河道标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('河道名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('河道名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="河道长度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('河道长度')"
          >
            <template #default="scope">
              {{ tableFieldShow('河道长度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="起点河底标高"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('起点河底标高')"
          >
            <template #default="scope">
              {{ tableFieldShow('起点河底标高', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="终点河底标高"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('终点河底标高')"
          >
            <template #default="scope">
              {{ tableFieldShow('终点河底标高', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="常水位"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('河道宽度')"
          >
            <template #default="scope">
              {{ tableFieldShow('河道宽度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="河道坡度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('河道坡度')"
          >
            <template #default="scope">
              {{ tableFieldShow('河道坡度', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="过流能力"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('过流能力')"
          >
            <template #default="scope">
              {{ tableFieldShow('过流能力', scope.row.field) }} 立方米每秒(m³/s)
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:riverledger:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:riverledger:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:riverledger:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>河道标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>河道名称：</span>{{ tableFieldShow('河道名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>河道长度：</span>{{ tableFieldShow('河道长度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>起点河底标高：</span
                >{{ tableFieldShow('起点河底标高', queryData.field) }}</li
              >
              <li class="width50"
                ><span>终点河底标高：</span
                >{{ tableFieldShow('终点河底标高', queryData.field) }}</li
              >
              <li class="width50"
              ...
              >
              <li class="width50"
                ><span>河道宽度：</span>{{ tableFieldShow('河道宽度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>河道坡度：</span>{{ tableFieldShow('河道坡度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>过流能力：</span>{{ tableFieldShow('过流能力', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入河道标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入河道标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="河道名称："
                prop="field.河道名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入河道名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入河道名称"
                  v-model="formData.field.河道名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.河道长度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入河道长度', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入河道长度" v-model="formData.field.河道长度" clearable>
              ...
              <el-form-item
                ...
                prop="field.起点河底标高"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入起点河底标高', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入起点河底标高"
                  v-model="formData.field.起点河底标高"
                  clearable
                >
                  <template #append>米(m)</template>
                </el-input>
              ...
              <el-form-item
                ...
                prop="field.终点河底标高"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入终点河底标高', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入终点河底标高"
                  v-model="formData.field.终点河底标高"
                  clearable
                >
                  <template #append>米(m)</template>
                </el-input>
              ...
              <el-form-item
                ...
                prop="field.河道宽度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入河道宽度', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入河道宽度" v-model="formData.field.河道宽度" clearable>
                  <template #append>米(m)</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.河道坡度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入河道坡度', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入河道坡度"
                  v-model="formData.field.河道坡度"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.过流能力"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入过流能力', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入过流能力" v-model="formData.field.过流能力" clearable>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '2d59eec0-780a-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '河道台账管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.河道名称 = tableFieldShow('河道名称', row.field) //河道名称
  formData.value.field.河道长度 = tableFieldShow('河道长度', row.field) //河道长度
  formData.value.field.起点河底标高 = tableFieldShow('起点河底标高', row.field) //起点河底标高
  formData.value.field.终点河底标高 = tableFieldShow('终点河底标高', row.field) //终点河底标高
  formData.value.field.常水位 = tableFieldShow('常水位', row.field) //常水位
  formData.value.field.警戒水位 = tableFieldShow('警戒水位', row.field) //警戒水位
  formData.value.field.河道宽度 = tableFieldShow('河道宽度', row.field) //河道宽度
  formData.value.field.河道坡度 = tableFieldShow('河道坡度', row.field) //河道坡度
  formData.value.field.过流能力 = tableFieldShow('过流能力', row.field) //过流能力

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑河道台账管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增河道台账管理'
}
...
const tableColumns = ref([
  { label: '河道标识码', visible: true },
  { label: '河道名称', visible: true },
  { label: '河道长度', visible: true },
  { label: '起点河底标高', visible: true },
  { label: '终点河底标高', visible: true },
  { label: '常水位', visible: true },
  { label: '警戒水位', visible: true },
  { label: '河道宽度', visible: true },
  { label: '河道坡度', visible: true },
  { label: '过流能力', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
