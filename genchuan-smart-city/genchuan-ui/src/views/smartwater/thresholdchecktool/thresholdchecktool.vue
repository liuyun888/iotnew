    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:thresholdchecktool:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="阈值校验编号" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入阈值校验编号"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('校验字段')"
          >
            <template #default="scope">
              {{ tableFieldShow('校验字段', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="字段值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('字段值')"
          >
            <template #default="scope">
              {{ tableFieldShow('字段值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="阈值范围"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('阈值范围')"
          >
            <template #default="scope">
              {{ tableFieldShow('阈值范围', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="预警状态"
          ...
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('预警状态', scope.row.field),
                  optionsLisi.预警状态Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('校验时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('校验时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:thresholdchecktool:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:thresholdchecktool:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:thresholdchecktool:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>阈值校验编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设施编码：</span>{{ tableFieldShow('设施编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>校验字段：</span>{{ tableFieldShow('校验字段', queryData.field) }}</li
              >
              <li class="width50"
                ><span>字段值：</span>{{ tableFieldShow('字段值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>阈值范围：</span>{{ tableFieldShow('阈值范围', queryData.field) }}</li
              >
              <li class="width50"
                ><span>预警状态：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('预警状态', queryData.field),
                    optionsLisi.预警状态Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>校验时间：</span>{{ dateFormatter(null, null, tableFieldShow('校验时间', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入阈值校验编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入阈值校验编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施编码："
                prop="field.设施编码"
              ...
              <el-form-item
                ...
                prop="field.校验字段"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入校验字段', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入校验字段"
                  v-model="formData.field.校验字段"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input placeholder="请输入字段值" v-model="formData.field.字段值" clearable />
              </el-form-item>
              <el-form-item
                label="阈值范围："
                prop="field.阈值范围"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入阈值范围', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入阈值范围"
                  v-model="formData.field.阈值范围"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.预警状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择预警状态', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择预警状态"
                  v-model="formData.field.预警状态"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.预警状态Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.校验时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入校验时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.校验时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择校验时间"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '2e5ecba0-78ba-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '阈值校验工具-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.校验字段 = tableFieldShow('校验字段', row.field) //校验字段
  formData.value.field.字段值 = tableFieldShow('字段值', row.field) //字段值
  formData.value.field.阈值范围 = tableFieldShow('阈值范围', row.field) //阈值范围
  formData.value.field.预警状态 = tableFieldShow('预警状态', row.field) //预警状态
  formData.value.field.校验时间 = tableFieldShow('校验时间', row.field) //校验时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑阈值校验工具'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增阈值校验工具'
}
...
const tableColumns = ref([
  { label: '阈值校验编号', visible: true },
  { label: '设施编码', visible: true },
  { label: '校验字段', visible: true },
  { label: '字段值', visible: true },
  { label: '阈值范围', visible: true },
  { label: '预警状态', visible: true },
  { label: '校验时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
