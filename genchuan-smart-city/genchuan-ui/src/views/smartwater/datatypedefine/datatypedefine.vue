    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:datatypedefine:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="数据类型编码" prop="name" label-width="120px">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('数据类型名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('数据类型名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="数据长度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('数据长度')"
          >
            <template #default="scope">
              {{ tableFieldShow('数据长度', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="小数位数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('小数位数')"
          >
            <template #default="scope">
              {{ tableFieldShow('小数位数', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="说明"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('说明')"
          >
            <template #default="scope">
              {{ tableFieldShow('说明', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:datatypedefine:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:datatypedefine:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:datatypedefine:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>数据类型编码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>数据类型名称：</span
                >{{ tableFieldShow('数据类型名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>数据长度：</span>{{ tableFieldShow('数据长度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>小数位数：</span>{{ tableFieldShow('小数位数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>说明：</span>{{ tableFieldShow('说明', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入数据类型编码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入数据类型编码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="数据类型名称："
                prop="field.数据类型名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入数据类型名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入数据类型名称"
                  v-model="formData.field.数据类型名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.数据长度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入数据长度', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入数据长度"
                  v-model="formData.field.数据长度"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input placeholder="请输入说明" v-model="formData.field.说明" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'd98239e0-78a6-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '数据类型定义-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.数据类型名称 = tableFieldShow('数据类型名称', row.field) //数据类型名称
  formData.value.field.数据长度 = tableFieldShow('数据长度', row.field) //数据长度
  formData.value.field.小数位数 = tableFieldShow('小数位数', row.field) //小数位数
  formData.value.field.说明 = tableFieldShow('说明', row.field) //说明

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑数据类型定义'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增数据类型定义'
}
...
const tableColumns = ref([
  { label: '数据类型编码', visible: true },
  { label: '数据类型名称', visible: true },
  { label: '数据长度', visible: true },
  { label: '小数位数', visible: true },
