      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:gatevalvestatus:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('启闭度')"
          >
            <template #default="scope">
              {{ tableFieldShow('启闭度', scope.row.field) }} %
            </template>
          </el-table-column>
          <el-table-column
            label="控制指令"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('控制指令')"
          >
            <template #default="scope">
              {{ tableFieldShow('控制指令', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="状态码"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('状态码')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('状态码', scope.row.field),
                  optionsLisi.状态码Options
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:gatevalvestatus:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:gatevalvestatus:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:gatevalvestatus:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              >
              <li class="width50"
                ><span>启闭度：</span>{{ tableFieldShow('启闭度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>控制指令：</span>{{ tableFieldShow('控制指令', queryData.field) }}</li
              >
              <li class="width50"
                ><span>状态码：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('状态码', queryData.field),
                    optionsLisi.状态码Options
                  )
                }}</li
              >
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.启闭度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入启闭度', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入启闭度" v-model="formData.field.启闭度" clearable>
              ...
              <el-form-item
                ...
                prop="field.控制指令"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入控制指令', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入控制指令"
                  v-model="formData.field.控制指令"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.状态码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择状态码', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择状态码"
                  v-model="formData.field.状态码"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.状态码Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'f5fb8a50-78d7-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '闸门/阀门状态监测', //资产配置，都是写死固定的资产配置
  field: {
    监测时间: '', //监测时间
    启闭度: '', //启闭度
    控制指令: '', //控制指令
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '闸门/阀门状态监测-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.启闭度 = tableFieldShow('启闭度', row.field) //启闭度
  formData.value.field.控制指令 = tableFieldShow('控制指令', row.field) //控制指令
  formData.value.field.状态码 = tableFieldShow('状态码', row.field) //状态码

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑闸门/阀门状态监测'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增闸门/阀门状态监测'
}
...
const tableColumns = ref([
  ...
  { label: '启闭度', visible: true },
  { label: '控制指令', visible: true },
  { label: '状态码', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
