    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:attributedataclasscode:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="属性项编码" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入属性项编码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('属性项名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('属性项名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="编码值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('编码值')"
          >
            <template #default="scope">
              {{ tableFieldShow('编码值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="编码说明"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('编码说明')"
          >
            <template #default="scope">
              {{ tableFieldShow('编码说明', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="是否启用"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('是否启用')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('是否启用', scope.row.field),
                  optionsLisi.是否启用Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('创建时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('创建时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:attributedataclasscode:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:attributedataclasscode:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:attributedataclasscode:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>属性项编码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>属性项名称：</span>{{ tableFieldShow('属性项名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>编码值：</span>{{ tableFieldShow('编码值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>编码说明：</span>{{ tableFieldShow('编码说明', queryData.field) }}</li
              >
              <li class="width50"
                ><span>是否启用：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('是否启用', queryData.field),
                    optionsLisi.是否启用Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>创建时间：</span>{{ dateFormatter(null, null, tableFieldShow('创建时间', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入属性项编码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入属性项编码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="属性项名称："
                prop="field.属性项名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入属性项名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入属性项名称"
                  v-model="formData.field.属性项名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input placeholder="请输入编码值" v-model="formData.field.编码值" clearable />
              </el-form-item>
              <el-form-item
                label="编码说明："
                prop="field.编码说明"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入编码说明', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入编码说明"
                  v-model="formData.field.编码说明"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-select
                  placeholder="请选择是否启用"
                  v-model="formData.field.是否启用"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.是否启用Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'fdf71540-78a9-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '属性数据分类编码', //资产配置，都是写死固定的资产配置
  field: {
    属性项名称: '', //属性项名称
    编码值: '', //编码值
    编码说明: '', //编码说明
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '属性数据分类编码-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.属性项名称 = tableFieldShow('属性项名称', row.field) //属性项名称
  formData.value.field.编码值 = tableFieldShow('编码值', row.field) //编码值
  formData.value.field.编码说明 = tableFieldShow('编码说明', row.field) //编码说明
  formData.value.field.是否启用 = tableFieldShow('是否启用', row.field) //是否启用
  formData.value.field.创建时间 = tableFieldShow('创建时间', row.field) //创建时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑属性数据分类编码'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增属性数据分类编码'
}
...
const tableColumns = ref([
  { label: '属性项编码', visible: true },
  { label: '属性项名称', visible: true },
  { label: '编码值', visible: true },
  { label: '编码说明', visible: true },
  { label: '是否启用', visible: true },
