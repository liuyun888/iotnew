      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:operationlimit:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('参数名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('参数名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="额定值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('额定值')"
          >
            <template #default="scope">
              {{ tableFieldShow('额定值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="上限值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('上限值')"
          >
            <template #default="scope">
              {{ tableFieldShow('上限值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="下限值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('下限值')"
          >
            <template #default="scope">
              {{ tableFieldShow('下限值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="壁厚"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:operationlimit:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:operationlimit:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:operationlimit:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              <li class="width50"
                ><span>参数名称：</span>{{ tableFieldShow('参数名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>额定值：</span>{{ tableFieldShow('额定值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>上限值：</span>{{ tableFieldShow('上限值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>下限值：</span>{{ tableFieldShow('下限值', queryData.field) }}</li
              >
              <li class="width50"
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.额定值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入额定值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入额定值" v-model="formData.field.额定值" clearable />
              </el-form-item>
              <el-form-item
                label="上限值："
                prop="field.上限值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入上限值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入上限值" v-model="formData.field.上限值" clearable />
              </el-form-item>
              <el-form-item
                label="下限值："
                prop="field.下限值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入下限值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入下限值" v-model="formData.field.下限值" clearable />
              </el-form-item>
              <el-form-item
                label="壁厚："
                prop="field.壁厚"
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '5df801f0-781b-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '运行限值维护-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.参数名称 = tableFieldShow('参数名称', row.field) //参数名称
  formData.value.field.额定值 = tableFieldShow('额定值', row.field) //额定值
  formData.value.field.上限值 = tableFieldShow('上限值', row.field) //上限值
  formData.value.field.下限值 = tableFieldShow('下限值', row.field) //下限值
  ...
  drawerConfig.value.title = '编辑运行限值维护'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增运行限值维护'
}
...
const tableColumns = ref([
  ...
  { label: '参数名称', visible: true },
  { label: '额定值', visible: true },
  { label: '上限值', visible: true },
  { label: '下限值', visible: true },
