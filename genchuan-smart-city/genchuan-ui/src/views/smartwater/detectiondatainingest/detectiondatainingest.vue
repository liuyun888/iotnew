    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:detectiondatainingest:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="探测编号" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('探测日期')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('探测日期', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="探测单位"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('探测单位')"
          >
            <template #default="scope">
              {{ tableFieldShow('探测单位', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:detectiondatainingest:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:detectiondatainingest:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:detectiondatainingest:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>探测编号：</span>{{ queryData.name }}</li>
              <li class="width50"
              ...
              >
              <li class="width50"
                ><span>探测日期：</span>{{ dateFormatter2(null, null, tableFieldShow('探测日期', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>探测单位：</span>{{ tableFieldShow('探测单位', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入探测编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入探测编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施编码："
                prop="field.设施编码"
              ...
              <el-form-item
                ...
                prop="field.探测日期"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入探测日期', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.探测日期"
                  type="date"
                  value-format="x"
                  placeholder="选择探测日期"
                />
              ...
              <el-form-item
                ...
                prop="field.探测单位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入探测单位', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入探测单位"
                  v-model="formData.field.探测单位"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'f480ea50-78ad-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '探测数据接入-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.探测日期 = tableFieldShow('探测日期', row.field) //探测日期
  formData.value.field.探测单位 = tableFieldShow('探测单位', row.field) //探测单位

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑探测数据接入'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增探测数据接入'
}
...
const tableColumns = ref([
  { label: '探测编号', visible: true },
  { label: '设施编码', visible: true },
  { label: '坐标X', visible: true },
  { label: '坐标Y', visible: true },
  { label: '探测日期', visible: true },
  { label: '探测单位', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
