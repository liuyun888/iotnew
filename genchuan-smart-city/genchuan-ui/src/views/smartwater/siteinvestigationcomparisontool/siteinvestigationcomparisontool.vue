    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:siteinvestigationcomparisontool:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="勘察编号" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('系统坐标')"
          >
            <template #default="scope">
              {{ tableFieldShow('系统坐标', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="现场坐标"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('现场坐标')"
          >
            <template #default="scope">
              {{ tableFieldShow('现场坐标', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="偏差值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('偏差值')"
          >
            <template #default="scope">
              {{ tableFieldShow('偏差值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="勘察人员"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('勘察人员')"
          >
            <template #default="scope">
              {{ tableFieldShow('勘察人员', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="勘察日期"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('勘察日期')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('勘察日期', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:siteinvestigationcomparisontool:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:siteinvestigationcomparisontool:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:siteinvestigationcomparisontool:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>勘察编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设施编码：</span>{{ tableFieldShow('设施编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>系统坐标：</span>{{ tableFieldShow('系统坐标', queryData.field) }}</li
              >
              <li class="width50"
                ><span>现场坐标：</span>{{ tableFieldShow('现场坐标', queryData.field) }}</li
              >
              <li class="width50"
                ><span>偏差值：</span>{{ tableFieldShow('偏差值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>勘察人员：</span>{{ tableFieldShow('勘察人员', queryData.field) }}</li
              >
              <li class="width50"
                ><span>勘察日期：</span
                >{{ dateFormatter2(null, null, tableFieldShow('勘察日期', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入勘察编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入勘察编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施编码："
                prop="field.设施编码"
              ...
              <el-form-item
                ...
                prop="field.系统坐标"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入系统坐标', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入系统坐标"
                  v-model="formData.field.系统坐标"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.现场坐标"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入现场坐标', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入现场坐标"
                  v-model="formData.field.现场坐标"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.偏差值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入偏差值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入偏差值" v-model="formData.field.偏差值" clearable />
              </el-form-item>
              <el-form-item
                label="勘察人员："
                prop="field.勘察人员"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入勘察人员', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入勘察人员"
                  v-model="formData.field.勘察人员"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.勘察日期"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入勘察日期', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.勘察日期"
                  type="date"
                  value-format="x"
                  placeholder="选择勘察日期"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '01be3c20-78ba-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '现场勘查比对工具', //资产配置，都是写死固定的资产配置
  field: {
    设施编码: '', //设施编码
    系统坐标: '', //系统坐标
    现场坐标: '', //现场坐标
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '现场勘查比对工具-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.系统坐标 = tableFieldShow('系统坐标', row.field) //系统坐标
  formData.value.field.现场坐标 = tableFieldShow('现场坐标', row.field) //现场坐标
  formData.value.field.偏差值 = tableFieldShow('偏差值', row.field) //偏差值
  formData.value.field.勘察人员 = tableFieldShow('勘察人员', row.field) //勘察人员
  formData.value.field.勘察日期 = tableFieldShow('勘察日期', row.field) //勘察日期

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑现场勘查比对工具'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增现场勘查比对工具'
}
...
const tableColumns = ref([
  { label: '勘察编号', visible: true },
  { label: '设施编码', visible: true },
  { label: '系统坐标', visible: true },
  { label: '现场坐标', visible: true },
  { label: '偏差值', visible: true },
  { label: '勘察人员', visible: true },
  { label: '勘察日期', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
