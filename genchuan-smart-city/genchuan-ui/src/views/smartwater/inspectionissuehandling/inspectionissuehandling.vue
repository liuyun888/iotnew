    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:inspectionissuehandling:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('关联巡检记录ID')"
          >
            <template #default="scope">
              {{ tableFieldShow('关联巡检记录ID', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="设施编码"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('现场照片')"
          >
            <template #default="scope">
              {{ tableFieldShow('现场照片', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="问题类型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('问题类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('问题类型', scope.row.field),
                  optionsLisi.问题类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('工单状态')"
          >
            <template #default="scope">
              {{ tableFieldShow('工单状态', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="处理结果描述"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('处理结果描述')"
          >
            <template #default="scope">
              {{ tableFieldShow('处理结果描述', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="复核结果"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('复核结果')"
          >
            <template #default="scope">
              {{ tableFieldShow('复核结果', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="上报人"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('上报人')"
          >
            <template #default="scope">
              {{ tableFieldShow('上报人', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="上报时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('上报时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('上报时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:inspectionissuehandling:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:inspectionissuehandling:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:inspectionissuehandling:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>问题ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>关联巡检记录ID：</span
                >{{ tableFieldShow('关联巡检记录ID', queryData.field) }}</li
              >
              <li class="width50"
                ><span>设施编码：</span>{{ tableFieldShow('设施编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>现场照片：</span>{{ tableFieldShow('现场照片', queryData.field) }}</li
              >
              <li class="width50"
                ><span>问题类型：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('问题类型', queryData.field),
                    optionsLisi.问题类型Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>工单状态：</span>{{ tableFieldShow('工单状态', queryData.field) }}</li
              >
              <li class="width50"
                ><span>处理结果描述：</span
                >{{ tableFieldShow('处理结果描述', queryData.field) }}</li
              >
              <li class="width50"
                ><span>复核结果：</span>{{ tableFieldShow('复核结果', queryData.field) }}</li
              >
              <li class="width50"
                ><span>上报人：</span>{{ tableFieldShow('上报人', queryData.field) }}</li
              >
              <li class="width50"
                ><span>上报时间：</span>{{ dateFormatter(null, null, tableFieldShow('上报时间', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入问题ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入问题ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="关联巡检记录ID："
                prop="field.关联巡检记录ID"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入关联巡检记录ID', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入关联巡检记录ID"
                  v-model="formData.field.关联巡检记录ID"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.现场照片"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入现场照片', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入现场照片"
                  v-model="formData.field.现场照片"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-select
                  placeholder="请选择问题类型"
                  v-model="formData.field.问题类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.问题类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.工单状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入工单状态', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入工单状态"
                  v-model="formData.field.工单状态"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.处理结果描述"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入处理结果描述', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入处理结果描述"
                  v-model="formData.field.处理结果描述"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.复核结果"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入复核结果', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入复核结果"
                  v-model="formData.field.复核结果"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.上报人"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入上报人', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入上报人" v-model="formData.field.上报人" clearable />
              </el-form-item>
              <el-form-item
                label="上报时间："
                prop="field.上报时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入上报时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.上报时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择上报时间"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '34f19260-798a-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '巡检问题处理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.关联巡检记录ID = tableFieldShow('关联巡检记录ID', row.field) //关联巡检记录ID
  formData.value.field.设施编码 = tableFieldShow('设施编码', row.field) //设施编码
  formData.value.field.现场照片 = tableFieldShow('现场照片', row.field) //现场照片
  formData.value.field.问题类型 = tableFieldShow('问题类型', row.field) //问题类型
  formData.value.field.工单状态 = tableFieldShow('工单状态', row.field) //工单状态
  formData.value.field.处理结果描述 = tableFieldShow('处理结果描述', row.field) //处理结果描述
  formData.value.field.复核结果 = tableFieldShow('复核结果', row.field) //复核结果
  formData.value.field.上报人 = tableFieldShow('上报人', row.field) //上报人
  formData.value.field.上报时间 = tableFieldShow('上报时间', row.field) //上报时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑巡检问题处理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增巡检问题处理'
}
...
const tableColumns = ref([
  { label: '问题ID', visible: true },
  { label: '关联巡检记录ID', visible: true },
  { label: '设施编码', visible: true },
  { label: '现场照片', visible: true },
  { label: '问题类型', visible: true },
  { label: '工单状态', visible: true },
  { label: '处理结果描述', visible: true },
  { label: '复核结果', visible: true },
  { label: '上报人', visible: true },
  { label: '上报时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
