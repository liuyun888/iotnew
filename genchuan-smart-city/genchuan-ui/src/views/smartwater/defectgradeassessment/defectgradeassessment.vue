    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:defectgradeassessment:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="评估ID" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('缺陷等级')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('缺陷等级', scope.row.field),
                  optionsLisi.缺陷等级Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('修复优先级')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('修复优先级', scope.row.field),
                  optionsLisi.修复优先级Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('评估时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('评估时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:defectgradeassessment:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:defectgradeassessment:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:defectgradeassessment:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>评估ID：</span>{{ queryData.name }}</li>
              <li class="width50"
              ...
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('缺陷等级', queryData.field),
                    optionsLisi.缺陷等级Options
                  )
                }}</li
              >
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('修复优先级', queryData.field),
                    optionsLisi.修复优先级Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>评估时间：</span>{{ dateFormatter(null, null, tableFieldShow('评估时间', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入评估ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入评估ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="管道编码："
                prop="field.管道编码"
              ...
              <el-form-item
                ...
                prop="field.缺陷等级"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择缺陷等级', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择缺陷等级"
                  v-model="formData.field.缺陷等级"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.缺陷等级Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.修复优先级"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择修复优先级', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择修复优先级"
                  v-model="formData.field.修复优先级"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.修复优先级Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.评估时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入评估时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.评估时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择评估时间"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'b47db910-799d-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '缺陷等级评估-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.缺陷等级 = tableFieldShow('缺陷等级', row.field) //缺陷等级
  formData.value.field.修复优先级 = tableFieldShow('修复优先级', row.field) //修复优先级
  formData.value.field.评估时间 = tableFieldShow('评估时间', row.field) //评估时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑缺陷等级评估'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增缺陷等级评估'
}
...
const tableColumns = ref([
  { label: '评估ID', visible: true },
  { label: '管道编码', visible: true },
  { label: '缺陷类型', visible: true },
  { label: '缺陷等级', visible: true },
  { label: '修复优先级', visible: true },
  { label: '评估时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
