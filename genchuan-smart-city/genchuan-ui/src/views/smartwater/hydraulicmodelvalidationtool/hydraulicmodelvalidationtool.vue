    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:hydraulicmodelvalidationtool:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="模型验证编号" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入模型验证编号"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('模拟区域')"
          >
            <template #default="scope">
              {{ tableFieldShow('模拟区域', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="实测水位"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('实测水位')"
          >
            <template #default="scope">
              {{ tableFieldShow('实测水位', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="模拟水位"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('模拟水位')"
          >
            <template #default="scope">
              {{ tableFieldShow('模拟水位', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="误差值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('误差值')"
          >
            <template #default="scope">
              {{ tableFieldShow('误差值', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="验证日期"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('验证日期')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('验证日期', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:hydraulicmodelvalidationtool:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:hydraulicmodelvalidationtool:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:hydraulicmodelvalidationtool:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>模型验证编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>模拟区域：</span>{{ tableFieldShow('模拟区域', queryData.field) }}</li
              >
              <li class="width50"
                ><span>实测水位：</span>{{ tableFieldShow('实测水位', queryData.field) }}</li
              >
              <li class="width50"
                ><span>模拟水位：</span>{{ tableFieldShow('模拟水位', queryData.field) }}</li
              >
              <li class="width50"
                ><span>误差值：</span>{{ tableFieldShow('误差值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>验证日期：</span
                >{{ dateFormatter2(null, null, tableFieldShow('验证日期', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入模型验证编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入模型验证编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="模拟区域："
                prop="field.模拟区域"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入模拟区域', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入模拟区域"
                  v-model="formData.field.模拟区域"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.实测水位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入实测水位', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入实测水位" v-model="formData.field.实测水位" clearable>
              ...
              <el-form-item
                ...
                prop="field.模拟水位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入模拟水位', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入模拟水位" v-model="formData.field.模拟水位" clearable>
                  <template #append>米(m)</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.误差值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入误差值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入误差值" v-model="formData.field.误差值" clearable>
                  <template #append>米(m)</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.验证日期"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入验证日期', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.验证日期"
                  type="date"
                  value-format="x"
                  placeholder="选择验证日期"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'cb085850-78b9-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '水力模型验证工具', //资产配置，都是写死固定的资产配置
  field: {
    模拟区域: '', //模拟区域
    实测水位: '', //实测水位
    模拟水位: '', //模拟水位
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '水力模型验证工具-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.模拟区域 = tableFieldShow('模拟区域', row.field) //模拟区域
  formData.value.field.实测水位 = tableFieldShow('实测水位', row.field) //实测水位
  formData.value.field.模拟水位 = tableFieldShow('模拟水位', row.field) //模拟水位
  formData.value.field.误差值 = tableFieldShow('误差值', row.field) //误差值
  formData.value.field.验证日期 = tableFieldShow('验证日期', row.field) //验证日期

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑水力模型验证工具'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增水力模型验证工具'
}
...
const tableColumns = ref([
  { label: '模型验证编号', visible: true },
  { label: '模拟区域', visible: true },
  { label: '实测水位', visible: true },
  { label: '模拟水位', visible: true },
  { label: '误差值', visible: true },
  { label: '验证日期', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
