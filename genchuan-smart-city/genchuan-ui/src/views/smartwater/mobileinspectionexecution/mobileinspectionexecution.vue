    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:mobileinspectionexecution:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="巡检记录ID" prop="name" label-width="100px">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('关联计划ID')"
          >
            <template #default="scope">
              {{ tableFieldShow('关联计划ID', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="设施编码"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('状态描述')"
          >
            <template #default="scope">
              {{ tableFieldShow('状态描述', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="现场照片URL"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('现场照片URL')"
          >
            <template #default="scope">
              {{ tableFieldShow('现场照片URL', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="巡检人员ID"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('巡检人员ID')"
          >
            <template #default="scope">
              {{ tableFieldShow('巡检人员ID', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="位置坐标X"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('位置坐标X')"
          >
            <template #default="scope">
              {{ tableFieldShow('位置坐标X', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="位置坐标Y"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('位置坐标Y')"
          >
            <template #default="scope">
              {{ tableFieldShow('位置坐标Y', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:mobileinspectionexecution:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:mobileinspectionexecution:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:mobileinspectionexecution:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>巡检记录ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>关联计划ID：</span>{{ tableFieldShow('关联计划ID', queryData.field) }}</li
              >
              <li class="width50"
                ><span>设施编码：</span>{{ tableFieldShow('设施编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>状态描述：</span>{{ tableFieldShow('状态描述', queryData.field) }}</li
              >
              <li class="width50"
                ><span>现场照片URL：</span>{{ tableFieldShow('现场照片URL', queryData.field) }}</li
              >
              <li class="width50"
                ><span>巡检人员ID：</span>{{ tableFieldShow('巡检人员ID', queryData.field) }}</li
              >
              <li class="width50"
                ><span>位置坐标X：</span>{{ tableFieldShow('位置坐标X', queryData.field) }}</li
              >
              <li class="width50"
                ><span>位置坐标Y：</span>{{ tableFieldShow('位置坐标Y', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入巡检记录ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入巡检记录ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="关联计划ID："
                prop="field.关联计划ID"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入关联计划ID', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入关联计划ID"
                  v-model="formData.field.关联计划ID"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.现场照片URL"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入现场照片URL', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入现场照片URL"
                  v-model="formData.field.现场照片URL"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.巡检人员ID"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入巡检人员ID', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入巡检人员ID"
                  v-model="formData.field.巡检人员ID"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.位置坐标X"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入位置坐标X', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入位置坐标X"
                  v-model="formData.field.位置坐标X"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.位置坐标Y"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入位置坐标Y', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入位置坐标Y"
                  v-model="formData.field.位置坐标Y"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'f4a35270-7989-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '移动端巡检执行-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.关联计划ID = tableFieldShow('关联计划ID', row.field) //关联计划ID
  formData.value.field.设施编码 = tableFieldShow('设施编码', row.field) //设施编码
  formData.value.field.状态描述 = tableFieldShow('状态描述', row.field) //状态描述
  formData.value.field.现场照片URL = tableFieldShow('现场照片URL', row.field) //现场照片URL
  formData.value.field.巡检人员ID = tableFieldShow('巡检人员ID', row.field) //巡检人员ID
  formData.value.field.位置坐标X = tableFieldShow('位置坐标X', row.field) //位置坐标X
  formData.value.field.位置坐标Y = tableFieldShow('位置坐标Y', row.field) //位置坐标Y

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑移动端巡检执行'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增移动端巡检执行'
}
...
const tableColumns = ref([
  { label: '巡检记录ID', visible: true },
  { label: '关联计划ID', visible: true },
  { label: '设施编码', visible: true },
  { label: '状态描述', visible: true },
  { label: '现场照片URL', visible: true },
  { label: '巡检人员ID', visible: true },
  { label: '位置坐标X', visible: true },
  { label: '位置坐标Y', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
