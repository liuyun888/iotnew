    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:constructioninformation:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="审核单号" prop="name">
            <el-input
        ...
        >
          ...
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('变更类型', scope.row.field),
                  optionsLisi.变更类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('变更内容摘要')"
          >
            <template #default="scope">
              {{ tableFieldShow('变更内容摘要', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="当前审核节点"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('当前审核节点')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('当前审核节点', scope.row.field),
                  optionsLisi.当前审核节点Options
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('申请人')"
          >
            <template #default="scope">
              {{ tableFieldShow('申请人', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="申请时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('申请时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('申请时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:constructioninformation:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:constructioninformation:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:constructioninformation:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>审核单号：</span>{{ queryData.name }}</li>
              <li class="width50"
              ...
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('变更类型', queryData.field),
                    optionsLisi.变更类型Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>变更内容摘要：</span
                >{{ tableFieldShow('变更内容摘要', queryData.field) }}</li
              >
              <li class="width50"
                ><span>当前审核节点：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('当前审核节点', queryData.field),
                    optionsLisi.当前审核节点Options
                  )
                }}</li
              >
              ...
              >
              <li class="width50"
                ><span>申请人：</span>{{ tableFieldShow('申请人', queryData.field) }}</li
              >
              <li class="width50"
                ><span>申请时间：</span>{{ dateFormatter(null, null, tableFieldShow('申请时间', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入审核单号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入审核单号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施标识码："
                prop="field.设施标识码"
              ...
              <el-form-item
                ...
                prop="field.变更内容摘要"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入变更内容摘要', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入变更内容摘要"
                  v-model="formData.field.变更内容摘要"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.当前审核节点"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择当前审核节点', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择当前审核节点"
                  v-model="formData.field.当前审核节点"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.当前审核节点Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              >
                <el-input placeholder="请输入申请人" v-model="formData.field.申请人" clearable />
              </el-form-item>
              <el-form-item
                label="申请时间："
                prop="field.申请时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入申请时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.申请时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择申请时间"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '8bad5240-781a-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '建设信息维护-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.变更内容摘要 = tableFieldShow('变更内容摘要', row.field) //变更内容摘要
  formData.value.field.当前审核节点 = tableFieldShow('当前审核节点', row.field) //当前审核节点
  formData.value.field.审核意见 = tableFieldShow('审核意见', row.field) //审核意见
  formData.value.field.申请人 = tableFieldShow('申请人', row.field) //申请人
  formData.value.field.申请时间 = tableFieldShow('申请时间', row.field) //申请时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑建设信息维护'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增建设信息维护'
}
...
const tableColumns = ref([
  { label: '审核单号', visible: true },
  { label: '设施标识码', visible: true },
  { label: '变更类型', visible: true },
  { label: '变更内容摘要', visible: true },
  { label: '当前审核节点', visible: true },
  { label: '审核意见', visible: true },
  { label: '申请人', visible: true },
  { label: '申请时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
