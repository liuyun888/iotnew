    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:dataupdatesynchronization:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="更新设施标识码" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入更新设施标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('更新时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('更新时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="更新内容"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('更新内容')"
          >
            <template #default="scope">
              {{ tableFieldShow('更新内容', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="更新单位"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('更新单位')"
          >
            <template #default="scope">
              {{ tableFieldShow('更新单位', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:dataupdatesynchronization:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:dataupdatesynchronization:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:dataupdatesynchronization:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>更新设施标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>更新时间：</span>{{ dateFormatter(null, null, tableFieldShow('更新时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>更新内容：</span>{{ tableFieldShow('更新内容', queryData.field) }}</li
              >
              <li class="width50"
                ><span>更新单位：</span>{{ tableFieldShow('更新单位', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入更新设施标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入更新设施标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="更新时间："
                prop="field.更新时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入更新时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.更新时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择更新时间"
                />
              ...
              <el-form-item
                ...
                prop="field.更新内容"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入更新内容', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入更新内容"
                  v-model="formData.field.更新内容"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.更新单位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入更新单位', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入更新单位"
                  v-model="formData.field.更新单位"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '0b31ab10-797e-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '数据更新同步-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.更新时间 = tableFieldShow('更新时间', row.field) //更新时间
  formData.value.field.更新内容 = tableFieldShow('更新内容', row.field) //更新内容
  formData.value.field.更新单位 = tableFieldShow('更新单位', row.field) //更新单位

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑数据更新同步'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增数据更新同步'
}
...
const tableColumns = ref([
  { label: '更新设施标识码', visible: true },
  { label: '更新时间', visible: true },
  { label: '更新内容', visible: true },
  { label: '更新单位', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
