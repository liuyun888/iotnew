    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:sparepartmgmt:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('备件名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('备件名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="库存数量"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('库存数量')"
          >
            <template #default="scope">
              {{ tableFieldShow('库存数量', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="预警阈值"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('采购状态')"
          >
            <template #default="scope">
              {{ tableFieldShow('采购状态', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="供应商"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('供应商')"
          >
            <template #default="scope">
              {{ tableFieldShow('供应商', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:sparepartmgmt:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:sparepartmgmt:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:sparepartmgmt:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>备件编码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>备件名称：</span>{{ tableFieldShow('备件名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>库存数量：</span>{{ tableFieldShow('库存数量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>预警阈值：</span>{{ tableFieldShow('预警阈值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>采购状态：</span>{{ tableFieldShow('采购状态', queryData.field) }}</li
              >
              <li class="width50"
                ><span>供应商：</span>{{ tableFieldShow('供应商', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              >
                <el-input placeholder="请输入备件编码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="备件名称："
                prop="field.备件名称"
              ...
              <el-form-item
                ...
                prop="field.采购状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入采购状态', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入采购状态"
                  v-model="formData.field.采购状态"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input placeholder="请输入供应商" v-model="formData.field.供应商" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '40d20060-79a3-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '备件管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.备件名称 = tableFieldShow('备件名称', row.field) //备件名称
  formData.value.field.库存数量 = tableFieldShow('库存数量', row.field) //库存数量
  formData.value.field.预警阈值 = tableFieldShow('预警阈值', row.field) //预警阈值
  formData.value.field.采购状态 = tableFieldShow('采购状态', row.field) //采购状态
  formData.value.field.供应商 = tableFieldShow('供应商', row.field) //供应商

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑备件管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增备件管理'
}
...
const tableColumns = ref([
  ...
  { label: '备件名称', visible: true },
  { label: '库存数量', visible: true },
  { label: '预警阈值', visible: true },
  { label: '采购状态', visible: true },
