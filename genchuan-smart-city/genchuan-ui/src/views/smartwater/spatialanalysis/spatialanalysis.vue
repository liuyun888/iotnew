    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:spatialanalysis:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="分析任务ID" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入分析任务ID"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('设计设施标识码')"
          >
            <template #default="scope">
              {{ tableFieldShow('设计设施标识码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="缓冲半径"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('缓冲半径')"
          >
            <template #default="scope">
              {{ tableFieldShow('缓冲半径', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="空间关系结果"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('空间关系结果')"
          >
            <template #default="scope">
              {{ tableFieldShow('空间关系结果', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="分析时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('分析时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('分析时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="分析人"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('分析人')"
          >
            <template #default="scope">
              {{ tableFieldShow('分析人', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:spatialanalysis:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:spatialanalysis:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:spatialanalysis:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>分析任务ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设计设施标识码：</span
                >{{ tableFieldShow('设计设施标识码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>缓冲半径：</span>{{ tableFieldShow('缓冲半径', queryData.field) }}</li
              >
              <li class="width50"
                ><span>空间关系结果：</span
                >{{ tableFieldShow('空间关系结果', queryData.field) }}</li
              >
              <li class="width50"
                ><span>分析时间：</span
                >{{ dateFormatter(null, null, tableFieldShow('分析时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>分析人：</span>{{ tableFieldShow('分析人', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入分析任务ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入分析任务ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设计设施标识码："
                prop="field.设计设施标识码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入设计设施标识码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入设计设施标识码"
                  v-model="formData.field.设计设施标识码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.缓冲半径"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入缓冲半径', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入缓冲半径" v-model="formData.field.缓冲半径" clearable>
              ...
              <el-form-item
                ...
                prop="field.空间关系结果"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入空间关系结果', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入空间关系结果"
                  v-model="formData.field.空间关系结果"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.分析时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入分析时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.分析时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择分析时间"
                />
              ...
              <el-form-item
                ...
                prop="field.分析人"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入分析人', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入分析人" v-model="formData.field.分析人" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '05c26e40-7815-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '空间分析管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.设计设施标识码 = tableFieldShow('设计设施标识码', row.field) //设计设施标识码
  formData.value.field.缓冲半径 = tableFieldShow('缓冲半径', row.field) //缓冲半径
  formData.value.field.空间关系结果 = tableFieldShow('空间关系结果', row.field) //空间关系结果
  formData.value.field.分析时间 = tableFieldShow('分析时间', row.field) //分析时间
  formData.value.field.分析人 = tableFieldShow('分析人', row.field) //分析人

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑空间分析管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增空间分析管理'
}
...
const tableColumns = ref([
  { label: '分析任务ID', visible: true },
  { label: '设计设施标识码', visible: true },
  { label: '缓冲半径', visible: true },
  { label: '空间关系结果', visible: true },
  { label: '分析时间', visible: true },
  { label: '分析人', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
