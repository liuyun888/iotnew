    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:overflowweirledger:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="溢流堰标识码" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入溢流堰标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('堰顶高程')"
          >
            <template #default="scope">
              {{ tableFieldShow('堰顶高程', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="设计流量"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('流量系数')"
          >
            <template #default="scope">
              {{ tableFieldShow('流量系数', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="堰型分类"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('堰型分类')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('堰型分类', scope.row.field),
                  optionsLisi.堰型分类Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('堰底宽度')"
          >
            <template #default="scope">
              {{ tableFieldShow('堰底宽度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="关联排水系统编码"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('关联排水系统编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('关联排水系统编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="经度"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:overflowweirledger:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:overflowweirledger:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:overflowweirledger:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>溢流堰标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>堰顶高程：</span>{{ tableFieldShow('堰顶高程', queryData.field) }}</li
              >
              <li class="width50"
                ><span>设计流量：</span>{{ tableFieldShow('设计流量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>流量系数：</span>{{ tableFieldShow('流量系数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>堰型分类：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('堰型分类', queryData.field),
                    optionsLisi.堰型分类Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>堰底宽度：</span>{{ tableFieldShow('堰底宽度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>关联排水系统编码：</span
                >{{ tableFieldShow('关联排水系统编码', queryData.field) }}</li
              >
              <li class="width50"
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入溢流堰标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入溢流堰标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="堰顶高程："
                prop="field.堰顶高程"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入堰顶高程', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入堰顶高程"
                  v-model="formData.field.堰顶高程"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.设计流量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入设计流量', trigger: 'blur' }]"
              >
              ...
              <el-form-item
                ...
                prop="field.流量系数"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入流量系数', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入流量系数"
                  v-model="formData.field.流量系数"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.堰型分类"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择堰型分类', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择堰型分类"
                  v-model="formData.field.堰型分类"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.堰型分类Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.堰底宽度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入堰底宽度', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入堰底宽度" v-model="formData.field.堰底宽度" clearable>
              ...
              >
                <el-input
                  placeholder="请输入关联排水系统编码"
                  v-model="formData.field.关联排水系统编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '54934be0-780e-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '溢流堰台账管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.堰顶高程 = tableFieldShow('堰顶高程', row.field) //堰顶高程
  formData.value.field.设计流量 = tableFieldShow('设计流量', row.field) //设计流量
  formData.value.field.流量系数 = tableFieldShow('流量系数', row.field) //流量系数
  formData.value.field.堰型分类 = tableFieldShow('堰型分类', row.field) //堰型分类
  formData.value.field.堰底宽度 = tableFieldShow('堰底宽度', row.field) //堰底宽度
  formData.value.field.关联排水系统编码 = tableFieldShow('关联排水系统编码', row.field) //关联排水系统编码
  ...
  drawerConfig.value.title = '编辑溢流堰台账管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增溢流堰台账管理'
}
...
const tableColumns = ref([
  { label: '溢流堰标识码', visible: true },
  { label: '堰顶高程', visible: true },
  { label: '设计流量', visible: true },
  { label: '流量系数', visible: true },
  { label: '堰型分类', visible: true },
  { label: '堰底宽度', visible: true },
  { label: '关联排水系统编码', visible: true },
