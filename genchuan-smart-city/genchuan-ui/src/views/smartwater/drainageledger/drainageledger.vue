    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:drainageledger:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="排水系统标识码" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入排水系统标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('系统名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('系统名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="系统类型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('系统类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('系统类型', scope.row.field),
                  optionsLisi.系统类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('汇水面积')"
          >
            <template #default="scope">
              {{ tableFieldShow('汇水面积', scope.row.field) }} 平方米(㎡)
            </template>
          </el-table-column>
          <el-table-column
            label="服务人口数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('服务人口数')"
          >
            <template #default="scope">
              {{ tableFieldShow('服务人口数', scope.row.field) }} 人
            </template>
          </el-table-column>
          <el-table-column
            label="径流系数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('径流系数')"
          >
            <template #default="scope">
              {{ tableFieldShow('径流系数', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:drainageledger:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:drainageledger:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:drainageledger:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>排水系统标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>系统名称：</span>{{ tableFieldShow('系统名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>系统类型：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('系统类型', queryData.field),
                    optionsLisi.系统类型Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>汇水面积：</span>{{ tableFieldShow('汇水面积', queryData.field) }}</li
              >
              <li class="width50"
                ><span>服务人口数：</span>{{ tableFieldShow('服务人口数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>径流系数：</span>{{ tableFieldShow('径流系数', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入排水系统标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入排水系统标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="系统名称："
                prop="field.系统名称"
              ...
              <el-form-item
                ...
                prop="field.系统类型"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择系统类型', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择系统类型"
                  v-model="formData.field.系统类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.系统类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.汇水面积"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入汇水面积', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入汇水面积" v-model="formData.field.汇水面积" clearable>
              ...
              <el-form-item
                ...
                prop="field.服务人口数"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入服务人口数', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入服务人口数"
                  v-model="formData.field.服务人口数"
                  clearable
                >
              ...
              <el-form-item
                ...
                prop="field.径流系数"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入径流系数', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入径流系数"
                  v-model="formData.field.径流系数"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'de7d1b50-77f1-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '排水系统台账管理', //资产配置，都是写死固定的资产配置
  field: {
    系统名称: '', //系统名称
    系统类型: '', //系统类型
    汇水面积: '', //汇水面积
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '排水系统台账管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.系统名称 = tableFieldShow('系统名称', row.field) //系统名称
  formData.value.field.系统类型 = tableFieldShow('系统类型', row.field) //系统类型
  formData.value.field.汇水面积 = tableFieldShow('汇水面积', row.field) //汇水面积
  formData.value.field.服务人口数 = tableFieldShow('服务人口数', row.field) //服务人口数
  formData.value.field.径流系数 = tableFieldShow('径流系数', row.field) //径流系数

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑排水系统台账管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增排水系统台账管理'
}
...
const tableColumns = ref([
  { label: '排水系统标识码', visible: true },
  { label: '系统名称', visible: true },
  { label: '系统类型', visible: true },
  { label: '汇水面积', visible: true },
  { label: '服务人口数', visible: true },
  { label: '径流系数', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
