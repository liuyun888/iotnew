    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:manholecoverstatus:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="井盖标识码" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入井盖标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('定位坐标X')"
          >
            <template #default="scope">
              {{ tableFieldShow('定位坐标X', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="定位坐标Y"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('定位坐标Y')"
          >
            <template #default="scope">
              {{ tableFieldShow('定位坐标Y', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="状态"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('状态')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(tableFieldShow('状态', scope.row.field), optionsLisi.状态Options)
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:manholecoverstatus:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:manholecoverstatus:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:manholecoverstatus:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>井盖标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>监测时间：</span>{{ dateFormatter(null, null, tableFieldShow('监测时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>定位坐标X：</span>{{ tableFieldShow('定位坐标X', queryData.field) }}</li
              >
              <li class="width50"
                ><span>定位坐标Y：</span>{{ tableFieldShow('定位坐标Y', queryData.field) }}</li
              >
              <li class="width50"
                ><span>状态：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('状态', queryData.field),
                    optionsLisi.状态Options
                  )
                }}</li
              >
            </ul>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入井盖标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入井盖标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="监测时间："
                prop="field.监测时间"
              ...
              <el-form-item
                ...
                prop="field.定位坐标X"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入定位坐标X', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入定位坐标X"
                  v-model="formData.field.定位坐标X"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.定位坐标Y"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入定位坐标Y', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入定位坐标Y"
                  v-model="formData.field.定位坐标Y"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-select
                  placeholder="请选择状态"
                  v-model="formData.field.状态"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.状态Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '5db00090-78d8-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '井盖状态监测-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.定位坐标X = tableFieldShow('定位坐标X', row.field) //定位坐标X
  formData.value.field.定位坐标Y = tableFieldShow('定位坐标Y', row.field) //定位坐标Y
  formData.value.field.状态 = tableFieldShow('状态', row.field) //状态

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑井盖状态监测'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增井盖状态监测'
}
...
const tableColumns = ref([
  { label: '井盖标识码', visible: true },
  { label: '监测时间', visible: true },
  { label: '定位坐标X', visible: true },
  { label: '定位坐标Y', visible: true },
