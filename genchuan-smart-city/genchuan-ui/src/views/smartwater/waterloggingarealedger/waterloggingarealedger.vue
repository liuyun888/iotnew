    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:waterloggingarealedger:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="易涝区标识码" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入易涝区标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('易涝区名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('易涝区名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="易涝区面积"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('易涝区面积')"
          >
            <template #default="scope">
              {{ tableFieldShow('易涝区面积', scope.row.field) }} 平方米(㎡)
            </template>
          </el-table-column>
          <el-table-column
            label="最大内涝水深"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('最大内涝水深')"
          >
            <template #default="scope">
              {{ tableFieldShow('最大内涝水深', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="内涝主要原因"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('内涝主要原因')"
          >
            <template #default="scope">
              {{ tableFieldShow('内涝主要原因', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="调查日期"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('调查日期')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('调查日期', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="中心点经度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('中心点经度')"
          >
            <template #default="scope">
              {{ tableFieldShow('中心点经度', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="中心点纬度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('中心点纬度')"
          >
            <template #default="scope">
              {{ tableFieldShow('中心点纬度', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:waterloggingarealedger:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:waterloggingarealedger:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:waterloggingarealedger:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>易涝区标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>易涝区名称：</span>{{ tableFieldShow('易涝区名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>易涝区面积：</span>{{ tableFieldShow('易涝区面积', queryData.field) }}</li
              >
              <li class="width50"
                ><span>最大内涝水深：</span
                >{{ tableFieldShow('最大内涝水深', queryData.field) }}</li
              >
              <li class="width50"
                ><span>内涝主要原因：</span
                >{{ tableFieldShow('内涝主要原因', queryData.field) }}</li
              >
              <li class="width50"
                ><span>调查日期：</span>{{ dateFormatter2(null, null, tableFieldShow('调查日期', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>中心点经度：</span>{{ tableFieldShow('中心点经度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>中心点纬度：</span>{{ tableFieldShow('中心点纬度', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入易涝区标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入易涝区标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="易涝区名称："
                prop="field.易涝区名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入易涝区名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入易涝区名称"
                  v-model="formData.field.易涝区名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.易涝区面积"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入易涝区面积', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入易涝区面积"
                  v-model="formData.field.易涝区面积"
                  clearable
                >
              ...
              <el-form-item
                ...
                prop="field.最大内涝水深"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入最大内涝水深', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入最大内涝水深"
                  v-model="formData.field.最大内涝水深"
                  clearable
                >
              ...
              <el-form-item
                ...
                prop="field.内涝主要原因"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入内涝主要原因', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入内涝主要原因"
                  v-model="formData.field.内涝主要原因"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.调查日期"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入调查日期', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.调查日期"
                  type="date"
                  value-format="x"
                  placeholder="选择调查日期"
                />
              ...
              <el-form-item
                ...
                prop="field.中心点经度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入中心点经度', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入中心点经度"
                  v-model="formData.field.中心点经度"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.中心点纬度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入中心点纬度', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入中心点纬度"
                  v-model="formData.field.中心点纬度"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'f30751a0-7808-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '易涝区台账管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.易涝区名称 = tableFieldShow('易涝区名称', row.field) //易涝区名称
  formData.value.field.易涝区面积 = tableFieldShow('易涝区面积', row.field) //易涝区面积
  formData.value.field.最大内涝水深 = tableFieldShow('最大内涝水深', row.field) //最大内涝水深
  formData.value.field.内涝主要原因 = tableFieldShow('内涝主要原因', row.field) //内涝主要原因
  formData.value.field.调查日期 = tableFieldShow('调查日期', row.field) //调查日期
  formData.value.field.中心点经度 = tableFieldShow('中心点经度', row.field) //中心点经度
  formData.value.field.中心点纬度 = tableFieldShow('中心点纬度', row.field) //中心点纬度

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑易涝区台账管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增易涝区台账管理'
}
...
const tableColumns = ref([
  { label: '易涝区标识码', visible: true },
  { label: '易涝区名称', visible: true },
  { label: '易涝区面积', visible: true },
  { label: '最大内涝水深', visible: true },
  { label: '内涝主要原因', visible: true },
  { label: '调查日期', visible: true },
  { label: '中心点经度', visible: true },
  { label: '中心点纬度', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
