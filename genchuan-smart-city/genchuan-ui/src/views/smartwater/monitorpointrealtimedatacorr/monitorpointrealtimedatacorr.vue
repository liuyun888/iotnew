    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:monitorpointrealtimedatacorr:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="监测点编码" prop="name" label-width="100px">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('监测数值')"
          >
            <template #default="scope">
              {{ tableFieldShow('监测数值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="地理位置"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('地理位置')"
          >
            <template #default="scope">
              {{ tableFieldShow('地理位置', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="数据状态"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:monitorpointrealtimedatacorr:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:monitorpointrealtimedatacorr:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:monitorpointrealtimedatacorr:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>监测点编码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>监测时间：</span>{{ dateFormatter(null, null, tableFieldShow('监测时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>监测数值：</span>{{ tableFieldShow('监测数值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>地理位置：</span>{{ tableFieldShow('地理位置', queryData.field) }}</li
              >
              <li class="width50"
                ><span>数据状态：</span
                >{{
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入监测点编码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入监测点编码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="监测时间："
                prop="field.监测时间"
              ...
              <el-form-item
                ...
                prop="field.监测数值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入监测数值', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入监测数值"
                  v-model="formData.field.监测数值"
                  clearable
                />
              </el-form-item>
              <el-form-item
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'b9dadf80-797c-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '监测点实时数据关联', //资产配置，都是写死固定的资产配置
  field: {
    监测时间: '', //监测时间
    监测数值: '', //监测数值
    地理位置: '', //地理位置
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '监测点实时数据关联-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.监测数值 = tableFieldShow('监测数值', row.field) //监测数值
  formData.value.field.地理位置 = tableFieldShow('地理位置', row.field) //地理位置
  formData.value.field.数据状态 = tableFieldShow('数据状态', row.field) //数据状态

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑监测点实时数据关联'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增监测点实时数据关联'
}
...
const tableColumns = ref([
  { label: '监测点编码', visible: true },
  { label: '监测时间', visible: true },
  { label: '监测数值', visible: true },
  { label: '地理位置', visible: true },
