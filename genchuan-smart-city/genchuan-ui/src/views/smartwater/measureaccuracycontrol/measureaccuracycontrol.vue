    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:measureaccuracycontrol:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="精度控制编号" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入精度控制编号"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('测量方法')"
          >
            <template #default="scope">
              {{ tableFieldShow('测量方法', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="定向边长"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('定向边长')"
          >
            <template #default="scope">
              {{ tableFieldShow('定向边长', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="测点边长"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('测点边长')"
          >
            <template #default="scope">
              {{ tableFieldShow('测点边长', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="误差值"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:measureaccuracycontrol:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:measureaccuracycontrol:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:measureaccuracycontrol:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>精度控制编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>测量方法：</span>{{ tableFieldShow('测量方法', queryData.field) }}</li
              >
              <li class="width50"
                ><span>定向边长：</span>{{ tableFieldShow('定向边长', queryData.field) }}</li
              >
              <li class="width50"
                ><span>测点边长：</span>{{ tableFieldShow('测点边长', queryData.field) }}</li
              >
              <li class="width50"
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入精度控制编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入精度控制编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="测量方法："
                prop="field.测量方法"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入测量方法', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入测量方法"
                  v-model="formData.field.测量方法"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.定向边长"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入定向边长', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入定向边长" v-model="formData.field.定向边长" clearable>
              ...
              <el-form-item
                ...
                prop="field.测点边长"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入测点边长', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入测点边长" v-model="formData.field.测点边长" clearable>
                  <template #append>米(m)</template>
                </el-input>
              </el-form-item>
              <el-form-item
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '8c63fc00-78b2-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '测量精度控制-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.测量方法 = tableFieldShow('测量方法', row.field) //测量方法
  formData.value.field.定向边长 = tableFieldShow('定向边长', row.field) //定向边长
  formData.value.field.测点边长 = tableFieldShow('测点边长', row.field) //测点边长
  ...
  drawerConfig.value.title = '编辑测量精度控制'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增测量精度控制'
}
...
const tableColumns = ref([
  { label: '精度控制编号', visible: true },
  { label: '测量方法', visible: true },
  { label: '定向边长', visible: true },
  { label: '测点边长', visible: true },
