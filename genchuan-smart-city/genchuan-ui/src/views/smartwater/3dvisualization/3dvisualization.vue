    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:3dvisualization:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('关联设施标识码')"
          >
            <template #default="scope">
              {{ tableFieldShow('关联设施标识码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="三维坐标点"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('三维坐标点')"
          >
            <template #default="scope">
              {{ tableFieldShow('三维坐标点', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="模型精度等级"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('模型精度等级')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('模型精度等级', scope.row.field),
                  optionsLisi.模型精度等级Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('模型创建时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('模型创建时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="模型更新时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('模型更新时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('模型更新时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:3dvisualization:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:3dvisualization:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:3dvisualization:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>模型ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>关联设施标识码：</span
                >{{ tableFieldShow('关联设施标识码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>三维坐标点：</span>{{ tableFieldShow('三维坐标点', queryData.field) }}</li
              >
              <li class="width50"
                ><span>模型精度等级：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('模型精度等级', queryData.field),
                    optionsLisi.模型精度等级Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>模型创建时间：</span
                >{{
                  dateFormatter(null, null, tableFieldShow('模型创建时间', queryData.field))
                }}</li
              >
              <li class="width50"
                ><span>模型更新时间：</span
                >{{
                  dateFormatter(null, null, tableFieldShow('模型更新时间', queryData.field))
                }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入模型ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入模型ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="关联设施标识码："
                prop="field.关联设施标识码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入关联设施标识码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入关联设施标识码"
                  v-model="formData.field.关联设施标识码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.三维坐标点"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入三维坐标点', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入三维坐标点"
                  v-model="formData.field.三维坐标点"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.模型精度等级"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择模型精度等级', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择模型精度等级"
                  v-model="formData.field.模型精度等级"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.模型精度等级Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.模型创建时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入模型创建时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.模型创建时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择模型创建时间"
                />
              </el-form-item>
              <el-form-item
                label="模型更新时间："
                prop="field.模型更新时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入模型更新时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.模型更新时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择模型更新时间"
                />
              </el-form-item>
            </el-form>
          </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'a9b4c1c0-7814-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const optionsLisi = ref({
  //模型精度等级 选项
  模型精度等级Options: [
    { label: 'L1（5cm）', value: '1' },
    { label: 'L2（2cm）', value: '2' },
    { label: 'L3（1cm）', value: '3' }
  ]
})
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '三维可视化管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.关联设施标识码 = tableFieldShow('关联设施标识码', row.field) //关联设施标识码
  formData.value.field.三维坐标点 = tableFieldShow('三维坐标点', row.field) //三维坐标点
  formData.value.field.模型精度等级 = tableFieldShow('模型精度等级', row.field) //模型精度等级
  formData.value.field.模型创建时间 = tableFieldShow('模型创建时间', row.field) //模型创建时间
  formData.value.field.模型更新时间 = tableFieldShow('模型更新时间', row.field) //模型更新时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑三维可视化管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增三维可视化管理'
}
...
const tableColumns = ref([
  { label: '模型ID', visible: true },
  { label: '关联设施标识码', visible: true },
  { label: '三维坐标点', visible: true },
  { label: '模型精度等级', visible: true },
  { label: '模型创建时间', visible: true },
  { label: '模型更新时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
