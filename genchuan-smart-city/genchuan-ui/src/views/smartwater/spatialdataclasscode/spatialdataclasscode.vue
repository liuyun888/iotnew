    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:spatialdataclasscode:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="大类代码" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('大类名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('大类名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="小类代码"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('小类代码')"
          >
            <template #default="scope">
              {{ tableFieldShow('小类代码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="小类名称"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('小类名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('小类名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="空间要素类型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('空间要素类型')"
          >
            <template #default="scope">
              {{ tableFieldShow('空间要素类型', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="数据表名"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('数据表名')"
          >
            <template #default="scope">
              {{ tableFieldShow('数据表名', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:spatialdataclasscode:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:spatialdataclasscode:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:spatialdataclasscode:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>大类代码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>大类名称：</span>{{ tableFieldShow('大类名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>小类代码：</span>{{ tableFieldShow('小类代码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>小类名称：</span>{{ tableFieldShow('小类名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>空间要素类型：</span
                >{{ tableFieldShow('空间要素类型', queryData.field) }}</li
              >
              <li class="width50"
                ><span>数据表名：</span>{{ tableFieldShow('数据表名', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入大类代码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入大类代码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="大类名称："
                prop="field.大类名称"
              ...
              <el-form-item
                ...
                prop="field.小类代码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入小类代码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入小类代码"
                  v-model="formData.field.小类代码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.小类名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入小类名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入小类名称"
                  v-model="formData.field.小类名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.空间要素类型"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入空间要素类型', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入空间要素类型"
                  v-model="formData.field.空间要素类型"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.数据表名"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入数据表名', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入数据表名"
                  v-model="formData.field.数据表名"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '985c7e50-78a9-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '空间数据分类编码', //资产配置，都是写死固定的资产配置
  field: {
    大类名称: '', //大类名称
    小类代码: '', //小类代码
    小类名称: '', //小类名称
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '空间数据分类编码-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.大类名称 = tableFieldShow('大类名称', row.field) //大类名称
  formData.value.field.小类代码 = tableFieldShow('小类代码', row.field) //小类代码
  formData.value.field.小类名称 = tableFieldShow('小类名称', row.field) //小类名称
  formData.value.field.空间要素类型 = tableFieldShow('空间要素类型', row.field) //空间要素类型
  formData.value.field.数据表名 = tableFieldShow('数据表名', row.field) //数据表名

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑空间数据分类编码'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增空间数据分类编码'
}
...
const tableColumns = ref([
  { label: '大类代码', visible: true },
  { label: '大类名称', visible: true },
  { label: '小类代码', visible: true },
  { label: '小类名称', visible: true },
  { label: '空间要素类型', visible: true },
  { label: '数据表名', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
