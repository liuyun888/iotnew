      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:facilitystatus:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('当前状态')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('当前状态', scope.row.field),
                  optionsLisi.当前状态Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('状态变更原因')"
          >
            <template #default="scope">
              {{ tableFieldShow('状态变更原因', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="变更时间"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('变更人')"
          >
            <template #default="scope">
              {{ tableFieldShow('变更人', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="处理计划"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('处理计划')"
          >
            <template #default="scope">
              {{ tableFieldShow('处理计划', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:facilitystatus:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:facilitystatus:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:facilitystatus:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
          ...
          >
            <ul>
              ...
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('当前状态', queryData.field),
                    optionsLisi.当前状态Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>状态变更原因：</span
                >{{ tableFieldShow('状态变更原因', queryData.field) }}</li
              >
              <li class="width50"
                ><span>变更时间：</span>{{ dateFormatter(null, null, tableFieldShow('变更时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>变更人：</span>{{ tableFieldShow('变更人', queryData.field) }}</li
              >
              <li class="width50"
                ><span>处理计划：</span>{{ tableFieldShow('处理计划', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              >
                <el-select
                  placeholder="请选择当前状态"
                  v-model="formData.field.当前状态"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.当前状态Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.状态变更原因"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入状态变更原因', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入状态变更原因"
                  v-model="formData.field.状态变更原因"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.变更人"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入变更人', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入变更人" v-model="formData.field.变更人" clearable />
              </el-form-item>
              <el-form-item
                label="处理计划："
                prop="field.处理计划"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入处理计划', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入处理计划"
                  v-model="formData.field.处理计划"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'ec5e9630-781a-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '设施状态维护-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.当前状态 = tableFieldShow('当前状态', row.field) //当前状态
  formData.value.field.状态变更原因 = tableFieldShow('状态变更原因', row.field) //状态变更原因
  formData.value.field.变更时间 = tableFieldShow('变更时间', row.field) //变更时间
  formData.value.field.变更人 = tableFieldShow('变更人', row.field) //变更人
  formData.value.field.处理计划 = tableFieldShow('处理计划', row.field) //处理计划

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑设施状态维护'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增设施状态维护'
}
...
const tableColumns = ref([
  ...
  { label: '当前状态', visible: true },
  { label: '状态变更原因', visible: true },
  { label: '变更时间', visible: true },
  { label: '变更人', visible: true },
  { label: '处理计划', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
