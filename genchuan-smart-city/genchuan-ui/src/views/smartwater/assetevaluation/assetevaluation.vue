    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:assetevaluation:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="评估报告编号" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入评估报告编号"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('原值')"
          >
            <template #default="scope">
              {{ tableFieldShow('原值', scope.row.field) }} 个/米
            </template>
          </el-table-column>
          <el-table-column
            label="评估值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('评估值')"
          >
            <template #default="scope">
              {{ tableFieldShow('评估值', scope.row.field) }} 个/米
            </template>
          </el-table-column>
          <el-table-column
            label="评估方法"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('评估方法')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('评估方法', scope.row.field),
                  optionsLisi.评估方法Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('成新率')"
          >
            <template #default="scope">
              {{ tableFieldShow('成新率', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="折旧参数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('折旧参数')"
          >
            <template #default="scope">
              {{ tableFieldShow('折旧参数', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="市场参照案例"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('市场参照案例')"
          >
            <template #default="scope">
              {{ tableFieldShow('市场参照案例', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="收益预测模型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('收益预测模型')"
          >
            <template #default="scope">
              {{ tableFieldShow('收益预测模型', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="评估日期"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('评估日期')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('评估日期', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="评估人"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('评估人')"
          >
            <template #default="scope">
              {{ tableFieldShow('评估人', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:assetevaluation:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:assetevaluation:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:assetevaluation:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>评估报告编号：</span>{{ queryData.name }}</li>
              <li class="width50"
              ...
              >
              <li class="width50"
                ><span>原值：</span>{{ tableFieldShow('原值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>评估值：</span>{{ tableFieldShow('评估值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>评估方法：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('评估方法', queryData.field),
                    optionsLisi.评估方法Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>成新率：</span>{{ tableFieldShow('成新率', queryData.field) }}</li
              >
              <li class="width50"
                ><span>折旧参数：</span>{{ tableFieldShow('折旧参数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>市场参照案例：</span
                >{{ tableFieldShow('市场参照案例', queryData.field) }}</li
              >
              <li class="width50"
                ><span>收益预测模型：</span
                >{{ tableFieldShow('收益预测模型', queryData.field) }}</li
              >
              <li class="width50"
                ><span>评估日期：</span>{{ dateFormatter2(null, null, tableFieldShow('评估日期', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>评估人：</span>{{ tableFieldShow('评估人', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入评估报告编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入评估报告编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施标识码："
                prop="field.设施标识码"
              ...
              <el-form-item
                ...
                prop="field.原值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入原值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入原值" v-model="formData.field.原值" clearable>
                  <template #append>个/米</template>
                </el-input>
              </el-form-item>
              <el-form-item
                label="评估值："
                prop="field.评估值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入评估值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入评估值" v-model="formData.field.评估值" clearable>
                  <template #append>个/米</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.评估方法"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择评估方法', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择评估方法"
                  v-model="formData.field.评估方法"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.评估方法Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                :rules="[]"
              >
                <el-input placeholder="请输入成新率" v-model="formData.field.成新率" clearable />
              </el-form-item>
              <el-form-item
                label="折旧参数："
                prop="field.折旧参数"
              ...
              >
                <el-input
                  placeholder="请输入市场参照案例"
                  v-model="formData.field.市场参照案例"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input
                  placeholder="请输入收益预测模型"
                  v-model="formData.field.收益预测模型"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.评估日期"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入评估日期', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.评估日期"
                  type="date"
                  value-format="x"
                  placeholder="选择评估日期"
                />
              ...
              <el-form-item
                ...
                prop="field.评估人"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入评估人', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入评估人" v-model="formData.field.评估人" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '1dab5430-7811-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '资产评估管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.原值 = tableFieldShow('原值', row.field) //原值
  formData.value.field.评估值 = tableFieldShow('评估值', row.field) //评估值
  formData.value.field.评估方法 = tableFieldShow('评估方法', row.field) //评估方法
  formData.value.field.成新率 = tableFieldShow('成新率', row.field) //成新率
  formData.value.field.折旧参数 = tableFieldShow('折旧参数', row.field) //折旧参数
  formData.value.field.市场参照案例 = tableFieldShow('市场参照案例', row.field) //市场参照案例
  formData.value.field.收益预测模型 = tableFieldShow('收益预测模型', row.field) //收益预测模型
  formData.value.field.评估日期 = tableFieldShow('评估日期', row.field) //评估日期
  formData.value.field.评估人 = tableFieldShow('评估人', row.field) //评估人

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑资产评估管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增资产评估管理'
}
...
const tableColumns = ref([
  { label: '评估报告编号', visible: true },
  { label: '设施标识码', visible: true },
  { label: '设施类型', visible: true },
  { label: '原值', visible: true },
  { label: '评估值', visible: true },
  { label: '评估方法', visible: true },
  { label: '成新率', visible: true },
  { label: '折旧参数', visible: true },
  { label: '市场参照案例', visible: true },
  { label: '收益预测模型', visible: true },
  { label: '评估日期', visible: true },
  { label: '评估人', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
