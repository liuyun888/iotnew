      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:materialparam:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('材质类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('材质类型', scope.row.field),
                  optionsLisi.材质类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('管材等级')"
          >
            <template #default="scope">
              {{ tableFieldShow('管材等级', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="防腐措施"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('防腐措施')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('防腐措施', scope.row.field),
                  optionsLisi.防腐措施Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('抗渗等级')"
          >
            <template #default="scope">
              {{ tableFieldShow('抗渗等级', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="壁厚"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('壁厚')"
          >
            <template #default="scope">
              {{ tableFieldShow('壁厚', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="使用年限"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('使用年限')"
          >
            <template #default="scope">
              {{ tableFieldShow('使用年限', scope.row.field) }} 年
            </template>
          </el-table-column>
          <el-table-column
            label="检测报告编号"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检测报告编号')"
          >
            <template #default="scope">
              {{ tableFieldShow('检测报告编号', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="检测日期"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检测日期')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('检测日期', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:materialparam:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:materialparam:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:materialparam:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
          ...
          >
            <ul>
              ...
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('材质类型', queryData.field),
                    optionsLisi.材质类型Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>管材等级：</span>{{ tableFieldShow('管材等级', queryData.field) }}</li
              >
              <li class="width50"
                ><span>防腐措施：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('防腐措施', queryData.field),
                    optionsLisi.防腐措施Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>抗渗等级：</span>{{ tableFieldShow('抗渗等级', queryData.field) }}</li
              >
              <li class="width50"
                ><span>壁厚：</span>{{ tableFieldShow('壁厚', queryData.field) }}</li
              >
              <li class="width50"
                ><span>使用年限：</span>{{ tableFieldShow('使用年限', queryData.field) }}</li
              >
              <li class="width50"
                ><span>检测报告编号：</span
                >{{ tableFieldShow('检测报告编号', queryData.field) }}</li
              >
              <li class="width50"
                ><span>检测日期：</span>{{ dateFormatter2(null, null, tableFieldShow('检测日期', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.材质类型"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择材质类型', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择材质类型"
                  v-model="formData.field.材质类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.材质类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.管材等级"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入管材等级', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入管材等级"
                  v-model="formData.field.管材等级"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.防腐措施"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择防腐措施', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择防腐措施"
                  v-model="formData.field.防腐措施"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.防腐措施Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.抗渗等级"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入抗渗等级', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入抗渗等级"
                  v-model="formData.field.抗渗等级"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.壁厚"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入壁厚', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入壁厚" v-model="formData.field.壁厚" clearable />
              </el-form-item>
              <el-form-item
                label="使用年限："
                prop="field.使用年限"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入使用年限', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入使用年限" v-model="formData.field.使用年限" clearable>
              ...
              <el-form-item
                ...
                prop="field.检测报告编号"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检测报告编号', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入检测报告编号"
                  v-model="formData.field.检测报告编号"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.检测日期"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检测日期', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.检测日期"
                  type="date"
                  value-format="x"
                  placeholder="选择检测日期"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '18319c80-781b-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '材质参数维护-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.材质类型 = tableFieldShow('材质类型', row.field) //材质类型
  formData.value.field.管材等级 = tableFieldShow('管材等级', row.field) //管材等级
  formData.value.field.防腐措施 = tableFieldShow('防腐措施', row.field) //防腐措施
  formData.value.field.抗渗等级 = tableFieldShow('抗渗等级', row.field) //抗渗等级
  formData.value.field.壁厚 = tableFieldShow('壁厚', row.field) //壁厚
  formData.value.field.使用年限 = tableFieldShow('使用年限', row.field) //使用年限
  formData.value.field.检测报告编号 = tableFieldShow('检测报告编号', row.field) //检测报告编号
  formData.value.field.检测日期 = tableFieldShow('检测日期', row.field) //检测日期

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑材质参数维护'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增材质参数维护'
}
...
const tableColumns = ref([
  ...
  { label: '材质类型', visible: true },
  { label: '管材等级', visible: true },
  { label: '防腐措施', visible: true },
  { label: '抗渗等级', visible: true },
  { label: '壁厚', visible: true },
  { label: '使用年限', visible: true },
  { label: '检测报告编号', visible: true },
  { label: '检测日期', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
