    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:safetytrainingrecord:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="培训记录ID" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入培训记录ID"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('人员ID')"
          >
            <template #default="scope">
              {{ tableFieldShow('人员ID', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="培训内容"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('培训内容')"
          >
            <template #default="scope">
              {{ tableFieldShow('培训内容', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="考核成绩"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('考核成绩')"
          >
            <template #default="scope">
              {{ tableFieldShow('考核成绩', scope.row.field) }} 分
            </template>
          </el-table-column>
          <el-table-column
            label="培训时长"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('培训时长')"
          >
            <template #default="scope">
              {{ tableFieldShow('培训时长', scope.row.field) }} 小时(h)
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:safetytrainingrecord:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:safetytrainingrecord:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:safetytrainingrecord:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>培训记录ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>人员ID：</span>{{ tableFieldShow('人员ID', queryData.field) }}</li
              >
              <li class="width50"
                ><span>培训内容：</span>{{ tableFieldShow('培训内容', queryData.field) }}</li
              >
              <li class="width50"
                ><span>考核成绩：</span>{{ tableFieldShow('考核成绩', queryData.field) }}</li
              >
              <li class="width50"
                ><span>培训时长：</span>{{ tableFieldShow('培训时长', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入培训记录ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入培训记录ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="人员ID："
                prop="field.人员ID"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入人员ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入人员ID" v-model="formData.field.人员ID" clearable />
              </el-form-item>
              <el-form-item
                label="培训内容："
                prop="field.培训内容"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入培训内容', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入培训内容"
                  v-model="formData.field.培训内容"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.考核成绩"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入考核成绩', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入考核成绩" v-model="formData.field.考核成绩" clearable>
              ...
              <el-form-item
                ...
                prop="field.培训时长"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入培训时长', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入培训时长" v-model="formData.field.培训时长" clearable>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '3ce2b2e0-79a5-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '安全培训记录-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.人员ID = tableFieldShow('人员ID', row.field) //人员ID
  formData.value.field.培训内容 = tableFieldShow('培训内容', row.field) //培训内容
  formData.value.field.考核成绩 = tableFieldShow('考核成绩', row.field) //考核成绩
  formData.value.field.培训时长 = tableFieldShow('培训时长', row.field) //培训时长

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑安全培训记录'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增安全培训记录'
}
...
const tableColumns = ref([
  { label: '培训记录ID', visible: true },
  { label: '人员ID', visible: true },
  { label: '培训内容', visible: true },
  { label: '考核成绩', visible: true },
  { label: '培训时长', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
