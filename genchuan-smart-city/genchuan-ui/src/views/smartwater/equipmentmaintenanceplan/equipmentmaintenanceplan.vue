      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:equipmentmaintenanceplan:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('保养项目')"
          >
            <template #default="scope">
              {{ tableFieldShow('保养项目', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="计划保养时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('计划保养时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('计划保养时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="设备运行时长"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('设备运行时长')"
          >
            <template #default="scope">
              {{ tableFieldShow('设备运行时长', scope.row.field) }} 小时(h)
            </template>
          </el-table-column>
          <el-table-column
            label="责任方"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('责任方')"
          >
            <template #default="scope">
              {{ tableFieldShow('责任方', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:equipmentmaintenanceplan:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:equipmentmaintenanceplan:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:equipmentmaintenanceplan:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              >
              <li class="width50"
                ><span>保养项目：</span>{{ tableFieldShow('保养项目', queryData.field) }}</li
              >
              <li class="width50"
                ><span>计划保养时间：</span
                >{{
                  dateFormatter(null, null, tableFieldShow('计划保养时间', queryData.field))
                }}</li
              >
              <li class="width50"
                ><span>设备运行时长：</span
                >{{ tableFieldShow('设备运行时长', queryData.field) }}</li
              >
              <li class="width50"
                ><span>责任方：</span>{{ tableFieldShow('责任方', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.保养项目"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入保养项目', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入保养项目"
                  v-model="formData.field.保养项目"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.计划保养时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入计划保养时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.计划保养时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择计划保养时间"
                />
              </el-form-item>
              <el-form-item
                label="设备运行时长："
                prop="field.设备运行时长"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入设备运行时长', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入设备运行时长"
                  v-model="formData.field.设备运行时长"
                  clearable
                >
              ...
              <el-form-item
                ...
                prop="field.责任方"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入责任方', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入责任方" v-model="formData.field.责任方" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'c41da210-79a0-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '设备保养计划-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.保养项目 = tableFieldShow('保养项目', row.field) //保养项目
  formData.value.field.计划保养时间 = tableFieldShow('计划保养时间', row.field) //计划保养时间
  formData.value.field.设备运行时长 = tableFieldShow('设备运行时长', row.field) //设备运行时长
  formData.value.field.责任方 = tableFieldShow('责任方', row.field) //责任方

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑设备保养计划'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增设备保养计划'
}
...
const tableColumns = ref([
  ...
  { label: '保养项目', visible: true },
  { label: '计划保养时间', visible: true },
  { label: '设备运行时长', visible: true },
  { label: '责任方', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
