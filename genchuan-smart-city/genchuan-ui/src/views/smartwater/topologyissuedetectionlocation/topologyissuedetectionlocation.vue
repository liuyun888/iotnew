    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:topologyissuedetectionlocation:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="问题管段标识码" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入问题管段标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('标高高差')"
          >
            <template #default="scope">
              {{ tableFieldShow('标高高差', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="错接类型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('错接类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('错接类型', scope.row.field),
                  optionsLisi.错接类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('所在道路')"
          >
            <template #default="scope">
              {{ tableFieldShow('所在道路', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:topologyissuedetectionlocation:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:topologyissuedetectionlocation:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:topologyissuedetectionlocation:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>问题管段标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>问题类型：</span
                >{{
                  handleTypeOptions(
              ...
              >
              <li class="width50"
                ><span>标高高差：</span>{{ tableFieldShow('标高高差', queryData.field) }}</li
              >
              <li class="width50"
                ><span>错接类型：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('错接类型', queryData.field),
                    optionsLisi.错接类型Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>所在道路：</span>{{ tableFieldShow('所在道路', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入问题管段标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入问题管段标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="问题类型："
                prop="field.问题类型"
              ...
              <el-form-item
                ...
                :rules="[]"
              >
                <el-input placeholder="请输入标高高差" v-model="formData.field.标高高差" clearable>
              ...
              >
                <el-select
                  placeholder="请选择错接类型"
                  v-model="formData.field.错接类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.错接类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.所在道路"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入所在道路', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入所在道路"
                  v-model="formData.field.所在道路"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '83485f00-7978-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '拓扑问题检测与定位', //资产配置，都是写死固定的资产配置
  field: {
    问题类型: '', //问题类型
    标高高差: '', //标高高差
    错接类型: '', //错接类型
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '拓扑问题检测与定位-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.标高高差 = tableFieldShow('标高高差', row.field) //标高高差
  formData.value.field.错接类型 = tableFieldShow('错接类型', row.field) //错接类型
  formData.value.field.所在道路 = tableFieldShow('所在道路', row.field) //所在道路

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑拓扑问题检测与定位'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增拓扑问题检测与定位'
}
...
const tableColumns = ref([
  { label: '问题管段标识码', visible: true },
  { label: '问题类型', visible: true },
  { label: '标高高差', visible: true },
  { label: '错接类型', visible: true },
  { label: '所在道路', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
