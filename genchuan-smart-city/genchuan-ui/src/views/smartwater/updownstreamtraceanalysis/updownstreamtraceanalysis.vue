    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:updownstreamtraceanalysis:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="起点编码" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('终点编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('终点编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="管线标识码"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('管线标识码')"
          >
            <template #default="scope">
              {{ tableFieldShow('管线标识码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="起点管底标高"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('流向')"
          >
            <template #default="scope">
              {{ tableFieldShow('流向', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:updownstreamtraceanalysis:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:updownstreamtraceanalysis:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:updownstreamtraceanalysis:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>起点编码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>终点编码：</span>{{ tableFieldShow('终点编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>管线标识码：</span>{{ tableFieldShow('管线标识码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>起点管底标高：</span
              ...
              >
              <li class="width50"
                ><span>流向：</span>{{ tableFieldShow('流向', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入起点编码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入起点编码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="终点编码："
                prop="field.终点编码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入终点编码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入终点编码"
                  v-model="formData.field.终点编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.管线标识码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入管线标识码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入管线标识码"
                  v-model="formData.field.管线标识码"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.流向"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入流向', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入流向" v-model="formData.field.流向" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '58bebe10-7977-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '上下游追溯分析-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.终点编码 = tableFieldShow('终点编码', row.field) //终点编码
  formData.value.field.管线标识码 = tableFieldShow('管线标识码', row.field) //管线标识码
  ...
  formData.value.field.流向 = tableFieldShow('流向', row.field) //流向

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑上下游追溯分析'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增上下游追溯分析'
}
...
const tableColumns = ref([
  { label: '起点编码', visible: true },
  { label: '终点编码', visible: true },
  { label: '管线标识码', visible: true },
  { label: '起点管底标高', visible: true },
  { label: '终点管底标高', visible: true },
  { label: '流向', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
