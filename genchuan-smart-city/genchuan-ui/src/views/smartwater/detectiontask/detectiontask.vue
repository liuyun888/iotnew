    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:detectiontask:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('任务名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('任务名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="检查范围"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检查范围')"
          >
            <template #default="scope">
              {{ tableFieldShow('检查范围', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="检测单位"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检测单位')"
          >
            <template #default="scope">
              {{ tableFieldShow('检测单位', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="完成时限"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('完成时限')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('完成时限', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:detectiontask:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:detectiontask:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:detectiontask:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>任务ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>任务名称：</span>{{ tableFieldShow('任务名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>检查范围：</span>{{ tableFieldShow('检查范围', queryData.field) }}</li
              >
              <li class="width50"
                ><span>检测单位：</span>{{ tableFieldShow('检测单位', queryData.field) }}</li
              >
              <li class="width50"
                ><span>完成时限：</span>{{ dateFormatter2(null, null, tableFieldShow('完成时限', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              >
                <el-input placeholder="请输入任务ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="任务名称："
                prop="field.任务名称"
              ...
              <el-form-item
                ...
                prop="field.检查范围"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检查范围', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入检查范围"
                  v-model="formData.field.检查范围"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.检测单位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检测单位', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入检测单位"
                  v-model="formData.field.检测单位"
                  clearable
                />
              </el-form-item>
              <el-form-item
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '4b0d13e0-799d-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '检测任务管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.任务名称 = tableFieldShow('任务名称', row.field) //任务名称
  formData.value.field.检查范围 = tableFieldShow('检查范围', row.field) //检查范围
  formData.value.field.检测单位 = tableFieldShow('检测单位', row.field) //检测单位
  formData.value.field.完成时限 = tableFieldShow('完成时限', row.field) //完成时限

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑检测任务管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增检测任务管理'
}
...
const tableColumns = ref([
  { label: '任务ID', visible: true },
  { label: '任务名称', visible: true },
  { label: '检查范围', visible: true },
  { label: '检测单位', visible: true },
  { label: '完成时限', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
