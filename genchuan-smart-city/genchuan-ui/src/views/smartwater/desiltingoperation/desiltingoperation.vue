    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:desiltingoperation:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="作业ID" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('清淤量')"
          >
            <template #default="scope">
              {{ tableFieldShow('清淤量', scope.row.field) }} 立方米(m³)
            </template>
          </el-table-column>
          <el-table-column
            label="作业时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('作业时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('作业时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="设备编号"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('设备编号')"
          >
            <template #default="scope">
              {{ tableFieldShow('设备编号', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="操作人员ID"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('操作人员ID')"
          >
            <template #default="scope">
              {{ tableFieldShow('操作人员ID', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="视频文件URL"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('视频文件URL')"
          >
            <template #default="scope">
              {{ tableFieldShow('视频文件URL', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:desiltingoperation:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:desiltingoperation:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:desiltingoperation:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>作业ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>管道编码：</span>{{ tableFieldShow('管道编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>清淤量：</span>{{ tableFieldShow('清淤量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>作业时间：</span>{{ dateFormatter(null, null, tableFieldShow('作业时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>设备编号：</span>{{ tableFieldShow('设备编号', queryData.field) }}</li
              >
              <li class="width50"
                ><span>操作人员ID：</span>{{ tableFieldShow('操作人员ID', queryData.field) }}</li
              >
              <li class="width50"
                ><span>视频文件URL：</span>{{ tableFieldShow('视频文件URL', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入作业ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入作业ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="管道编码："
                prop="field.管道编码"
              ...
              <el-form-item
                ...
                prop="field.清淤量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入清淤量', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入清淤量" v-model="formData.field.清淤量" clearable>
              ...
              <el-form-item
                ...
                prop="field.作业时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入作业时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.作业时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择作业时间"
                />
              ...
              <el-form-item
                ...
                prop="field.操作人员ID"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入操作人员ID', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入操作人员ID"
                  v-model="formData.field.操作人员ID"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.视频文件URL"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入视频文件URL', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入视频文件URL"
                  v-model="formData.field.视频文件URL"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '09e80120-798b-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '清淤作业管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.清淤量 = tableFieldShow('清淤量', row.field) //清淤量
  formData.value.field.作业时间 = tableFieldShow('作业时间', row.field) //作业时间
  formData.value.field.设备编号 = tableFieldShow('设备编号', row.field) //设备编号
  formData.value.field.操作人员ID = tableFieldShow('操作人员ID', row.field) //操作人员ID
  formData.value.field.视频文件URL = tableFieldShow('视频文件URL', row.field) //视频文件URL

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑清淤作业管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增清淤作业管理'
}
...
const tableColumns = ref([
  { label: '作业ID', visible: true },
  { label: '管道编码', visible: true },
  { label: '清淤量', visible: true },
  { label: '作业时间', visible: true },
  { label: '设备编号', visible: true },
  { label: '操作人员ID', visible: true },
  { label: '视频文件URL', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
