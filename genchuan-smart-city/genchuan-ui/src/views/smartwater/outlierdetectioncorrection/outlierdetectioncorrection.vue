    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:outlierdetectioncorrection:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="异常编号" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('异常值')"
          >
            <template #default="scope">
              {{ tableFieldShow('异常值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="修正值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('修正值')"
          >
            <template #default="scope">
              {{ tableFieldShow('修正值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="处理人"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:outlierdetectioncorrection:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:outlierdetectioncorrection:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:outlierdetectioncorrection:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>异常编号：</span>{{ queryData.name }}</li>
              <li class="width50"
              ...
              >
              <li class="width50"
                ><span>异常值：</span>{{ tableFieldShow('异常值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>修正值：</span>{{ tableFieldShow('修正值', queryData.field) }}</li
              >
              <li class="width50"
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入异常编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入异常编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施编码："
                prop="field.设施编码"
              ...
              <el-form-item
                ...
                prop="field.异常值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入异常值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入异常值" v-model="formData.field.异常值" clearable />
              </el-form-item>
              <el-form-item
                label="修正值："
                prop="field.修正值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入修正值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入修正值" v-model="formData.field.修正值" clearable />
              </el-form-item>
              <el-form-item
                label="处理人："
                prop="field.处理人"
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '5d4cbfd0-78b5-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '异常值检测与修正', //资产配置，都是写死固定的资产配置
  field: {
    设施编码: '', //设施编码
    异常字段: '', //异常字段
    异常值: '', //异常值
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '异常值检测与修正-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.异常值 = tableFieldShow('异常值', row.field) //异常值
  formData.value.field.修正值 = tableFieldShow('修正值', row.field) //修正值
  formData.value.field.处理人 = tableFieldShow('处理人', row.field) //处理人

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑异常值检测与修正'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增异常值检测与修正'
}
...
const tableColumns = ref([
  { label: '异常编号', visible: true },
  { label: '设施编码', visible: true },
  { label: '异常字段', visible: true },
  { label: '异常值', visible: true },
  { label: '修正值', visible: true },
