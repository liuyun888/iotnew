    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:watersheddivisionattribute:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="汇水区标识码" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入汇水区标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('面积')"
          >
            <template #default="scope">
              {{ tableFieldShow('面积', scope.row.field) }} 平方米(㎡)
            </template>
          </el-table-column>
          <el-table-column
            label="径流系数"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('出水口编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('出水口编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="下垫面类型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('下垫面类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('下垫面类型', scope.row.field),
                  optionsLisi.下垫面类型Options
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:watersheddivisionattribute:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:watersheddivisionattribute:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:watersheddivisionattribute:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>汇水区标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>面积：</span>{{ tableFieldShow('面积', queryData.field) }}</li
              >
              <li class="width50"
                ><span>径流系数：</span>{{ tableFieldShow('径流系数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>出水口编码：</span>{{ tableFieldShow('出水口编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>下垫面类型：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('下垫面类型', queryData.field),
                    optionsLisi.下垫面类型Options
                  )
                }}</li
              >
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入汇水区标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入汇水区标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="面积："
                prop="field.面积"
              ...
              <el-form-item
                ...
                prop="field.出水口编码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入出水口编码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入出水口编码"
                  v-model="formData.field.出水口编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.下垫面类型"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择下垫面类型', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择下垫面类型"
                  v-model="formData.field.下垫面类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.下垫面类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'da00af40-7983-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '汇水区划分与属性管理', //资产配置，都是写死固定的资产配置
  field: {
    面积: '', //面积
    径流系数: '', //径流系数
    出水口编码: '', //出水口编码
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '汇水区划分与属性管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.出水口编码 = tableFieldShow('出水口编码', row.field) //出水口编码
  formData.value.field.下垫面类型 = tableFieldShow('下垫面类型', row.field) //下垫面类型

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑汇水区划分与属性管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增汇水区划分与属性管理'
}
...
const titleConfig = ref({
  name: '汇水区划分与属性管理', //标题名称
  addButName: '新增汇水区划分与属性管理' //新增按钮名称
})
// 搜索 字段
...
const tableColumns = ref([
  { label: '汇水区标识码', visible: true },
  { label: '面积', visible: true },
  { label: '径流系数', visible: true },
  { label: '出水口编码', visible: true },
  { label: '下垫面类型', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
