      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:regularwaterquality:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('pH值')"
          >
            <template #default="scope"> {{ tableFieldShow('pH值', scope.row.field) }} pH </template>
          </el-table-column>
          <el-table-column
            label="溶解氧DO"
            min-width="150"
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('溶解氧DO')"
          >
            <template #default="scope">
              {{ tableFieldShow('溶解氧DO', scope.row.field) }} mg/L
            </template>
          </el-table-column>
          <el-table-column
            label="悬浮物SS"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('悬浮物SS')"
          >
            <template #default="scope">
              {{ tableFieldShow('悬浮物SS', scope.row.field) }} mg/L
            </template>
          </el-table-column>
          <el-table-column
            label="pH值最高阈值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('pH值最高阈值')"
          >
            <template #default="scope">
              {{ tableFieldShow('pH值最高阈值', scope.row.field) }} pH
            </template>
          </el-table-column>
          <el-table-column
            label="pH值最低阈值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('pH值最低阈值')"
          >
            <template #default="scope">
              {{ tableFieldShow('pH值最低阈值', scope.row.field) }} pH
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:regularwaterquality:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:regularwaterquality:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:regularwaterquality:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              >
              <li class="width50"
                ><span>pH值：</span>{{ tableFieldShow('pH值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>溶解氧DO：</span>{{ tableFieldShow('溶解氧DO', queryData.field) }}</li
              >
              <li class="width50"
                ><span>悬浮物SS：</span>{{ tableFieldShow('悬浮物SS', queryData.field) }}</li
              >
              <li class="width50"
                ><span>pH值最高阈值：</span
                >{{ tableFieldShow('pH值最高阈值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>pH值最低阈值：</span
                >{{ tableFieldShow('pH值最低阈值', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.pH值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入pH值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入pH值" v-model="formData.field.pH值" clearable>
              ...
              <el-form-item
                ...
                prop="field.溶解氧DO"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入溶解氧DO', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入溶解氧DO" v-model="formData.field.溶解氧DO" clearable>
              ...
              <el-form-item
                ...
                prop="field.悬浮物SS"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入悬浮物SS', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入悬浮物SS" v-model="formData.field.悬浮物SS" clearable>
                  <template #append>mg/L</template>
                </el-input>
              </el-form-item>
              <el-form-item
                ...
                prop="field.pH值最高阈值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入pH值最高阈值', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入pH值最高阈值"
                  v-model="formData.field.pH值最高阈值"
                  clearable
                >
                  <template #append>pH</template>
                </el-input>
              ...
              <el-form-item
                ...
                prop="field.pH值最低阈值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入pH值最低阈值', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入pH值最低阈值"
                  v-model="formData.field.pH值最低阈值"
                  clearable
                >
                  <template #append>pH</template>
                </el-input>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '36ef2ed0-78d4-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '常规水质指标监测', //资产配置，都是写死固定的资产配置
  field: {
    采样时间: '', //采样时间
    pH值: '', //pH值
    溶解氧DO: '', //溶解氧DO
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '常规水质指标监测-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.pH值 = tableFieldShow('pH值', row.field) //pH值
  formData.value.field.溶解氧DO = tableFieldShow('溶解氧DO', row.field) //溶解氧DO
  formData.value.field.悬浮物SS = tableFieldShow('悬浮物SS', row.field) //悬浮物SS
  formData.value.field.pH值最高阈值 = tableFieldShow('pH值最高阈值', row.field) //pH值最高阈值
  formData.value.field.pH值最低阈值 = tableFieldShow('pH值最低阈值', row.field) //pH值最低阈值

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑常规水质指标监测'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增常规水质指标监测'
}
...
const tableColumns = ref([
  ...
  { label: 'pH值', visible: true },
  { label: '溶解氧DO', visible: true },
  { label: '悬浮物SS', visible: true },
  { label: 'pH值最高阈值', visible: true },
  { label: 'pH值最低阈值', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
