    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:operationfiling:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="档案编号" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('统计时段')"
          >
            <template #default="scope">
              {{ tableFieldShow('统计时段', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="作业次数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('作业次数')"
          >
            <template #default="scope">
              {{ tableFieldShow('作业次数', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="合格检测次数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('合格检测次数')"
          >
            <template #default="scope">
              {{ tableFieldShow('合格检测次数', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="应急事件数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('应急事件数')"
          >
            <template #default="scope">
              {{ tableFieldShow('应急事件数', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="归档时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('归档时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('归档时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:operationfiling:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:operationfiling:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:operationfiling:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>档案编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>统计时段：</span>{{ tableFieldShow('统计时段', queryData.field) }}</li
              >
              <li class="width50"
                ><span>作业次数：</span>{{ tableFieldShow('作业次数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>合格检测次数：</span
                >{{ tableFieldShow('合格检测次数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>应急事件数：</span>{{ tableFieldShow('应急事件数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>归档时间：</span>{{ dateFormatter(null, null, tableFieldShow('归档时间', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              >
                <el-input placeholder="请输入档案编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="统计时段："
                prop="field.统计时段"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入统计时段', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入统计时段"
                  v-model="formData.field.统计时段"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.作业次数"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入作业次数', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入作业次数"
                  v-model="formData.field.作业次数"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.合格检测次数"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入合格检测次数', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入合格检测次数"
                  v-model="formData.field.合格检测次数"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.应急事件数"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入应急事件数', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入应急事件数"
                  v-model="formData.field.应急事件数"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.归档时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入归档时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.归档时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择归档时间"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '00c57270-79a5-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '作业档案管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.统计时段 = tableFieldShow('统计时段', row.field) //统计时段
  formData.value.field.作业次数 = tableFieldShow('作业次数', row.field) //作业次数
  formData.value.field.合格检测次数 = tableFieldShow('合格检测次数', row.field) //合格检测次数
  formData.value.field.应急事件数 = tableFieldShow('应急事件数', row.field) //应急事件数
  formData.value.field.归档时间 = tableFieldShow('归档时间', row.field) //归档时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑作业档案管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增作业档案管理'
}
...
const tableColumns = ref([
  ...
  { label: '统计时段', visible: true },
  { label: '作业次数', visible: true },
  { label: '合格检测次数', visible: true },
  { label: '应急事件数', visible: true },
  { label: '归档时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
