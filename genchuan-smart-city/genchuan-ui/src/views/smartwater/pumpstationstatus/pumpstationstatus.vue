    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:pumpstationstatus:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="泵站标识码" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入泵站标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('电流')"
          >
            <template #default="scope">
              {{ tableFieldShow('电流', scope.row.field) }} 安(A)
            </template>
          </el-table-column>
          <el-table-column
            label="电压"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('电压')"
          >
            <template #default="scope">
              {{ tableFieldShow('电压', scope.row.field) }} 伏(V)
            </template>
          </el-table-column>
          <el-table-column
            label="运行状态"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:pumpstationstatus:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:pumpstationstatus:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:pumpstationstatus:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>泵站标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
              ...
              >
              <li class="width50"
                ><span>电流：</span>{{ tableFieldShow('电流', queryData.field) }}</li
              >
              <li class="width50"
                ><span>电压：</span>{{ tableFieldShow('电压', queryData.field) }}</li
              >
              <li class="width50"
                ><span>运行状态：</span
                >{{
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入泵站标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入泵站标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设备编号："
                prop="field.设备编号"
              ...
              >
                <el-input placeholder="请输入电流" v-model="formData.field.电流" clearable>
                  <template #append>安(A)</template>
                </el-input>
              </el-form-item>
              <el-form-item
                label="电压："
              ...
              >
                <el-input placeholder="请输入电压" v-model="formData.field.电压" clearable>
                  <template #append>伏(V)</template>
                </el-input>
              </el-form-item>
              <el-form-item
                label="运行状态："
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '5339bd00-78d7-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '泵站设备状态监测', //资产配置，都是写死固定的资产配置
  field: {
    设备编号: '', //设备编号
    监测时间: '', //监测时间
    电流: '', //电流
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '泵站设备状态监测-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.电流 = tableFieldShow('电流', row.field) //电流
  formData.value.field.电压 = tableFieldShow('电压', row.field) //电压
  formData.value.field.运行状态 = tableFieldShow('运行状态', row.field) //运行状态

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑泵站设备状态监测'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增泵站设备状态监测'
}
...
const tableColumns = ref([
  { label: '泵站标识码', visible: true },
  { label: '设备编号', visible: true },
  { label: '监测时间', visible: true },
  { label: '电流', visible: true },
  { label: '电压', visible: true },
