    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:coordinatetransformtool:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="转换编号" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('原坐标X')"
          >
            <template #default="scope">
              {{ tableFieldShow('原坐标X', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="原坐标Y"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('原坐标Y')"
          >
            <template #default="scope">
              {{ tableFieldShow('原坐标Y', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="目标坐标X"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('目标坐标X')"
          >
            <template #default="scope">
              {{ tableFieldShow('目标坐标X', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="目标坐标Y"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('目标坐标Y')"
          >
            <template #default="scope">
              {{ tableFieldShow('目标坐标Y', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="转换方法"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('转换方法')"
          >
            <template #default="scope">
              {{ tableFieldShow('转换方法', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:coordinatetransformtool:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:coordinatetransformtool:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:coordinatetransformtool:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>转换编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设施编码：</span>{{ tableFieldShow('设施编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>原坐标X：</span>{{ tableFieldShow('原坐标X', queryData.field) }}</li
              >
              <li class="width50"
                ><span>原坐标Y：</span>{{ tableFieldShow('原坐标Y', queryData.field) }}</li
              >
              <li class="width50"
                ><span>目标坐标X：</span>{{ tableFieldShow('目标坐标X', queryData.field) }}</li
              >
              <li class="width50"
                ><span>目标坐标Y：</span>{{ tableFieldShow('目标坐标Y', queryData.field) }}</li
              >
              <li class="width50"
                ><span>转换方法：</span>{{ tableFieldShow('转换方法', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入转换编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入转换编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施编码："
                prop="field.设施编码"
              ...
              <el-form-item
                ...
                prop="field.原坐标X"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入原坐标X', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入原坐标X" v-model="formData.field.原坐标X" clearable />
              </el-form-item>
              <el-form-item
                label="原坐标Y："
                prop="field.原坐标Y"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入原坐标Y', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入原坐标Y" v-model="formData.field.原坐标Y" clearable />
              </el-form-item>
              <el-form-item
                label="目标坐标X："
                prop="field.目标坐标X"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入目标坐标X', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入目标坐标X"
                  v-model="formData.field.目标坐标X"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.目标坐标Y"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入目标坐标Y', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入目标坐标Y"
                  v-model="formData.field.目标坐标Y"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.转换方法"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入转换方法', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入转换方法"
                  v-model="formData.field.转换方法"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'ee6db920-78b4-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '坐标转换工具-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.原坐标X = tableFieldShow('原坐标X', row.field) //原坐标X
  formData.value.field.原坐标Y = tableFieldShow('原坐标Y', row.field) //原坐标Y
  formData.value.field.目标坐标X = tableFieldShow('目标坐标X', row.field) //目标坐标X
  formData.value.field.目标坐标Y = tableFieldShow('目标坐标Y', row.field) //目标坐标Y
  formData.value.field.转换方法 = tableFieldShow('转换方法', row.field) //转换方法

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑坐标转换工具'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增坐标转换工具'
}
...
const tableColumns = ref([
  { label: '转换编号', visible: true },
  { label: '设施编码', visible: true },
  { label: '原坐标X', visible: true },
  { label: '原坐标Y', visible: true },
  { label: '目标坐标X', visible: true },
  { label: '目标坐标Y', visible: true },
  { label: '转换方法', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
