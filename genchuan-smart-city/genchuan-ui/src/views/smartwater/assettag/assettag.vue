    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:assettag:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="资产标签ID" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入资产标签ID"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('标签类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('标签类型', scope.row.field),
                  optionsLisi.标签类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('生成日期')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('生成日期', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="绑定状态"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('绑定状态')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('绑定状态', scope.row.field),
                  optionsLisi.绑定状态Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('绑定时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('绑定时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="RFID芯片ID"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('RFID芯片ID')"
          >
            <template #default="scope">
              {{ tableFieldShow('RFID芯片ID', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:assettag:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:assettag:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:assettag:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>资产标签ID：</span>{{ queryData.name }}</li>
              <li class="width50"
              ...
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('标签类型', queryData.field),
                    optionsLisi.标签类型Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>生成日期：</span>{{ dateFormatter2(null, null, tableFieldShow('生成日期', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>绑定状态：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('绑定状态', queryData.field),
                    optionsLisi.绑定状态Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>绑定时间：</span>{{ dateFormatter(null, null, tableFieldShow('绑定时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>RFID芯片ID：</span>{{ tableFieldShow('RFID芯片ID', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入资产标签ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入资产标签ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施标识码："
                prop="field.设施标识码"
              ...
              >
                <el-select
                  placeholder="请选择标签类型"
                  v-model="formData.field.标签类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.标签类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.生成日期"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入生成日期', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.生成日期"
                  type="date"
                  value-format="x"
                  placeholder="选择生成日期"
                />
              ...
              <el-form-item
                ...
                prop="field.绑定状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择绑定状态', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择绑定状态"
                  v-model="formData.field.绑定状态"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.绑定状态Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              >
                <el-input
                  placeholder="请输入RFID芯片ID"
                  v-model="formData.field.RFID芯片ID"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '29b89d60-7810-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '资产标签管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.标签类型 = tableFieldShow('标签类型', row.field) //标签类型
  formData.value.field.生成日期 = tableFieldShow('生成日期', row.field) //生成日期
  formData.value.field.绑定状态 = tableFieldShow('绑定状态', row.field) //绑定状态
  formData.value.field.绑定时间 = tableFieldShow('绑定时间', row.field) //绑定时间
  formData.value.field.RFID芯片ID = tableFieldShow('RFID芯片ID', row.field) //RFID芯片ID

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑资产标签管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增资产标签管理'
}
...
const tableColumns = ref([
  { label: '资产标签ID', visible: true },
  { label: '设施标识码', visible: true },
  { label: '标签类型', visible: true },
  { label: '生成日期', visible: true },
  { label: '绑定状态', visible: true },
  { label: '绑定时间', visible: true },
  { label: 'RFID芯片ID', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
