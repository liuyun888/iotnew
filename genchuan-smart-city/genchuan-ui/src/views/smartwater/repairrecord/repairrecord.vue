    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:repairrecord:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="维修记录ID" prop="name" label-width="100px">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('故障描述')"
          >
            <template #default="scope">
              {{ tableFieldShow('故障描述', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="备件编码"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('备件编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('备件编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="更换数量"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('更换数量')"
          >
            <template #default="scope">
              {{ tableFieldShow('更换数量', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="维修时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('维修时间')"
          >
            <template #default="scope">
              {{ tableFieldShow('维修时间', scope.row.field) }} 小时(h)
            </template>
          </el-table-column>
          <el-table-column
            label="维修人员"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('维修人员')"
          >
            <template #default="scope">
              {{ tableFieldShow('维修人员', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:repairrecord:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:repairrecord:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:repairrecord:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>维修记录ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设备编码：</span>{{ tableFieldShow('设备编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>故障描述：</span>{{ tableFieldShow('故障描述', queryData.field) }}</li
              >
              <li class="width50"
                ><span>备件编码：</span>{{ tableFieldShow('备件编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>更换数量：</span>{{ tableFieldShow('更换数量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>维修时间：</span>{{ tableFieldShow('维修时间', queryData.field) }}</li
              >
              <li class="width50"
                ><span>维修人员：</span>{{ tableFieldShow('维修人员', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入维修记录ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入维修记录ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设备编码："
                prop="field.设备编码"
              ...
              <el-form-item
                ...
                prop="field.备件编码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入备件编码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入备件编码"
                  v-model="formData.field.备件编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.更换数量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入更换数量', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入更换数量"
                  v-model="formData.field.更换数量"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.维修时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入维修时间', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入维修时间" v-model="formData.field.维修时间" clearable>
              ...
              <el-form-item
                ...
                prop="field.维修人员"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入维修人员', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入维修人员"
                  v-model="formData.field.维修人员"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '076bdb90-79a1-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '维修记录管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.故障描述 = tableFieldShow('故障描述', row.field) //故障描述
  formData.value.field.备件编码 = tableFieldShow('备件编码', row.field) //备件编码
  formData.value.field.更换数量 = tableFieldShow('更换数量', row.field) //更换数量
  formData.value.field.维修时间 = tableFieldShow('维修时间', row.field) //维修时间
  formData.value.field.维修人员 = tableFieldShow('维修人员', row.field) //维修人员

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑维修记录管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增维修记录管理'
}
...
const tableColumns = ref([
  { label: '维修记录ID', visible: true },
  { label: '设备编码', visible: true },
  { label: '故障描述', visible: true },
  { label: '备件编码', visible: true },
  { label: '更换数量', visible: true },
  { label: '维修时间', visible: true },
  { label: '维修人员', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
