    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:pumpstationstorageinternalstructure:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('设备类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('设备类型', scope.row.field),
                  optionsLisi.设备类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('位置坐标')"
          >
            <template #default="scope">
              {{ tableFieldShow('位置坐标', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="运行状态"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('实时功率')"
          >
            <template #default="scope">
              {{ tableFieldShow('实时功率', scope.row.field) }} 千瓦(kW)
            </template>
          </el-table-column>
          <el-table-column
            label="实时流量"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('实时流量')"
          >
            <template #default="scope">
              {{ tableFieldShow('实时流量', scope.row.field) }} 立方米每秒(m³/s)
            </template>
          </el-table-column>
          <el-table-column
            label="扬程或开度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('扬程或开度')"
          >
            <template #default="scope">
              {{ tableFieldShow('扬程或开度', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:pumpstationstorageinternalstructure:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:pumpstationstorageinternalstructure:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:pumpstationstorageinternalstructure:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>设备编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设备类型：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('设备类型', queryData.field),
                    optionsLisi.设备类型Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>位置坐标：</span>{{ tableFieldShow('位置坐标', queryData.field) }}</li
              >
              <li class="width50"
                ><span>运行状态：</span
                >{{
              ...
              >
              <li class="width50"
                ><span>实时功率：</span>{{ tableFieldShow('实时功率', queryData.field) }}</li
              >
              <li class="width50"
                ><span>实时流量：</span>{{ tableFieldShow('实时流量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>扬程或开度：</span>{{ tableFieldShow('扬程或开度', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              >
                <el-input placeholder="请输入设备编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设备类型："
                prop="field.设备类型"
              ...
              >
                <el-select
                  placeholder="请选择设备类型"
                  v-model="formData.field.设备类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.设备类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.位置坐标"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入位置坐标', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入位置坐标"
                  v-model="formData.field.位置坐标"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                :rules="[]"
              >
                <el-input placeholder="请输入实时功率" v-model="formData.field.实时功率" clearable>
              ...
              <el-form-item
                ...
                prop="field.实时流量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入实时流量', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入实时流量" v-model="formData.field.实时流量" clearable>
              ...
              >
                <el-input
                  placeholder="请输入扬程或开度"
                  v-model="formData.field.扬程或开度"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '0f947380-797f-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '泵站与调蓄池内部结构展示', //资产配置，都是写死固定的资产配置
  field: {
    设备类型: '', //设备类型
    位置坐标: '', //位置坐标
    运行状态: '', //运行状态
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '泵站与调蓄池内部结构展示-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.设备类型 = tableFieldShow('设备类型', row.field) //设备类型
  formData.value.field.位置坐标 = tableFieldShow('位置坐标', row.field) //位置坐标
  formData.value.field.运行状态 = tableFieldShow('运行状态', row.field) //运行状态
  formData.value.field.实时功率 = tableFieldShow('实时功率', row.field) //实时功率
  formData.value.field.实时流量 = tableFieldShow('实时流量', row.field) //实时流量
  formData.value.field.扬程或开度 = tableFieldShow('扬程或开度', row.field) //扬程或开度

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑泵站与调蓄池内部结构展示'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增泵站与调蓄池内部结构展示'
}
...
const titleConfig = ref({
  name: '泵站与调蓄池内部结构展示', //标题名称
  addButName: '新增泵站与调蓄池内部结构展示' //新增按钮名称
})
// 搜索 字段
...
const tableColumns = ref([
  ...
  { label: '设备类型', visible: true },
  { label: '位置坐标', visible: true },
  { label: '运行状态', visible: true },
  { label: '实时功率', visible: true },
  { label: '实时流量', visible: true },
  { label: '扬程或开度', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
