    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:gatevalveledger:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="闸门标识码" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入闸门标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('闸门名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('闸门名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="闸门净高度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('闸门净高度')"
          >
            <template #default="scope">
              {{ tableFieldShow('闸门净高度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="闸门净宽度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('闸门净宽度')"
          >
            <template #default="scope">
              {{ tableFieldShow('闸门净宽度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="控制方式"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('所属排水系统编号')"
          >
            <template #default="scope">
              {{ tableFieldShow('所属排水系统编号', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="关联泵站编号"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('关联泵站编号')"
          >
            <template #default="scope">
              {{ tableFieldShow('关联泵站编号', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="运行状态"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:gatevalveledger:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:gatevalveledger:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:gatevalveledger:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>闸门标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>闸门名称：</span>{{ tableFieldShow('闸门名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>闸门净高度：</span>{{ tableFieldShow('闸门净高度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>闸门净宽度：</span>{{ tableFieldShow('闸门净宽度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>控制方式：</span
                >{{
              ...
              <li class="width50"
                ><span>所属排水系统编号：</span
                >{{ tableFieldShow('所属排水系统编号', queryData.field) }}</li
              >
              <li class="width50"
                ><span>关联泵站编号：</span
                >{{ tableFieldShow('关联泵站编号', queryData.field) }}</li
              >
              <li class="width50"
                ><span>运行状态：</span
                >{{
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入闸门标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入闸门标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="闸门名称："
                prop="field.闸门名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入闸门名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入闸门名称"
                  v-model="formData.field.闸门名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.闸门净高度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入闸门净高度', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入闸门净高度"
                  v-model="formData.field.闸门净高度"
                  clearable
                >
              ...
              <el-form-item
                ...
                prop="field.闸门净宽度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入闸门净宽度', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入闸门净宽度"
                  v-model="formData.field.闸门净宽度"
                  clearable
                >
                  <template #append>米(m)</template>
                </el-input>
              ...
              >
                <el-input
                  placeholder="请输入所属排水系统编号"
                  v-model="formData.field.所属排水系统编号"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input
                  placeholder="请输入关联泵站编号"
                  v-model="formData.field.关联泵站编号"
                  clearable
                />
              </el-form-item>
              <el-form-item
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '03f4e8f0-780f-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '闸门/阀门台账管理', //资产配置，都是写死固定的资产配置
  field: {
    闸门名称: '', //闸门名称
    闸门净高度: '', //闸门净高度
    闸门净宽度: '', //闸门净宽度
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '闸门/阀门台账管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.闸门名称 = tableFieldShow('闸门名称', row.field) //闸门名称
  formData.value.field.闸门净高度 = tableFieldShow('闸门净高度', row.field) //闸门净高度
  formData.value.field.闸门净宽度 = tableFieldShow('闸门净宽度', row.field) //闸门净宽度
  formData.value.field.控制方式 = tableFieldShow('控制方式', row.field) //控制方式
  formData.value.field.所属排水系统编号 = tableFieldShow('所属排水系统编号', row.field) //所属排水系统编号
  formData.value.field.关联泵站编号 = tableFieldShow('关联泵站编号', row.field) //关联泵站编号
  formData.value.field.运行状态 = tableFieldShow('运行状态', row.field) //运行状态

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑闸门/阀门台账管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增闸门/阀门台账管理'
}
...
const tableColumns = ref([
  { label: '闸门标识码', visible: true },
  { label: '闸门名称', visible: true },
  { label: '闸门净高度', visible: true },
  { label: '闸门净宽度', visible: true },
  { label: '控制方式', visible: true },
  { label: '所属排水系统编号', visible: true },
  { label: '关联泵站编号', visible: true },
