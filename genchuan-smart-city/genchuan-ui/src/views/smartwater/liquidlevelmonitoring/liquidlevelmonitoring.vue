    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:liquidlevelmonitoring:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="检查井标识码" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入检查井标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('液位值')"
          >
            <template #default="scope">
              {{ tableFieldShow('液位值', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="液位状态码"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('液位状态码')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('液位状态码', scope.row.field),
                  optionsLisi.液位状态码Options
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:liquidlevelmonitoring:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:liquidlevelmonitoring:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:liquidlevelmonitoring:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>检查井标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>监测时间：</span
                >{{ dateFormatter(null, null, tableFieldShow('监测时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>液位值：</span>{{ tableFieldShow('液位值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>液位状态码：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('液位状态码', queryData.field),
                    optionsLisi.液位状态码Options
                  )
                }}</li
              >
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入检查井标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入检查井标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="监测时间："
                prop="field.监测时间"
              ...
              <el-form-item
                ...
                prop="field.液位值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入液位值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入液位值" v-model="formData.field.液位值" clearable>
              ...
              <el-form-item
                ...
                prop="field.液位状态码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择液位状态码', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择液位状态码"
                  v-model="formData.field.液位状态码"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.液位状态码Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '69ee9c50-78be-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '液位监测-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.液位值 = tableFieldShow('液位值', row.field) //液位值
  formData.value.field.液位状态码 = tableFieldShow('液位状态码', row.field) //液位状态码

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑液位监测'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增液位监测'
}
...
const tableColumns = ref([
  { label: '检查井标识码', visible: true },
  { label: '监测时间', visible: true },
  { label: '液位值', visible: true },
  { label: '液位状态码', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
