    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:trackingmeasure:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="测量编号" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('起点X')"
          >
            <template #default="scope">
              {{ tableFieldShow('起点X', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="起点Y"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('起点Y')"
          >
            <template #default="scope">
              {{ tableFieldShow('起点Y', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="终点X"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('终点X')"
          >
            <template #default="scope">
              {{ tableFieldShow('终点X', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="终点Y"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('终点Y')"
          >
            <template #default="scope">
              {{ tableFieldShow('终点Y', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="底部高程"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:trackingmeasure:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:trackingmeasure:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:trackingmeasure:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>测量编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设施编码：</span>{{ tableFieldShow('设施编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>起点X：</span>{{ tableFieldShow('起点X', queryData.field) }}</li
              >
              <li class="width50"
                ><span>起点Y：</span>{{ tableFieldShow('起点Y', queryData.field) }}</li
              >
              <li class="width50"
                ><span>终点X：</span>{{ tableFieldShow('终点X', queryData.field) }}</li
              >
              <li class="width50"
                ><span>终点Y：</span>{{ tableFieldShow('终点Y', queryData.field) }}</li
              >
              <li class="width50"
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入测量编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入测量编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施编码："
                prop="field.设施编码"
              ...
              <el-form-item
                ...
                prop="field.起点X"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入起点X', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入起点X" v-model="formData.field.起点X" clearable />
              </el-form-item>
              <el-form-item
                label="起点Y："
                prop="field.起点Y"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入起点Y', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入起点Y" v-model="formData.field.起点Y" clearable />
              </el-form-item>
              <el-form-item
                label="终点X："
                prop="field.终点X"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入终点X', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入终点X" v-model="formData.field.终点X" clearable />
              </el-form-item>
              <el-form-item
                label="终点Y："
                prop="field.终点Y"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入终点Y', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入终点Y" v-model="formData.field.终点Y" clearable />
              </el-form-item>
              <el-form-item
                label="底部高程："
                prop="field.底部高程"
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'a779b080-78b1-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '跟踪测量管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.起点X = tableFieldShow('起点X', row.field) //起点X
  formData.value.field.起点Y = tableFieldShow('起点Y', row.field) //起点Y
  formData.value.field.终点X = tableFieldShow('终点X', row.field) //终点X
  formData.value.field.终点Y = tableFieldShow('终点Y', row.field) //终点Y
  formData.value.field.底部高程 = tableFieldShow('底部高程', row.field) //底部高程

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑跟踪测量管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增跟踪测量管理'
}
...
const tableColumns = ref([
  { label: '测量编号', visible: true },
  { label: '设施编码', visible: true },
  { label: '起点X', visible: true },
  { label: '起点Y', visible: true },
  { label: '终点X', visible: true },
  { label: '终点Y', visible: true },
