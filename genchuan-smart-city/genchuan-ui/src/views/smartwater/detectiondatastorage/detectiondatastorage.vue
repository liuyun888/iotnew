      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:detectiondatastorage:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检测方法')"
          >
            <template #default="scope">
              {{ tableFieldShow('检测方法', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="视频URL"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('视频URL')"
          >
            <template #default="scope">
              {{ tableFieldShow('视频URL', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="缺陷类型"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('缺陷位置')"
          >
            <template #default="scope">
              {{ tableFieldShow('缺陷位置', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="缺陷长度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('缺陷长度')"
          >
            <template #default="scope">
              {{ tableFieldShow('缺陷长度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="缺陷面积"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('缺陷面积')"
          >
            <template #default="scope">
              {{ tableFieldShow('缺陷面积', scope.row.field) }} 平方米(㎡)
            </template>
          </el-table-column>
          <el-table-column
            label="检测时间"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:detectiondatastorage:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:detectiondatastorage:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:detectiondatastorage:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              >
              <li class="width50"
                ><span>检测方法：</span>{{ tableFieldShow('检测方法', queryData.field) }}</li
              >
              <li class="width50"
                ><span>视频URL：</span>{{ tableFieldShow('视频URL', queryData.field) }}</li
              >
              <li class="width50"
                ><span>缺陷类型：</span>{{ tableFieldShow('缺陷类型', queryData.field) }}</li
              >
              <li class="width50"
                ><span>缺陷位置：</span>{{ tableFieldShow('缺陷位置', queryData.field) }}</li
              >
              <li class="width50"
                ><span>缺陷长度：</span>{{ tableFieldShow('缺陷长度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>缺陷面积：</span>{{ tableFieldShow('缺陷面积', queryData.field) }}</li
              >
              <li class="width50"
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.检测方法"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检测方法', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入检测方法"
                  v-model="formData.field.检测方法"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.视频URL"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入视频URL', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入视频URL" v-model="formData.field.视频URL" clearable />
              </el-form-item>
              <el-form-item
                label="缺陷类型："
                prop="field.缺陷类型"
              ...
              <el-form-item
                ...
                prop="field.缺陷位置"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入缺陷位置', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入缺陷位置"
                  v-model="formData.field.缺陷位置"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                :rules="[]"
              >
                <el-input placeholder="请输入缺陷长度" v-model="formData.field.缺陷长度" clearable>
              ...
              <el-form-item
                ...
                :rules="[]"
              >
                <el-input placeholder="请输入缺陷面积" v-model="formData.field.缺陷面积" clearable>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '8318eac0-799d-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '检测数据入库-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.检测方法 = tableFieldShow('检测方法', row.field) //检测方法
  formData.value.field.视频URL = tableFieldShow('视频URL', row.field) //视频URL
  formData.value.field.缺陷类型 = tableFieldShow('缺陷类型', row.field) //缺陷类型
  formData.value.field.缺陷位置 = tableFieldShow('缺陷位置', row.field) //缺陷位置
  formData.value.field.缺陷长度 = tableFieldShow('缺陷长度', row.field) //缺陷长度
  formData.value.field.缺陷面积 = tableFieldShow('缺陷面积', row.field) //缺陷面积
  formData.value.field.检测时间 = tableFieldShow('检测时间', row.field) //检测时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑检测数据入库'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增检测数据入库'
}
...
const tableColumns = ref([
  ...
  { label: '检测方法', visible: true },
  { label: '视频URL', visible: true },
  { label: '缺陷类型', visible: true },
  { label: '缺陷位置', visible: true },
  { label: '缺陷长度', visible: true },
  { label: '缺陷面积', visible: true },
