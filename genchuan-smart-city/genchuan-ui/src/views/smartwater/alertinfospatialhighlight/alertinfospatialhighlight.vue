    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:alertinfospatialhighlight:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="预警编码" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('当前值')"
          >
            <template #default="scope">
              {{ tableFieldShow('当前值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="预警阈值"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('预警阈值')"
          >
            <template #default="scope">
              {{ tableFieldShow('预警阈值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="触发时间"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('空间位置')"
          >
            <template #default="scope">
              {{ tableFieldShow('空间位置', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:alertinfospatialhighlight:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:alertinfospatialhighlight:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:alertinfospatialhighlight:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>预警编码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>预警类型：</span>{{ tableFieldShow('预警类型', queryData.field) }}</li
              >
              <li class="width50"
                ><span>当前值：</span>{{ tableFieldShow('当前值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>预警阈值：</span>{{ tableFieldShow('预警阈值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>触发时间：</span>{{ dateFormatter(null, null, tableFieldShow('触发时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>空间位置：</span>{{ tableFieldShow('空间位置', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入预警编码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入预警编码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="预警类型："
                prop="field.预警类型"
              ...
              <el-form-item
                ...
                prop="field.当前值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入当前值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入当前值" v-model="formData.field.当前值" clearable />
              </el-form-item>
              <el-form-item
                label="预警阈值："
                prop="field.预警阈值"
              ...
              <el-form-item
                ...
                prop="field.触发时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择触发时间', trigger: 'blur' }]"
              >
                <el-date-picker
              ...
              <el-form-item
                ...
                prop="field.空间位置"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入空间位置', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入空间位置"
                  v-model="formData.field.空间位置"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '18c9da00-797d-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '预警信息空间高亮', //资产配置，都是写死固定的资产配置
  field: {
    预警类型: '', //预警类型
    当前值: '', //当前值
    预警阈值: '', //预警阈值
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '预警信息空间高亮-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.当前值 = tableFieldShow('当前值', row.field) //当前值
  formData.value.field.预警阈值 = tableFieldShow('预警阈值', row.field) //预警阈值
  formData.value.field.触发时间 = tableFieldShow('触发时间', row.field) //触发时间
  formData.value.field.空间位置 = tableFieldShow('空间位置', row.field) //空间位置

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑预警信息空间高亮'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增预警信息空间高亮'
}
...
const tableColumns = ref([
  { label: '预警编码', visible: true },
  { label: '预警类型', visible: true },
  { label: '当前值', visible: true },
  { label: '预警阈值', visible: true },
  { label: '触发时间', visible: true },
  { label: '空间位置', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
