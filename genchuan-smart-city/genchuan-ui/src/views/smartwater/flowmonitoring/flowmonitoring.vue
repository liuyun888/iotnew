    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:flowmonitoring:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="管道标识码" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入管道标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('瞬时流量')"
          >
            <template #default="scope">
              {{ tableFieldShow('瞬时流量', scope.row.field) }} 立方米每秒(m³/s)
            </template>
          </el-table-column>
          <el-table-column
            label="累计流量"
          ...
          >
            <template #default="scope">
              {{ tableFieldShow('累计流量', scope.row.field) }} 立方米(m³)
            </template>
          </el-table-column>
          <el-table-column
            label="降雨类型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('降雨类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('降雨类型', scope.row.field),
                  optionsLisi.降雨类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('日期类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('日期类型', scope.row.field),
                  optionsLisi.日期类型Options
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:flowmonitoring:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:flowmonitoring:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:flowmonitoring:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>管道标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>监测时间：</span>{{ dateFormatter(null, null, tableFieldShow('监测时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>瞬时流量：</span>{{ tableFieldShow('瞬时流量', queryData.field) }}</li
              >
              <li class="width50"
              ...
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('降雨类型', queryData.field),
                    optionsLisi.降雨类型Options
                  )
                }}</li
              >
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('日期类型', queryData.field),
                    optionsLisi.日期类型Options
                  )
                }}</li
              >
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入管道标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入管道标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="监测时间："
                prop="field.监测时间"
              ...
              <el-form-item
                ...
                prop="field.瞬时流量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入瞬时流量', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入瞬时流量" v-model="formData.field.瞬时流量" clearable>
              ...
              <el-form-item
                ...
                prop="field.降雨类型"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择降雨类型', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择降雨类型"
                  v-model="formData.field.降雨类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.降雨类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.日期类型"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择日期类型', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择日期类型"
                  v-model="formData.field.日期类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.日期类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'a1570740-78be-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '流量监测-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.瞬时流量 = tableFieldShow('瞬时流量', row.field) //瞬时流量
  formData.value.field.累计流量 = tableFieldShow('累计流量', row.field) //累计流量
  formData.value.field.降雨类型 = tableFieldShow('降雨类型', row.field) //降雨类型
  formData.value.field.日期类型 = tableFieldShow('日期类型', row.field) //日期类型

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑流量监测'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增流量监测'
}
...
const tableColumns = ref([
  { label: '管道标识码', visible: true },
  { label: '监测时间', visible: true },
  { label: '瞬时流量', visible: true },
  { label: '累计流量', visible: true },
  { label: '降雨类型', visible: true },
  { label: '日期类型', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
