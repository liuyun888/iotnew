    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:maintenanceplangeneration:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('管道编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('管道编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="淤积厚度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('淤积厚度')"
          >
            <template #default="scope">
              {{ tableFieldShow('淤积厚度', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="预计位置"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('预计位置')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('预计位置', scope.row.field),
                  optionsLisi.预计位置Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('计划养护方式')"
          >
            <template #default="scope">
              {{ tableFieldShow('计划养护方式', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="所需设备"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('所需设备')"
          >
            <template #default="scope">
              {{ tableFieldShow('所需设备', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="审核状态"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:maintenanceplangeneration:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:maintenanceplangeneration:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:maintenanceplangeneration:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>计划ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>管道编码：</span>{{ tableFieldShow('管道编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>淤积厚度：</span>{{ tableFieldShow('淤积厚度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>预计位置：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('预计位置', queryData.field),
                    optionsLisi.预计位置Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>计划养护方式：</span
                >{{ tableFieldShow('计划养护方式', queryData.field) }}</li
              >
              <li class="width50"
                ><span>所需设备：</span>{{ tableFieldShow('所需设备', queryData.field) }}</li
              >
              <li class="width50"
                ><span>审核状态：</span>{{ tableFieldShow('审核状态', queryData.field) }}</li
              >
              <li class="width50"
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入计划ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入计划ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="管道编码："
                prop="field.管道编码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入管道编码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入管道编码"
                  v-model="formData.field.管道编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.淤积厚度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入淤积厚度', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入淤积厚度" v-model="formData.field.淤积厚度" clearable>
              ...
              <el-form-item
                ...
                prop="field.预计位置"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择预计位置', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择预计位置"
                  v-model="formData.field.预计位置"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.预计位置Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.计划养护方式"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入计划养护方式', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入计划养护方式"
                  v-model="formData.field.计划养护方式"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.所需设备"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入所需设备', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入所需设备"
                  v-model="formData.field.所需设备"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.审核状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入审核状态', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入审核状态"
              ...
              <el-form-item
                ...
                prop="field.审核人"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入审核人', trigger: 'blur' }]"
              >
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'b8295690-798a-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '养护计划生成-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.管道编码 = tableFieldShow('管道编码', row.field) //管道编码
  formData.value.field.淤积厚度 = tableFieldShow('淤积厚度', row.field) //淤积厚度
  formData.value.field.预计位置 = tableFieldShow('预计位置', row.field) //预计位置
  formData.value.field.计划养护方式 = tableFieldShow('计划养护方式', row.field) //计划养护方式
  formData.value.field.所需设备 = tableFieldShow('所需设备', row.field) //所需设备
  ...
  drawerConfig.value.title = '编辑养护计划生成'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增养护计划生成'
}
...
const tableColumns = ref([
  { label: '计划ID', visible: true },
  { label: '管道编码', visible: true },
  { label: '淤积厚度', visible: true },
  { label: '预计位置', visible: true },
  { label: '计划养护方式', visible: true },
  { label: '所需设备', visible: true },
