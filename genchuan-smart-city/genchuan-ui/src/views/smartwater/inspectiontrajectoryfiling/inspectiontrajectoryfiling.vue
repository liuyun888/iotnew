    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:inspectiontrajectoryfiling:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="轨迹ID" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('轨迹时间戳集合')"
          >
            <template #default="scope">
              {{ tableFieldShow('轨迹时间戳集合', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="完成率"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('完成率')"
          >
            <template #default="scope">
              {{ tableFieldShow('完成率', scope.row.field) }} %
            </template>
          </el-table-column>
          <el-table-column
            label="问题发现数量"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('问题发现数量')"
          >
            <template #default="scope">
              {{ tableFieldShow('问题发现数量', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="档案编号"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('档案编号')"
          >
            <template #default="scope">
              {{ tableFieldShow('档案编号', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:inspectiontrajectoryfiling:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:inspectiontrajectoryfiling:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:inspectiontrajectoryfiling:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>轨迹ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>巡检人员ID：</span>{{ tableFieldShow('巡检人员ID', queryData.field) }}</li
              >
              <li class="width50"
                ><span>轨迹时间戳集合：</span
                >{{ tableFieldShow('轨迹时间戳集合', queryData.field) }}</li
              >
              <li class="width50"
                ><span>完成率：</span>{{ tableFieldShow('完成率', queryData.field) }}</li
              >
              <li class="width50"
                ><span>问题发现数量：</span
                >{{ tableFieldShow('问题发现数量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>档案编号：</span>{{ tableFieldShow('档案编号', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入轨迹ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入轨迹ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="巡检人员ID："
                prop="field.巡检人员ID"
              ...
              <el-form-item
                ...
                prop="field.轨迹时间戳集合"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入轨迹时间戳集合', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入轨迹时间戳集合"
                  v-model="formData.field.轨迹时间戳集合"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.完成率"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入完成率', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入完成率" v-model="formData.field.完成率" clearable>
              ...
              <el-form-item
                ...
                prop="field.问题发现数量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入问题发现数量', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入问题发现数量"
                  v-model="formData.field.问题发现数量"
                  clearable
                />
              </el-form-item>
              <el-form-item
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '75cdba70-798a-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '巡检轨迹与档案管理', //资产配置，都是写死固定的资产配置
  field: {
    巡检人员ID: '', //巡检人员ID
    轨迹时间戳集合: '', //轨迹时间戳集合
    完成率: '', //完成率
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '巡检轨迹与档案管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.轨迹时间戳集合 = tableFieldShow('轨迹时间戳集合', row.field) //轨迹时间戳集合
  formData.value.field.完成率 = tableFieldShow('完成率', row.field) //完成率
  formData.value.field.问题发现数量 = tableFieldShow('问题发现数量', row.field) //问题发现数量
  formData.value.field.档案编号 = tableFieldShow('档案编号', row.field) //档案编号

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑巡检轨迹与档案管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增巡检轨迹与档案管理'
}
...
const tableColumns = ref([
  { label: '轨迹ID', visible: true },
  { label: '巡检人员ID', visible: true },
  { label: '轨迹时间戳集合', visible: true },
  { label: '完成率', visible: true },
  { label: '问题发现数量', visible: true },
  { label: '档案编号', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
