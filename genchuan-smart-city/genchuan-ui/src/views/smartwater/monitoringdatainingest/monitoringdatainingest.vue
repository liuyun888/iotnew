    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:monitoringdatainingest:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="监测点编号" prop="name" label-width="100px">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('监测值')"
          >
            <template #default="scope">
              {{ tableFieldShow('监测值', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="采集时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('采集时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('采集时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="数据状态"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('数据状态')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('数据状态', scope.row.field),
                  optionsLisi.数据状态Options
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:monitoringdatainingest:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:monitoringdatainingest:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:monitoringdatainingest:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>监测点编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>监测值：</span>{{ tableFieldShow('监测值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>采集时间：</span>{{ dateFormatter(null, null, tableFieldShow('采集时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>数据状态：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('数据状态', queryData.field),
                    optionsLisi.数据状态Options
                  )
                }}</li
              >
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入监测点编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入监测点编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="监测值："
                prop="field.监测值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入监测值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入监测值" v-model="formData.field.监测值" clearable />
              </el-form-item>
              <el-form-item
                label="采集时间："
                prop="field.采集时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入采集时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.采集时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择采集时间"
                />
              ...
              <el-form-item
                ...
                prop="field.数据状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择数据状态', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择数据状态"
                  v-model="formData.field.数据状态"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.数据状态Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '4bd09990-78ae-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '监测数据接入-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.监测值 = tableFieldShow('监测值', row.field) //监测值
  formData.value.field.采集时间 = tableFieldShow('采集时间', row.field) //采集时间
  formData.value.field.数据状态 = tableFieldShow('数据状态', row.field) //数据状态
  formData.value.field.设备编码 = tableFieldShow('设备编码', row.field) //设备编码

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑监测数据接入'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增监测数据接入'
}
...
const tableColumns = ref([
  { label: '监测点编号', visible: true },
  { label: '监测值', visible: true },
  { label: '采集时间', visible: true },
  { label: '数据状态', visible: true },
