    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:terrainmapping:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="测绘编号" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('泵站编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('泵站编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="构筑物名称"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('构筑物名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('构筑物名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="平面坐标"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('平面坐标')"
          >
            <template #default="scope">
              {{ tableFieldShow('平面坐标', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="高程"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('高程')"
          >
            <template #default="scope">
              {{ tableFieldShow('高程', scope.row.field) }} 米(m)
            </template>
          </el-table-column>
          <el-table-column
            label="测绘日期"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('测绘日期')"
          >
            <template #default="scope">
              {{ dateFormatter2(null, null, tableFieldShow('测绘日期', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:terrainmapping:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:terrainmapping:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:terrainmapping:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>测绘编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>泵站编码：</span>{{ tableFieldShow('泵站编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>构筑物名称：</span>{{ tableFieldShow('构筑物名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>平面坐标：</span>{{ tableFieldShow('平面坐标', queryData.field) }}</li
              >
              <li class="width50"
                ><span>高程：</span>{{ tableFieldShow('高程', queryData.field) }}</li
              >
              <li class="width50"
                ><span>测绘日期：</span>{{ dateFormatter2(null, null, tableFieldShow('测绘日期', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入测绘编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入测绘编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="泵站编码："
                prop="field.泵站编码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入泵站编码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入泵站编码"
                  v-model="formData.field.泵站编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.构筑物名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入构筑物名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入构筑物名称"
                  v-model="formData.field.构筑物名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.平面坐标"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入平面坐标', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入平面坐标"
                  v-model="formData.field.平面坐标"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.高程"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入高程', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入高程" v-model="formData.field.高程" clearable>
              ...
              <el-form-item
                ...
                prop="field.测绘日期"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入测绘日期', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.测绘日期"
                  type="date"
                  value-format="x"
                  placeholder="选择测绘日期"
                />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '48067560-78b2-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '地形测绘管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.泵站编码 = tableFieldShow('泵站编码', row.field) //泵站编码
  formData.value.field.构筑物名称 = tableFieldShow('构筑物名称', row.field) //构筑物名称
  formData.value.field.平面坐标 = tableFieldShow('平面坐标', row.field) //平面坐标
  formData.value.field.高程 = tableFieldShow('高程', row.field) //高程
  formData.value.field.测绘日期 = tableFieldShow('测绘日期', row.field) //测绘日期

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑地形测绘管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增地形测绘管理'
}
...
const tableColumns = ref([
  { label: '测绘编号', visible: true },
  { label: '泵站编码', visible: true },
  { label: '构筑物名称', visible: true },
  { label: '平面坐标', visible: true },
  { label: '高程', visible: true },
  { label: '测绘日期', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
