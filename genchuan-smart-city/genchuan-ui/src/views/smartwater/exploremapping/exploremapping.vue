    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:exploremapping:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="探查编号" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检查井编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('检查井编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="管道方位"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('管道方位')"
          >
            <template #default="scope">
              {{ tableFieldShow('管道方位', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="管径"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('修正记录')"
          >
            <template #default="scope">
              {{ tableFieldShow('修正记录', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:exploremapping:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:exploremapping:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:exploremapping:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>探查编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>检查井编码：</span>{{ tableFieldShow('检查井编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>管道方位：</span>{{ tableFieldShow('管道方位', queryData.field) }}</li
              >
              <li class="width50"
              ...
              >
              <li class="width50"
                ><span>修正记录：</span>{{ tableFieldShow('修正记录', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入探查编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入探查编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="检查井编码："
                prop="field.检查井编码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检查井编码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入检查井编码"
                  v-model="formData.field.检查井编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.管道方位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入管道方位', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入管道方位"
                  v-model="formData.field.管道方位"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.修正记录"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入修正记录', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入修正记录"
                  v-model="formData.field.修正记录"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'f70d16a0-78b1-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '探查测绘管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.检查井编码 = tableFieldShow('检查井编码', row.field) //检查井编码
  formData.value.field.管道方位 = tableFieldShow('管道方位', row.field) //管道方位
  formData.value.field.管径 = tableFieldShow('管径', row.field) //管径
  formData.value.field.埋深 = tableFieldShow('埋深', row.field) //埋深
  formData.value.field.修正记录 = tableFieldShow('修正记录', row.field) //修正记录

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑探查测绘管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增探查测绘管理'
}
...
const tableColumns = ref([
  { label: '探查编号', visible: true },
  { label: '检查井编码', visible: true },
  { label: '管道方位', visible: true },
  { label: '管径', visible: true },
  { label: '埋深', visible: true },
  { label: '修正记录', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
