      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:velocitymonitoring:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('流速值')"
          >
            <template #default="scope">
              {{ tableFieldShow('流速值', scope.row.field) }} 米每秒(m/s)
            </template>
          </el-table-column>
          <el-table-column
            label="流速状态"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('流速状态')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('流速状态', scope.row.field),
                  optionsLisi.流速状态Options
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:velocitymonitoring:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:velocitymonitoring:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:velocitymonitoring:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              >
              <li class="width50"
                ><span>流速值：</span>{{ tableFieldShow('流速值', queryData.field) }}</li
              >
              <li class="width50"
                ><span>流速状态：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('流速状态', queryData.field),
                    optionsLisi.流速状态Options
                  )
                }}</li
              >
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.流速值"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入流速值', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入流速值" v-model="formData.field.流速值" clearable>
                  <template #append>米每秒(m/s)</template>
                </el-input>
              </el-form-item>
              <el-form-item
                label="流速状态："
                prop="field.流速状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择流速状态', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择流速状态"
                  v-model="formData.field.流速状态"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.流速状态Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'fd8003f0-78be-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '流速监测-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.流速值 = tableFieldShow('流速值', row.field) //流速值
  formData.value.field.流速状态 = tableFieldShow('流速状态', row.field) //流速状态

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑流速监测'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增流速监测'
}
...
const tableColumns = ref([
  ...
  { label: '流速值', visible: true },
  { label: '流速状态', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
