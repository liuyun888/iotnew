    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:preventivemaintenancealert:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="预警ID" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('设备坐标X')"
          >
            <template #default="scope">
              {{ tableFieldShow('设备坐标X', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="设备坐标Y"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('设备坐标Y')"
          >
            <template #default="scope">
              {{ tableFieldShow('设备坐标Y', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="预警类型"
          ...
          >
            <template #default="scope">
              {{ tableFieldShow('预警类型', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="关联运行参数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('关联运行参数')"
          >
            <template #default="scope">
              {{ tableFieldShow('关联运行参数', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="预警时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('预警时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('预警时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="处理状态"
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:preventivemaintenancealert:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:preventivemaintenancealert:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:preventivemaintenancealert:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>预警ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设备编码：</span>{{ tableFieldShow('设备编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>设备坐标X：</span>{{ tableFieldShow('设备坐标X', queryData.field) }}</li
              >
              <li class="width50"
                ><span>设备坐标Y：</span>{{ tableFieldShow('设备坐标Y', queryData.field) }}</li
              >
              <li class="width50"
                ><span>预警类型：</span>{{ tableFieldShow('预警类型', queryData.field) }}</li
              >
              <li class="width50"
                ><span>关联运行参数：</span
                >{{ tableFieldShow('关联运行参数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>预警时间：</span>{{ dateFormatter(null, null, tableFieldShow('预警时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>处理状态：</span>{{ tableFieldShow('处理状态', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入预警ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入预警ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设备编码："
                prop="field.设备编码"
              ...
              >
                <el-input
                  placeholder="请输入设备坐标X"
                  v-model="formData.field.设备坐标X"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input
                  placeholder="请输入设备坐标Y"
                  v-model="formData.field.设备坐标Y"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.预警类型"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入预警类型', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入预警类型"
                  v-model="formData.field.预警类型"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.关联运行参数"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入关联运行参数', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入关联运行参数"
                  v-model="formData.field.关联运行参数"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.预警时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入预警时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.预警时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择预警时间"
                />
              ...
              <el-form-item
                ...
                prop="field.处理状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入处理状态', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入处理状态"
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '693de6e0-79a3-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '预防性维护预警-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.设备坐标X = tableFieldShow('设备坐标X', row.field) //设备坐标X
  formData.value.field.设备坐标Y = tableFieldShow('设备坐标Y', row.field) //设备坐标Y
  formData.value.field.预警类型 = tableFieldShow('预警类型', row.field) //预警类型
  formData.value.field.关联运行参数 = tableFieldShow('关联运行参数', row.field) //关联运行参数
  formData.value.field.预警时间 = tableFieldShow('预警时间', row.field) //预警时间
  formData.value.field.处理状态 = tableFieldShow('处理状态', row.field) //处理状态

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑预防性维护预警'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增预防性维护预警'
}
...
const tableColumns = ref([
  ...
  { label: '设备坐标X', visible: true },
  { label: '设备坐标Y', visible: true },
  { label: '预警类型', visible: true },
  { label: '关联运行参数', visible: true },
  { label: '预警时间', visible: true },
