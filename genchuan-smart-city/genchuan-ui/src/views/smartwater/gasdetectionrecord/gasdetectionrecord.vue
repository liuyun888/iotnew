    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:gasdetectionrecord:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="检测记录ID" prop="name" label-width="100px">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检测时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('检测时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="硫化氢浓度"
          ...
          >
            <template #default="scope">
              {{ tableFieldShow('硫化氢浓度', scope.row.field) }} ppm
            </template>
          </el-table-column>
          <el-table-column
            label="氧气浓度"
          ...
          >
            <template #default="scope">
              {{ tableFieldShow('氧气浓度', scope.row.field) }} %
            </template>
          </el-table-column>
          <el-table-column
            label="甲烷浓度"
          ...
          >
            <template #default="scope">
              {{ tableFieldShow('甲烷浓度', scope.row.field) }} %LEL
            </template>
          </el-table-column>
          <el-table-column
            label="一氧化碳浓度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('一氧化碳浓度')"
          >
            <template #default="scope">
              {{ tableFieldShow('一氧化碳浓度', scope.row.field) }} ppm
            </template>
          </el-table-column>
          <el-table-column
            label="检测人员"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检测人员')"
          >
            <template #default="scope">
              {{ tableFieldShow('检测人员', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="是否合格"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('是否合格')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('是否合格', scope.row.field),
                  optionsLisi.是否合格Options
                )
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:gasdetectionrecord:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:gasdetectionrecord:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:gasdetectionrecord:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>检测记录ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设施编码：</span>{{ tableFieldShow('设施编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>检测时间：</span>{{ dateFormatter(null, null, tableFieldShow('检测时间', queryData.field)) }}</li
              >
              <li class="width50"
              ...
              <li class="width50"
                ><span>一氧化碳浓度：</span
                >{{ tableFieldShow('一氧化碳浓度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>检测人员：</span>{{ tableFieldShow('检测人员', queryData.field) }}</li
              >
              <li class="width50"
                ><span>是否合格：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('是否合格', queryData.field),
                    optionsLisi.是否合格Options
                  )
                }}</li
              >
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入检测记录ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入检测记录ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施编码："
                prop="field.设施编码"
              ...
              <el-form-item
                ...
                prop="field.检测时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检测时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.检测时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择检测时间"
                />
              ...
              <el-form-item
                ...
                prop="field.硫化氢浓度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入硫化氢浓度', trigger: 'blur' }]"
              >
                <el-input
              ...
              <el-form-item
                ...
                prop="field.一氧化碳浓度"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入一氧化碳浓度', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入一氧化碳浓度"
                  v-model="formData.field.一氧化碳浓度"
                  clearable
                >
                  <template #append>ppm</template>
                </el-input>
              ...
              <el-form-item
                ...
                prop="field.检测人员"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检测人员', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入检测人员"
                  v-model="formData.field.检测人员"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-select
                  placeholder="请选择是否合格"
                  v-model="formData.field.是否合格"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.是否合格Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '570db940-79a4-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '气体检测记录-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.检测时间 = tableFieldShow('检测时间', row.field) //检测时间
  ...
  formData.value.field.一氧化碳浓度 = tableFieldShow('一氧化碳浓度', row.field) //一氧化碳浓度
  formData.value.field.检测人员 = tableFieldShow('检测人员', row.field) //检测人员
  formData.value.field.是否合格 = tableFieldShow('是否合格', row.field) //是否合格

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑气体检测记录'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增气体检测记录'
}
