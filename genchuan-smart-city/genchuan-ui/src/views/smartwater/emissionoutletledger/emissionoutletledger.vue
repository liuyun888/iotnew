    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:emissionoutletledger:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="排放口标识码" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入排放口标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('收纳水体编码')"
          >
            <template #default="scope">
              {{ tableFieldShow('收纳水体编码', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="类别"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('类别')"
          >
            <template #default="scope">
              {{ tableFieldShow('类别', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="出流形式"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('出流形式')"
          >
            <template #default="scope">
              {{ tableFieldShow('出流形式', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="底部高程"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('坐标系')"
          >
            <template #default="scope">
              {{ tableFieldShow('坐标系', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="管理维护单位"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('管理维护单位')"
          >
            <template #default="scope">
              {{ tableFieldShow('管理维护单位', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:emissionoutletledger:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:emissionoutletledger:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:emissionoutletledger:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>排放口标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>收纳水体编码：</span
                >{{ tableFieldShow('收纳水体编码', queryData.field) }}</li
              >
              <li class="width50"
                ><span>类别：</span>{{ tableFieldShow('类别', queryData.field) }}</li
              >
              <li class="width50"
                ><span>出流形式：</span>{{ tableFieldShow('出流形式', queryData.field) }}</li
              >
              <li class="width50"
              ...
              >
              <li class="width50"
                ><span>坐标系：</span>{{ tableFieldShow('坐标系', queryData.field) }}</li
              >
              <li class="width50"
                ><span>管理维护单位：</span
                >{{ tableFieldShow('管理维护单位', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入排放口标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入排放口标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="收纳水体编码："
                prop="field.收纳水体编码"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入收纳水体编码', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入收纳水体编码"
                  v-model="formData.field.收纳水体编码"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input placeholder="请输入类别" v-model="formData.field.类别" clearable />
              </el-form-item>
              <el-form-item
                label="出流形式："
                prop="field.出流形式"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入出流形式', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入出流形式"
                  v-model="formData.field.出流形式"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.坐标系"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入坐标系', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入坐标系" v-model="formData.field.坐标系" clearable />
              </el-form-item>
              <el-form-item
                label="管理维护单位："
                prop="field.管理维护单位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入管理维护单位', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入管理维护单位"
                  v-model="formData.field.管理维护单位"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '4a074520-7807-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '排放口台账管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.收纳水体编码 = tableFieldShow('收纳水体编码', row.field) //收纳水体编码
  formData.value.field.类别 = tableFieldShow('类别', row.field) //类别
  formData.value.field.出流形式 = tableFieldShow('出流形式', row.field) //出流形式
  ...
  formData.value.field.坐标系 = tableFieldShow('坐标系', row.field) //坐标系
  formData.value.field.管理维护单位 = tableFieldShow('管理维护单位', row.field) //管理维护单位

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑排放口台账管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增排放口台账管理'
}
...
const tableColumns = ref([
  { label: '排放口标识码', visible: true },
  { label: '收纳水体编码', visible: true },
  { label: '类别', visible: true },
  { label: '出流形式', visible: true },
  { label: '底部高程', visible: true },
  { label: '经度', visible: true },
  { label: '纬度', visible: true },
  { label: '坐标系', visible: true },
  { label: '管理维护单位', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
