    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:emergencyrescuelinkage:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="时间ID" prop="name">
            <el-input
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('触发时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="应急小组响应时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('应急小组响应时间')"
          >
            <template #default="scope">
              {{ tableFieldShow('应急小组响应时间', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="处理结果"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('伤亡情况')"
          >
            <template #default="scope">
              {{ tableFieldShow('伤亡情况', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:emergencyrescuelinkage:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:emergencyrescuelinkage:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:emergencyrescuelinkage:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>时间ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>作业地点：</span>{{ tableFieldShow('作业地点', queryData.field) }}</li
              >
              <li class="width50"
                ><span>触发时间：</span>{{ dateFormatter(null, null, tableFieldShow('触发时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>应急小组响应时间：</span
                >{{ tableFieldShow('应急小组响应时间', queryData.field) }}</li
              >
              <li class="width50"
                ><span>处理结果：</span>{{ tableFieldShow('处理结果', queryData.field) }}</li
              >
              <li class="width50"
                ><span>伤亡情况：</span>{{ tableFieldShow('伤亡情况', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入时间ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入时间ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="作业地点："
                prop="field.作业地点"
              ...
              <el-form-item
                ...
                prop="field.触发时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入触发时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.触发时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择触发时间"
                />
              ...
              <el-form-item
                ...
                prop="field.应急小组响应时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入应急小组响应时间', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入应急小组响应时间"
                  v-model="formData.field.应急小组响应时间"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.伤亡情况"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入伤亡情况', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入伤亡情况"
                  v-model="formData.field.伤亡情况"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '81ed10c0-79a4-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '应急救援联动-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.触发时间 = tableFieldShow('触发时间', row.field) //触发时间
  formData.value.field.应急小组响应时间 = tableFieldShow('应急小组响应时间', row.field) //应急小组响应时间
  formData.value.field.处理结果 = tableFieldShow('处理结果', row.field) //处理结果
  formData.value.field.伤亡情况 = tableFieldShow('伤亡情况', row.field) //伤亡情况

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑应急救援联动'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增应急救援联动'
}
...
const tableColumns = ref([
  { label: '时间ID', visible: true },
  { label: '作业地点', visible: true },
  { label: '触发时间', visible: true },
  { label: '应急小组响应时间', visible: true },
  { label: '处理结果', visible: true },
  { label: '伤亡情况', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
