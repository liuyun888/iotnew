    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:layerformatcompatibility:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('要素数量')"
          >
            <template #default="scope">
              {{ tableFieldShow('要素数量', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="坐标系统"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('坐标系统')"
          >
            <template #default="scope">
              {{ tableFieldShow('坐标系统', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="导出时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('导出时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('导出时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="操作人"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('操作人')"
          >
            <template #default="scope">
              {{ tableFieldShow('操作人', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:layerformatcompatibility:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:layerformatcompatibility:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:layerformatcompatibility:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>图层名称：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>要素数量：</span>{{ tableFieldShow('要素数量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>坐标系统：</span>{{ tableFieldShow('坐标系统', queryData.field) }}</li
              >
              <li class="width50"
                ><span>导出时间：</span>{{ dateFormatter(null, null, tableFieldShow('导出时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>操作人：</span>{{ tableFieldShow('操作人', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入图层名称', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入图层名称" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="要素数量："
                prop="field.要素数量"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入要素数量', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入要素数量"
                  v-model="formData.field.要素数量"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.坐标系统"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入坐标系统', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入坐标系统"
                  v-model="formData.field.坐标系统"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.导出时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入导出时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.导出时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择导出时间"
                />
              ...
              >
                <el-input placeholder="请输入操作人" v-model="formData.field.操作人" clearable />
              </el-form-item>
            </el-form>
          </div>
        </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '01fe5680-7977-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '图层格式兼容与导入导出', //资产配置，都是写死固定的资产配置
  field: {
    要素数量: '', //要素数量
    坐标系统: '', //坐标系统
    导出时间: '', //导出时间
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '图层格式兼容与导入导出-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.要素数量 = tableFieldShow('要素数量', row.field) //要素数量
  formData.value.field.坐标系统 = tableFieldShow('坐标系统', row.field) //坐标系统
  formData.value.field.导出时间 = tableFieldShow('导出时间', row.field) //导出时间
  formData.value.field.操作人 = tableFieldShow('操作人', row.field) //操作人

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑图层格式兼容与导入导出'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增图层格式兼容与导入导出'
}
...
const titleConfig = ref({
  name: '图层格式兼容与导入导出', //标题名称
  addButName: '新增图层格式兼容与导入导出' //新增按钮名称
})
// 搜索 字段
...
const tableColumns = ref([
  { label: '图层名称', visible: true },
  { label: '要素数量', visible: true },
  { label: '坐标系统', visible: true },
  { label: '导出时间', visible: true },
  { label: '操作人', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
