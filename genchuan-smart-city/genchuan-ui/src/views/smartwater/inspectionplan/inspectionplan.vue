    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:inspectionplan:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('计划名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('计划名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="巡检范围"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('巡检范围')"
          >
            <template #default="scope">
              {{ tableFieldShow('巡检范围', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="计划频次"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('计划频次')"
          >
            <template #default="scope">
              {{ tableFieldShow('计划频次', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="责任人"
          ...
          >
            <template #default="scope">
              {{ tableFieldShow('责任人', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="审核状态"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('审核状态')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('审核状态', scope.row.field),
                  optionsLisi.审核状态Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('审核意见')"
          >
            <template #default="scope">
              {{ tableFieldShow('审核意见', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="审核人"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('审核人')"
          >
            <template #default="scope">
              {{ tableFieldShow('审核人', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="计划开始时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('计划开始时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('计划开始时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="计划结束时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('计划结束时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('计划结束时间', scope.row.field)) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:inspectionplan:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:inspectionplan:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:inspectionplan:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>计划编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>计划名称：</span>{{ tableFieldShow('计划名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>巡检范围：</span>{{ tableFieldShow('巡检范围', queryData.field) }}</li
              >
              <li class="width50"
                ><span>计划频次：</span>{{ tableFieldShow('计划频次', queryData.field) }}</li
              >
              <li class="width50"
                ><span>责任人：</span>{{ tableFieldShow('责任人', queryData.field) }}</li
              >
              <li class="width50"
                ><span>审核状态：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('审核状态', queryData.field),
                    optionsLisi.审核状态Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>审核意见：</span>{{ tableFieldShow('审核意见', queryData.field) }}</li
              >
              <li class="width50"
                ><span>审核人：</span>{{ tableFieldShow('审核人', queryData.field) }}</li
              >
              <li class="width50"
                ><span>计划开始时间：</span
                >{{ dateFormatter(null, null, tableFieldShow('计划开始时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>计划结束时间：</span
                >{{ dateFormatter(null, null, tableFieldShow('计划结束时间', queryData.field)) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入计划编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入计划编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="计划名称："
                prop="field.计划名称"
              ...
              <el-form-item
                ...
                prop="field.巡检范围"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入巡检范围', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入巡检范围"
                  v-model="formData.field.巡检范围"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.计划频次"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入计划频次', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入计划频次"
                  v-model="formData.field.计划频次"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input placeholder="请输入责任人" v-model="formData.field.责任人" clearable />
              </el-form-item>
              <el-form-item
                label="审核状态："
                prop="field.审核状态"
              ...
              >
                <el-select
                  placeholder="请选择审核状态"
                  v-model="formData.field.审核状态"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.审核状态Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                :rules="[]"
              >
                <el-input placeholder="请输入审核人" v-model="formData.field.审核人" clearable />
              </el-form-item>
              <el-form-item
                label="计划开始时间："
                prop="field.计划开始时间"
              ...
              <el-form-item
                ...
                prop="field.计划结束时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入计划结束时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.计划结束时间"
                  type="datetime"
                  value-format="x"
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '9e9a2cf0-7989-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '巡检计划管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.计划名称 = tableFieldShow('计划名称', row.field) //计划名称
  formData.value.field.巡检范围 = tableFieldShow('巡检范围', row.field) //巡检范围
  formData.value.field.计划频次 = tableFieldShow('计划频次', row.field) //计划频次
  formData.value.field.责任人 = tableFieldShow('责任人', row.field) //责任人
  formData.value.field.审核状态 = tableFieldShow('审核状态', row.field) //审核状态
  formData.value.field.审核意见 = tableFieldShow('审核意见', row.field) //审核意见
  formData.value.field.审核人 = tableFieldShow('审核人', row.field) //审核人
  formData.value.field.计划开始时间 = tableFieldShow('计划开始时间', row.field) //计划开始时间
  formData.value.field.计划结束时间 = tableFieldShow('计划结束时间', row.field) //计划结束时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑巡检计划管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增巡检计划管理'
}
...
const tableColumns = ref([
  { label: '计划编号', visible: true },
  { label: '计划名称', visible: true },
  { label: '巡检范围', visible: true },
  { label: '计划频次', visible: true },
  { label: '责任人', visible: true },
  { label: '审核状态', visible: true },
  { label: '审核意见', visible: true },
  { label: '审核人', visible: true },
  { label: '计划开始时间', visible: true },
  { label: '计划结束时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
