    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:basicdataitemintegritycheck:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="完整性检查编号" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入完整性检查编号"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('缺失字段列表')"
          >
            <template #default="scope">
              {{ tableFieldShow('缺失字段列表', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="检查时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('检查时间')"
          >
            <template #default="scope">
              {{ dateFormatter(null, null, tableFieldShow('检查时间', scope.row.field)) }}
            </template>
          </el-table-column>
          <el-table-column
            label="校验人员"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('责任单位')"
          >
            <template #default="scope">
              {{ tableFieldShow('责任单位', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:basicdataitemintegritycheck:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:basicdataitemintegritycheck:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:basicdataitemintegritycheck:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>完整性检查编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>设施类型：</span>{{ tableFieldShow('设施类型', queryData.field) }}</li
              >
              <li class="width50"
                ><span>缺失字段列表：</span
                >{{ tableFieldShow('缺失字段列表', queryData.field) }}</li
              >
              <li class="width50"
                ><span>检查时间：</span
                >{{ dateFormatter(null, null, tableFieldShow('检查时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>校验人员：</span>{{ tableFieldShow('校验人员', queryData.field) }}</li
              >
              <li class="width50"
                ><span>责任单位：</span>{{ tableFieldShow('责任单位', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入完整性检查编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入完整性检查编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="设施类型："
                prop="field.设施类型"
              ...
              <el-form-item
                ...
                prop="field.缺失字段列表"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入缺失字段列表', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入缺失字段列表"
                  v-model="formData.field.缺失字段列表"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.检查时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入检查时间', trigger: 'blur' }]"
              >
                <el-date-picker
                  v-model="formData.field.检查时间"
                  type="datetime"
                  value-format="x"
                  placeholder="选择检查时间"
                />
              ...
              <el-form-item
                ...
                prop="field.责任单位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入责任单位', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入责任单位"
                  v-model="formData.field.责任单位"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '91b4e640-78b9-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '基础数据项完整性检查', //资产配置，都是写死固定的资产配置
  field: {
    设施类型: '', //设施类型
    缺失字段列表: '', //缺失字段列表
    检查时间: '', //检查时间
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '基础数据项完整性检查-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.缺失字段列表 = tableFieldShow('缺失字段列表', row.field) //缺失字段列表
  formData.value.field.检查时间 = tableFieldShow('检查时间', row.field) //检查时间
  formData.value.field.校验人员 = tableFieldShow('校验人员', row.field) //校验人员
  formData.value.field.责任单位 = tableFieldShow('责任单位', row.field) //责任单位

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑基础数据项完整性检查'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增基础数据项完整性检查'
}
...
const titleConfig = ref({
  name: '基础数据项完整性检查', //标题名称
  addButName: '新增基础数据项完整性检查' //新增按钮名称
})
// 搜索 字段
...
const tableColumns = ref([
  { label: '完整性检查编号', visible: true },
  { label: '设施类型', visible: true },
  { label: '缺失字段列表', visible: true },
  { label: '检查时间', visible: true },
  { label: '校验人员', visible: true },
  { label: '责任单位', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
