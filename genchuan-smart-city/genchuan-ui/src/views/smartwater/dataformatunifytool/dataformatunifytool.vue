    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:dataformatunifytool:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="格式转换编号" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入格式转换编号"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('源文件格式')"
          >
            <template #default="scope">
              {{ tableFieldShow('源文件格式', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="目标格式"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('目标格式')"
          >
            <template #default="scope">
              {{ tableFieldShow('目标格式', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="字段映射关系"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('字段映射关系')"
          >
            <template #default="scope">
              {{ tableFieldShow('字段映射关系', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="转换状态"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('转换状态')"
          >
            <template #default="scope">
              {{ tableFieldShow('转换状态', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:dataformatunifytool:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:dataformatunifytool:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:dataformatunifytool:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>格式转换编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>源文件格式：</span>{{ tableFieldShow('源文件格式', queryData.field) }}</li
              >
              <li class="width50"
                ><span>目标格式：</span>{{ tableFieldShow('目标格式', queryData.field) }}</li
              >
              <li class="width50"
                ><span>字段映射关系：</span
                >{{ tableFieldShow('字段映射关系', queryData.field) }}</li
              >
              <li class="width50"
                ><span>转换状态：</span>{{ tableFieldShow('转换状态', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入格式转换编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入格式转换编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="源文件格式："
                prop="field.源文件格式"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入源文件格式', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入源文件格式"
                  v-model="formData.field.源文件格式"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.目标格式"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入目标格式', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入目标格式"
                  v-model="formData.field.目标格式"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.字段映射关系"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入字段映射关系', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入字段映射关系"
                  v-model="formData.field.字段映射关系"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.转换状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入转换状态', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入转换状态"
                  v-model="formData.field.转换状态"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '31bda050-78b5-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '数据格式统一工具', //资产配置，都是写死固定的资产配置
  field: {
    源文件格式: '', //源文件格式
    目标格式: '', //目标格式
    字段映射关系: '', //字段映射关系
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '数据格式统一工具-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.源文件格式 = tableFieldShow('源文件格式', row.field) //源文件格式
  formData.value.field.目标格式 = tableFieldShow('目标格式', row.field) //目标格式
  formData.value.field.字段映射关系 = tableFieldShow('字段映射关系', row.field) //字段映射关系
  formData.value.field.转换状态 = tableFieldShow('转换状态', row.field) //转换状态

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑数据格式统一工具'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增数据格式统一工具'
}
...
const tableColumns = ref([
  { label: '格式转换编号', visible: true },
  { label: '源文件格式', visible: true },
  { label: '目标格式', visible: true },
  { label: '字段映射关系', visible: true },
  { label: '转换状态', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
