    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:operationapproval:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="审批单ID" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入审批单ID"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('作业地点')"
          >
            <template #default="scope">
              {{ tableFieldShow('作业地点', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="作业内容"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('作业内容')"
          >
            <template #default="scope">
              {{ tableFieldShow('作业内容', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="安全措施描述"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('安全措施描述')"
          >
            <template #default="scope">
              {{ tableFieldShow('安全措施描述', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="审核状态"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('许可编号')"
          >
            <template #default="scope">
              {{ tableFieldShow('许可编号', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:operationapproval:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:operationapproval:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:operationapproval:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>审批单ID：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>作业地点：</span>{{ tableFieldShow('作业地点', queryData.field) }}</li
              >
              <li class="width50"
                ><span>作业内容：</span>{{ tableFieldShow('作业内容', queryData.field) }}</li
              >
              <li class="width50"
                ><span>安全措施描述：</span
                >{{ tableFieldShow('安全措施描述', queryData.field) }}</li
              >
              <li class="width50"
              ...
              >
              <li class="width50"
                ><span>许可编号：</span>{{ tableFieldShow('许可编号', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入审批单ID', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入审批单ID" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="作业地点："
                prop="field.作业地点"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入作业地点', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入作业地点"
                  v-model="formData.field.作业地点"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.安全措施描述"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入安全措施描述', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入安全措施描述"
                  v-model="formData.field.安全措施描述"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              <el-form-item
                ...
                prop="field.许可编号"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入许可编号', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入许可编号"
                  v-model="formData.field.许可编号"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'e5d11380-79a3-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '作业审批管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.作业地点 = tableFieldShow('作业地点', row.field) //作业地点
  formData.value.field.作业内容 = tableFieldShow('作业内容', row.field) //作业内容
  formData.value.field.安全措施描述 = tableFieldShow('安全措施描述', row.field) //安全措施描述
  ...
  formData.value.field.许可编号 = tableFieldShow('许可编号', row.field) //许可编号

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑作业审批管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增作业审批管理'
}
...
const tableColumns = ref([
  { label: '审批单ID', visible: true },
  { label: '作业地点', visible: true },
  { label: '作业内容', visible: true },
  { label: '安全措施描述', visible: true },
  { label: '审核状态', visible: true },
  { label: '审核意见', visible: true },
  { label: '审核人', visible: true },
  { label: '许可编号', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
