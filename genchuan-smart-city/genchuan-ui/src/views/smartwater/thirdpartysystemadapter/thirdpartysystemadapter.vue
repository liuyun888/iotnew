    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:thirdpartysystemadapter:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="接口编号编号" prop="name" label-width="120px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入接口编号编号"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('第三方系统名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('第三方系统名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="数据类型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('数据类型')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('数据类型', scope.row.field),
                  optionsLisi.数据类型Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('接口协议')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('接口协议', scope.row.field),
                  optionsLisi.接口协议Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('更新频率')"
          >
            <template #default="scope">
              {{ tableFieldShow('更新频率', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:thirdpartysystemadapter:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:thirdpartysystemadapter:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:thirdpartysystemadapter:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>接口编号编号：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>第三方系统名称：</span
                >{{ tableFieldShow('第三方系统名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>数据类型：</span
                >{{
                  handleTypeOptions(
                    tableFieldShow('数据类型', queryData.field),
                    optionsLisi.数据类型Options
                  )
                }}</li
              >
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('接口协议', queryData.field),
                    optionsLisi.接口协议Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>更新频率：</span>{{ tableFieldShow('更新频率', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入接口编号编号', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入接口编号编号" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="第三方系统名称："
                prop="field.第三方系统名称"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入第三方系统名称', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入第三方系统名称"
                  v-model="formData.field.第三方系统名称"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-select
                  placeholder="请选择数据类型"
                  v-model="formData.field.数据类型"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.数据类型Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.接口协议"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择接口协议', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择接口协议"
                  v-model="formData.field.接口协议"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.接口协议Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.更新频率"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入更新频率', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入更新频率"
                  v-model="formData.field.更新频率"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '8a0d3790-78ae-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '第三方系统接口适配', //资产配置，都是写死固定的资产配置
  field: {
    第三方系统名称: '', //第三方系统名称
    数据类型: '', //数据类型
    接口协议: '', //接口协议
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '第三方系统接口适配-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.第三方系统名称 = tableFieldShow('第三方系统名称', row.field) //第三方系统名称
  formData.value.field.数据类型 = tableFieldShow('数据类型', row.field) //数据类型
  formData.value.field.接口协议 = tableFieldShow('接口协议', row.field) //接口协议
  formData.value.field.更新频率 = tableFieldShow('更新频率', row.field) //更新频率

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑第三方系统接口适配'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增第三方系统接口适配'
}
...
const tableColumns = ref([
  { label: '接口编号编号', visible: true },
  { label: '第三方系统名称', visible: true },
  { label: '数据类型', visible: true },
  { label: '接口协议', visible: true },
  { label: '更新频率', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
