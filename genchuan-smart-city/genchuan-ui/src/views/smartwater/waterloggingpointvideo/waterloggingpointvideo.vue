    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smartwater:waterloggingpointvideo:create']"
          >
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          ...
          <el-form-item label="监控点标识码" prop="name" label-width="100px">
            <el-input
              v-model="searchForm.name"
              placeholder="请输入监控点标识码"
              clearable
              class="!w-240px"
            />
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('视频流')"
          >
            <template #default="scope">
              {{ tableFieldShow('视频流', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="积水识别结果"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('积水识别结果')"
          >
            <template #default="scope">
              {{ tableFieldShow('积水识别结果', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smartwater:waterloggingpointvideo:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smartwater:waterloggingpointvideo:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smartwater:waterloggingpointvideo:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              <li class="width50"><span>监控点标识码：</span>{{ queryData.name }}</li>
              <li class="width50"
                ><span>监测时间：</span>{{ dateFormatter(null, null, tableFieldShow('监测时间', queryData.field)) }}</li
              >
              <li class="width50"
                ><span>视频流：</span>{{ tableFieldShow('视频流', queryData.field) }}</li
              >
              <li class="width50"
                ><span>积水识别结果：</span
                >{{ tableFieldShow('积水识别结果', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              <el-form-item
                ...
                :rules="[{ required: true, message: '请输入监控点标识码', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入监控点标识码" v-model="formData.name" clearable />
              </el-form-item>
              <el-form-item
                label="监测时间："
                prop="field.监测时间"
              ...
              <el-form-item
                ...
                prop="field.视频流"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入视频流', trigger: 'blur' }]"
              >
                <el-input placeholder="请输入视频流" v-model="formData.field.视频流" clearable />
              </el-form-item>
              <el-form-item
                label="积水识别结果："
                prop="field.积水识别结果"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入积水识别结果', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入积水识别结果"
                  v-model="formData.field.积水识别结果"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '36e1bd20-78db-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '内涝点视频监控-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.视频流 = tableFieldShow('视频流', row.field) //视频流
  formData.value.field.积水识别结果 = tableFieldShow('积水识别结果', row.field) //积水识别结果

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑内涝点视频监控'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增内涝点视频监控'
}
...
const tableColumns = ref([
  { label: '监控点标识码', visible: true },
  { label: '监测时间', visible: true },
  { label: '视频流', visible: true },
  { label: '积水识别结果', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
