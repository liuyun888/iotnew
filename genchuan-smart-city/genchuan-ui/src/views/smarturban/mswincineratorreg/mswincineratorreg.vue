    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smarturban:mswincineratorreg:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          >
            <template #default="scope">
              {{ tableFieldShow('占地面积', scope.row.field) }} ㎡
            </template>
          </el-table-column>
          <el-table-column
            label="建筑面积"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('单台炉处理能力')"
          >
            <template #default="scope">
              {{ tableFieldShow('单台炉处理能力', scope.row.field) }} t/d
            </template>
          </el-table-column>
          <el-table-column
            label="焚烧炉台数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('焚烧炉台数')"
          >
            <template #default="scope">
              {{ tableFieldShow('焚烧炉台数', scope.row.field) }} 台
            </template>
          </el-table-column>
          <el-table-column
            label="焚烧线年累计运行时长"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('焚烧线年累计运行时长')"
          >
            <template #default="scope">
              {{ tableFieldShow('焚烧线年累计运行时长', scope.row.field) }} h
            </template>
          </el-table-column>
          <el-table-column
            label="主管单位"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('烟气排放指标')"
          >
            <template #default="scope">
              {{ tableFieldShow('烟气排放指标', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="渗沥液处理能力"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('渗沥液处理能力')"
          >
            <template #default="scope">
              {{ tableFieldShow('渗沥液处理能力', scope.row.field) }} t/d
            </template>
          </el-table-column>
          <el-table-column
            label="飞灰处理方式"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('飞灰处理方式')"
          >
            <template #default="scope">
              {{ tableFieldShow('飞灰处理方式', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smarturban:mswincineratorreg:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smarturban:mswincineratorreg:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smarturban:mswincineratorreg:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              <li class="width50"
                ><span>设计处理能力：</span
                >{{ tableFieldShow('设计处理能力', queryData.field) }}</li
              >
              <li class="width50"
                ><span>实际处理负荷：</span
                >{{ tableFieldShow('实际处理负荷', queryData.field) }}</li
              >
              <li class="width50"
                ><span>单台炉处理能力：</span
                >{{ tableFieldShow('单台炉处理能力', queryData.field) }}</li
              >
              <li class="width50"
                ><span>焚烧炉台数：</span>{{ tableFieldShow('焚烧炉台数', queryData.field) }}</li
              >
              <li class="width50"
                ><span style="width: 30%">焚烧线年累计运行时长：</span
                >{{ tableFieldShow('焚烧线年累计运行时长', queryData.field) }}</li
              >
              <li class="width50"
                ><span>主管单位：</span>{{ tableFieldShow('主管单位', queryData.field) }}</li
              >
              <li class="width50"
                ><span>运营单位：</span>{{ tableFieldShow('运营单位', queryData.field) }}</li
              >
              <li class="width50"
                ><span>烟气排放指标：</span
                >{{ tableFieldShow('烟气排放指标', queryData.field) }}</li
              >
              <li class="width50"
                ><span>渗沥液处理能力：</span
                >{{ tableFieldShow('渗沥液处理能力', queryData.field) }}</li
              >
              <li class="width50"
                ><span>飞灰处理方式：</span
                >{{ tableFieldShow('飞灰处理方式', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.设计处理能力"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入设计处理能力', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入设计处理能力"
                  v-model="formData.field.设计处理能力"
                  clearable
                >
              ...
              >
                <el-input
                  placeholder="请输入实际处理负荷"
                  v-model="formData.field.实际处理负荷"
                  clearable
                >
                  <template #append>t/d</template>
                </el-input>
              ...
              <el-form-item
                ...
                prop="field.单台炉处理能力"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入单台炉处理能力', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入单台炉处理能力"
                  v-model="formData.field.单台炉处理能力"
                  clearable
                >
                  <template #append>t/d</template>
                </el-input>
              ...
              <el-form-item
                ...
                prop="field.焚烧炉台数"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入焚烧炉台数', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入焚烧炉台数"
                  v-model="formData.field.焚烧炉台数"
                  clearable
                >
              ...
              <el-form-item
                label="焚烧线年累计运行时长："
                prop="field.焚烧线年累计运行时长"
                class="edit-content-form-li width50"
                :rules="[]"
                label-width="180px"
              >
                <el-input
                  placeholder="请输入焚烧线年累计运行时长"
                  v-model="formData.field.焚烧线年累计运行时长"
                  clearable
                >
              ...
              <el-form-item
                ...
                prop="field.主管单位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入主管单位', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入主管单位"
                  v-model="formData.field.主管单位"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.运营单位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入运营单位', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入运营单位"
                  v-model="formData.field.运营单位"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input
                  placeholder="请输入烟气排放指标"
                  v-model="formData.field.烟气排放指标"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.渗沥液处理能力"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入渗沥液处理能力', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入渗沥液处理能力"
                  v-model="formData.field.渗沥液处理能力"
                  clearable
                >
                  <template #append>t/d</template>
                </el-input>
              ...
              >
                <el-input
                  placeholder="请输入飞灰处理方式"
                  v-model="formData.field.飞灰处理方式"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '47b8eed0-7687-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '生活垃圾焚烧厂监管', //资产配置，都是写死固定的资产配置
  field: {
    名称: '', //名称
    地址: '', //地址
    坐标X: '', //坐标X
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '生活垃圾焚烧厂监管-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.单台炉处理能力 = tableFieldShow('单台炉处理能力', row.field) //单台炉处理能力
  formData.value.field.焚烧炉台数 = tableFieldShow('焚烧炉台数', row.field) //焚烧炉台数
  formData.value.field.焚烧线年累计运行时长 = tableFieldShow('焚烧线年累计运行时长', row.field) //焚烧线年累计运行时长
  formData.value.field.主管单位 = tableFieldShow('主管单位', row.field) //主管单位
  formData.value.field.运营单位 = tableFieldShow('运营单位', row.field) //运营单位
  formData.value.field.烟气排放指标 = tableFieldShow('烟气排放指标', row.field) //烟气排放指标
  formData.value.field.渗沥液处理能力 = tableFieldShow('渗沥液处理能力', row.field) //渗沥液处理能力
  formData.value.field.飞灰处理方式 = tableFieldShow('飞灰处理方式', row.field) //飞灰处理方式

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑生活垃圾焚烧厂监管'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增生活垃圾焚烧厂监管'
}
...
const tableColumns = ref([
  ...
  { label: '单台炉处理能力', visible: true },
  { label: '焚烧炉台数', visible: true },
  { label: '焚烧线年累计运行时长', visible: true },
  { label: '主管单位', visible: true },
  { label: '运营单位', visible: true },
  { label: '烟气排放指标', visible: true },
  { label: '渗沥液处理能力', visible: true },
  { label: '飞灰处理方式', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
