      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smarturban:outdooradmgmt:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('载体性质')"
          >
            <template #default="scope">
              {{ tableFieldShow('载体性质', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="所属单位"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('所属单位')"
          >
            <template #default="scope">
              {{ tableFieldShow('所属单位', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="长度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('长度')"
          >
            <template #default="scope">
              {{ tableFieldShow('长度', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="宽度"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('宽度')"
          >
            <template #default="scope">
              {{ tableFieldShow('宽度', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="许可开始时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('许可开始时间')"
          >
            <template #default="scope">
              {{ tableFieldShow('许可开始时间', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="许可结束时间"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('许可结束时间')"
          >
            <template #default="scope">
              {{ tableFieldShow('许可结束时间', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smarturban:outdooradmgmt:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smarturban:outdooradmgmt:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smarturban:outdooradmgmt:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              >
              <li class="width50"
                ><span>载体性质：</span>{{ tableFieldShow('载体性质', queryData.field) }}</li
              >
              <li class="width50"
                ><span>所属单位：</span>{{ tableFieldShow('所属单位', queryData.field) }}</li
              >
              <li class="width50"
                ><span>长度：</span>{{ tableFieldShow('长度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>宽度：</span>{{ tableFieldShow('宽度', queryData.field) }}</li
              >
              <li class="width50"
                ><span>许可开始时间：</span
                >{{ tableFieldShow('许可开始时间', queryData.field) }}</li
              >
              <li class="width50"
                ><span>许可结束时间：</span
                >{{ tableFieldShow('许可结束时间', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              <el-form-item
                ...
                prop="field.载体性质"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入载体性质', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入载体性质"
                  v-model="formData.field.载体性质"
                  clearable
                />
              </el-form-item>
              <el-form-item
                ...
                prop="field.所属单位"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入所属单位', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入所属单位"
                  v-model="formData.field.所属单位"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input placeholder="请输入宽度" v-model="formData.field.宽度" clearable />
              </el-form-item>
              <el-form-item
                label="许可开始时间："
                prop="field.许可开始时间"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入许可开始时间', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入许可开始时间"
                  v-model="formData.field.许可开始时间"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input
                  placeholder="请输入许可结束时间"
                  v-model="formData.field.许可结束时间"
                  clearable
                />
              </el-form-item>
            </el-form>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'ecaed7c0-7717-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '户外广告管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.载体性质 = tableFieldShow('载体性质', row.field) //载体性质
  formData.value.field.所属单位 = tableFieldShow('所属单位', row.field) //所属单位
  formData.value.field.长度 = tableFieldShow('长度', row.field) //长度
  formData.value.field.宽度 = tableFieldShow('宽度', row.field) //宽度
  formData.value.field.许可开始时间 = tableFieldShow('许可开始时间', row.field) //许可开始时间
  formData.value.field.许可结束时间 = tableFieldShow('许可结束时间', row.field) //许可结束时间

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑户外广告管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增户外广告管理'
}
...
const tableColumns = ref([
  ...
  { label: '载体性质', visible: true },
  { label: '所属单位', visible: true },
  { label: '长度', visible: true },
  { label: '宽度', visible: true },
  { label: '许可开始时间', visible: true },
  { label: '许可结束时间', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
