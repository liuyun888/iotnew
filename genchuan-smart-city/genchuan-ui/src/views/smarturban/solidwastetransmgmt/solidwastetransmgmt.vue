    ...
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smarturban:solidwastetransmgmt:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          >
            <template #default="scope">
              {{ tableFieldShow('类型', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="地址"
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('垃圾来源分类')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('垃圾来源分类', scope.row.field),
                  optionsLisi.垃圾来源分类Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('垃圾分类状态')"
          >
            <template #default="scope">
              {{
                handleTypeOptions(
                  tableFieldShow('垃圾分类状态', scope.row.field),
                  optionsLisi.垃圾分类状态Options
                )
              }}
            </template>
          </el-table-column>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('垃圾收集量')"
          >
            <template #default="scope">
              {{ tableFieldShow('垃圾收集量', scope.row.field) }} t/d（日均）
            </template>
          </el-table-column>
          <el-table-column
            label="清运次数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('清运次数')"
          >
            <template #default="scope">
              {{ tableFieldShow('清运次数', scope.row.field) }} 次
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smarturban:solidwastetransmgmt:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smarturban:solidwastetransmgmt:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smarturban:solidwastetransmgmt:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              >
              <li class="width50"
                ><span>类型：</span>{{ tableFieldShow('类型', queryData.field) }}</li
              >
              <li class="width50"
              ...
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('垃圾来源分类', queryData.field),
                    optionsLisi.垃圾来源分类Options
                  )
                }}</li
              >
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('垃圾分类状态', queryData.field),
                    optionsLisi.垃圾分类状态Options
                  )
                }}</li
              >
              <li class="width50"
                ><span>垃圾收集量：</span>{{ tableFieldShow('垃圾收集量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>清运次数：</span>{{ tableFieldShow('清运次数', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              >
                <el-input placeholder="请输入类型" v-model="formData.field.类型" clearable />
              </el-form-item>
              <el-form-item
                label="地址："
                prop="field.地址"
              ...
              <el-form-item
                ...
                prop="field.垃圾来源分类"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择垃圾来源分类', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择垃圾来源分类"
                  v-model="formData.field.垃圾来源分类"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.垃圾来源分类Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.垃圾分类状态"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请选择垃圾分类状态', trigger: 'blur' }]"
              >
                <el-select
                  placeholder="请选择垃圾分类状态"
                  v-model="formData.field.垃圾分类状态"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.垃圾分类状态Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              >
                <el-input
                  placeholder="请输入垃圾收集量"
                  v-model="formData.field.垃圾收集量"
                  clearable
                >
                  <template #append>t/d（日均）</template>
                </el-input>
              </el-form-item>
              <el-form-item
                label="清运次数："
                ...
                :rules="[]"
              >
                <el-input placeholder="请输入清运次数" v-model="formData.field.清运次数" clearable>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '59de43d0-7682-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '垃圾固废收运管理', //资产配置，都是写死固定的资产配置
  field: {
    名称: '', //名称
    类型: '', //类型
    地址: '', //地址
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '垃圾固废收运管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.垃圾来源分类 = tableFieldShow('垃圾来源分类', row.field) //垃圾来源分类
  formData.value.field.垃圾分类状态 = tableFieldShow('垃圾分类状态', row.field) //垃圾分类状态
  formData.value.field.垃圾收集量 = tableFieldShow('垃圾收集量', row.field) //垃圾收集量
  formData.value.field.清运次数 = tableFieldShow('清运次数', row.field) //清运次数

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑垃圾固废收运管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增垃圾固废收运管理'
}
...
const tableColumns = ref([
  ...
  { label: '垃圾来源分类', visible: true },
  { label: '垃圾分类状态', visible: true },
  { label: '垃圾收集量', visible: true },
  { label: '清运次数', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
