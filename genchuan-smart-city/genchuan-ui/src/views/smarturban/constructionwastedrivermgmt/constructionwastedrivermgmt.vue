  ...
  <div class="cc-main">
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <!--右边的按钮-->
        <div class="right">
          <el-button type="primary" plain :icon="Plus" @click="addClick(titleConfig.addButName)" v-hasPermi="['smarturban:constructionwastedrivermgmt:create']"> {{ titleConfig.addButName }}</el-button>
        </div>
      </div>
<!--搜索内容字段-->
      <div class="cc-search-css" ref="searchRef">
        <el-form class="-mb-15px" :model="searchForm" ref="searchFormRef" :inline="true" label-width="68px">
          ...
          <el-form-item label="身份证号" prop="name">
            <el-input v-model="searchForm.name" placeholder="请输入身份证号" clearable class="!w-240px"/>
          </el-form-item>
          <el-form-item>
...
<el-table-column label="驾驶证号" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('驾驶证号')">
            <template #default="scope">
              {{ tableFieldShow('驾驶证号', scope.row.field) }}
            </template>
          </el-table-column>
<el-table-column label="姓名" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('姓名')">
            <template #default="scope">
              {{ tableFieldShow('姓名', scope.row.field) }}
            </template>
          </el-table-column>
<el-table-column label="身份证号" min-width="150" align="center" prop="name" :show-overflow-tooltip="true" v-if="tableColumnShow('身份证号')"/>
<el-table-column label="性别" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('性别')">
            <template #default="scope">
              {{ tableFieldShow('性别', scope.row.field) }}
            </template>
          </el-table-column>
<el-table-column label="年龄" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('年龄')">
            <template #default="scope">
              {{ tableFieldShow('年龄', scope.row.field) }} 岁
            </template>
          </el-table-column>
<el-table-column label="工龄" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('工龄')">
            <template #default="scope">
              {{ tableFieldShow('工龄', scope.row.field) }} 年
            </template>
          </el-table-column>
...
<el-table-column label="联系方式" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('联系方式')">
            <template #default="scope">
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button type="success" :icon="View" :plain="true" @click="queryClick(scope.row)" v-hasPermi="['smarturban:constructionwastedrivermgmt:query']">详细</el-button>
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['smarturban:constructionwastedrivermgmt:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['smarturban:constructionwastedrivermgmt:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
 ...
 <li class="width50"><span>驾驶证号：</span>{{ tableFieldShow('驾驶证号', queryData.field) }}</li>
 <li class="width50"><span>姓名：</span>{{ tableFieldShow('姓名', queryData.field) }}</li>
<li class="width50"><span>身份证号：</span>{{ queryData.name }}</li>
 <li class="width50"><span>性别：</span>{{ tableFieldShow('性别', queryData.field) }}</li>
 <li class="width50"><span>年龄：</span>{{ tableFieldShow('年龄', queryData.field) }}</li>
 <li class="width50"><span>工龄：</span>{{ tableFieldShow('工龄', queryData.field) }}</li>
 <li class="width50"><span>企业编号：</span>{{ tableFieldShow('企业编号', queryData.field) }}</li>
 <li class="width50"><span>联系方式：</span>{{ tableFieldShow('联系方式', queryData.field) }}</li>
...
<el-form-item label="驾驶证号：" prop="field.驾驶证号" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入驾驶证号', trigger: 'blur' }]">
                <el-input placeholder="请输入驾驶证号" v-model="formData.field.驾驶证号" clearable/>
              </el-form-item>
<el-form-item label="姓名：" prop="field.姓名" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入姓名', trigger: 'blur' }]">
                <el-input placeholder="请输入姓名" v-model="formData.field.姓名" clearable/>
              </el-form-item>
 <el-form-item label="身份证号：" prop="name" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入身份证号', trigger: 'blur' }]">
                <el-input placeholder="请输入身份证号" v-model="formData.name" clearable/>
              </el-form-item>
<el-form-item label="性别：" prop="field.性别" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入性别', trigger: 'blur' }]">
                <el-input placeholder="请输入性别" v-model="formData.field.性别" clearable/>
              </el-form-item>
<el-form-item label="年龄：" prop="field.年龄" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入年龄', trigger: 'blur' }]">
                <el-input placeholder="请输入年龄" v-model="formData.field.年龄" clearable>
                  <template #append>岁</template>
                </el-input>
              </el-form-item>
<el-form-item label="工龄：" prop="field.工龄" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入工龄', trigger: 'blur' }]">
                <el-input placeholder="请输入工龄" v-model="formData.field.工龄" clearable>
...
<el-form-item label="联系方式：" prop="field.联系方式" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入联系方式', trigger: 'blur' }]">
                <el-input placeholder="请输入联系方式" v-model="formData.field.联系方式" clearable/>
              </el-form-item>
        ...
        <div class="action-button" v-if="['add', 'edit'].includes(drawerConfig.type) && !drawerConfig.loading">
          ...
          <el-button type="primary" :icon="Check" @click="drawerSubmitForm(drawerFromRef)">保 存</el-button>
            ...
            const tableInitData = async () => {
              ...
              try {
                let params = {
                  ...
                  sortOrder: 'DESC',
                  assetProfileId: 'e5f383e0-7717-11f0-bd39-2dc5ccdb39ef',//资产配置id
                }
            ...
            // 表单字段
              const initFormData = {
                ...
                type: '建筑垃圾运输司机管理',//资产配置，都是写死固定的资产配置
                field: {
                   行政区划代码: '',//行政区划代码
            驾驶证号: '',//驾驶证号
            姓名: '',//姓名
            ...
            const queryClick = async (row) => {
              ...
              drawerConfig.value.title = '建筑垃圾运输司机管理-详细信息';
            }
            // 点击编辑按钮
            ...
            formData.value.field.驾驶证号 = tableFieldShow('驾驶证号', row.field);//驾驶证号
            formData.value.field.姓名 = tableFieldShow('姓名', row.field);//姓名
            formData.value.field.性别 = tableFieldShow('性别', row.field);//性别
            formData.value.field.年龄 = tableFieldShow('年龄', row.field);//年龄
            formData.value.field.工龄 = tableFieldShow('工龄', row.field);//工龄
            ...
            formData.value.field.联系方式 = tableFieldShow('联系方式', row.field);//联系方式
              ...
              drawerConfig.value.title = '编辑建筑垃圾运输司机管理';
            }
            // 点击删除按钮
            const delClick = (row) => {
            ...
            const addClick = async () => {
              ...
              drawerConfig.value.title = '新增建筑垃圾运输司机管理';
            }
            ...
            const titleConfig = ref({
              name: '建筑垃圾运输司机管理',//标题名称
              addButName: '新增建筑垃圾运输司机管理',//新增按钮名称
            })
            // 搜索 字段
          ...
          const tableColumns = ref([
            ...
            {label: '驾驶证号', visible: true},
            {label: '姓名', visible: true},
            {label: '身份证号', visible: true},
            {label: '性别', visible: true},
            {label: '年龄', visible: true},
            {label: '工龄', visible: true},
