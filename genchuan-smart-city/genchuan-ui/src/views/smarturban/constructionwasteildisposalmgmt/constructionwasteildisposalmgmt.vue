  ...
  <div class="cc-main">
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <!--右边的按钮-->
        <div class="right">
          <el-button type="primary" plain :icon="Plus" @click="addClick(titleConfig.addButName)" v-hasPermi="['smarturban:constructionwasteildisposalmgmt:create']"> {{ titleConfig.addButName }}</el-button>
        </div>
      </div>
<!--搜索内容字段-->
...
<el-table-column label="运输司机驾驶证号" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('运输司机驾驶证号')">
            <template #default="scope">
              {{ tableFieldShow('运输司机驾驶证号', scope.row.field) }}
            </template>
          </el-table-column>
<el-table-column label="违章时间" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('违章时间')">
            <template #default="scope">
              {{ tableFieldShow('违章时间', scope.row.field) }}
            </template>
          </el-table-column>
<el-table-column label="处罚信息" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('处罚信息')">
            <template #default="scope">
              {{ tableFieldShow('处罚信息', scope.row.field) }}
            </template>
          </el-table-column>
<el-table-column label="违章地点" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('违章地点')">
            <template #default="scope">
              {{ tableFieldShow('违章地点', scope.row.field) }}
            </template>
          </el-table-column>

          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button type="success" :icon="View" :plain="true" @click="queryClick(scope.row)" v-hasPermi="['smarturban:constructionwasteildisposalmgmt:query']">详细</el-button>
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['smarturban:constructionwasteildisposalmgmt:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['smarturban:constructionwasteildisposalmgmt:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
...
<li class="width50"><span>车牌号：</span>{{ queryData.name }}</li>
 <li class="width50"><span>运输司机驾驶证号：</span>{{ tableFieldShow('运输司机驾驶证号', queryData.field) }}</li>
 <li class="width50"><span>违章时间：</span>{{ tableFieldShow('违章时间', queryData.field) }}</li>
 <li class="width50"><span>处罚信息：</span>{{ tableFieldShow('处罚信息', queryData.field) }}</li>
 <li class="width50"><span>违章地点：</span>{{ tableFieldShow('违章地点', queryData.field) }}</li>
...
<el-form-item label="运输司机驾驶证号：" prop="field.运输司机驾驶证号" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入运输司机驾驶证号', trigger: 'blur' }]">
                <el-input placeholder="请输入运输司机驾驶证号" v-model="formData.field.运输司机驾驶证号" clearable/>
              </el-form-item>
<el-form-item label="违章时间：" prop="field.违章时间" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入违章时间', trigger: 'blur' }]">
                <el-input placeholder="请输入违章时间" v-model="formData.field.违章时间" clearable/>
              </el-form-item>
<el-form-item label="处罚信息：" prop="field.处罚信息" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入处罚信息', trigger: 'blur' }]">
                <el-input placeholder="请输入处罚信息" v-model="formData.field.处罚信息" clearable/>
              </el-form-item>
<el-form-item label="违章地点：" prop="field.违章地点" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入违章地点', trigger: 'blur' }]">
                <el-input placeholder="请输入违章地点" v-model="formData.field.违章地点" clearable/>
              </el-form-item>
        ...
        <div class="action-button" v-if="['add', 'edit'].includes(drawerConfig.type) && !drawerConfig.loading">
          ...
          <el-button type="primary" :icon="Check" @click="drawerSubmitForm(drawerFromRef)">保 存</el-button>
            ...
            const tableInitData = async () => {
              ...
              try {
                let params = {
                  ...
                  sortOrder: 'DESC',
                  assetProfileId: '81a281f0-772d-11f0-bd39-2dc5ccdb39ef',//资产配置id
                }
            ...
            // 表单字段
              const initFormData = {
                ...
                type: '建筑垃圾运输违法处置数据管理',//资产配置，都是写死固定的资产配置
                field: {
                   行政区划代码: '',//行政区划代码
            运输司机驾驶证号: '',//运输司机驾驶证号
            违章时间: '',//违章时间
            ...
            const queryClick = async (row) => {
              ...
              drawerConfig.value.title = '建筑垃圾运输违法处置数据管理-详细信息';
            }
            // 点击编辑按钮
            ...
            formData.value.field.运输司机驾驶证号 = tableFieldShow('运输司机驾驶证号', row.field);//运输司机驾驶证号
            formData.value.field.违章时间 = tableFieldShow('违章时间', row.field);//违章时间
            formData.value.field.处罚信息 = tableFieldShow('处罚信息', row.field);//处罚信息
            formData.value.field.违章地点 = tableFieldShow('违章地点', row.field);//违章地点

              drawerConfig.value.isDrawer = true;
              drawerConfig.value.loading = false;
              drawerConfig.value.type = 'edit';
              drawerConfig.value.title = '编辑建筑垃圾运输违法处置数据管理';
            }
            // 点击删除按钮
            const delClick = (row) => {
            ...
            const addClick = async () => {
              ...
              drawerConfig.value.title = '新增建筑垃圾运输违法处置数据管理';
            }
            ...
            const titleConfig = ref({
              name: '建筑垃圾运输违法处置数据管理',//标题名称
              addButName: '新增建筑垃圾运输违法处置数据管理',//新增按钮名称
            })
            // 搜索 字段
          ...
          const tableColumns = ref([
            ...
            {label: '运输司机驾驶证号', visible: true},
            {label: '违章时间', visible: true},
            {label: '处罚信息', visible: true},
            {label: '违章地点', visible: true},
