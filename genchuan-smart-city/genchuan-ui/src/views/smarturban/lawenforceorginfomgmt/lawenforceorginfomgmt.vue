      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smarturban:lawenforceorginfomgmt:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('机构名称')"
          >
            <template #default="scope">
              {{ tableFieldShow('机构名称', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="机构类型"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('机构类型')"
          >
            <template #default="scope">
              {{ tableFieldShow('机构类型', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smarturban:lawenforceorginfomgmt:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smarturban:lawenforceorginfomgmt:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smarturban:lawenforceorginfomgmt:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
        ...
        <div class="drawer-content">
          ...
          >
            <ul>
              ...
              <li class="width50"
                ><span>机构名称：</span>{{ tableFieldShow('机构名称', queryData.field) }}</li
              >
              <li class="width50"
                ><span>机构类型：</span>{{ tableFieldShow('机构类型', queryData.field) }}</li
              >
            </ul>
          </div>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: 'e0428c60-771d-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '执法机构信息管理', //资产配置，都是写死固定的资产配置
  field: {
    机构名称: '', //机构名称
    机构类型: '' //机构类型
  }
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '执法机构信息管理-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.机构名称 = tableFieldShow('机构名称', row.field) //机构名称
  formData.value.field.机构类型 = tableFieldShow('机构类型', row.field) //机构类型

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑执法机构信息管理'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增执法机构信息管理'
}
