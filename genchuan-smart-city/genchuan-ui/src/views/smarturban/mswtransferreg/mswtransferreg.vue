      ...
      <div class="cc-title-css">
        ...
        <div class="right">
          <el-button
            ...
            v-hasPermi="['smarturban:mswtransferreg:create']"
          >
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          >
            <template #default="scope">
              {{
                handleTypeOptions(tableFieldShow('类别', scope.row.field), optionsLisi.类别Options)
              }}
            </template>
          </el-table-column>
          <el-table-column
          ...
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('设计转运能力')"
          >
            <template #default="scope">
              {{ tableFieldShow('设计转运能力', scope.row.field) }} t（日均）
            </template>
          </el-table-column>
          <el-table-column
            label="转运量"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('转运量')"
          >
            <template #default="scope">
              {{ tableFieldShow('转运量', scope.row.field) }} t（日均）
            </template>
          </el-table-column>
          <el-table-column
            label="转运作业单元数"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('转运作业单元数')"
          >
            <template #default="scope">
              {{ tableFieldShow('转运作业单元数', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="分类转运情况"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('分类转运情况')"
          >
            <template #default="scope">
              {{ tableFieldShow('分类转运情况', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="污水产生量"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('污水产生量')"
          >
            <template #default="scope">
              {{ tableFieldShow('污水产生量', scope.row.field) }} t（日均）
            </template>
          </el-table-column>
          <el-table-column
            label="污水处理方式"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('污水处理方式')"
          >
            <template #default="scope">
              {{ tableFieldShow('污水处理方式', scope.row.field) }}
            </template>
          </el-table-column>
          <el-table-column
            label="出水标准"
            ...
            align="center"
            :show-overflow-tooltip="true"
            v-if="tableColumnShow('出水标准')"
          >
            <template #default="scope">
              {{ tableFieldShow('出水标准', scope.row.field) }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button
                ...
                v-hasPermi="['smarturban:mswtransferreg:query']"
                >详细</el-button
              >
              <el-button
                type="primary"
                ...
                v-hasPermi="['smarturban:mswtransferreg:update']"
                >编辑</el-button
              >
              <el-button
                type="danger"
                ...
                v-hasPermi="['smarturban:mswtransferreg:delete']"
                >删除</el-button
              >
            </template>
          </el-table-column>
          ...
          >
            <ul>
              ...
              <li class="width50"
                ...
                >{{
                  handleTypeOptions(
                    tableFieldShow('类别', queryData.field),
                    optionsLisi.类别Options
                  )
                }}</li
              >
              <li class="width50"
              ...
              <li class="width50"
                ><span>设计转运能力：</span
                >{{ tableFieldShow('设计转运能力', queryData.field) }}</li
              >
              <li class="width50"
                ><span>转运量：</span>{{ tableFieldShow('转运量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>转运作业单元数：</span
                >{{ tableFieldShow('转运作业单元数', queryData.field) }}</li
              >
              <li class="width50"
                ><span>分类转运情况：</span
                >{{ tableFieldShow('分类转运情况', queryData.field) }}</li
              >
              <li class="width50"
                ><span>污水产生量：</span>{{ tableFieldShow('污水产生量', queryData.field) }}</li
              >
              <li class="width50"
                ><span>污水处理方式：</span
                >{{ tableFieldShow('污水处理方式', queryData.field) }}</li
              >
              <li class="width50"
                ><span>出水标准：</span>{{ tableFieldShow('出水标准', queryData.field) }}</li
              >
            </ul>
          </div>
          ...
          <div class="edit-content" v-loading="drawerConfig.loading" v-else>
            ...
            >
              ...
              >
                <el-select
                  placeholder="请选择类别"
                  v-model="formData.field.类别"
                  filterable
                  clearable
                >
                  <el-option
                    v-for="item in optionsLisi.类别Options"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  />
              ...
              <el-form-item
                ...
                prop="field.设计转运能力"
                class="edit-content-form-li width50"
                :rules="[{ required: true, message: '请输入设计转运能力', trigger: 'blur' }]"
              >
                <el-input
                  placeholder="请输入设计转运能力"
                  v-model="formData.field.设计转运能力"
                  clearable
                >
                  <template #append>t（日均）</template>
                </el-input>
              </el-form-item>
              <el-form-item
                label="转运量："
                ...
                :rules="[]"
              >
                <el-input placeholder="请输入转运量" v-model="formData.field.转运量" clearable>
                  <template #append>t（日均）</template>
                </el-input>
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input
                  placeholder="请输入转运作业单元数"
                  v-model="formData.field.转运作业单元数"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input
                  placeholder="请输入分类转运情况"
                  v-model="formData.field.分类转运情况"
                  clearable
                />
              </el-form-item>
              <el-form-item
              ...
              >
                <el-input
                  placeholder="请输入污水产生量"
                  v-model="formData.field.污水产生量"
                  clearable
                >
                  <template #append>t（日均）</template>
                </el-input>
...
const tableInitData = async () => {
  ...
  try {
    let params = {
      ...
      sortOrder: 'DESC',
      assetProfileId: '541a0f40-7684-11f0-bd39-2dc5ccdb39ef' //资产配置id
    }
...
const initFormData = {
  ...
  type: '生活垃圾中转设施监管', //资产配置，都是写死固定的资产配置
  field: {
    名称: '', //名称
    类别: '', //类别
    地址: '', //地址
...
const queryClick = async (row) => {
  ...
  drawerConfig.value.title = '生活垃圾中转设施监管-详细信息'
}
// 点击编辑按钮
const editClick = async (row) => {
  ...
  formData.value.field.设计转运能力 = tableFieldShow('设计转运能力', row.field) //设计转运能力
  formData.value.field.转运量 = tableFieldShow('转运量', row.field) //转运量
  formData.value.field.转运作业单元数 = tableFieldShow('转运作业单元数', row.field) //转运作业单元数
  formData.value.field.分类转运情况 = tableFieldShow('分类转运情况', row.field) //分类转运情况
  formData.value.field.污水产生量 = tableFieldShow('污水产生量', row.field) //污水产生量
  formData.value.field.污水处理方式 = tableFieldShow('污水处理方式', row.field) //污水处理方式
  formData.value.field.出水标准 = tableFieldShow('出水标准', row.field) //出水标准

  drawerConfig.value.isDrawer = true
  drawerConfig.value.loading = false
  drawerConfig.value.type = 'edit'
  drawerConfig.value.title = '编辑生活垃圾中转设施监管'
}
// 点击删除按钮
const delClick = (row) => {
...
const addClick = async () => {
  ...
  drawerConfig.value.title = '新增生活垃圾中转设施监管'
}
...
const titleConfig = ref({
  name: '生活垃圾中转设施监管', //标题名称
  addButName: '新增生活垃圾中转设施监管' //新增按钮名称
})
// 搜索 字段
...
const tableColumns = ref([
  ...
  { label: '设计转运能力', visible: true },
  { label: '转运量', visible: true },
  { label: '转运作业单元数', visible: true },
  { label: '分类转运情况', visible: true },
  { label: '污水产生量', visible: true },
  { label: '污水处理方式', visible: true },
  { label: '出水标准', visible: true }
])
// table表格数据
const tableData = ref([])
//table 配置
