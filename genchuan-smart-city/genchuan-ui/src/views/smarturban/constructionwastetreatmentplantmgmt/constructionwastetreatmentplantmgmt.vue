  ...
  <div class="cc-main">
    <div class="cc-main-content">
      ...
      <div class="cc-title-css">
        ...
        <!--右边的按钮-->
        <div class="right">
          <el-button type="primary" plain :icon="Plus" @click="addClick(titleConfig.addButName)" v-hasPermi="['smarturban:constructionwastetreatmentplantmgmt:create']"> {{ titleConfig.addButName }}</el-button>
        </div>
      </div>
<!--搜索内容字段-->
...
<el-table-column label="消纳总量" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('消纳总量')">
            <template #default="scope">
              {{ tableFieldShow('消纳总量', scope.row.field) }} t
            </template>
          </el-table-column>
<el-table-column label="再生产品类型" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('再生产品类型')">
            <template #default="scope">
              {{ handleTypeOptions(tableFieldShow('再生产品类型', scope.row.field), optionsLisi.再生产品类型Options) }}
            </template>
          </el-table-column>
<el-table-column label="再生材料利用率" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('再生材料利用率')">
            <template #default="scope">
              {{ tableFieldShow('再生材料利用率', scope.row.field) }}
            </template>
          </el-table-column>

          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button type="success" :icon="View" :plain="true" @click="queryClick(scope.row)" v-hasPermi="['smarturban:constructionwastetreatmentplantmgmt:query']">详细</el-button>
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['smarturban:constructionwastetreatmentplantmgmt:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['smarturban:constructionwastetreatmentplantmgmt:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
 ...
 <li class="width50"><span>消纳总量：</span>{{ tableFieldShow('消纳总量', queryData.field) }}</li>
 <li class="width50"><span>再生产品类型：</span>{{ handleTypeOptions(tableFieldShow('再生产品类型', queryData.field), optionsLisi.再生产品类型Options) }}</li>
 <li class="width50"><span>再生材料利用率：</span>{{ tableFieldShow('再生材料利用率', queryData.field) }}</li>
...
<el-form-item label="消纳总量：" prop="field.消纳总量" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入消纳总量', trigger: 'blur' }]">
                <el-input placeholder="请输入消纳总量" v-model="formData.field.消纳总量" clearable>
                  <template #append>t</template>
                </el-input>
              </el-form-item>
<el-form-item label="再生产品类型：" prop="field.再生产品类型" class="edit-content-form-li width50" :rules="[{ required: true, message: '请选择再生产品类型', trigger: 'blur' }]">
                   <el-select placeholder="请选择再生产品类型" v-model="formData.field.再生产品类型" filterable clearable>
                  <el-option v-for="item in optionsLisi.再生产品类型Options" :key="item.value" :label="item.label" :value="item.value"/>
                </el-select>
              </el-form-item>
<el-form-item label="再生材料利用率：" prop="field.再生材料利用率" class="edit-content-form-li width50" :rules="[{ required: true, message: '请输入再生材料利用率', trigger: 'blur' }]">
                <el-input placeholder="请输入再生材料利用率" v-model="formData.field.再生材料利用率" clearable/>
              </el-form-item>
        ...
        <div class="action-button" v-if="['add', 'edit'].includes(drawerConfig.type) && !drawerConfig.loading">
          ...
          <el-button type="primary" :icon="Check" @click="drawerSubmitForm(drawerFromRef)">保 存</el-button>
            ...
            const tableInitData = async () => {
              ...
              try {
                let params = {
                  ...
                  sortOrder: 'DESC',
                  assetProfileId: 'ed55b000-7720-11f0-bd39-2dc5ccdb39ef',//资产配置id
                }
            ...
            // 表单字段
              const initFormData = {
                ...
                type: '建筑垃圾处理厂数据管理',//资产配置，都是写死固定的资产配置
                field: {
                   名称: '',//名称
            地址: '',//地址
            消纳总量: '',//消纳总量
            ...
            const optionsLisi = ref({
              //再生产品类型 选项
                                   再生产品类型Options:[
                                {label:'再生混凝土',value:'1'},{label:'再生砂浆',value:'2'},{label:'再生制品d',value:'3'},{label:'再生沥青混合料',value:'4'},{label:'再生无机混合料',value:'5'},
                           ],
            ...
            const queryClick = async (row) => {
              ...
              drawerConfig.value.title = '建筑垃圾处理厂数据管理-详细信息';
            }
            // 点击编辑按钮
            ...
            formData.value.field.消纳总量 = tableFieldShow('消纳总量', row.field);//消纳总量
            formData.value.field.再生产品类型 = tableFieldShow('再生产品类型', row.field);//再生产品类型
            formData.value.field.再生材料利用率 = tableFieldShow('再生材料利用率', row.field);//再生材料利用率

              drawerConfig.value.isDrawer = true;
              drawerConfig.value.loading = false;
              drawerConfig.value.type = 'edit';
              drawerConfig.value.title = '编辑建筑垃圾处理厂数据管理';
            }
            // 点击删除按钮
            const delClick = (row) => {
            ...
            const addClick = async () => {
              ...
              drawerConfig.value.title = '新增建筑垃圾处理厂数据管理';
            }
            ...
            const titleConfig = ref({
              name: '建筑垃圾处理厂数据管理',//标题名称
              addButName: '新增建筑垃圾处理厂数据管理',//新增按钮名称
            })
            // 搜索 字段
          ...
          const tableColumns = ref([
            ...
            {label: '消纳总量', visible: true},
            {label: '再生产品类型', visible: true},
            {label: '再生材料利用率', visible: true},
