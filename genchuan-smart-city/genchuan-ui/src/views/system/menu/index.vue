...
const columns = [
  ...
  },
  {
    ...
    title: '操作',
    align: 'center',
    width: JSON.parse(import.meta.env.VITE_DEV)?250:160,
    fixed: TableV2FixedDir.RIGHT,
    cellRenderer: ({rowData}) => {
      // 定义按钮列表
      const buttons = []
      ...
      if (JSON.parse(import.meta.env.VITE_DEV)) {
          buttons.push(
            <ElButton
              ...
              link
              type="primary"
              onClick={() => cchandleClick2(rowData.id,rowData)}
            >
              增删改查添加
            </ElButton>
          )
...
console.log(CACHE_KEY.ROLE_ROUTERS,33333)
...
const cchandleClick2 = async (id,rowData) => {
  ...
  for (let i = 0; i < dataList.length; i++) {
    await MenuApi.createMenu(dataList[i]);
  }
  // 刷新列表
  await getList()
