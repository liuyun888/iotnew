<template>
  ...
  <ContentWrap :bodyStyle="{ padding: '10px 20px 0' }" class="position-relative">
    <div class="processInstance-wrap-main">
      <!--{{processInstance.status}}  1 审核中 2 审核通过-->
      <el-scrollbar>
        <img
        ...
        <div class="b-t-solid border-t-1px border-[var(--el-border-color)]">
          <!-- 操作栏按钮 -->
          <ProcessInstanceOperationButton3
...
import ProcessInstanceOperationButton3 from './ProcessInstanceOperationButton3.vue'
...
import {remarksClear, remarksInfo} from "@/api/genchuan/thingsBoard/profiles";


defineOptions({name: 'BpmProcessInstanceDetail3'})
const props = defineProps<{
  id: string // 流程实例的编号
  taskId?: string // 任务编号
  activityId?: string //流程活动编号，用于抄送查看
  thingsBoardId?: string //thingsBoard id
}>()
const thingsBoardId = props.thingsBoardId;
...
const getApprovalDetail = async () => {
  ...
  try {
    ...
    // 审核到最后一步了
    if (processInstance.value.status == 2) {
      const res = await remarksInfo(thingsBoardId);
      if (res.status == 'ACTIVE_ACK') {
        await remarksClear(thingsBoardId)
      }
    }
