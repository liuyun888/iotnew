...
import {remarksAck, remarksAddEdit} from "@/api/genchuan/thingsBoard/profiles";

defineOptions({name: 'ProcessDefinitionDetail3'})
...
const thingsBoardId = route.query.thingsBoardId
...
const submitForm = async () => {
  ...
  try {
    // 保存事件返回事件id
    let continuousTableId = await ProcessInstanceApi.createProcessInstance({
      processDefinitionId: props.selectProcessDefinition.id,
      variables: detailForm.value.value,
      startUserSelectAssignees: startUserSelectAssignees.value
    })
    // 重新发起的时候id存过  id存过验证过了，告警应答这个没有做
    if (thingsBoardId) {
      let data = {
        "alarmId":{
          "entityType": "ALARM",
        ...
        "type": "OTHER",
        "comment": {
          "text": continuousTableId
        }
      }
      await remarksAddEdit(thingsBoardId, data);
      const routerName = route.query.routerName
      ...
      let data = {
        "alarmId": props.selectProcessDefinition.tableRow.id,
        "type": "OTHER",
        "comment": {
          "text": continuousTableId
        }
      }
      await remarksAddEdit(props.selectProcessDefinition.tableRow.id.id, data);
      // 应答
      await remarksAck(props.selectProcessDefinition.tableRow.id.id)
      emit('tableInitData')
    }
