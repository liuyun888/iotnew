...
import {assetsEditField} from "@/api/genchuan/thingsBoard/assets";

defineOptions({ name: 'ProcessDefinitionDetail1' })
const props = defineProps<{
  selectProcessDefinition: any
}>()
const emit = defineEmits(['cancel','tableInitData'])
const processInstanceStartLoading = ref(false) // 流程实例发起中
const { push, currentRoute } = useRouter() // 路由
const route = useRoute();
const message = useMessage() // 消息弹窗
const { delView } = useTagsViewStore() // 视图操作
//把保存的流程id存进thingsBoard资产里面
...
const submitForm = async () => {
  ...
  try {
    ...
    // 重新发起的时候id存过
    if(thingsBoardId){
      await assetsEditField(thingsBoardId, {'流程id':continuousTableId});
      ...
      await assetsEditField(props.selectProcessDefinition.tableRow.id.id, {'流程id':continuousTableId});
      emit('tableInitData')
    }
